...
class Statistics:
    ...
    def get_cached_graphobjects_total_count_trend(
        ...
        for (
            ...
            if timestamp > timeframe_start:
                relevant_data[timestamp] = graph_objects_count
    ...
    def _trend(cls, data: dict) -> int:
        ...
        # TODO: This is stupid. values are not fitting because slope is totaly distorted due to the generic generated x values.
        # try to understand what are you doing and fix it next week. see test.py

        if len(data) > 1:
            coeffs = numpy.polynomial.polynomial.Polynomial.fit(
                range(0, len(data)), list(data), 1
            )
            coeffs_convert = coeffs.convert()
