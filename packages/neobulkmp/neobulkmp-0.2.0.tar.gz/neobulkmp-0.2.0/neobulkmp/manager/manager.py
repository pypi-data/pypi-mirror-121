...
class Manager:
    ...
    def _manage_store(self):
        ...
        if (
            self.cache.store_is_available()
            and self.statistics.get_cache_storage_level() == "RED"
        ):
        ...
        elif self.cache.store_is_available() and (
            self.statistics.get_cached_graphobjects_total()
            - self.statistics.get_loaded_graphobjects_total()
            > self.strategy.max_advantage_cache_graphobjects_count
        ):
            self.cache.close_store()
            log.debug(
        ...
        elif not self.cache.store_is_available():
