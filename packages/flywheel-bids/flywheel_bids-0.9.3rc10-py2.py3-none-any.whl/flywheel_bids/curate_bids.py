...
def set_up_template(fw, project, template_type=None, template_file=None):
    ...
    if template_type:
        ...
        # The following is deprecated and will be removed before merging this branch into the main branch
        # Check for project-level template file
        ...
        if template_filename:
            ...
            os.close(fd)
            fw.download_file_from_project(project.id, template_filename, template_file)
...
def curate_bids(
    ...
    logger.info("Curated %s sessions", count.sessions)
    logger.info("Curated %s acquisitions", count.acquisitions)
