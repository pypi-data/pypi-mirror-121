...
<div>Uploading file: <b>{f['name']}</b></div>
        <div class="progress" style="margin-top: 5px;">
                                <div id='file_progress_bar_{c}' class="progress-bar bg-success" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%">
                                </div>
                            </div>
        ''')
...
class ModalAjaxFile:
    ...
    def start_upload_files(self, **kwargs):
        if not kwargs.get('files'):
            ...
            from django_modals.modals import Modal
           # return self.command_response('modal_html', html=Modal(message='x').get(self.request))

            return self.command_response('modal_html', html=render_modal(contents='<i class="text-danger fas fa-exclamation-circle"></i> No files selected',
