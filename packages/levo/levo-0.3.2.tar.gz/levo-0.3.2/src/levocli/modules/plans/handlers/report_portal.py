...
from .default import get_section_name, get_statistic, get_subsection, get_summary
...
def handle_after_execution(
    ...
    if event.payload.result.interactions:
        _send_request_response_bodies(event, service, test_item_id)


def _send_request_response_bodies(event, service, test_item_id):
    lines = []
...
def _get_single_error(
    ...
    for error in result.errors:
        lines.append(format_exception(error, context.show_errors_tracebacks))
    return "\n".join(lines)
...
def handle_finished(
    ...
    report_log(get_statistic(suite_to_status_summary), service)
    report_log(get_summary(context, event), service)
