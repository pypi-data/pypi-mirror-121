...
import typing
from typing import (Any, Optional, Dict, Generator, Tuple, List)

_traceparent_header_format_re = re.compile("^[ \t]*"  # whitespace
                                           "([0-9a-f]{32})?"  # trace_id
                                           "-?([0-9a-f]{16})?"  # span_id
                                           "-?([01])?"  # sampled
                                           "[ \t]*$"  # whitespace
...
class Span(object):
   ...
   def start_child(self, **kwargs):
      ...
      rv = Span(trace_id=self.trace_id,
                span_id=None,
   ...
   def new_span(self, **kwargs):
      ...
      logger.warning(
          "Deprecated: use Span.start_child instead of Span.new_span.")
   ...
   def continue_from_environ(
      ...
      """
        ...
        the 'warden-trace' header in the environ (if any) before returning the
        Transaction.

        If the 'warden-trace' header is malformed or missing, just create and
   ...
   def continue_from_headers(
      ...
      """
        Create a Transaction with the given params, then add in data pulled from
        the 'warden-trace' header (if any) before returning the Transaction.
      ...
      transaction = Transaction.from_traceparent(headers.get("warden-trace"),
                                                 **kwargs)
   ...
   def from_traceparent(
      ...
      """
        ...
        """
      if cls is Span:
         logger.warning("Deprecated: use Transaction.from_traceparent "
      ...
      return Transaction(trace_id=trace_id,
   ...
   def finish(self, hub=None):
      ...
      try:
         ...
         ) - self._start_timestamp_monotonic
...
def has_tracing_enabled(options):
   ...
   return bool(
       options.get("traces_sample_rate") is not None or
       options.get("traces_sampler") is not None)
