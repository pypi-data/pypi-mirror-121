...
from warden_sdk.utils import iteritems, text_type, string_types, format_timestamp
   ...
   for key, value in iteritems(options):
      ...
      if key == 'creds':
         ...
         if ['client_id', 'client_secret'] != list(value.keys()):
            raise TypeError("`creds` is missing either `client_id` or `client_secret`")
...
class _Client(object):
   ...
   def env(self, env: Optional[str] = None) -> str:
      ...
      if any(self.options['environment'] == _env_type for _env_type in _dev_types):
         return 'dev'

      _prod_types: List[str] = ['prod','production']
      if any(self.options['environment'] == _env_type for _env_type in _prod_types):
         return 'prod'
