...
Code reference:
- [sentry_sdk](https://github.com/getsentry/sentry-python/blob/master/sentry_sdk/serializer.py)
"""
import sys
import math
...
import warden_sdk.debug

from warden_sdk.utils import json_dumps, text_type, string_types, number_types, iteritems, disable_capture_event, format_timestamp
...
def serialize(event, smart_transaction_trimming=False, **kwargs):
   ...
   def _should_repr_strings():
      ...
      try:
         ...
         if (p0 in ("threads", "exception") and path[1] == "values" and
             path[3] == "stacktrace" and path[4] == "frames" and
             path[6] == "vars"):
            return True
      except IndexError:
         return None
   ...
   def _serialize_node_impl(obj, is_databag, should_repr_strings,
      ...
      if (smart_transaction_trimming and len(path) == 3 and
          path[0] == "spans" and path[-1] == "description"):
   ...
   try:
      ...
      if smart_transaction_trimming and sum_span_description_bytes > 0:
         ...
         # as event["_meta"].
         descriptions_budget_bytes = span_count * warden_sdk.debug.MAX_STRING_LENGTH
