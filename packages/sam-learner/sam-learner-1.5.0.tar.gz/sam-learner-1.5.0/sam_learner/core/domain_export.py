...
class DomainExporter:
	...
	def write_constants(self, constants: Dict[str, Type]) -> str:
		...
		for constant_name, constant_type in constants.items():
			sorted_consts_types[constant_type.parent.name].append(constant_name)

		constants_content = self.format_type_like_string(sorted_consts_types)
		return constants_str.format(constants="\n".join(constants_content))

	@staticmethod
	def format_type_like_string(sorted_type_like_objects: Dict[List[str]]) -> List[str]:
	...
	def export_domain(self, domain: Domain, export_path: Path) -> NoReturn:
		...
		if len(domain.constants) > 0:
			domain_content += self.write_constants(domain.constants)
