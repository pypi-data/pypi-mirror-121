...
NO_PLAN_FOUND = "Planner failed to solve the problem!"
TIMEOUT_DURING_PLANNING_OCCURRED = "Time out occurred during the solving process!"

POTENTIAL_ERRORS = [NO_PLAN_FOUND, TIMEOUT_DURING_PLANNING_OCCURRED]

PLAN_SEARCH_TIME_LIMIT = 300 if 'SEARCH_TIME' in os.environ else int(os.environ['SEARCH_TIME'])
...
class DomainValidator:
	...
	def generate_test_set_plans(
		...
		for problem_file_path in Path(test_set_directory).glob("*.pddl"):
			...
			try:
				...
				successful_plans[str(problem_file_path)] = TIMEOUT_DURING_PLANNING_OCCURRED
	...
	def search_plan_with_time_limit(
		...
		if plan_actions is not None and len(plan_actions) > 0:
			...
			successful_plans[str(problem_file_path)] = NO_PLAN_FOUND
	...
	def log_model_safety_report(
		...
		for generated_domain_path in Path(generated_domains_directory_path).glob("*.pddl"):
			...
			for problem_path, plan_path in domain_successful_plans.items():
				...
				if plan_path in POTENTIAL_ERRORS:
