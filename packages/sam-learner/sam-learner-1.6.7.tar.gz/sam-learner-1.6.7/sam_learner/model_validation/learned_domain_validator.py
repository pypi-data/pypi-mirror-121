...
TIMEOUT_WHILE_SOLVING_ERROR = f"Solver could not solve the problem within {PLAN_SEARCH_TIME_LIMIT} seconds."
	...
	def generate_test_set_plans(
		...
		for problem_file_path in Path(test_set_directory).glob("*.pddl"):
			...
			try:
				...
				successful_plans[str(problem_file_path)] = TIMEOUT_WHILE_SOLVING_ERROR
