...
class AMStatisticsExtractor:
	...
	def run_learner(
		...
		for trajectory in domain_trajectories:
			...
			try:
				...
				self.write_learned_actions_statistics(
	...
	def write_learned_actions_statistics(
		...
		for action_name in learned_actions:
			action_statistics["domain_name"] = self.expected_domain.name
			action_statistics["num_trajectories"] = num_trajectories
			action_statistics["learned_action_name"] = action_name
			action_preconditions = learned_model.actions[action_name].precondition
			action_add_effects = list(learned_model.actions[action_name].effect.addlist)
			action_delete_effects = list(learned_model.actions[action_name].effect.dellist)
			action_statistics["learned_preconditions"] = ",".join([str(precond) for precond in action_preconditions])
			action_statistics["learned_add_effects"] = ",".join([str(add_effect) for add_effect in action_add_effects])
			action_statistics["learned_delete_effects"] = ",".join([str(del_effect) for del_effect in action_delete_effects])
