...
def _get_crop_value(crop_id: str, column: str):
    value = safe_parse_float(get_crop_lookup_value(crop_id, column), 0)
...
def _crop_product_values(crop: dict, residue_id: str):
    ...
    debugRequirements(model=MODEL, term=residue_id,
                      ...
                      ratio=ratio)
    return (value * dm / 100 * yield_dm * (ratio or 1), n_content)


def _run_multiple_products(crop_residue_products: list, crop_products: list):
    def run_product(product: dict):
        term_id = product.get('term', {}).get('@id')
        values = [_crop_product_values(p, term_id) for p in crop_products]
...
def _should_run_multiple_crop_product(product: dict):
    ...
    prop = find_term_match(product.get('properties', []), PROPERTY_KEY).get('value')
    return all([value > 0, prop, _get_crop_value(term_id, 'IPCC_2019_Ratio_AGRes_YieldDM')])
...
def _should_run(cycle: dict):
    ...
    single_crop = crop_products[0] if len(crop_products) == 1 else None
    ...
    return should_run, crop_residue_products, crop_products


def run(cycle: dict):
    should_run, crop_residue_products, crop_products = _should_run(cycle)
    return (
        _run_multiple_products(crop_residue_products, crop_products) if len(crop_products) > 1
        else _run_single_product(crop_residue_products, crop_products[0])
    ) if should_run else []
