...
def _add_emission(input: dict):
    def run(mapping: tuple):
        ecoinventName, coeff = mapping
        emissions = ecoinventV3_impacts(ecoinventName)
        return [_emission(id, value * coeff * list_sum(input.get('value', [])), input) for id, value in emissions]
    return run
...
def _run_input(input: dict):
    ...
    return flatten(map(_add_emission(input), mappings)) if should_run else []
