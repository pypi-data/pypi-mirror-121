...
class BrainerClient(PhasioControlPort):
	...
	def __init__(self, brainer_config: BrainerConfig):
	...
	def register_candidate(self, brand: str, model: str, material: str) -> (int, str, str):
		r_id = str(uuid.uuid4())
		pairing_code = random.randint(9999, 999999)
		secret = self._generate_secret()
		pairing_request = self._generate_pairing_request(r_id, pairing_code, brand, model, material, secret)
	...
	@staticmethod
	def _generate_pairing_request(r_id: str, pairing_code: int, brand: str, model: str, material: str, secret: str):
		return {
			"requestId": r_id,
