...
class Pyzio(PyzioListener):
  ...
  def __init__(self, logger: PyzioLogger, listener: PyzioListener,
      printer: PyzioPrinter, settings: PyzioSettings):
    ...
    self._job_service = JobService(self._printer, self._brainer_client,
                                   self._local_job_repo,
                                   self._printer_state_repo, self._logger)
    self._update_service = UpdateService()
    self._printer_service = PrinterService(self._printer_state_repo)
    self._phasio_port = PhasioMQTTAdapter(self._logger, self._job_service,
  ...
  def _register_candidate(self):
    pairing_code, candidate_id, secret = self._brainer_client.register_candidate(
