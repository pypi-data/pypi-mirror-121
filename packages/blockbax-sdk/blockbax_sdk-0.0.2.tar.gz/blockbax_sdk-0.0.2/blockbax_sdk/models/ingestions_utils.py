...
def create_series_batches(ingestions: Iterator[ingestion.Ingestion], subjects: List[subject.Subject], auto_create_subjects: bool) -> List[dict]:
    grouped_ingestions = group_ingestions_per_subject(ingestions, subjects, auto_create_subjects)
    json_batches = []
        ...
        for batch in batches_to_send:
            ...
            for measurement in batch:
                measurements_to_send[measurement[0]].append({
                    ...
                    measurement[1]._get_data_type() : measurement[1].get_value()
                })

            series = []
            for ingestion_id, measurements in measurements_to_send.items():
                series.append({"ingestionId": ingestion_id, "measurements" : measurements})
...
def batch_measurements_by_date(ingestions: List[ingestion.Ingestion]) -> List[List[Tuple[str, measurement.Measurement]]]:
    ...
    if number_of_measurements > 500:
        ...
        sorted_list_of_measurements = sorted(list_of_measurements, key=lambda measurement_tuple: measurement_tuple[1].date)
