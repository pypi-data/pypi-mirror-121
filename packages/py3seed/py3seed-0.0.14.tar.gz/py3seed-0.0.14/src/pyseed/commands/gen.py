                ...
                with open(pp) as f:  # Seeds defined in views
                    for line in f:
                        ...
                        if key_value_found:
                            ...
                            # NOTE: Varibles name should be in snake format, i.e, two_words
                            # NOTE: Need to make sure the varibles name is not rows/seeds/name/name_xxx
                            view[key] = _parse_varible_value(key, value)
                            # NOTE: Varibles startwith seed_ will be updated into seeds, e.g, seed_is_card => is_card
                            if key.startswith('seed_'):
                                key = key[5:]
                                seed_varibles[key] = _parse_varible_value(key, value)
                        else:
                            row = {'columns': []}
                            if '|' in line:  # Nested column
                                for c in line.split('|'):
                                    ...
                                    for cc in c.split(','):
                                        cc = cc.strip()
                                        seed = _parse_seed(cc, models, seed_varibles)
                                        if seed:
                                ...
                                for c in line.split(','):
                                    c = c.strip()
                                    seed = _parse_seed(c, models, seed_varibles)
                                    if seed:
                                        view['seeds'].append(seed)
                                        context['seeds'].append(seed)
...
def _parse_seed(column, models, seed_varibles={}):
    ...
    if found:
        action = tokens[-1]
        return {'model': found, 'includes': includes, 'action': action, **_generate_names(column), **seed_varibles}
    else:
        return None
