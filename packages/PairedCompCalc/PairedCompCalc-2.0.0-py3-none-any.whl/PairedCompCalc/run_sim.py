...
one or more Groups of subjects randomly drawn from one specified Population,
...
ds.save(data_path, allow_over_write=True, fmt=file_format,
        ...
        Stim='G',
        SNR='H')
# NOTE: subject OR attribute may be stored in sheet name
logging.info(ds.__class__.__name__ + ' saved in ' + str(data_path) + f' as {file_format} files')

# Optionally, save as json files, too
# file_format = 'json'
# ds.save(data_path, allow_over_write=True, fmt=file_format)
# logging.info(ds.__class__.__name__ + ' saved in ' + str(data_path) + f' as {file_format} files')
...
# See run_pc.py for other combinations of display output

# ------------------------------- save all result displays in directory tree
result_path = work_path / 'result_sim'
