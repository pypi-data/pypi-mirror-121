...
NOTE: The accuracy of the resulting performance measures is rather crude.
...
* Version 2.0
2021-09-15, tested, using new version of module pc_planning, adapted for new version of pc_model
...
pc_logging.setup(result_path=Path.home() / 'Documents' / 'PairedComp_plan',  # or whatever...
                 log_file='run_plan_log.txt')  # to save the log file
# OR
...
m_diff = 0.5
# = anticipated mean quality difference B - A in the population, in Thurstone d-prime units.

q_std = 1.
# = anticipated inter-individual standard deviations for objects A and B in the population
...
logger.info(f'Ideal measurements with {n_subj_ideal} subjects,' +
            ...
            f'\n\t\t would achieve p_true_pos > {p_true_crit:.1%}; p_false_pos < {p_false_crit:.1%}')

# ------------------------------ Crude Experimental Sample Size Determination (SSD):
# This might take one or more hour(s) to calculate
...
#                                   p_false_pos=p_false_crit,
#                                   n_sim=5)
# logger.info(f'A paired-comparison experiment with {n_estim} subjects,' +
#             f'\n\t\tusing {n_difference_grades} difference grades, ' +
#             f'forced_choice={pcf.forced_choice}, ' +
#             f'{n_pres} pair presentations' +
#             f'\n\t\t might achieve p_true_pos > {p_true_crit:.1%}; p_false_pos < {p_false_crit:.1%}')


# ------------------------------ Performance Measures with single given group size:

logger.info(f'*** Estimating Test Performance for Population Mean,' +
            f' assuming true difference mean = {m_diff:.1f}, object std. = {q_std:.1f}:')

(p_true_pos, p_false_pos) = pcp.power_ab_mean(pcf, m_diff, q_std,
                                              n_subjects=n_estim,
                                              n_pres=n_pres,
                                              n_sim=10)
...
logger.info(f'A paired-comparison experiment with {n_estim} subjects,' +
            ...
            f'forced_choice={pcf.forced_choice}, ' +
            f'{n_pres} pair presentations' +
            f'\n\t\t might achieve p_true_pos = {p_true_pos:.1%}; p_false_pos = {p_false_pos:.1%}')


# ------------------------------ AND / OR: performance for Random Individual:

# logger.info(f'*** Estimating Performance for Population Individual,' +
#             f' assuming true difference mean = {m_diff:.1f}, object std. = {q_std:.1f}')
#
# (p_true_pos, p_false_pos) = pcp.power_ab_individual(pcf, m_diff, q_std,
#                                                     n_subjects=n_estim,
#                                                     n_pres=n_pres,
#                                                     n_sim=10)
#
