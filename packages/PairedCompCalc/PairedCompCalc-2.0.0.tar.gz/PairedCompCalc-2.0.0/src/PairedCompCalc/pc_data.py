...
pc_base.PairedCompItem: basic data block, specifying
...
StimRespItem: container for a single tuple of (stimulus, response, test-condition)
...
* xlsx: Data can be imported from (and exported to) Excel workbook (xlsx) files,
...
See script run_sim for an example, and module pc_file_xlsx for full details.

* json: An existing PairedCompDataset instance may also be
dumped to a set of pc_file_json.PairedCompFile instances, and re-loaded later.
This file format is a serialized sequence of PairedCompItem objects.
...
This format is somewhat restricted and should NOT be used for new experiments.
...
if the test-factor category is not specified within the data file itself.
...
ds = PairedCompDataSet.load(pcf, dir='~/pc_data', groups=['NH', 'HI'], fmt='xlsx')
...
and use all files that seem to contain paired-comparison data.
...
With data files saved in the 'xlsx' (or 'json') formats,
...
in the same way as for test conditions.

Obviously, the safest policy is to include all relevant data in the input file.
...
"""
# *** Future: allow PairedCompDataSet import from Pandas dataframe ? ****
...
# Future: in python v >=3.7: T can be omitted, using arg default=dict()
...
class PairedCompFrame:
    ...
    def __init__(self,
        ...
        :param objects_alias: (optional) sequence of object labels used for displays
...
class PairedCompDataSet:
    ...
    def load(cls, pcf, dir, groups=None, fmt=None, **file_args):
        """Create one class instance from selected paired-comparison session results.
        ...
        pcd = OrderedDict()  # *** could be just a dict in python >= 3.7
        ...
        for g in groups:
            ...
            for pc_item in gen_pc_items(dir, g):
                ...
                if pc_item.subject not in pcd[g][pc_item.attribute].keys():
                    ...
                    # first item in the list for this subject
                else:
    ...
    def save(self, dir, allow_over_write=False, fmt='xlsx', **file_args):
                ...
                for (s_id, s_results) in a_subjects.items():
                    ...
                    try:
                        ...
                        s_file = FILE_CLASS[fmt](file_path=p,
                                                 pcf=self.pcf,
                                                 **file_args)
                        s_file.save(s_items, allow_over_write=allow_over_write)
...
def _matching_test_cond_tuple(tcd, pcf):
    ...
    NOTE: this implies that tct is guaranteed to be one element of pcf.test_conditions()
    """
    try:
