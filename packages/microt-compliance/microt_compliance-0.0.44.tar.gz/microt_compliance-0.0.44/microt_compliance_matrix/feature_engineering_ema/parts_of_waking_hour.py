...
def convert_time_str_to_datetime(time_str_series):
    ...
    idx = 0
    for i in range(len(time_str_series)):
        if type(time_str_series[i]) == str:
            idx = i
            break
    else:
        return time_str_series

    if len(time_str_series[idx].split(".")) > 1:
    ...
    time_datetime_list = [x.split(" ")[0] + " " + x.split(" ")[1] if (type(x) == str) or (len(x.split(" ")) < 2) else x for x in
...
def create_column(feature_df, parts_num):
    ...
    proximity_to_wake_time_column = [
        (y - x).total_seconds() / 60 if (type(x) != float) and (type(y) != float) else np.nan for x, y in
        zip(wake_time_datetime_list, prompt_time_datetime_list)]
    proximity_to_sleep_time_column = [
        (y - x).total_seconds() / 60 if (type(x) != float) and (type(y) != float) else np.nan for x, y in
    ...
    duration_per_part = [x // parts_num if x == x else np.nan for x in wake_duration]
