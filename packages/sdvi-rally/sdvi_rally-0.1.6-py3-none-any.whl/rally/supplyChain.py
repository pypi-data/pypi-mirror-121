...
class SupplyChainStep:
    ...
    :param fail_step: Including this argument makes this SupplyChainStep a conditional when part of a
        SupplyChainSequence: `fail_step` will be executed instead of `name` if the immediately preceding step fails.
        Note: `fail_step` has no effect outside of a SupplyChainSequence.
    :type fail_step: str, or :class:`~rally.supplyChain.SupplyChainStep`, optional
    ...
    def __init__(self, name, dynamic_preset_data=None, preset=None, priority=None, supply_chain_deadline=None,
        ...
        if fail_step:
            if isinstance(fail_step, (str, SupplyChainStep)):
                self.failStepName = fail_step if isinstance(fail_step, SupplyChainStep) else SupplyChainStep(fail_step)
            else:
                raise TypeError(f"invalid type for step: {type(fail_step).__name__}, expected: str or SupplyChainStep")
