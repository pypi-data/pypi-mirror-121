...
from rally.context import _sdk_context as context, _API_TOKEN, _API_URL, JOB_UUID
...
class RallySession(Session):
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        if not url.startswith('http'):
            url = posixpath.join(context(_API_URL), url)
...
def _getSession():
    ...
    assert context(_API_URL), "api_root must be set"
    if 'dev.sdvi.com' in context(_API_URL):
