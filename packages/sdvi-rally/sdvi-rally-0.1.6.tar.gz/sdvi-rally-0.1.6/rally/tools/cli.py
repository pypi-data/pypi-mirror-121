...
from rally.context import _API_TOKEN, _API_URL, _CONTEXT
...
def validate_environment():
    ...
    """
    apiToken = os.environ.get(_API_TOKEN)
    rallyUrl = os.environ.get(_API_URL)

    assert apiToken, f"{_API_TOKEN} must be set in your environment"

    url_help = f"{_API_URL} must be set to 'https://<silo>.sdvi.com' in your environment"
    assert rallyUrl, url_help
    url_parts = urlparse(rallyUrl)
    # Ensure sdvi.com silo; permit custom port numbers
    assert re.compile('.*\.sdvi\.com(:[0-9]*)?$').match(url_parts.netloc), f"{url_help}, but found '{rallyUrl}'"
    ...
    os.environ[_API_URL] = urlunparse((url_parts.scheme, url_parts.netloc, path, '', '', ''))

    # Rally can cope with missing keys in this dict, but cannot cope with `None`. Set this env var to an empty dict
    # to avoid AttributeErrors when accessing _CONTEXT.
    # This validation is unneeded outside of the CLI, as eval2 provider creates this variable during bootstrap.
    if not os.environ.get(_CONTEXT):
        os.environ[_CONTEXT] = {}
...
def make_context(asset_name, dynamic_preset_data):
    ...
    os.environ[_CONTEXT] = json.dumps(ctx)

    # asset._getAssetByName only depends on the API token and URL, but will have the side effect of having loaded and
