...
class Base(Controller):
    ...
    @ex(
        ...
        arguments=[
            (['action'],{
                "help" : "One of {ls, rm, cp, mv, download, upload}",
                "action" : "store"
            }),
            (['target'],{
                "help" : "The target of the action. Can be a file or a directory",
                "action" : "store"
            })
        ]
    )
    def s3(self):
        if self.app.pargs.action == '-1':
            from ..ext.aws_s3 import get_lastest_file
            print(get_lastest_file(self.app.pargs.target))
        if self.app.pargs.action == 'ls':
            from ..ext.aws_s3 import get_latest_bucket
            get_latest_bucket(self.app.pargs.target)
