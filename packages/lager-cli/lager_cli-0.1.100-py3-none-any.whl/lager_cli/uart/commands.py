...
@click.option('--test-runner', help='End the UART session when end-of-test is detected', type=click.Choice(['none', 'unity', 'fixture']), default='none')
...
def uart(ctx, gateway, serial_device, baudrate, bytesize, parity, stopbits, xonxoff, rtscts, dsrdtr,
         test_runner, interactive, message_timeout, overall_timeout, eof_timeout, display_job_id, line_ending, opost, serial_channel):
    """
    ...
    run_job_output(connection_params, test_runner, interactive, line_ending, message_timeout, overall_timeout,
        eof_timeout, ctx.obj.debug, opost=opost, serial_channel=serial_channel)
