...
class HierarchicalSequenceProcessor(ABCSentenceProcessor):
    ...
    def transform(self,samples: TextSamplesVar,seq_length: int = None,doc_length:int = None) :
        ...
        if doc_length is None:
            doc_length = max([len(sample) for sample in split_samples])
            print("doc_len:"+ str(doc_length))
        doc_length = 20
        ...
        for doc_seq in split_samples:
            ...
            doc_masking.append(sub_doc_masking)
        ...
        doc_masking = np.array(doc_masking)
        return token_ids,doc_masking
