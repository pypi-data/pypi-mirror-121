...
def eval_build_config(**kwargs) ->common.BuildConfig:
    ...
    if _install_requires is None:
        ...
        raise TypeError(
            f'Invalid argument for install_requires: {type(_install_requires)}'
            )
    if install_requires:
        install_requires = {re.split('[\\^\\~<>=;]', req.strip())[0] for
            req in install_requires}
...
def init_build_package_dir(local_package_dir: common.PackageDir
    ) ->common.PackageDir:
    ...
    for package, src_package_dir in local_package_dir.items():
        ...
        if is_abs_path:
            raise Exception(
                f"package_dir must use relative paths, got '{src_package_dir}'"
                )
...
def build_package(cfg: common.BuildConfig, package: str, build_dir: str
    ) ->None:
...
def build_packages(cfg: common.BuildConfig, build_package_dir: common.
    PackageDir) ->None:
...
def fix(package_dir: common.PackageDir=None, target_version: str=transpile.
    DEFAULT_TARGET_VERSION, install_requires: typ.List[str]=None,
    default_mode: str='enabled') ->common.PackageDir:
    msg = (
        'Depricated: lib3to6.fix(). See https://github.com/mbarkhau/lib3to6#Deprications'
        )
...
class build_py(_build_py.build_py):
    ...
    def run_3to6(self) ->None:
        ...
        preq_match = isinstance(pyreq, str) and re.match('>=([0-9]+\\.[0-9]+)',
            pyreq)
        if preq_match:
            target_version = preq_match.group(1)
        else:
            raise ValueError(
                'lib3to6: missing python_requires=">=X.Y" in setup.py')
        install_requires = sorted(dist._lib3to6_install_requires)
        build_cfg = eval_build_config(target_version=target_version,
            install_requires=install_requires, default_mode=getattr(dist,
    ...
    def run(self) ->None:
        ...
        self.run_3to6()
        self.byte_compile(_build_py.orig.build_py.get_outputs(self,
            include_bytecode=0))
