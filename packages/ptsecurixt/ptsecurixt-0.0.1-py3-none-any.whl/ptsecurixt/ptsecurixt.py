...
class ptsecurixt:
    ...
    def run(self):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Finished", "TITLE", self.use_json))
        ptmisclib.ptprint(ptmisclib.out_if(self.ptjsonlib.get_all_json(), "", self.use_json))

    def get_security_txt(self, path):
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Searching for: {path}", "INFO", self.use_json))
        try:
            ...
            if response.status_code == 200 and "text/plain" in response.headers["content-type"]:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Found security.txt file", "NOTVULN", self.use_json))
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"File contents:", "INFO", self.use_json))
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"{response.text}", "", self.use_json))
                self.data.append({"url": path, "file_contents": response.text})
                self.found_txt_file = True
            else:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Not found", "VULN", self.use_json))
