...
), ids=["DatasetTest214", "SimpleVISPDataset-polarized", "SimpleVISPDataset", "SimpleVBIDataset",
        "SimpleVTFDataset-polarized", "SimpleVTFDataset", "SimpleCRYODataset-polarized",
        "SimpleCRYODataset", "SimpleDLNIRSPDataset-polarized", "SimpleDLNIRSPDataset"])
def test_generate_214(ds):
    ...
    tb = TransformBuilder(sorted_headers)
    wcs = tb.gwcs
    assert isinstance(wcs, gwcs.WCS)

    inv = extract_inventory(table_headers, tb)
