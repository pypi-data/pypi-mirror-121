...
__all__ = ['ValuesRangeKey', 'ExampleValuesKey', 'ValuesKey', 'TimeKey', 'DefaultKey', 'Key', 'Section', 'Schema']
...
class Key:
    ...
    def generate_value(self) -> Any:
        ...
        if self.type is bool:
            ...
            return ''.join(
                self.random.choice(list(string.ascii_uppercase + string.digits)) for _ in range(len_str))
...
def construct_key_from_dict(schema: dict, random=None) -> Key:
    core_keys = {'key', 'type', 'required', 'expected', 'format', 'values', 'values_range', 'example_values'}
...
class Section:
    ...
    def generate(self,
        ...
        section_header = {k.key: k.generate_value() for k in self.keys
                          if not filtering or ((expected_only and k.expected) or (required_only and k.required))
                          }
...
class Schema:
    ...
    @classmethod
    def sections_from_dicts(cls, sections: Iterable[schema_type_hint], random: Any = None):
        """
        Construct a schema from a list of section dicts.
        """
        random = random or cls.random
        return [Section.from_dict(schema_dict, random=random) for schema_dict in sections]
