...
from dkist_data_simulator.dataset import Dataset, key_function, Omit
...
class Spec214Schema(Schema):
    ...
    def __init__(self, naxis: int, dnaxis: int, deaxes: int, daaxes: int, instrument=None, random=None):
...
class Spec214Dataset(Dataset):
    ...
    def observer(self):
        ...
        dkist_hgs = dkist_pos.get_gcrs(date_avg).transform_to(HeliographicStonyhurst(obstime=date_avg))
...
class WCSAMixin:
    ...
    def fits_wcs_a(self):
        ...
        lat_ind = [i for i, ctype in enumerate(new_wcs.wcs.ctype) if ctype.startswith("HPLT")][0]
        lon_ind = [i for i, ctype in enumerate(new_wcs.wcs.ctype) if ctype.startswith("HPLN")][0]
