...
class ResponseStages():
    ...
    def __init__(self, attribute_list, channel_modif={}, selected_config={},
        ...
        :param attribute_list: list of information file dictionaries for
            stages
        ...
        :param selected_config: the configuration description that will
            override or complement default values
        :type selected_config: dict or object of :class:`ObsMetadata`
        :param delay_correction: used only for datalogger, it's the delay
            correction of the whole instrument
        :type delay_correction: float
        """

        if attribute_list is None:
            msg = 'No stages found in information file'
            warnings.warn(msg)
            logger.error(msg)
            raise TypeError(msg)
            ...
            self.stages = [Stage(ObsMetadata(s),
                           channel_modif,
                           selected_config,
                           delay_correction if i == len(attribute_list)-1
                           else 0 if delay_correction is not None else None,
                           i+1)
                           for s, i in zip(attribute_list,
                                           range(0, len(attribute_list)))]
            # last argument is stage sequence #
...
class Stage(object):
    ...
    def __init__(self, attributes_dict, channel_modif_list={},
                 selected_config={},  delay_correction=None,
                 sequence_number=-1):
        """
        ...
        :param delay_correction: used only for datalogger, it's the delay
            correction of the whole instrument
        :type delay_correction: float
        :param sequence_number: sequence number, starting at 1. It will first
                                be assigned within component and then for
                                the whole instrument response
        :type sequence_number: int
        """
        if attributes_dict is None:
            return None

        channel_modif = self.get_selected_modifications(channel_modif_list,
                                                        str(sequence_number))

        name = attributes_dict.get_configured_element('name', channel_modif,
                                                      selected_config, '')
        ...
        self.filter = Filter.dynamic_class_constructor(
            ObsMetadata(attributes_dict.get('filter', None)),
            channel_modif.get('filter', None),
            selected_config, self.name)
        if not self.filter:
        ...
        input_dict = input_units_description = \
        ...
        output_dict = output_units_description = \
            attributes_dict.get_configured_element(
    ...
    def calculate_delay(self):
        ...
        :param delay_correct: used only for datalogger, it's the delay
            correction of the whole instrument
    ...
    def get_selected_modifications(self, modif_dict, key_code):
        ...
        :type key_code: str with special regexp format: "*" or,
            e.g. "[1,2,4]" or "[1-3]"
        """
        ...
        if (isinstance(filt, PolesZeros) or isinstance(filt, Analog)):
            ...
            obspy_stage = FIRResponseStage(
                *args,
                ...
                description=self.description,
                decimation_input_sample_rate=self.input_sample_rate,
                decimation_factor=1 if self.decimation_factor is None
                else self.decimation_factor,
                decimation_offset=0 if filt.offset is None else filt.offset,
                decimation_delay=0 if self.delay is None else self.delay,
                decimation_correction=0 if self.correction is None
