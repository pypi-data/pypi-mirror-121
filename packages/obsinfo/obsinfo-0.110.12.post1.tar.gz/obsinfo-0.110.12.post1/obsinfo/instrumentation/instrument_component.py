...
class InstrumentComponent(object):
    ...
    def retrieve_configuration(comp_type, attributes_dict,
        ...
        all_configurations = attributes_dict.get(
        ...
        if selected_configuration == {}:
            msg = f'Configuration selected "{selected_configuration_key}" '\
                  f'in {comp_type} not found in configuration definitions, '\
    ...
    def dynamic_class_constructor(component_type, attributes_dict,
        ...
        """
        # print(f'InstrumentComponent({component_type=}, attributes_dict=..., {channel_modif=}, {config_selector=})')
        ...
        # print(f'{ObsMetadata(attributes_dict[component_type])=}')
        # print(f'{channel_modif.get(component_type, {})=}')
        ...
        if not obj.config_description:
            obj.config_description = '[config: ' + config_selector \
                                     if config_selector else ''
...
class Sensor(InstrumentComponent):
    ...
    def __init__(self, equipment, seed_band_base_code, seed_instrument_code,
                 response_stages=[], config_description=''):
        """
        Constructor
    ...
    def dynamic_class_constructor(cls, attributes_dict, channel_modif={},
        ...
        if not selected_config:
            selected_config = {}  # Avoids a syntax error in the yaml file: two
                                  # consecutive labels with no response stages
