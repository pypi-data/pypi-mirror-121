...
from pytmosph3r.util.util import get, make_array, timer, spectral_chunks
from pytmosph3r.constants import KBOLTZ, RGP

@numba.njit
def compute_optical_depth_mie(P, T, rays_lengths, cross_section, mie_abs_coeff,  opacity_indices, rays_list, n_rays, n_intersection, tau):
    """Compute tau, the optical depth."""
...
class Transmission(Logger):
    """The transmission module compute the transit depth given a set of :attr:`rays` crossing the atmosphere.
    ...
    def __init__(self, rays=None, store_transmittance=None, identical=None, memory_aware=None, per_angle=True):
        ...
        """Compute only once opacity for cells with same physical properties (True by default). You can use this to your advantage when an atmosphere has multiple cells with the same properties. If your atmosphere is completely heterogeneous however, consider removing this option (searching for identical cells is a waste of time)."""
        ...
        if rays is not None:
            if isinstance(rays, dict):
                self.rays = Rays(grid=rays) # transform dict into class
            else:
                if not isinstance(rays, Rays):
                    self.error("Rays object is not from pytmosph3r. Proceed with care...")
                self.rays = rays # already a class
        self.model = None
    ...
    def default_values(self):
        ...
        self.info("memory_aware = %s ; identical = %s ; per_angle = %s" % (self.memory_aware, self.identical, self.per_angle))
    ...
    def build(self, model):
        ...
        self.rays.set_observer(model.observer)
    ...
    def prepare_opacities(self, *args, **kwargs):
        """Prepare opacities list that need to be computed by exo_k.
    ...
    def _prepare_opacities(self, opacity_coords):
        ...
        Cells with the same physical properties are grouped together to compute their opacity only once.
        """
        self.opacity_indices = np.full(self.atmosphere.shape, np.nan, dtype='i')
        ...
        prepare_aerosols = PrepareAerosols(self.model, self.size)
        ...
        return indices, aer_reff_densities, cell_gas_vmr

    @timer
    def compute_integral(self, *args, **kwargs):
        """Compute integral by iterating over opacities given by :func:`prepate_opacities` and if :attr:`memory_aware`, use :func:`wn_chunks` to subdivide the work along the spectral axis.
         Timed function. Use :func:`compute_contribution` if time is not needed.
        """
        return self.compute_contribution(*args, **kwargs)

    def compute_contribution(self, opacities):
        """Compute integral by iterating over opacities given by :func:`prepate_opacities` and if :attr:`memory_aware`, use :func:`wn_chunks` to subdivide the work along the spectral axis.
        """
        indices = opacities[0]
        ...
        if self.identical:
            data = np.asarray(list(indices.keys())).T
            log_p = data[0]
            temperature = data[1]
            ...
            log_p = indices[0]
            temperature = indices[1]
            gas_vmr = indices[2]

        if self.model.parallel and self.model.parallel.dimension == "spectral":
            integrals = self.model.parallel.compute_integral(self, indices, log_p, temperature, gas_vmr, aer_reff_densities)
        else:
            ...
            for wn_chunk in wn_chunks:
                chunk_integral = self.wn_contribution(indices, log_p, temperature, gas_vmr, aer_reff_densities, wn_chunk)
    ...
    def wn_chunks(self, xsec_size):
        """Subdivide the work along the spectral axis.
        ...
        xsec_shape = (xsec_size, self.opacity.k_data.Nw)
        ...
        if self.memory_aware:
            ...
            try:
                ...
                new_xsec_shape = (xsec_size, int(np.ceil(self.opacity.k_data.Nw/nchunks)))
                if self.opacity.k_data.Ng:
    ...
    def wn_to_transmittance(self, indices, log_p, temperature, gas_vmr, aer_reff_densities, wn_range):
    ...
    def angle_to_integral(self):
        ...
        integral_angle = self.compute_contribution(opacities)
    ...
    def grid_to_transmittance(self, bounds=None):
        """Compute transmittance for rays between bounds.
    ...
    def compute(self, model):
        """Sum all optical depths and compute transit depth (ratio of visible radius of the planet on the radius of the star to the square)."""
        ...
        self.model = model
        self.rays = model.radiative_transfer.rays # may look disturbing, but it's for the parallel version to be synchronized
        ...
        if self.model.parallel and not self.model.parallel.dimension == "spectral":
            ...
            for self.rays.angle_idx, ray_angle in enumerate(tqdm(self.rays.angles, leave=False)):
                integrals.append(self.angle_to_integral())
                if self.store_transmittance:
                    try:
            ...
            integral = self.compute_integral(opacities)
            if self.store_transmittance:
                try:
        ...
        self.info("Transit depth computed")
        ...
        return xk.Spectrum(value, wns=self.opacity.k_data.wns, wnedges=self.opacity.k_data.wnedges)

    def inputs(self):
        return ["rays", "memory_aware", "identical", "per_angle"]
