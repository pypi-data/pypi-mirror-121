...
import requests
import logging
from upswingutil.db.model import ReservationModelV2, Status, ReservationGuestInfo, ReservationGuestShort, GuestsModel
...
def _get_package_info(package):
    ...
    for item in package:
        _package.append({
            ...
            'amount': sum([x['unitPrice'] for x in item.get('scheduleList')]) * item.get('consumptionDetails').get('totalQuantity')
        })
    return _package
...
def _generate_day_to_day_entry(record):
    ...
    for i in range((end_date - start_date).days):
        _rates = next((x for x in record.get('roomStay').get('roomRates') if x['start'] == str((start_date + i * day_delta).date())), None)
        _daily_activity.append({
            ...
            'revenue': {
                'accommodation': (record.get('cashiering').get('revenuesAndBalances').get('roomRevenue').get('amount') / (end_date - start_date).days),
                'accommodationTax': 0,
                'accommodationGST': 0,
                'foodAndBeverage': (record.get('cashiering').get('revenuesAndBalances').get('foodAndBevRevenue').get('amount') / (end_date - start_date).days),
    ...
    def _transform_reservation(self, record):
        reservationV2 = ReservationModelV2(
            ...
            property={
                ...
                'numberOfRooms': record.get('folioInformation').get('reservationInfo').get('roomStay').get('numberOfRooms'),
            ...
            bookingInfo={
                ...
                'travelAgentId': record.get('folioInformation').get('reservationInfo').get('roomStay').get('bookingChannelCode'),
                'travelAgentName': record.get('folioInformation').get('reservationInfo').get('roomStay').get('bookingChannelCode'),
            ...
            financeInfo={
                ...
                'rateTypeName': record.get('folioInformation').get('reservationInfo').get('roomStay').get('ratePlanCode'),
                'totalPoints': record.get('roomStay').get('totalPoints').get('points'),
                'totalSpending': {
                    'amountBeforeTax': record.get('cashiering').get('revenuesAndBalances').get('totalRevenue').get('amount'),
                    'taxAmount': record.get('cashiering').get('revenuesAndBalances').get('totalPayment').get('amount') - record.get('cashiering').get('revenuesAndBalances').get('totalRevenue').get('amount'),
                ...
                'revenue': {
                    ...
                    'foodAndBevRevenue': {
                        'amount': record.get('cashiering').get('revenuesAndBalances').get('foodAndBevRevenue').get('amount')
                    },
                    'otherRevenue': {
                        'amount': 0
                    },
                ...
                'deposit': {
                    'deposit': record.get('folioInformation').get('reservationInfo').get('roomStay').get('depositPayments').get('amount'),
                    'depositRequiredByDate': record.get('reservationPolicies').get('depositPolicies')[0].get('policy').get('deadline').get('absoluteDeadline') if len(record.get('reservationPolicies').get('depositPolicies')) > 0 else None,
                    'secondDeposit': 0,
            ...
            historyEvents=[],
            cancellation=_transform_cancellation_record(record.get('cancellation'), record.get('reservationPolicies').get('cancellationPolicies')),
            housekeeping=[],
            comments=[],
