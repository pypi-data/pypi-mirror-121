        ...
        for slot in intent["slots"]:
            ...
            if slot_types:
                for value in slot.get("values", []):
                    ...
                    values.append(Intent.Slot.Value(
                        body=value.get("body"),
                        type=value.get("type"),
...
def prediction_from_plute(prediction):
    try:
        intent = prediction["graph"]["output"][0][0]
        ...
        for slot in intent.get("slots", []):
            ...
            } for entity in slot.get("values", [])]

            prediction["slot"].append({
                "name": slot.get("name"),
                "type": slot.get("type"),
                "value": entities
            })
        ...
        return json.dumps(py_.flatten(entities_from_all_slots)), json.dumps(prediction)
    ...
    for call_dict in calls_dict:
        ...
        for conversation in tqdm(turn.get("conversations", [])):
            ...
            turn_ = {
                ...
                "utterances": make_utterances(conversation
                    .get("debug_metadata", {})
                    .get("plute_request", {})
                    .get("alternatives")),
