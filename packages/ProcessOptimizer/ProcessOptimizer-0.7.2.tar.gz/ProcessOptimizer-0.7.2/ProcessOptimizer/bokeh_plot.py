...
from bokeh.models import ColumnDataSource, Button, Span, CustomJS
...
Selectors: Sliders that appear on top of the plots with which the red markers
can be adjusted
Toggle_x button group: A group of buttons where we can toggle which parameters
we want to plot.
Generate button: Dependence is only calculated when this buton is pressed
Other buttons: With buttons we can toggle partial dependence on and off,
control the n_points variable (resolution when calculating dependence)
and control the evaluation method
Y values: A small text field which shows the expected value
as well as confidence interval
"""

# Here we define the global variables
result = None
# x_eval_selectors holds the sliders and selectors for parameter values
x_eval_selectors = None
# old_active_list determines which x_eval_selectors were present
# before last update
old_active_list = None
# The 'reds' field is a (NxN) list. The diagonal holds a Span
source = {'reds': []}
# x_eval_selectors_values is a "location" value,
# that determines the location for the red line in the 1d plots.
# The off diagonals
# holds coordinates for the red markers used in the contour plot.
x_eval_selectors_values = None
# max_pars defines the maximum number of parameters that can be plotted
max_pars = None
layout = None
...
def set_globals(parsed_result):
    global button_generate, buttons_toggle_x, dropdown_eval_method
    global slider_n_points, max_pars, source, old_active_list
    global result, x_eval_selectors, layout, x_eval_selectors_values
    global button_partial_dependence, y_value, button_color_map, colorbar
    global button_draw_confidence

    # Here we set the values of the global variables and create the layout
    ...
    dropdown_eval_method = Select(title="Evaluation method:",
                                  value='Result',
                                  options=['Result',
                                           'Exp min',
                                           'Exp min rand',
    ...
    # We initiate colorbar as an empty div first
    # and then change it to a plot in case we want to show it
    colorbar = Div()
    row_plots = row([])
    row_top = row(button_generate, buttons_toggle_x)
    col_right_side = column(button_partial_dependence,
                            button_color_map,
                            button_draw_confidence,
                            dropdown_eval_method,
                            slider_n_points,
                            y_value,
                            colorbar
                            )
...
def handle_button_generate(layout, result):
    ...
    # Remove plots when button is clicked.
    # If we dont do this we ge the "Cant apply patch" bug.
    ...
    if active_list:  # Only plot if there is at least one selection
        ...
        if colorbar_interval:
            colorbar_plot = figure(plot_height=200,
                                   plot_width=100,
                                   tools='',
                                   x_range=[0, 1],
                                   y_range=colorbar_interval)
            im = get_colorbar_as_rgba()
            colorbar_plot.image_rgba(
                image=[im],
                x=0,
                y=colorbar_interval[0],
                dw=1,
                dh=colorbar_interval[1]-colorbar_interval[0])
        ...
        layout.children[0].children[1] = Div(
            text="""<font size="10"><br><br>Let's select
            som parameters to plot!</font>""",
            width=500,
            height=100)
...
def get_x_eval_selectors_values():
    """Description of x_eval_selectors_values

    Returns the values for the x_eval_selectors. Uses the global
    x_eval_selectors_values and for each selector that is present in the GUI
    we replace the corresponding value. Even though a selector isnot present
    in the GUI the last value that it had i still retrived.
...
def get_active_list():
    """Description of get_active_list
...
def get_use_partial_dependence():
    """Description of get_use_partial_dependence

    Returns True or false depending on wether or not the partial dependence
    button is toggled in the GUI
...
def get_plot_list(layout, result, active_list, n_points, x_eval, confidence):
    """Description of get_plot_list

    Returns a NxN list of plots where N is the number of parameters to be
    plotted. The diagonal is 1d plots and the off diagonal is contour plots.
    ...
    if get_use_partial_dependence():
        # Not passing any eval values to dependency function
        # makes it calculate partial dependence
        dependence_eval = None
    else:
        dependence_eval = x_eval

    # First we calculate all the data that will be used for all the plots.
    # After this we draw the plots.
    plots_data = []
    # We need to know the maxmimum and minimum values before drawing the plots,
    # in order to create a correct color-mapping
    plots = []
    space = result.space
    model = result.models[-1]
    bounds = result.space.bounds
    # the iscat variable is a list of bools. True if parameter is categorical.
    # False otherwise. red_vals are the coordinates for the x_eval
    # that is mapped to integers in case they are categorical.
    _, red_vals, iscat = _map_categories(space, result.x_iters, x_eval)
    # We add 0.5 to the value of all categorical integers.
    # This is due to the way bokeh handles plotting of categorical values.
    red_vals = [val+0.5 if iscat[i] else val for i, val in enumerate(red_vals)]
    # While iterating through each plot
    # we keep track of the max and min values of y and z in case of 2d plots.
    # We use these values to set the axis of all plots,
    # to make them the same axis.
    ...
    for i_active in range(len(active_list)):
        ...
        for j_active in range(len(active_list)):
            ...
            current_plot += 1
            # We put the progress into the div
            # where we normally show the y-values (confidence bounds)
            y_value.text = """<font size="5"><b><br> Calculating objective.
                Please wait. <br> """ + \
    ...
    y_value.text = """<font size="5"><b><br> Drawing.
        Please wait. <br> """ + \
        str(int(current_plot)) + " / " + str(int(plots_to_do)) + """</font>"""
    for i_active in range(len(active_list)):
        # Only plot the selected parameters
        source['reds'].append([])
        plots.append([])
        for j_active in range(len(active_list)):
            i = active_list[i_active]
            ...
            if j > i:
                ...
                # Source red is what we end up appending
                # to the global source variable
                # So the location of the red line can be changed interactively
                source_red = Span(location=red_vals[i],
                                  dimension='height',
                                  line_color='red',
                                  line_width=3)
                ...
                if use_same_color_map:
                    ...
                    cmap = None
                # Get an rgba contour image from matplotlib
                # as bokeh does not support contour plots
                im = get_plt_contour_as_rgba(xi,
                                             yi,
                                             zi,
                                             confidence,
                                             cmap=cmap,
                ...
                # x and y samples are the coordinates of the parameter values
                # that have been sampled during the creation of the model.
            ...
            if iscat[i] and i != j:
                ...
                # and therefore should not be rotated.
...
def get_plots_layout(layout,
    ...
    """Description of get_plots_layout

    Create the layout using the lists of plots and selectors.
    The layout consists of rows that we append plots and selectors to. The
    selectors should be on top of each diagonal plot, and therefore the layout
    can be regarded as a (N+1,N) grid to make room for the top selector. The
    top row therefore consists of only on selector and the bottom row has no
    selector as the selector for the last diagonal was added in the second
    last row.
...
def get_x_eval_selectors_list(result, active_list, x_eval):
    ...
    """Description of get_x_eval_selectors_list

    Returns a list of selectors. The selectors are sliders for numerical
    values and dropdown menus ("Select" object) for categorical values. The
    selectors are interactive with callbacks everytime a changed by the user.
    ...
    bounds = result.space.bounds
    # Used for defining what values can be selected
    x_eval_selectors = []
    n = 0
    # Index of the plots. Example: If only parameter 3 and 5 is being plotted
    # the selectors for these parameters still have index n = 0 and n= 1.
    for i in active_list:
        # Only get selecters that is going to be shown in GUI
        if isinstance(result.space.dimensions[i], Categorical):  # Categorical
            ...
            # This object gets a title equal to the parameter number,
            # and the value is set to x_eval
            select = Select(title='X'+str(i),
                            value=x_eval[i],
                            options=cats,
                            width=200,
                            height=45)
            # Here we define a callback that updates the appropiate
            # red markers by changing with the current value of the selector
            # by changing the global "source" variable.
            # The callback function is written in javascript
            select.js_on_change(
                'value',
                CustomJS(args=dict(source=source, n=n, cats=cats), code="""
            ...
            slider = Slider(start=start,
                            ...
                            width=200,
                            height=30)
            # javascript callback function that gets called everytime a user
            # changes the slider value.
            slider.js_on_change(
                'value',
                CustomJS(args=dict(source=source, n=n), code="""
...
def get_colorbar_as_rgba():
    ...
    xdim = X.shape[1]
    ydim = X.shape[0]
    # Converting image so that bokeh's image_rgba can read it.
...
def get_plt_contour_as_rgba(xi,
                            yi,
                            zi,
                            confidence,
                            cmap=None,
                            draw_confidence=None):

    """Description of get_plt_contour_as_rgba

    Returns a matplotlib contour plot as an rgba image. We create a matplotlib
    figure and draws it so we can capture the figure as an image.
...
def get_x_eval(result, active_list):
    # Returns the evaluation values that are defined by
    # the evaluation method dropdown menu
    ...
    if eval_method == 'Result':
        ...
        x = expected_minimum_random_sampling(result,
...
def get_step_size(start, end):
    # Returns the stepsize to be used for sliders
    # the stepsize will always be of the form 10**x
