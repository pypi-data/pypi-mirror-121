...
try:
    from aug_sfutils import sfobj, SFREAD
except:
    from sfread import SFREAD
    import sfobj
...
class EQU:
    ...
    def __init__(self, nshot, **kwargs):
        ...
        if 'diag' in kwargs.keys():
            ...
            diag = kwargs['diagnostic']
        else:
            diag = 'EQH'
        if 'ed' in kwargs.keys():
            ed = kwargs['ed']
        elif 'edition' in kwargs.keys():
            ed = kwargs['edition']
        else:
            ed = 0
        self.sf = SFREAD(nshot, diag, ed=ed)
        ...
        if self.sf.status:
            ...
            if 'exp' in kwargs.keys():
                self.exp = kwargs['exp']
            elif 'experiment' in kwargs.keys():
                self.exp = kwargs['experiment']
            else:
                self.exp = 'AUGD'
            ...
            print('Problems opening %s:%s:%d for shot %d' %(exp, diag, ed, nshot))
        print('Reading equ scalars')
        self.read_scalars()
        print('Reading equ 1d profiles')
        self.read_profiles()
        print('Reading equ PFM')
        self.read_pfm()

        print('COCOS %d' %self.cocos)

        if 'tbeg' is kwargs.keys():
            tbeg = kwargs['tbeg']
        else:
            tbeg = np.min(self.time)
        if 'tend' is kwargs.keys():
            tend = kwargs['tend']
        else:
