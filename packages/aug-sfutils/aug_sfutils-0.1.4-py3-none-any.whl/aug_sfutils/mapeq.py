...
# Trivial case
    ...
    unique_idx, idx =  get_nearest_index(eqm.time, tarr)
...
# COCOS, Eq. 19, 24
    ...
    for i in unique_idx:
        ...
        jt = (idx == i)
        Psi = eqm.pfm[:, :, i]
        Br =  np.gradient(Psi, axis=1)/eqm.Rmesh[:, None]
        Bz = -np.gradient(Psi, axis=0)/eqm.Rmesh[:, None]
...
# Eq 12 in COCOs paper
        Br[jt] = np.gradient(Psi, axis=1)/eqm.Rmesh[:, None]
        Bz[jt] = -np.gradient(Psi, axis=0)/eqm.Rmesh[:, None]
        fBt = interp1d(eqm.pfl[i], eqm.jpol[i], kind='linear', fill_value='extrapolate')
...
# matplotlib's contour creation
        try:
            ...
            print(R.shape, Z.shape, eqm.pfm[:nr, :nz, i].T.shape)
            raise e 
...
def get_q_surf(eqm, qvalue=1, t_in=None, coord_out='rho_tor'):
    ...
    rho_surf = []
    rho_q = rho2rho(eqm, eqm.pfl[unique_idx], t_in=t_in, coord_in='Psi', coord_out=coord_out)

    for i in unique_idx:
        (jt, ) = np.where(idx == i)[0]
        (nrho, ) = eqm.lpfp[i]
        q_surf = np.interp(qvalue, np.abs(eqm.q[i, :nrho]), rho_q[jt, :nrho], left=np.nan, right=np.nan)
        rho_surf.append(q_surf)
