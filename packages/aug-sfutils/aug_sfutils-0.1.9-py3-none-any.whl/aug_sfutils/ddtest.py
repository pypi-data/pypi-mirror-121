...
ts06 = sfread.get_cti_ts06(nshot)
ts06b = rmc.get_ts06()
print('TS06', dev['TS06'] - ts06, dev['TS06'] - ts06b)
...
mirn = sfread.SFREAD(39790, 'MHI')
nbeg = int(1e5)
nend = nbeg + int(1e7)
tmirn   = mirn.getobject('Time'  , nbeg=nbeg, nend=nend)
sigmirn = mirn.getobject('B31-14', nbeg=nbeg, nend=nend)

tmirn2 = time.time()
print('t Mirnov', tmirn2 - tmirn1)

try:
    import matplotilb.pylab as plt
    plt.figure('MirnovC')
    plt.plot(tmirn, sigmirn)
    plt.show()
except:
    print(tmirn.shape, sigmirn.shape, mirn.cache.keys())
