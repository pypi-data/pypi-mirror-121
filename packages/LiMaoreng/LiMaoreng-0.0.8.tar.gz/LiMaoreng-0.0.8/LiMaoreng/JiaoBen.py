...
def suo_ding(name, dianjicishu=1, region=(0, 0, 1920, 1080), confidence=0.7):  # 锁定目标图片并捕捉（点击），区域默认全屏。当函数后有注释时，开头空两行。
    f = False
    f1, f2, f3, f4, n = (0, 0, 0, 0, 0)
    while True:
        ...
        if location:
            ...
            if x1 > f1+1.1*f3 or x1 < f1-0.1*f3:  # 检测第二次点击位置是否不在同一位置，横坐标
                n+=1
            x2 = x1+w
            y2 = y1+h  # 计算得到图片右下角的坐标。
            X = r.randint(int(x1 + 0.1 * w), int(x2 - 0.1 * w))  # 将坐标区域向图片中心靠近，即图片边缘区域不点击。
            Y = r.randint(int(y1 + 0.1 * h), int(y2 - 0.1 * h))  # 获取随机位置X,Y。
            bu_zhuo(X, Y)
        elif n == dianjicishu:
            break
    if f:
