...
class SqliteTable(BaseTable):
    ...
    def db_load(self):
        ...
        if len(files) > 0:
            ...
            downer.down_file_by_id(files[-1]['id'],
    ...
    def db_save(self):
        ...
        downer.ignore_limits()
        downer.login_by_cookie()
        file_name = os.path.join(os.path.dirname(self.db_path),
                                 f'{datetime.now().strftime("%Y%m%d%H%M%S")}-{os.path.basename(self.db_path)}')
        downer.upload_file(file_path=self.db_path, folder_id=self.lanzou_fid, file_name=file_name)
