...
class LanZouCloud(object):
    ...
    def _post_bak(self, url, data, **kwargs):
        try:
            kwargs.setdefault('timeout', self._timeout)
            kwargs.setdefault('headers', self._headers)
    ...
    def _post(self, url, data, **kwargs):
        ...
        for possiable_url in self.all_possiable_urls(url):
            try:
                ...
                logger.warning(f"_post({possiable_url}, {data}) failed, try another domain")
    ...
    def all_possiable_urls(self, lanzouyun_url: str) -> List[str]:
        ...
        base_host_urls = ['pan.lanzoux', 'wws.lanzoux', 'www.lanzoux', 'wwx.lanzoux',
                          'wws.lanzous', 'www.lanzous', 'wwx.lanzous', 'up.lanzoui'
                          ]
        ...
        # ]
        return [lanzouyun_url, *[lanzouyun_url.replace(old_domain, base) for base in base_host_urls]]
    ...
    def down_file_by_url(self, share_url, pwd='', save_path='./download', file_name=None) -> int:
        """通过分享链接下载文件(需提取码)"""
    ...
    def down_file_by_id(self, fid, save_path='./Download', file_name=None) -> int:
        ...
        if info['code'] != CodeDetail.SUCCESS:
            return info['code']
        return self.down_file_by_url(info['url'], info['pwd'], save_path, file_name=file_name)
