...
from pt.ptutils.encoding import decode_json, decode_yaml
from pt.ptutils.undefined import is_undefined
from pt.ptutils.typing import is_string, is_sequence
from pt.ptutils.text import strip_quotes
...
def boolify(x: Any) -> bool:
    ...
    * If `x` is a string, return true when `x.lower()` is one of 'yes', 'on', 'true', 'enabled', or '1'.
    ...
    if isinstance(x, str):
        return x.lower() in ['yes', 'on', 'true', 'enabled', '1']
