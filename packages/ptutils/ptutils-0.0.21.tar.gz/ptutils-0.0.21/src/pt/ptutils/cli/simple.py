...
from pt.ptutils.undefined import UNDEFINED, is_undefined, is_defined, default_if_undefined
from pt.ptutils.typing import is_sequence, is_mapping
from pt.ptutils.encoding import encode_json
from pt.ptutils.punning import listify
from pt.ptutils.logging import getLogger, set_verbosity, DEFAULT_VERBOSITY
...
def _arg_name(args: Tuple[Any], kwargs: Dict[str, Any]) -> str:
    ...
    return _normalize_arg_name(_d[-1])


# ------------------------------------------------------------------------------------------------------------------------
def _resolve(name: Union[str, Callable]) -> str:
    """
    ...
    ------
    TypeError
        When `name` is not a string and not callable.
    """
    if callable(name):
        f = name
        name = f.__name__
        if name.endswith('dummy'):
            return f.__cmd_info__['arg_info'][0]
        else:
            return name
        ...
        raise TypeError(
            f"Object of type '{type(name).__name__}' may not "
            "be used to reference a command, arg, arg_group, or shared_arg."
        )


# ------------------------------------------------------------------------------------------------------------------------
def _resolve_command(name: Union[str, Callable]) -> str:
    """
    Given a string or a function, determine the name the cli simple system will use for lookups
    for commands.
    ...
    TypeError
        When `name` is not a string and not callable.
    """
    return normalize_command_name(_resolve(name))
...
def remove_shared_arg(name: Union[str, Callable]) -> None:
    ...
    """
    name_ = _resolve(name)
    if _resolve(name_) in __ARG_GROUPS__:
        ...
        if kind == "arg":
            ...
            if name not in __ARG_GROUPS__:
                raise Exception(
                    ...
                    "not a registered argument group."
                )
...
# ------------------------------------------------------------------------------------------------------------------------
def use_arg_group(name: str) -> Callable:
    """
...
def arg_group(func):
    ...
    _register_arg_group(name, dummy)
    return dummy
...
def shared_arg(*args, **kwargs) -> None:
    ...
    **kwargs:
    ...
    dummy.__cmd_info__['arg_info'].append(spec)
...
def get_args_parser(
    ...
    """
    Get ath global argument parser for all registered commands and associated arguments.
    ...
    copyright_message : Optional[str]
        The copyright message for the program.
...
def run_command() -> int:
    ...
    try:
        ...
        logger.exception("Error building parser(s).")
        return -1
    ...
    if is_defined(verbosity):
        delattr(args, 'verbosity')
    set_verbosity( DEFAULT_VERBOSITY + default_if_undefined(verbosity, 0) - default_if_undefined(quiet, 0) )
    ...
    for (k, v) in kwargs.items():
        logger.debug(f"  {(k+':'):{mk}} {encode_json(v)}")
    logger.debug('=======================================================')

    # invoke the command
    try:
        command.__cmd_info__['call'](kwargs)
        return 0
