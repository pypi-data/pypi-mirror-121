...
@dataclass
class ServerErrorResponse(SerdeMixin):
    """Server error (matches HTTP 5xx responses)."""

    message: str


@dataclass
class ClientErrorResponse(SerdeMixin):
    """Error dues to the client request (retrying won't help, matches HTTP 4xx responses)."""

    message: str
    # Later on, we'll probably extend this with a type and a stacktrace, because some errors thrown
    # by pandas are hard to understand without the corresponding stacktrace.
