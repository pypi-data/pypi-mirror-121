...
from common_client_scheduler import headers, ClientErrorResponse, ServerErrorResponse

from terality.version import __version__
from terality.exceptions import TeralityServerError, TeralityClientError
    ...
    * retries at the HTTP level
    ...
    def _deserialize_response(r: Response) -> Any:
        ...
        try:
            ...
            raise TeralityServerError(r.text + " " + additional_info) from e

        # Deprecated: the deserialization process can currently return exceptions to indicate that the client should
        # raise an error. We're moving away from this pattern.
        ...
        if isinstance(parsed, ServerErrorResponse):
            additional_info = HttpTransport._basic_info_from_response(r)
            raise TeralityServerError(parsed.message + " " + additional_info)

        if isinstance(parsed, ClientErrorResponse):
            # Avoid noise by not displaying additional info on ClientErrors.
            raise TeralityClientError(parsed.message)
