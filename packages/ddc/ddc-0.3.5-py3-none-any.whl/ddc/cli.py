...
class Cli(object):
    ...
    def meta(self, workdir=None, port=9999, build="apisgarpun/metaplatform:latest", auto_git_add=True, eap=False):
        """
        ...
        :param auto_git_add: bool. Watch files and try add them into git. By default is true
        :param eap: bool. Early access program build. if true, build param rewrite to apisgarpun/metaplatform:eap-latest. By default is false.
        :return:
        """
        if not workdir:
            workdir = self.cwd
        if eap:
            build = "apisgarpun/metaplatform:eap-latest"
        logging.info("Starter Meta. Work dir: " + workdir)
        start_meta(self.cwd, port, build, auto_git_add)
