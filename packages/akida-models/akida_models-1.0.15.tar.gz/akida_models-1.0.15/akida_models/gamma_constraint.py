...
def add_gamma_constraint(model):
    ...
    of huge values for `threshold_fire` or `threshold_fire_step` with a message
    indicating that a value cannot fit in a 20 bit signed or unsigned integer.
    ...
    Args:
        model (tf.keras.Model): the model for which gamma constraints will be
            added.

    Returns:
        tf.keras.Model: the same model with BatchNormalisation layers updated.
    """
