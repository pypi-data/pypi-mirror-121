...
def freeze_model_before(model, freeze_before):
    ...
    Args:
        model (tf.keras.Model): the model to freeze
...
def evaluate_akida_model(model, x, activation):
    ...
    steps = None
    if isinstance(x, tf.keras.preprocessing.image.DirectoryIterator):
    ...
    for batch, label_batch in x:
        ...
        if labels is None:
            labels = label_batch
            pots = pots_batch.squeeze()
        else:
            labels = np.concatenate((labels, label_batch))
            pots = np.concatenate((pots, pots_batch.squeeze()))
