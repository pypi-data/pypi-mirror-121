...
class Client:
    ...
    def get_gc_request_header(self):
    ...
    async def set_active(self):
        ...
        if not is_active or timed_out:
            ...
            if self._email is None:
                try:
                    get_self_user_status_response = await self.get_self_user_status(
                        googlechat_pb2.GetSelfUserStatusRequest(
                    ...
                    return
                try:
                    get_members_response = await self.get_members(
                        googlechat_pb2.GetMembersRequest(
                            request_header=self.get_gc_request_header(),
                            member_ids=[googlechat_pb2.MemberId(
                                user_id=googlechat_pb2.UserId(id=get_self_user_status_response.user_status.user_id.id)
                            )]
                        )
                    )
                except exceptions.NetworkError as e:
                ...
                self._email = (
                    get_members_response.members[0].user.email
                )
    ...
    async def download_attachment(self, url: str, max_size: int) -> Union[bytes, str, str]:
        """
    ...
    async def upload_image(self, image_file, group_id, *, filename=None):
        ...
        res = await self._base_request(upload_url, None, '', image_data, headers=headers, method='PUT')
        ...
        if array[0] == 'noop':
            ...
            if 'data' in array[0]:
                ...
                for body in embedded_bodies:
                    resp.event.body.CopyFrom(body)
                    resp.event.type = body.event_type

                    await self.on_stream_event.fire(resp.event)

                # now restore everything back in case anyone needs to do
                # something with it.
                resp.event.bodies.extend(embedded_bodies)
                resp.event.type = original_type
    ...
    async def _gc_request(self, endpoint, request_pb, response_pb):
        """Send a Protocol Buffer formatted chat API request.
    ...
    async def _base_request(self, url, content_type, response_type, data, headers=None, params=None, method='POST'):
    ...
    async def get_user_presence(self, get_user_presence_request):
    ...
    async def get_members(self, get_members_request):
    ...
    async def paginated_world(self, paginate_world_request):
    ...
    async def get_self_user_status(self, get_self_user_status_request):
        """Return info about the current user.
    ...
    async def create_topic(self, create_topic_request):
    ...
    async def create_message(self, create_message_request):
