        ...
        while True:
            for seq_folder in watch_dir.get_seq_folders():
                try:
                    ...
                    uploader.ua_parameters = self.ua_parameters
...
class DxUploaderBase(object):
    ...
    def upload(self, seq_folder):
        ...
        if self.ua_path and os.path.isfile(self.ua_path):
            with poolcontext(processes=self.process_count) as pool:
    ...
    def upload_file(self, data_file):
        ...
        if self.ua_path and os.path.isfile(self.ua_path):
            file_id = self.ua_upload_file_with_details(data_file, remote_folder, types, properties)
        else:
            dx_file = self.dx_upload_file_with_details(data_file, remote_folder, types, properties)
            file_id = dx_file.get_id()
    ...
    def dx_upload_file_with_details(self, data_file, remote_folder, types, properties):
        dx_file = self.dx_upload_file(data_file, remote_folder)
        if dx_file:
    ...
    def dx_upload_file(self, data_file, remote_folder):
...
class PersonalisDxUploader(DxUploaderBase):
    ...
    def _create_sentinel(self, seq_folder_name):
        return PersonalisSentinel(self._basedir, seq_folder_name)
