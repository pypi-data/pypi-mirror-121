...
import gin
from transformers import AutoModelForSequenceClassification, AlbertForSequenceClassification, \
    BertForSequenceClassification, AutoTokenizer, AutoConfig, AutoModel, BertTokenizer
import numpy as np
import logging
import os
...
class AutoBertForSequenceClassification(nn.Module):
    def __init__(
            self,
            pretrained: str,
            config,
        ...
        super(AutoBertForSequenceClassification, self).__init__()
        ...
        self._model = AutoModel.from_pretrained(self._pretrained, config=self.config)
        if self._task == 'ranking':
            self._dense = nn.Linear(self.config.hidden_size, 1)
        elif self._task == 'classification':
            self._dense = nn.Linear(self.config.hidden_size, self.config.num_labels)
        else:
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor = None,
        ...
        if token_type_ids is not None:
            output = self._model(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        else:
...
class SentenceEncoder():
    def __init__(self, model_name, tokenzier, config, num_labels: int = None, max_length: int = None, device: str = None):
        """
        ...
        :param model: Any model from Huggingface that can be loaded with AutoModel
        :param tokenzier: Any tokenizer from Huggingface that can be loaded with AutoTokenizer
        :param config: Any model config from Huggingface Models Repository that can be loaded with AutoConfig.
    ...
    def fit(self,
        ...
        print("check self.config.num_labels", self.config.num_labels)
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                ...
                # self.best_prediction = pred
                # self.predict_time = pred_time
                if save_best_model:
