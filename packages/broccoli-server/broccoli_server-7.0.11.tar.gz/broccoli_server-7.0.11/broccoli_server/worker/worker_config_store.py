...
class WorkerConfigStore(object):
    ...
    def get_all(self, namespace: str = None) -> Dict[str, WorkerMetadata]:
        ...
        if not namespace:
            ...
            cursor = self.collection.find(
                {"module_name": {"$regex": re.compile("^" + namespace)}}
            )
    ...
    def update_interval_seconds(
        self, worker_id: str, interval_seconds: int
    ) -> Tuple[bool, str]:
        if not self._if_worker_exists(worker_id):
            return False, f"Worker with id {worker_id} does not exist"
        ...
        self.collection.update_one(
            ...
            update={"$set": {"interval_seconds": interval_seconds}},
        )
        return True, ""

    def update_error_resiliency(
        self, worker_id: str, error_resiliency: int
    ) -> Tuple[bool, str]:
        if not self._if_worker_exists(worker_id):
            return False, f"Worker with id {worker_id} does not exist"
        # todo: update_one fails
        self.collection.update_one(
            filter={"worker_id": worker_id},
            update={"$set": {"error_resiliency": error_resiliency}},
        )
        return True, ""
    ...
    def update_args(self, worker_id: str, args: Dict) -> Tuple[bool, str]:
        ...
        # todo: update_one fails
        self.collection.update_one(
            filter={"worker_id": worker_id}, update={"$set": {"args": args}}
        )
        return True, ""
    ...
    def increment_error_count(self, worker_id: str) -> Tuple[bool, str]:
        if not self._if_worker_exists(worker_id):
            return False, f"Worker with id {worker_id} does not exist"
        self.collection.update_one(
            filter={"worker_id": worker_id}, update={"$inc": {"error_count": 1}}
        )
        return True, ""
    ...
    def reset_error_count(self, worker_id: str) -> Tuple[bool, str]:
        if not self._if_worker_exists(worker_id):
            return False, f"Worker with id {worker_id} does not exist"
        self.collection.update_one(
            filter={"worker_id": worker_id}, update={"$set": {"error_count": 0}}
        )
        return True, ""

    def get_error_count(self, worker_id: str) -> Tuple[bool, int, str]:
        document = self.collection.find_one(filter={"worker_id": worker_id})
        if not document:
    ...
    def set_last_executed_seconds(self, worker_id: str, last_executed_seconds: int):
        ...
        self.collection.update_one(
            filter={"worker_id": worker_id},
            update={"$set": {"last_executed_seconds": last_executed_seconds}},
        )
