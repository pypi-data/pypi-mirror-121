...
class Logger(object):
    ...
    def __init__(self,filename,level='info',when="D", backCount=180, fmt='%(asctime)s - %(levelname)s: %(module)s %(lineno)d %(message)s'):
    ...
    @classmethod
    def get_logger(cls, filename, **kwargs):
        if filename not in cls.loggers:
            cls.loggers[filename] = Logger(filename, **kwargs)
