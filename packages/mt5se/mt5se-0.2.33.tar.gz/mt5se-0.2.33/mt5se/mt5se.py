...
def order_type(x):
    """
    Returns a string correspondig to the order type (or unkonw). 
    Possible values and Meaning:
     'BUY' - Market Buy order
   'SELL' -  Market Sell order
    'LIMTED_BUY' -     Buy Limit pending order
    'LIMITED_SELL' -    Sell Limit pending order
...
def order_reason(x):
    """
    Returns a string correspondig to the order reason (or unkonw)
"""
    if x==0:
