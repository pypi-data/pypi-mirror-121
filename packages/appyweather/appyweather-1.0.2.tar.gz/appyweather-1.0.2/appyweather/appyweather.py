...
class Weather:
    ...
    temperature units (optional). If no units (metric / imperial) are given, kelvin units are applied as the default
    temperature units convention.
    """
    ...
    def next_24_hours(self):
        """
        Returns full 3-hour weather data for the next 24 hours as a dict.
        """
        return self.data['list'][:9]

    def next_24_hours_basic(self):
        """
        Returns 3-hour basic weather data (datetime, temperature, sky condition, sky condition icon code)
