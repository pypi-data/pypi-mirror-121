...
class Job:
  ...
  def update_from_jobrow(self, jobrow):
    ...
    self._set_correct_ncpus(int(jobrow.ncpus))
    self.run_id = jobrow.run_id
  
  def _set_correct_ncpus(self, jobrow_ncpus):
    # type : (int) -> None
    if jobrow_ncpus > 0 and jobrow_ncpus != self.ncpus:
      self.ncpus = jobrow_ncpus
...
class SimJob(Job):
  ...
  def set_years_per_sim(self, years_per_sim):
    # type : (float) -> None
    if years_per_sim and self.section == util.JobSection.SIM:
