...
class DrbNode(DrbItem, NodeImpl, abc.ABC):
    ...
    def get_named_child(self, name: str, namespace_uri: str = None,
                        occurrence: int = None) -> Union[DrbNode,
                                                         List[DrbNode]]:
        """
        Returns an occurrence(s) of child matching a specific name. The
        occurrence number starts from 1. If occurrence is not provided: the
        list of children is returned.
        The name is a local name without any prefix.
        :param name: The name to match.
        :param namespace_uri: child namespace URI
        :param occurrence: The occurrence number of the desired child.
        :return: A reference to the children or DrbException exception when
           occurrence is outside the list range.
        """
        raise NotImplementedError
    ...
    def get_first_child(self) -> Optional[DrbNode]:
        """
        Reference to the first child of the current node. If the current
        node has no child a null reference if returned. The children order
        depends on the implementation. This operation time performance
        depends therefore from the implementation. However,
        the implementations shall try to prefer the firstChild()
        performances against the lastChild() as far as possible.
        :return: A reference to the first child or None if no such node.
        """
        raise NotImplementedError
    ...
    def get_last_child(self) -> Optional[DrbNode]:
        """
        reference to the last child of the current node. If the current node
        has no child a null reference if returned. The children order
        depends on the implementation. This operation time performance
        depends therefore from the implementation. However,
        the implementations shall try to prefer the firstChild()
        performances against the lastChild() as far as possible.
        :return: A reference to the last child or None if no such node.
        """
        raise NotImplementedError
    ...
    def get_child_at(self, index: int) -> DrbNode:
        """
        Reference to a specific child. The child is identified by its
        position (i.e. index or sequence number) in the children list of
        this node. The children order depends on the implementation. This
        operation time performance depends therefore of this
        implementation.The provided index starts from 0 and shall not be
        ...
        and not greater or equal to the number of children. Negative position
        means start from end.
        :return: A reference to specified child or
           DrbException exception if no such node.
        """
        raise NotImplementedError
    ...
    def get_children_count(self) -> int:
        ...
        current node. According to the implementation the computation of
        children number may require more or less time. The number of
        children may actually require to parse a big file if it is not
        constant among document instances. It is therefore recommended to
        use this operation only if required. It is preferable to browse the
        tree from first children to last using getFirstChild() and
        getNextSibling() operations. If the current node has no child,
        0 is returned.
