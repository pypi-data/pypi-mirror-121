...
class ScriptRuntimeHandler(CondaRuntimeHandler):
    ...
    def _translate_env(self, runtime: ScriptRuntime) -> dict:
        ...
        if runtime.entrypoint:
            # Entrypoint must include the name of the directory
            # Add the directory to entrypoint if the user did not include it.
            dir_name = os.path.basename(runtime.source_uri.rstrip("/"))
            if dir_name != runtime.entrypoint.split("/", 1)[0]:
                runtime.with_entrypoint(os.path.join(dir_name, runtime.entrypoint))
