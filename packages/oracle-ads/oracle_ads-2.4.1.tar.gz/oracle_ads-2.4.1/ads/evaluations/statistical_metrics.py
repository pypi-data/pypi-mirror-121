...
class ModelEvaluator:
    ...
    def _get_binary_metrics(self):
        ...
        self.metrics['balanced accuracy'] = metrics.balanced_accuracy_score(self.y_true, self.y_pred)
        self.metrics['precision'] = metrics.precision_score(self.y_true, self.y_pred,
                                                            pos_label=self.positive_class)
        self.metrics['recall'] = metrics.recall_score(self.y_true, self.y_pred,
                                                      pos_label=self.positive_class)
        self.metrics['f1'] = metrics.f1_score(self.y_true, self.y_pred,
                                              pos_label=self.positive_class, average='binary')
    ...
    def _get_multiclass_metrics(self):
        ...
        self.metrics["precision_weighted"] = metrics.precision_score(
            y_true, y_pred, self.classes, average="weighted"
        )
        self.metrics["precision_micro"] = metrics.precision_score(
            y_true, y_pred, self.classes, average="micro"
        )
        self.metrics["precision_by_label"] = metrics.precision_score(
            y_true, y_pred, self.classes, average=None
        ).tolist()
