...
class BaseCart(
    ...
    def __init__(
        ...
        self._user = get_collection("user").find_one({"owner": identity})
        self._cache_cart = self._load_cart(identity, user=self._user)
    ...
    def _load_cart(self, identity: str, user: Any) -> MutableMapping[str, Any]:
        cart = get_collection("cart").find_one({"owner": identity})
        if not cart:
            cart = {
                "owner": identity,
                **self.INITIAL_CART,
                **self.config_cls.validate_optional(self._load_cart_extra(user)).dict(),
            }
    ...
    @abc.abstractmethod
    def _load_cart_extra(self, user: Any) -> Dict[str, Any]:
        raise NotImplementedError("_load_cart_extra")
