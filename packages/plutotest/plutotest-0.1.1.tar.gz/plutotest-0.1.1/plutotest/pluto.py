...
class Facebook(PlutoObject):
    ...
    def analyse2(self, img=None):
        ...
        test = expand_to_rows((255 - to_grayscale(img.copy())), True, 10)
        ...
        for i in range(1, len(slices), 2):
            ...
            for r in range(len(temp)):
                if stop: continue
                for p in range(len(temp[r])):
                    if stop: continue
                    minpix, maxpix = np.min(temp[r][p]), np.max(temp[r][p])
    ...
    def engagement_str(self, ocr: str):
        ...
        print(comsplit, sharesplit, viewsplit)
    ...
    def classify(self, img=None):
        ...
        net.load_state_dict(torch.load("FB models/test_2.pt"))
...
class NYT(PlutoObject):
    ...
    def analyse(self, img=None):
        ...
        top, color_images, bottomnp = self.slice(analyse_img)
        ...
        bottom = np.array(bottom, np.uint8)
...
class Discord(PlutoObject):
    ...
    def analyse(self, img=None):
        ...
        exptr = expand_to_rows(img.copy(), False, 65)
...
class FBM(PlutoObject):
    ...
    def io_classification(self, img=None):
        ...
        net.load_state_dict(torch.load("fbm2.pt"))
    ...
    def slice(self, img=None): #  --> List
        ...
        test = expand_to_rows(to_grayscale(img), True, 5)
...
def watest():
    net = ConvNet(3, 6, 12, 100, 50, 2)
    print(os.listdir())
    net.load_state_dict(torch.load("wa1.pt"))
...
if __name__ == "__main__":
    try:
        ...
        if arg_c == "NYT":
            nyt = NYT(img)
            if arg_o != None:
                nyt.to_json(None, arg_o)
            result = nyt.analyse()
        elif arg_c == "Tagesschau":
            result = Tagesschau(img).analyse()
        elif arg_c == "Facebook":
            result = Facebook(img).analyse()
