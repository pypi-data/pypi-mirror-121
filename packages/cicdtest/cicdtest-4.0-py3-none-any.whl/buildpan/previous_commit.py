...
def prev_commit(path, repo_name, project_id, username, provider):
    try:
        ...
        if provider == "github":
            ...
            result = subprocess.run(["git", "-c", f"http.extraHeader=Authorization: Bearer {token}", "checkout", "FETCH_HEAD"], stdout= subprocess.PIPE, stderr = subprocess.STDOUT, cwd=path)
