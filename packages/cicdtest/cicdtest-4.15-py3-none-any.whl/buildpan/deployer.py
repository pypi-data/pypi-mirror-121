    ...
    try:
        ...
        if result_install.returncode:
            ...
            if process_fails:
                ...
                add = status + 20
                msg = status[add:160]
                requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=deployment = '+str(message)+'&status = '+ str(msg) +'&operation=deployment')
                return success
    except Exception as e:
