    ...
    try:
        ...
        if result_install.returncode:
            ...
            if process_fails:
                ...
                success = True
                requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=deployment = '+str(message)+'&status=success&operation=deployment')
                return success
    except Exception as e:
