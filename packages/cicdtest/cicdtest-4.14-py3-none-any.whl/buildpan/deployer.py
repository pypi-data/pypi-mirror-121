    ...
    try:
        ...
        if result_install.returncode:
            ...
            if result_install_pm2.returncode:
                ...
                try:
                    ...
                    if file_found.returncode:
                        ...
                        file_found = subprocess.run(popen_arg, stdout = subprocess.PIPE, stderr = subprocess.STDOUT,cwd=cwd)
                        if file_found.returncode:
                            requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=main file not found'+'&status=failed&operation=deployment')
                            return success
                        else:
                            app_name = "index.js"
                    else:
                ...
                if service_status.stdout.decode().find("online") == -1:
                    popen_arg = ['pm2', 'start', app_name, '--name', project_id, "--namespace", "buildpan"]
            ...
            if process_fails:
                ...
                success = True
                show = subprocess.run(["pm2", "show", project_id], stdout= subprocess.PIPE, stderr = subprocess.STDOUT)
                decode = show.stdout.decode()
                status = decode.find("online")
                if status != -1:
                    requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=deployment = '+str(message)+'&status=online&operation=deployment')
                else:
                    requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=deployment = '+str(message)+'&status=errored&operation=deployment')
                return success
    except Exception as e:
