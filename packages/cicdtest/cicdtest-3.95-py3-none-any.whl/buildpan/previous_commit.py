...
def prev_commit(path, repo_name, project_id, username, provider):
    try:
        ...
        if provider == "github":
            ...
            requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=Pull'+'&status=success&operation=previous commit')
            ...
            result = subprocess.run(["git", "-c", f"http.extraHeader=Authorization: Bearer {access_token}", "checkout", "FETCH_HEAD"], cwd=path)

            requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=pull operation performed : '+str(result.stdout.decode())+'&status=success&operation=previous commit')
