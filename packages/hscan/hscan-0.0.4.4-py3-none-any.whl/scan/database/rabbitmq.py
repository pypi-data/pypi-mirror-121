...
class RabbitMQ:
    ...
    async def publish(self, data, routing_key, priority=0):
        """
        ...
        try:
            async with self.channel_pool.acquire() as channel:
                ...
                for _ in range(3):
                    try:
                        await channel.default_exchange.publish(aio_pika.Message(body=data.encode(), priority=priority), routing_key=routing_key)
                        return
                    except Exception as e:
    ...
    async def message_count(self, queue_name):
        try:
            async with self.channel_pool.acquire() as channel:
                for _ in range(3):
                    try:
