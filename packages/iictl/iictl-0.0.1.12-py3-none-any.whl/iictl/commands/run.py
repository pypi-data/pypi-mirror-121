...
@click.option('--name', type=str, help='Instance Name.', required=True) # TODO: create with generated name
...
@click.option('-e', '--env', type=str, multiple=True, help='Environment variables. NAME=VALUE format.') # name=value
@click.option('-v', '--volume', type=str, multiple=True, help='Volume mount. VOLUME_NAME:MOUNT_PATH format.') # pvcname:mountpath:ro
@click.option('--domain', type=str, multiple=True, help='Assign domain to port. PORT:DOMAIN format.') # port:domain
