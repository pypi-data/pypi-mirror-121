...
class Sharepoint:
    ...
    def create_file(self, headers, data, drive_id, folder_path, filename, folder_id=None, overwrite=True):
        """:param headers: used for authentication, parsed from the result of the bearer_token function
        ...
        file_content = ''
        try:
            result = requests.get(f'{self.graph_url}/drives/{drive_id}/root:/{full_rel_path}',
                                  headers=headers)
            if result.status_code == 200:
                file_exists = 1
                ...
                if folder_id is None:
                    # Get the folder ID
                    result = requests.get(f'{self.graph_url}/drives/{drive_id}/root:/{folder_rel_path}',
                                          headers=headers)
            ...
            if file_exists == 1 and overwrite:
                ...
                result = requests.put(
                    f'{self.graph_url}/drives/{drive_id}/items/{folder_id}:/{file_rel_path}:/content'
                    , headers=headers
                    , data=data
                )
