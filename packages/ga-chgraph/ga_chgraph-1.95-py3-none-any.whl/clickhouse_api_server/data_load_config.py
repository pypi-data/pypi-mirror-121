...
def load_config():
    ...
    db_user = os.getenv("DB_USERNAME") if os.getenv("DB_USERNAME") else \
    clickhouse_config["dbuser"]
    db_pwd = os.getenv("DB_PASSWORD") if os.getenv("DB_PASSWORD") else\
    ...
    clickhouse_connect = {"ip": clickhouse_ip, "port": clickhouse_port, "user": clickhouse_user, "password": clickhouse_pwd}
