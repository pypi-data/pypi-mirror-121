...
class DiskHashTree:
    def __init__(self, root: str, min_prefix_size: int=2):
        ...
        '''
        # Ensure the trailing slash on the directory
    ...
    def __iter__(self, current_root=None):
        ...
        glob_results = glob.glob(current_root + '*')

        dir_list = list(util.filter_valid_directories(glob_results))

        for d in dir_list:
            yield from self.__iter__(current_root + d + '/')
    ...
    def load_meta(self):
        ...
        '''
        with open(self.root + '.meta', 'r', encoding='utf-8') as fp:
            meta = json.load(fp)
    ...
    def write_meta(self):
        ...
        with open(self.root + '.meta', 'w', encoding='utf-8') as fp:
            json.dump(meta, fp)
    ...
    def add(self, key: str, current_root: str=None):
        ...
        try:
            ...
            # This will be the absolute path of hte next subdirectory
            next_root = '{}{}/'.format(current_root, key_prefix_rel)
        except IndexError:
            # If we reached the limit of subdirectories for the length of the
            # key, then we simply store the key here.
            with open(current_root + key, 'w', encoding='utf-8') as fp:
                return current_root + key
        ...
        if bool(colliding_files):
            ...
            for f in colliding_files:
                os.rename(current_root + f, new_subdir + f)

            # And finally, emplace the key
            with open(new_subdir + key, 'w', encoding='utf-8') as fp:
                return new_subdir + key
        else:
            # Emplace the key
            with open(current_root + key, 'w', encoding='utf-8') as fp:
                return current_root + key
    ...
    def is_empty(self):
        ...
        '''
        glob_results = glob.glob(self.root + '*')
        file_list = list(util.filter_valid_files(glob_results))
        dir_list = list(util.filter_valid_directories(glob_results))
