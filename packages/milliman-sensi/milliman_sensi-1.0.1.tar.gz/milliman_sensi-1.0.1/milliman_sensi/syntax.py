...
def parse_param(param_string):
    ...
    if re.findall(r'(?<=[^=])=(?=[^=]+)', param_string):
        param_string, param_value = re.split(r'(?<=[^=])=(?=[^=]+)', param_string)[:2]

    param_string = param_string.strip('"')

    # Checks if the strings starts with 'file::'
    if param_string.startswith("file::"):
        param_string = param_string[len("file::"):]
        ...
        if re.findall(r'(\[[^\]]*\])$', param_expression):
            param_expression, param_col = re.split(r'(?=(\[[^\]]*\])$)', param_expression)[:2]
            # param_col = param_col.strip("[]")
        else:
            # print("Didn't find the column in {}".format(param_expression))
            pass
