...
class DisnakeTogether:
    ...
    Parameters
    ----------
    client/bot: Union[:class:`disnake.Client`, :class:`disnake.AutoShardedClient`, :class:`commands.Bot`, :class:`commands.AutoShardedBot`]
    ...
    def __init__(self, client: Union[Client, Bot, AutoShardedClient, AutoShardedBot], *, debug: Optional[bool] = False):
        ...
        if isinstance(debug, bool):
            ...
            self.debug = False
            print('\033[93m'+"[WARN] (disnake_together) Debug parameter did not receive a bool object. Reverting to Debug = False."+'\033[0m')
        ...
        if option and (str(option).lower().replace(" ", "") in defaultApplications.keys()):
            ...
            #Error Handling
                ...
                if e.code == 10003 or "channel_id: snowflake value" in e.text:
                    ...
                    raise ConnectionError(f"[status: {e.status}] (code: {e.code}) : An unknown error occurred while retrieving data from disnake API.")
