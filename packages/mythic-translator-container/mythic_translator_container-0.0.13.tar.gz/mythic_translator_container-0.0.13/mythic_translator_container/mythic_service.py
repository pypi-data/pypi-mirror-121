...
def mythic_service(debugSetting):
    global debug
    debug = debugSetting
    try:
...
def heartbeat_loop(debugSetting):
    global debug
    debug = debugSetting
    try:
        hostname = settings.get("name", "hostname")
        ...
        print_flush("[-] Exception in heartbeat_loop, exiting: {}".format(str(f)))
        sys.exit(1)
...
def start_service_and_heartbeat(debug: bool = False):
    ...
    thread1 = threading.Thread(target=mythic_service, args=(debug,))
    thread2 = threading.Thread(target=heartbeat_loop, args=(debug,))
    thread1.start()
    thread2.start()
    loop.run_forever()
    thread1.join()
