...
class DBClient:
    ...
    def init_defenses(self, transaction_id: str, engine_id: str) -> Dict[str, dict]:
        defenses: List = self.GetAllDefense(transaction_id, engine_id)
    ...
    def GetDefense(self, transactionid: str, uuid: str, subcode: str, node: str) -> (str, str, str):
        if subcode.strip() == "":
            ...
            query = f"SELECT excerpt, defenseuuid FROM demodb.defense WHERE uuid='{uuid}' AND subcode='{node}' AND nodename='{node}'"
    ...
    def GetAllDefense(self, transactionid: str, uuid: str) -> Optional[List[Dict[str, str]]]:
