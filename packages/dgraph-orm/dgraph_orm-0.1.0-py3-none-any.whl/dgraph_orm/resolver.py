...
class Resolver(BaseModel, T.Generic[Model]):
    ...
    class GetInput(Input):
        ...
        def min_one_arg(cls, values: dict):
            if not any(values.values()):
                raise GQLException(
                    f".get requires one field to be given of {list(cls.__fields__.keys())}"
                )
            return values

    model: T.ClassVar[T.Type[Model]]
    children_resolvers: Children
    ...
    def get_params_and_fields(self, input: Input) -> str:
        return f"{input.to_str()}{self.gql_fields_str()}"

    def _get(self, input: GetInput) -> T.Optional[Model]:
        query_name = f"get{self.model.__name__}"
        s = f"{{ {query_name}{self.get_params_and_fields(input=input)} }}"
        print(s)
    ...
    def first(self, _: int) -> Resolver:
        self.query_input.first = _
        return self

    def offset(self, _: int) -> Resolver:
