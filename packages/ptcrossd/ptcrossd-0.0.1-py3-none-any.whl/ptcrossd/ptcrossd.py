...
class ptcrossd:
    ...
    def run(self):
        ...
        ptmisclib.ptprint(ptmisclib.out_if(self.ptjsonlib.get_all_json(), condition=self.use_json))
    ...
    def proccess_crossdomain(self, response):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot("File contents:\n", "TITLE", self.use_json))
        ptmisclib.ptprint(ptmisclib.out_ifnot(ptmisclib.get_colored_text(response.text, "ADDITIONS"), "", self.use_json))
        vulnerable = None
        ...
        if acf_elements:
            for acf in acf_elements:
                if acf.attrib["domain"] and acf.attrib["domain"] == "*":
                    vulnerable = True
            if vulnerable:
                ptmisclib.ptprint(ptmisclib.out_ifnot("file is vulnerable to open policy", "VULN", self.use_json))
                self.ptjsonlib.set_vulnerable(self.json_no, "True")
            else:
                ptmisclib.ptprint(ptmisclib.out_ifnot("no vulnerabilities found", "NOTVULN", self.use_json))
                self.ptjsonlib.set_vulnerable(self.json_no, "False")
        else:
            ptmisclib.ptprint(ptmisclib.out_ifnot("allow-access-from element not found", "INFO", self.use_json))
            self.ptjsonlib.set_vulnerable(self.json_no, "False")
