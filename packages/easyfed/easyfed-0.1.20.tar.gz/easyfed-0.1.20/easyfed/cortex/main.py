...
class client():
    def __init__(self,clientName,host,type='plain'):
    ...
    def submit(self,model,omodel=None,loss=None):
        if self.type=='plain' and omodel is None:
            omodel=model
        req=get_status(self.host,self.key,self.type)
        state,clientstatus,url=req['result'],req['clientstatus'],req['modelurl']
        save_path='' 
        while int(state)!=0:
            if int(clientstatus)==0:
                send_model(self.host,self.key,self.type,model,loss)
            req=get_status(self.host,self.key,self.type)
