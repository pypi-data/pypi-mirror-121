...
def render(info, template=None, format=None, headers=None, fragment=False,
    ...
    if isinstance(info, dict):
        if info.get('tg_template'):
            info.setdefault('tg_format', 'default')
            format = info['tg_format']
        ...
        if 'tg_flash' not in info:
            tg_flash = _get_flash()
            if tg_flash:
                info['tg_flash'] = tg_flash

        if config.get('tg.empty_flash', True):
            info.setdefault('tg_flash', None)
...
def _get_flash():
    ...
    if 'tg_flash' in response_cookie:
        ...
        # If there was an old tg_flash value in the request cookie, clear it;
        # its value is overridden by the new flash in the response
        if 'tg_flash' in request_cookie:
            clearcookie()
    elif 'tg_flash' in request_cookie:
        message = \
            request_cookie.value_decode(request_cookie['tg_flash'].value)[0]
