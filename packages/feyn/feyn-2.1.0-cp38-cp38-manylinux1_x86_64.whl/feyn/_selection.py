...
def _remove_duplicates(models):
    keep = [True]*len(models)
    for i in range(len(models)):
        for j in range(i+1, len(models)):
            equation1 = models[i].sympify(symbolic_lr=False, include_weights=False)
            equation2 = models[j].sympify(symbolic_lr=False, include_weights=False)
            if equation1 - equation2 == 0:
                keep[j] = False

    selected_models = [model for (i, model) in enumerate(models) if keep[i]]
