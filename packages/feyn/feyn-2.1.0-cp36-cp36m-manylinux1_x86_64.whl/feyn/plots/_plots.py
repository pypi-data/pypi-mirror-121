...
def plot_confusion_matrix(
    ...
    Returns:
        matplotlib.axes.Axes -- matplotlib confusion matrix
    """
    ...
    if not len(y_true) == len(y_pred):
        raise ValueError(f"the lengths of y_true {len(y_true)} and y_pred {len(y_pred)} do not match.")
    ...
    if filename:
        return _save_plot(plot_confusion_matrix, filename, fig, y_true=y_true, y_pred=y_pred)
...
def plot_segmented_loss(
    ...
    Returns:
        matplotlib.axes.Axes -- the axes for plotting.
    """
    if by is None:
        by = model.target
    elif by not in data:
        raise ValueError(
            "Argument `by` not in data"
        )
...
def _save_plot(plotting_function, filename, figure=None, **kwargs):
