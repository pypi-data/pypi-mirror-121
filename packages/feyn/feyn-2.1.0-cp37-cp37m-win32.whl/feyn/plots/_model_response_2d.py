...
def plot_partial2d(
    ...
    1. A colored background indicating the response of the model in a 2D space given the fixed values.
...
def plot_model_response_2d(
    ...
    Raises:
        ...
        ValueError: if fixed contains a feature not in the model inputs.
    """
    if len(np.unique(model.inputs)) != len(model.inputs):
        warnings.warn(
            "One should not use plot_model_response_2d when a categorical variable has multiple inputs. The plotted boundary in this case is unreliable."
        )

    if len(np.unique(model.inputs)) < 2:
        raise ValueError(
...
class PartialPlot2D:
    ...
    def __init__(
        ...
        all_features = set(filter(None, model.names[1:]))
        plot = sorted(list(all_features.difference(self.fixed.keys())))
    ...
    def _replace_registers(self):
        ...
        new_fnames = []
        new_params = []
        for param, fname in zip(self.model.params, self.model.fnames):
            if "categories" in param:
                new_params.append({"scale": 1, "w": 1, "bias": param["bias"]})
...
def _validate_fixed(model, fixed):
    ...
    if not len(np.unique(model.inputs)) - len(fixed) == 2:
