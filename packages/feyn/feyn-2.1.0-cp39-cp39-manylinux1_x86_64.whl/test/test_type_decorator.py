...
@pytest.mark.skipif(sys.version_info >= (3, 7), reason="Tests specific for 3.6")
class TestTypeDecorator36(unittest.TestCase):
    def setUp(self):
        exclude = []
    ...
    def test_types_are_not_enforced(self):
        self.assertTrue(function_to_decorate("whatever wrong type", 1337, "bad type", 1336, "Not a list", "etc"))
...
class TestTypeDecorator(ErrorTestCase):
    ...
    def test_check_types_handles_untyped_params(self):
        ...
        with self.subTest("Can handle positional arguments with untyped in between"):
            self.assertTrue(
                self.decorated_function(1, "Hello", True, 1, ['Hello'])
            )
    ...
    def test_check_types_handles_iterables(self):
        ...
        with self.subTest(
            ...
            with self.assertRaisesTypeErrorAndContainsParam("iterable_param"):
                self.decorated_function(1, iterable_param=quickmodels.get_unary_model(["x"], "y"))

    def test_check_types_handles_feyn_models(self):
        self.assertTrue(
            self.decorated_function(1, model_param=quickmodels.get_unary_model(["x"], "y")),
        ...
        with self.subTest("Other types should not be accepted"):
            ...
            with self.assertRaisesTypeErrorAndContainsParam("model_param"):
                self.decorated_function(1, model_param="_feyn.Model (1 interactions)")

    def test_check_types_handles_lists_of_feyn_models(self):
        self.assertTrue(
            self.decorated_function(1, list_model_param=[quickmodels.get_unary_model(["x"], "y")]),
        ...
        with self.subTest("Other lists should not be accepted"):
            ...
            with self.assertRaisesTypeErrorAndContainsParam("list_model_param"):
                self.decorated_function(
                    1, list_model_param=["_feyn.Model (1 interactions)"]
                )

            with self.assertRaisesTypeErrorAndContainsParam("list_model_param"):
                self.decorated_function(1, list_model_param=set([quickmodels.get_unary_model(["x"], "y")]))
    ...
    def test_check_types_handles_dictionaries_with_union_values(self):
        self.assertTrue(
            self.decorated_function(1, dict_with_union_param={"Hello": 1, "World": 2., "Other": "Value"}),
            "Dict should be allowed",
        )
        ...
        with self.subTest("But should react to bad types in the dict"):
            with self.assertRaisesTypeErrorAndContainsParam("dict_with_union_param"):
                self.decorated_function(1, dict_with_union_param={"Hello": [1], "World": [2]})
    ...
    def test_check_types_can_handle_callables(self):
        ...
        self.assertTrue(self.decorated_function(1, typed_callable_param=lambda x: x), "The typed callable_params are actually not evaluated or enforced")
        self.assertTrue(self.decorated_function(1, typed_callable_param=untyped_func), "The typed callable_params are actually not evaluated or enforced")
    ...
    def test_check_types_dict_with_iterable(self):
        ...
        with self.subTest("Doesn't accept bad types"):
            ...
            with self.assertRaisesTypeErrorAndContainsParam("dict_with_iterable"):
                self.decorated_function(1, dict_with_iterable={1: [1, "not_right_type"]})