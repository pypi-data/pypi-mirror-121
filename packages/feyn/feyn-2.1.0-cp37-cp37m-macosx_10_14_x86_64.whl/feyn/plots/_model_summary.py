...
def plot_model_summary(
    ...
    gtk = _calculate_and_add_summary_information(gtk, model, dataframe, compare_data, labels)
    ...
    if filename:
        ...
        if file_ext and file_ext != ".html":
            raise ValueError(f"Extension of filename must be .html but was {file_ext}")

        filename = filename+".html"
        with open(filename, "w") as fd:
            fd.write(output_html)
