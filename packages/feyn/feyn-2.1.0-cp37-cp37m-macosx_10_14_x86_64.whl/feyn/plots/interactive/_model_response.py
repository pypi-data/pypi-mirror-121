...
def interactive_model_response_1d(model: feyn.Model, data: DataFrame, input_constraints: Dict[str, Union[Iterable, float, str]] = None):
    ...
    free_features = _get_free_features(unique_model_features, input_constraints)
    by0 = free_features[0]
    fixed_combinations = _input_constraints_to_their_value_combinations(
        data[unique_model_features], by0, input_constraints, cat_list
    )
    ...
    kwargs = {}
    kwargs['by'] = free_features

    return interactive(_update, **kwargs)


def _get_free_features(model_features, input_constraints):
    if input_constraints is None:
        input_constraints = {}
    free_features = list(set(model_features) - set(input_constraints.keys()))
