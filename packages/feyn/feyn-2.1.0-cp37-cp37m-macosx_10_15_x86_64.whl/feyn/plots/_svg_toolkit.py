...
class SVGGraphToolkit:
    ...
    def _add_bold_text(self, parent, text, start, end, insert=(0,0), size='medium'):
        ...
        bold_width = Theme._get_string_length(bold_part, size=size)
        start_width = 0
        ...
        if start == 0:
            ...
            start_part = text[:start]
            start_width = Theme._get_string_length(start_part, size=size)
            ...
            self._add_txt(
                parent,
                bold_part,
                insert=(x_pos + start_width, y_pos),
                font_size=size,
                font_weight="bold"
            )
        if len(text) > end:
            ...
            self._add_txt(
                parent,
                end_part,
                insert=(x_pos+start_width+bold_width, y_pos),
                font_size=size,
            )
    ...
    def add_input_table(self, model):
        ...
        for idx, name in enumerate(inputs):
            if name in rendered_inputs:
                continue
            rendered_inputs.append(name)
    ...
    def add_graph(self, model, label=None, show_loss=True, show_sources=False):
        ...
        trunc_features = truncate_input_names(model.features)
        for ix, elem in enumerate(model):
            ...
            # The node text
            if elem.name in model.features:
                idx = model.features.index(elem.name)
                trunc_name = trunc_features[idx]
            elif elem.name == model.target:
                trunc_name = truncate_input_names([model.target])[0]
            else:
