...
class IOCParser(object):
    ...
    def parse_iocs(self, text, defang=False, whitelist_regex='', iocs_to_parse=None):
        if iocs_to_parse is not None:
            if len(iocs_to_parse) == 1 and iocs_to_parse[0] == 'all':
                iocs_to_parse = IOCTyper.IOC_TYPES.copy()
            for ioc in iocs_to_parse:
                if ioc not in IOCTyper.IOC_TYPES:
                    raise InvalidInput(f"valid IOCs are: {', '.join(IOCTyper.IOC_TYPES)}", "IOCs to parse", ioc)
        else:
            iocs_to_parse = IOCTyper.IOC_TYPES.copy()

        result = {}
        for key in IOCTyper.IOC_TYPES:
            result[key] = []
        ...
        for ioc in text_chunks:
            typ = ioc_typer.type_ioc(ioc, iocs_to_parse)
            if typ != "unknown":
                result[typ].append(ioc)
            for test in self.possible_entries(ioc):
                typ = ioc_typer.type_ioc(test, iocs_to_parse)
                if typ != "unknown":
...
$                                    #end of line""", re.VERBOSE | re.UNICODE)
    ...
    def type_ioc(self, ioc, types_to_find=None):
        iocs = types_to_find if types_to_find is not None else IOCTyper.IOC_TYPES
