...
class PyAvataaar(object):
    ...
    def random(self):
        ...
        for k, _ in self.__dict__.items():
            if k in __attrs:
                self.__dict__[k] = random.choice(list(__attrs[k]))

        self.render_png_file('/tmp/pyavatar.png')
        self.render_svg_file('/tmp/pyavatar.svg')
        print('avatar exported to /tmp/pyavatar.png(svg)')
    ...
    def __template_path(path: str, enum_type: AvatarEnum) -> str:
        return str(
            pathlib.PurePosixPath(path).joinpath(
                f"{enum_type.name.lower()}.svg"))

    @staticmethod
    def __template_name(context):
        template_name = getattr(context, '_TemplateReference__context',
    ...
    def __simplify_ids(self, rendered_template):
        ...
        id_list_multi = {
            key: value
            for key, value in Counter(id_list).items() if value > 1
        }
        if id_list_multi:
    ...
    def __render_svg(self):
        env = Environment(loader=PackageLoader('dofast.pyavatar', 'templates'),
                          trim_blocks=True,
                          lstrip_blocks=True,
                          autoescape=True,
    ...
    def unique_id(self) -> str:
        return "".join([
            f'{y.value:02x}' for x, y in sorted(vars(self).items())
            if isinstance(y, AvatarEnum)
        ])
    ...
    def unique_id(self, value: str) -> None:
        ...
        for idx, (key, param_value) in enumerate({
                x: y
                for x, y in sorted(vars(self).items())
                if isinstance(y, AvatarEnum)
        }.items()):
