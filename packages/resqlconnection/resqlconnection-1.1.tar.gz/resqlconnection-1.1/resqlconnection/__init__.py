from resqlconnection import methods, sql_connection
...
class resqlconnection:
    ...
    def __init__(self, server_name, db_name):
        ...
        """
        try:
            self.sql_conn = sql_connection.SQLConnection(server_name, db_name)
            self.method = methods
            print("Connection created")
            self.method=methods.Method(255)
        except Exception as e:
            print(str(e))
            raise Exception(f"something went wrong with creating a connection with {server_name} and {db_name}")
    ...
    def preview_table(self, table_name, schema_name, nr_of_lines=5):
        ...
        """
        return self.method.preview_table(table_name, schema_name, nr_of_lines, self.sql_conn)

    def push_data(self, table_name, schema_name, input_data, column_name_list, value_columns,
                  crawldate_column='CrawlDate', replace=False,
                  ignore_duplicates_check=False, recipients=None):
        """
        ...
        """
        return self.method.push_data(table_name, schema_name, input_data, column_name_list, value_columns,
                                     crawldate_column, replace, ignore_duplicates_check, self.sql_conn, recipients)
    ...
    def remove_duplicates(self, table_name, schema_name, replace, value_columns):
        ...
        :return:
        """
        return self.method.remove_duplicates(table_name, schema_name, replace, value_columns, self.sql_conn)
    ...
    def get_newest_date(self, table_name, schema_name, date_column_name):
        ...
        """
        return self.method.get_newest_date(table_name, schema_name, date_column_name, self.sql_conn)
