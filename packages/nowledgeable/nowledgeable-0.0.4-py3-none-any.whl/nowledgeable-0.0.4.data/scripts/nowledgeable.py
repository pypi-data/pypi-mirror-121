...
from external_exercice_tools import *
from custom_parser import create_custom_parser
...
'''.format(command_description, available_commands, command_helps)
    ...
    if args.command == RUN_CHECK_COMMAND:
        ...
        command_parser.add_argument(
            'yaml_path', nargs=1, help=texts["yaml_path"][lang])

        command_args = command_parser.parse_args(sys.argv[2:])

        yaml_path = command_args.yaml_path[0]
        run_checks(yaml_path)

    elif args.command == commands[1]:
        command_parser, lang = create_custom_parser({
            "main_description": available_commands[commands[1]]
        }, "python3 nowledgeable.py {}".format(commands[1]), "Nowledgeable Standalone Tester v.{}".format(VERSION))
        command_parser.add_argument(
            'yaml_path', nargs=1, help=texts["yaml_path"][lang])
        ...
        prepareNotebook(command_args.teacher_path[0],
...
'''.format(texts["mode_desc"][lang], texts["mode_list"][lang],
        ...
        command_args = command_parser.parse_args(sys.argv[2:])
        testNotebook(command_args.teacher_path[0],
