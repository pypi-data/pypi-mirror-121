...
def test_notebook(reference_nb_path, student_nb_path, lang="en", mode="cell"):
    ...
    if os.path.isfile(abs_student_path):
        p = subprocess.Popen(["jupyter nbconvert --to notebook --inplace --execute {}".format(os.path.basename(student_nb_path))], shell=True,
                             cwd=os.path.dirname(abs_student_path))
        p.communicate()
