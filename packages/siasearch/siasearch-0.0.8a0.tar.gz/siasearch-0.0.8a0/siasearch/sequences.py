...
from .util import filter_primary_camera, parse_query_url, URLString
...
class Sequence:
    ...
    def __init__(self, session: Session, drive_id: str, start_timestamp: pd.Timestamp, end_timestamp: pd.Timestamp):
        ...
        self._frames = dict()  # camera_name => list of frames
    ...
    def sensor_names(self):
        ...
        Returns:
            The sensor names of this sequence, organized by modality, for example:
            >>> self.sensor_names()
            {'cameras': [{'name': 'camera_00', 'is_primary': True}, {'name': 'camera_back', 'is_primary': False}]}
        """
    ...
    def frames(self, camera_name: Optional[str] = None):
        ...
        camera_name = camera_name or filter_primary_camera(self.sensor_names()["cameras"])
...
class Sequences(AbstractSequence):
    ...
    def sensor_names(self):
        """Get available sensor names for the first Sequence all sequences
        ...
        url_filled = URLS.sensor_names.replace("<drive_id>", first_sequence_drive_id)
        return self._session.get(url_filled, format="json")
