...
class Frames(AbstractSequence):
    ...
    @classmethod
    def from_sequences(cls, sequences, camera_name: str):
        ...
        response = sequences._session.post(URLS.frames_url, json={"camera_name": camera_name, "df_clips": json_df})
        df_results = pd.DataFrame.from_dict(response.json())
        df_results.rename(
            columns={"timestamps": "timestamp", "img_paths": "image_fpath", "drive_ids": "drive_id"}, inplace=True
        )
    ...
    def subsample(
        self,
        num_samples_per_drive: int = None,
        min_time_between_samples_per_drive: float = None,
    ):
