...
class HttpClient:
    ...
    async def delete(
        self, url: str, timeout: int = DEFAULT_TIMEOUT_SEC, retry_on_failure: bool = True,
    ) -> Optional[dict]:
    ...
    async def _request(
        self, method: str, url: str, data: Optional[dict] = None, timeout: int = DEFAULT_TIMEOUT_SEC
    ) -> Optional[dict]:
    ...
    def _handle_http_error(status: HTTPStatus, content: Optional[dict] = None):
        ...
        if short_message is not None:
            raise AporiaError(short_message=short_message, verbose_message=verbose_message)
        ...
        # This should cover authentication and routing errors.
            ...
            raise AporiaError(HTTP_ERROR_MESSAGES_BY_STATUS[status])
        ...
        # Bad gateway requires a more verbose error message, so we handle it separately
            ...
            raise AporiaError(f"Unexpected HTTP error {status}")

    @staticmethod
    def _serialize(data: dict) -> bytes:
        try:
            return orjson.dumps(
                data, option=orjson.OPT_SERIALIZE_NUMPY, default=orjson_serialize_default_handler,
            )
