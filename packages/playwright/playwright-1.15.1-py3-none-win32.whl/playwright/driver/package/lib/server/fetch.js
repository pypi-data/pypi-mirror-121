...
class FetchRequest extends _instrumentation.SdkObject {
  ...
  async fetch(params) {
    try {
      ...
      let postData;
      if (['POST', 'PUSH', 'PATCH'].includes(method)) postData = params.formData ? serilizeFormData(params.formData, headers) : params.postData;else if (params.postData || params.formData) throw new Error(`Method ${method} does not accept post data`);
...
function serilizeFormData(data, headers) {
  ...
  if (contentType === 'application/json') {
    ...
    (_headers$_contentType = headers[_contentType = 'content-type']) !== null && _headers$_contentType !== void 0 ? _headers$_contentType : headers[_contentType] = contentType;
    return Buffer.from(json, 'utf8');
  } else if (contentType === 'application/x-www-form-urlencoded') {
    const searchParams = new URLSearchParams();

    for (const [name, value] of Object.entries(data)) searchParams.append(name, String(value));
