...
def enrichmentAnalysis(gmtfile, genelist, ranked, tab_out, custom):
    if ranked == False:
...
def main():
    ...
    parser.add_argument("--db", required=True, help="Option of selecting the database.")
    ...
    resulttable = enrichmentAnalysis(gmtfile=gmt, genelist=glist, ranked=args.ranked, tab_out=args.tab_out, custom=args.custom)
    #else:
