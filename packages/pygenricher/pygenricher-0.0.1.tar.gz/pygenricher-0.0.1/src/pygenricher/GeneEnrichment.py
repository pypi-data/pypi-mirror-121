...
def enrichmentAnalysis(gmtfile, genelist, gmtdf, ranked, tab_out, custom):
    if ranked == False:
    ...
    if tab_out == True:
        ...
        try:
            ...
            print("Directory ", dirName, " already exists")
        listnames.to_csv("~/Desktop/pygenricher/src/pygenricher/" + dirName + "enrTable.csv")

    return listnames

def plotting(table):
    fig = px.scatter(table.sort_values(by='Adjusted P-value', ascending=False), x='-log(p)', y=table.sort_values(by='Adjusted P-value', ascending=False).Name.str.split(';', expand=True)[0], size="size",
...
def main():
    ...
    #if args.custom == True:
    gmtfiledf = processGmt(args.gmt)
    #else:
    #    gmtfiledf = args.db
    gene_list = preProcess(args.gene_list)
    glist = genelistGen(gene_list)
    #if args.custom == True:
    resulttable = enrichmentAnalysis(gmtfile=args.gmt, genelist=glist, gmtdf=gmtfiledf, ranked=args.ranked, tab_out=args.tab_out, custom=args.custom)
    #else:
