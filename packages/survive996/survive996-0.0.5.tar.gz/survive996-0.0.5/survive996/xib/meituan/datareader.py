# coding:utf-8
import pandas as pd
from .metadata.table import metadata


def read_plaintext(path, product, table_name=None, names=None, dtype=None, skip_rows=0, iterator=False, chunk_size=None, **kwargs):
    '''
    Parameters
    ----------
    dtype
    ...
    Returns:same as read_csv
    -------

    '''

    if table_name is None:
        table_name = "_".join(path.replace("\\", "/").split("/")[-1].split("_")[:-1])
    if dtype is None:
        dtype = metadata["dtype"]
    if names is None:
        names = metadata[product][table_name]
    return pd.read_csv(path, skiprows=skip_rows, encoding="utf-8", sep="\|\+\|", engine="python", header=None, names=names, dtype=dtype, iterator=iterator, chunksize=chunk_size, **kwargs)


class MT:

    def __init__(self, path, table_name=None, iterator=False, chunk_size=10000, skip_rows=1):
        self.path = path
        if table_name is None:
            self.table_name = "_".join(self.path.replace("\\", "/").split("/")[-1].split("_")[:-1])
        else:
        ...
        self.STRLIZED_COLS = {}
        self.KEYVALUE_META = {}
        self.datareader = load_data

    def load_data(self):
        if self.table_name is not None:
            self.table_name = "_".join(self.path.replace("\\", "/").split("/")[-1].split("_")[:-1])
        return pd.read_csv(self.path, skiprows=self.skip_rows,
                           encoding="utf-8", sep="\|\+\|", engine="python", header=None,
                           names=self.METADATA[self.table_name], iterator=self.iterator, chunksize=self.chunk_size)


class MTF(MT):

    def __init__(self, path, table_name=None, iterator=False, chunk_size=10000, skip_rows=1):
        super(__class__, self).__init__(path, table_name=table_name, iterator=iterator, chunk_size=chunk_size,
                                        skip_rows=skip_rows)
        # bank_period_create,bank_instmnt_init两张表在美团的基础上新增借据编号
        self.METADATA[
...
class MTM(MT):
    ...
    def __init__(self, path, table_name=None, iterator=False, chunk_size=10000, skip_rows=1):
        ...
        self.METADATA[
            ...
            ",")
        self.METADATA["bank_period_create"] = "客户号,借据号,期数单编号,分期开始日期,计划项账单日,计划项还款日,应还本金,应还利息,应还罚息".split(",")
        self.METADATA[
            "bank_repay_period"] = "客户号,借据号,交易流水号,期数单号,还款类型,还款日期,总金额（单位分）,本次实还正常本金（单位分）,本次实还正常利息金额（单位分）,本次实还逾期罚息金额（单位分）,本次实还逾期本金（单位分）".split(
            ",")
        self.METADATA["bank_loan_calc"] = "借据号,正常利息（单位分）,逾期本金罚息（单位分）,交易发生时间,入账状态".split(",")
        self.METADATA["bank_repay_change"] = "借据号,变动类型,日期,本金发生额,利息发生额,罚息发生额".split(",")
        self.METADATA[
        ...
        self.METADATA[
            ...
            ",")
        self.METADATA["fund_fee"] = "借据号,本金余额,贴息费率,贴息发生额".split(",")
        ...
        self.KEYVALUE_META = {"借据类型": {8: "普通消费", 1: "账单延期", 2: "账单分期", 7: "订单分期", 5: "备用金取现"}, "还款方式": {
            1: "等额本息", 2: "等本等息", 3: "信用付到期还本", 4: "到期一次性还本付息（取现）", 6: "先付息到期一次性还本（延付）"},
                              "还款类型": {1: "主动还款", 2: "退款还款", 3: "转循环"}}
...
if __name__ == "__main__":
    ...
    # df = MLF().load_data("E:\\Documents\\Scripts\\20201023_美团等本等息\\acc_2035-05-01\\2035-09-01\\bank_instmnt_init_20350901", "bank_instmnt_init")
    # print(df)
    ...
    # print(MLF.METADATA["bank_repay_loan"])
