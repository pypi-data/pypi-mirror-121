...
from pyrasgo.primitives import Collection, FeatureList, DataSource
...
class Publish():
    ...
    def execute_transform(
        ...
        utilities: Optional[List[api.TransformUtility]] = None
    ):
        # pull datasource and transform to ensure they exist before passing to execute transform
        data_source = self.get.data_source(id=data_source_id)
        transform = self.get.transform(transform_id=transform_id)

        if " " in new_source_name:
            raise ValueError(f"The new source is going to be a table. Please pass a name without whitespace")
        transform_exe = api.ExecuteTransform(
            arguments=arguments,
            dimensions=dimensions,
        ...
        response = self.api._post(f"/transform/{transform_id}/execute", transform_exe.dict(exclude_unset=True), api_version=1).json()
