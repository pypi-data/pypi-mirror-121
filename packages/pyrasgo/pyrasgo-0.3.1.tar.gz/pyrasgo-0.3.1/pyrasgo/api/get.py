...
class Get():
    ...
    def transforms(self):
        ...
        return api.Transforms(transforms=[api.Transform(**r) for r in response])
    ...
    def transform_utilities(self):
        """Returns the list of transform utilities available to a user
        """
        response = self.api._get(f"/transform/utility", api_version=1).json()
        print(f"utilities: {response}")
        return([api.TransformUtilityDefinition(**r) for r in response])
