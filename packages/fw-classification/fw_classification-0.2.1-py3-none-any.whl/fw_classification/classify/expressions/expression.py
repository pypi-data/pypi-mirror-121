...
class Add(ActionExpression):
    ...
    def validate(
        ...
        **kwargs,
        ...
        if "allow-duplicate" in kwargs and not isinstance(
            kwargs["allow-duplicate"], bool
        ):
            err.append(
                "Add allow-duplicate must be boolean, found "
                f"'{kwargs['allow-duplicate']}'"
            )
        return err

    def apply(self, i_dict: Dotty) -> bool:
        """Evaluate action."""
        duplicate = "allow-duplicate" in self.config and self.config["allow-duplicate"]
