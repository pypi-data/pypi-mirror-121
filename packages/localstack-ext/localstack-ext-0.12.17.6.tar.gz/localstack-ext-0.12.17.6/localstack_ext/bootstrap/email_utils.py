...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qOadG()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qOado(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qOadA,from_name=qOadA,smtp_host=qOadA,smtp_user=qOadA,smtp_pass=qOadA,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qOadP(recipients,qOadE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qOadA):
 ...
 result["From"]=formataddr((qOado(Header(from_name,"utf-8")),from_email))
 return result
