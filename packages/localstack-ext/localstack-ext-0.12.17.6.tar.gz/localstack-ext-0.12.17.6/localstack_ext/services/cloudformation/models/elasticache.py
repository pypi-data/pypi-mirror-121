...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=kHDPy(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not kHDPf
   return result
  except kHDPa:
   props=self.props
   ...
   if parts[0]in candidates and kHDPL(parts)>1:
