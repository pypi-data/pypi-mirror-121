...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Ugnbp
 @Ugnbh
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return Ugnbc(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @Ugnbx
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if UgnbY(origins,Ugnbs)else origins.get("Items")
  origins_to_create=Ugnbl([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if UgnbY(aliases,Ugnbs)else aliases.get("Items")
  aliases_to_create=Ugnbl(aliases)
  for distr in distrs:
   origins=Ugnbl([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=Ugnbl(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Ugnbm)
   config["Enabled"]=Ugnbq(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if UgnbY(custom_errors,Ugnbk)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Ugnbq(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Ugnbm}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Ugnbq(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Ugnbp):
    parent=config if parent is Ugnbp else parent
    items=parent.get(key)or[]
    if UgnbY(items,Ugnbk)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Ugnbz(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if UgnbY(cache_behavs,Ugnbk)else cache_behavs)
