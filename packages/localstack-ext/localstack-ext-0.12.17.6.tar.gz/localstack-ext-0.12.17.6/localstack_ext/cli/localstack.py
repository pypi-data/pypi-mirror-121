...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return DFMYR if e else DFMYm
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->DFMYa:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(DFMYa)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:DFMYO):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,DFMYa)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:DFMYO):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,DFMYa)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[DFMYO,DFMYO],rows:List[Dict[DFMYO,Any]],formatters:Dict[DFMYO,Callable[[Any],DFMYO]]=DFMYa):
 from rich.table import Table
 if formatters is DFMYa:
  formatters=DFMYB()
 t=Table()
