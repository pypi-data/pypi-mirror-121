...
SENT_EMAILS=[]
EMAIL_BLACKLIST=pEqDK()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",pEqDR(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=pEqDJ,from_name=pEqDJ,smtp_host=pEqDJ,smtp_user=pEqDJ,smtp_pass=pEqDJ,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if pEqDw(recipients,pEqDW)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=pEqDJ):
 ...
 result["From"]=formataddr((pEqDR(Header(from_name,"utf-8")),from_email))
 return result
