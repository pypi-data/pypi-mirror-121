...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=UAlzB(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not UAlzm
   return result
  except UAlzS:
   props=self.props
   ...
   if parts[0]in candidates and UAlzi(parts)>1:
