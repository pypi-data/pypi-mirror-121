...
def canonicalize_name(name:hoVlI)->hoVlI:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=[z for z in result if z["Name"]==canonicalize_name(self.props.get("Name"))]
  return(result or[hoVle])[0]
 def get_physical_resource_id(self,attribute=hoVle,**kwargs):
  return self.props.get("Id")
 def get_cfn_attribute(self,attribute_name):
  if attribute_name=="NameServers":
   return[self.props.get("Name")]
  return hoVlf(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @hoVlm
