...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return HBUbj if e else HBUbg
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->HBUbp:
...
@click.option("--revert",is_flag=HBUbj,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:HBUbI):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(HBUbp)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:HBUbt):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,HBUbp)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:HBUbt):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,HBUbp)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[HBUbt,HBUbt],rows:List[Dict[HBUbt,Any]],formatters:Dict[HBUbt,Callable[[Any],HBUbt]]=HBUbp):
 from rich.table import Table
 if formatters is HBUbp:
  formatters=HBUbu()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  HBUbl().__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->HBUbp:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->HBUbp:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->HBUbp:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->HBUbp:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
