...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=RPaIN(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not RPaIO
   return result
  except RPaIc:
   props=self.props
   ...
   if parts[0]in candidates and RPaIM(parts)>1:
