...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or NuwJM
 @NuwJn
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return NuwJT(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @NuwJj
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if NuwJi(origins,NuwJp)else origins.get("Items")
  origins_to_create=NuwJk([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if NuwJi(aliases,NuwJp)else aliases.get("Items")
  aliases_to_create=NuwJk(aliases)
  for distr in distrs:
   origins=NuwJk([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=NuwJk(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",NuwJh)
   config["Enabled"]=NuwJs(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if NuwJi(custom_errors,NuwJU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=NuwJs(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":NuwJh}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=NuwJs(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=NuwJM):
    parent=config if parent is NuwJM else parent
    items=parent.get(key)or[]
    if NuwJi(items,NuwJU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":NuwJz(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if NuwJi(cache_behavs,NuwJU)else cache_behavs)
