...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=BGWgD,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if BGWgL(perm["IpProtocol"])!=BGWgL(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",BGWgD)
   vpc_id=BGWgD
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or BGWgD
    vpc_id=groups and groups[0].get("VpcId")or BGWgD
   dst_group_id=result.pop("DestinationSecurityGroupId",BGWgD)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=BGWgL(result.get("IpProtocol"))
   description=result.pop("Description",BGWgD)
   cidr_ipv6=result.pop("CidrIpv6",BGWgD)
