...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Omzdp()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",Omzdv(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Omzdo,from_name=Omzdo,smtp_host=Omzdo,smtp_user=Omzdo,smtp_pass=Omzdo,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OmzdT(recipients,OmzdR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Omzdo):
 ...
 result["From"]=formataddr((Omzdv(Header(from_name,"utf-8")),from_email))
 return result
