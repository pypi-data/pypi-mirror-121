...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SpAot()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SpAoV(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SpAoq,from_name=SpAoq,smtp_host=SpAoq,smtp_user=SpAoq,smtp_pass=SpAoq,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SpAoO(recipients,SpAoU)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SpAoq):
 ...
 result["From"]=formataddr((SpAoV(Header(from_name,"utf-8")),from_email))
 return result
