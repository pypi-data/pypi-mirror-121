...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or NIrEQ
 @NIrEG
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return NIrEM(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @NIrEn
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if NIrEB(origins,NIrEU)else origins.get("Items")
  origins_to_create=NIrEL([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if NIrEB(aliases,NIrEU)else aliases.get("Items")
  aliases_to_create=NIrEL(aliases)
  for distr in distrs:
   origins=NIrEL([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=NIrEL(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",NIrEm)
   config["Enabled"]=NIrEq(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if NIrEB(custom_errors,NIrEh)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=NIrEq(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":NIrEm}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=NIrEq(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=NIrEQ):
    parent=config if parent is NIrEQ else parent
    items=parent.get(key)or[]
    if NIrEB(items,NIrEh)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":NIrEx(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if NIrEB(cache_behavs,NIrEh)else cache_behavs)
