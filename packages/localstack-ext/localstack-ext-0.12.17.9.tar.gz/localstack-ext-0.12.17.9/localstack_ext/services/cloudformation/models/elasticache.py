...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ALHiI(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ALHib
   return result
  except ALHiK:
   props=self.props
   ...
   if parts[0]in candidates and ALHiw(parts)>1:
