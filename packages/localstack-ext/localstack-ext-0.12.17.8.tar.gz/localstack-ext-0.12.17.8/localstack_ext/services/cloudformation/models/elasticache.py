...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ueqYl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ueqYh
   return result
  except ueqYN:
   props=self.props
   ...
   if parts[0]in candidates and ueqYO(parts)>1:
