...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or lUPfN
 @lUPfd
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return lUPfs(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @lUPfw
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if lUPfy(origins,lUPfx)else origins.get("Items")
  origins_to_create=lUPfc([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if lUPfy(aliases,lUPfx)else aliases.get("Items")
  aliases_to_create=lUPfc(aliases)
  for distr in distrs:
   origins=lUPfc([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=lUPfc(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",lUPft)
   config["Enabled"]=lUPfT(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if lUPfy(custom_errors,lUPfG)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=lUPfT(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":lUPft}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=lUPfT(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=lUPfN):
    parent=config if parent is lUPfN else parent
    items=parent.get(key)or[]
    if lUPfy(items,lUPfG)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":lUPfR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if lUPfy(cache_behavs,lUPfG)else cache_behavs)
