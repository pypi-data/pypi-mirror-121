...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PSaVL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PSaVm(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PSaVr,from_name=PSaVr,smtp_host=PSaVr,smtp_user=PSaVr,smtp_pass=PSaVr,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PSaVk(recipients,PSaVy)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PSaVr):
 ...
 result["From"]=formataddr((PSaVm(Header(from_name,"utf-8")),from_email))
 return result
