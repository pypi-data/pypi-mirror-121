...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return bCRnf if e else bCRna
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->bCRnX:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(bCRnX)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:bCRnB):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,bCRnX)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:bCRnB):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,bCRnX)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[bCRnB,bCRnB],rows:List[Dict[bCRnB,Any]],formatters:Dict[bCRnB,Callable[[Any],bCRnB]]=bCRnX):
 from rich.table import Table
 if formatters is bCRnX:
  formatters=bCRnW()
 t=Table()
