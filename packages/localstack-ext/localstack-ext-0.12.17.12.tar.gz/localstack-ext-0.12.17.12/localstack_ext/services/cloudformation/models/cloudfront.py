...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or fdiYT
 @fdiYA
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return fdiYG(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @fdiYO
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if fdiYy(origins,fdiYD)else origins.get("Items")
  origins_to_create=fdiYk([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if fdiYy(aliases,fdiYD)else aliases.get("Items")
  aliases_to_create=fdiYk(aliases)
  for distr in distrs:
   origins=fdiYk([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=fdiYk(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",fdiYs)
   config["Enabled"]=fdiYa(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if fdiYy(custom_errors,fdiYM)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=fdiYa(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":fdiYs}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=fdiYa(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=fdiYT):
    parent=config if parent is fdiYT else parent
    items=parent.get(key)or[]
    if fdiYy(items,fdiYM)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":fdiYu(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if fdiYy(cache_behavs,fdiYM)else cache_behavs)
