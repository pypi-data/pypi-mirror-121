...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=jyFtV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not jyFtI
   return result
  except jyFtB:
   props=self.props
   ...
   if parts[0]in candidates and jyFti(parts)>1:
