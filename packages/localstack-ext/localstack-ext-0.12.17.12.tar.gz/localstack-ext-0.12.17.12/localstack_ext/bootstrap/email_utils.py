...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Qgoel()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",QgoeT(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=QgoeI,from_name=QgoeI,smtp_host=QgoeI,smtp_user=QgoeI,smtp_pass=QgoeI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if QgoeX(recipients,Qgoed)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=QgoeI):
 ...
 result["From"]=formataddr((QgoeT(Header(from_name,"utf-8")),from_email))
 return result
