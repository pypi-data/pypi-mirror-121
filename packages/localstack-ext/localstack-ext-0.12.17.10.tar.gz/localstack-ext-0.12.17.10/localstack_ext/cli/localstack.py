...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return atKYd if e else atKYC
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->atKYJ:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(atKYJ)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:atKYP):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,atKYJ)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:atKYP):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,atKYJ)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[atKYP,atKYP],rows:List[Dict[atKYP,Any]],formatters:Dict[atKYP,Callable[[Any],atKYP]]=atKYJ):
 from rich.table import Table
 if formatters is atKYJ:
  formatters=atKYm()
 t=Table()
