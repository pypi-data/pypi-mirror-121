...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or KfFna
 @KfFng
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return KfFnI(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @KfFnT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if KfFnl(origins,KfFne)else origins.get("Items")
  origins_to_create=KfFnj([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if KfFnl(aliases,KfFne)else aliases.get("Items")
  aliases_to_create=KfFnj(aliases)
  for distr in distrs:
   origins=KfFnj([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=KfFnj(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",KfFnL)
   config["Enabled"]=KfFno(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if KfFnl(custom_errors,KfFnV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=KfFno(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":KfFnL}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=KfFno(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=KfFna):
    parent=config if parent is KfFna else parent
    items=parent.get(key)or[]
    if KfFnl(items,KfFnV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":KfFnq(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if KfFnl(cache_behavs,KfFnV)else cache_behavs)
