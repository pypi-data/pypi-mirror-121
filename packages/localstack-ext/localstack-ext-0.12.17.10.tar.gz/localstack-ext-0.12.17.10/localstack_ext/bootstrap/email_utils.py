...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PqFsn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PqFsJ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PqFsC,from_name=PqFsC,smtp_host=PqFsC,smtp_user=PqFsC,smtp_pass=PqFsC,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PqFsa(recipients,PqFsM)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PqFsC):
 ...
 result["From"]=formataddr((PqFsJ(Header(from_name,"utf-8")),from_email))
 return result
