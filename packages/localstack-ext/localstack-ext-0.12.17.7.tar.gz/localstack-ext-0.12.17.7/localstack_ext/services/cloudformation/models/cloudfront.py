...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or AMfRn
 @AMfRp
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return AMfRH(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @AMfRo
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if AMfRs(origins,AMfRu)else origins.get("Items")
  origins_to_create=AMfRE([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if AMfRs(aliases,AMfRu)else aliases.get("Items")
  aliases_to_create=AMfRE(aliases)
  for distr in distrs:
   origins=AMfRE([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=AMfRE(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",AMfRt)
   config["Enabled"]=AMfRL(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if AMfRs(custom_errors,AMfRT)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=AMfRL(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":AMfRt}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=AMfRL(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=AMfRn):
    parent=config if parent is AMfRn else parent
    items=parent.get(key)or[]
    if AMfRs(items,AMfRT)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":AMfRb(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if AMfRs(cache_behavs,AMfRT)else cache_behavs)
