...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=kbgcm(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not kbgcC
   return result
  except kbgcq:
   props=self.props
   ...
   if parts[0]in candidates and kbgcT(parts)>1:
