...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Eunwl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",EunwA(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Eunwr,from_name=Eunwr,smtp_host=Eunwr,smtp_user=Eunwr,smtp_pass=Eunwr,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if EunwR(recipients,Eunwz)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Eunwr):
 ...
 result["From"]=formataddr((EunwA(Header(from_name,"utf-8")),from_email))
 return result
