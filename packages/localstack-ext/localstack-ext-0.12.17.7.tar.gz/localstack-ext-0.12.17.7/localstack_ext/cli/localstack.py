...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return lwpNA if e else lwpNT
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->lwpNv:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(lwpNv)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:lwpNk):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,lwpNv)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:lwpNk):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,lwpNv)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[lwpNk,lwpNk],rows:List[Dict[lwpNk,Any]],formatters:Dict[lwpNk,Callable[[Any],lwpNk]]=lwpNv):
 from rich.table import Table
 if formatters is lwpNv:
  formatters=lwpNR()
 t=Table()
