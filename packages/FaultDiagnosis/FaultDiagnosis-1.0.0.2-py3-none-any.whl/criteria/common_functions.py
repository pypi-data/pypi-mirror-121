...
# MSSQL 접속
def conn():
    db = pymssql.connect(host="192.168.11.55", user="sunghan", password="Sunghan!2345", database='sunghan',
                         charset='UTF8')  # charset='EUC-KR'
    return db

# 냉방, 난방 구분 (T_OPERATING_CONFIG)
def air_condition(SiteCode):  # GGTMC042
    db = conn()
    cursor = db.cursor()
    sql = "SELECT  ColdStartDate, HeatingStartDate, ColdStartHour, ColdEndHour, HeatingStartHour, HeatingEndHour \
    ...
    if (ColdStartDate != None):
        AirCondition = '1'
    if (HeatingStartDate != None):
        AirCondition = '2'
...
def start_date(SiteCode):  # GGTMC042
    db = conn()
    cursor = db.cursor()
...
def ahu_info(EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo):
    ...
    sql = "SELECT  ColdSupplySetupTemp, HeatSupplySetupTemp, SupplyAirVolume, OutdoorAirVolume, ExhaustAirVolume \
            FROM T_AHU_INFO \
            WHERE EquipmentClassCode = %s AND EquipmentNo = %d \
              AND SiteCode = %s \
...
def T_AHU_CRITERIA_SIMILARITY_INFO(EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo,
