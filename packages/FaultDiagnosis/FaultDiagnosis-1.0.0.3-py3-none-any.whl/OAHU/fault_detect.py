...
from criteria.common_functions import air_condition
from criteria.common_functions import conn
from criteria.common_functions import T_AHU_SENSING_INFO
from criteria.common_functions import start_date
from criteria.common_functions import ahu_info
from criteria.common_functions import T_AHU_CRITERIA_SIMILARITY_INFO
from criteria.common_functions import T_STD_SENSING_INFO
...
def fault_detect(previous_day, EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, CoolHeatingIndication):
    ...
    AirCondition = air_condition(SiteCode)
    fault_total_data = T_AHU_SENSING_INFO(EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo)
    startdate = start_date(SiteCode)
    ...
    sa_set_temp_c, sa_set_temp_h, sa_flow, oa_flow, ea_flow = ahu_info(EquipmentClassCode, EquipmentNo, SiteCode,
                                                                       BuildingNo, ZoneNo)
    ...
    cv_rmse, wss_dstnc, enrgy_dstnc, polydegree = T_AHU_CRITERIA_SIMILARITY_INFO(EquipmentClassCode, EquipmentNo,
    ...
    if (AirCondition == '1'):
        poly_reg = joblib.load("./KSB-cooling-polynomial.pkl")

    if (AirCondition == '2'):
        poly_reg = joblib.load("./KSB-heating-polynomial.pkl")
    ...
    for i in range(predict_len):
        ...
        ### MMI센싱정보 테이블 수정
        ### T_MMI_SENSING_INFO Update start
        db = conn()
        cursor = db.cursor()

        sql = "UPDATE T_MMI_SENSING_INFO           \
                   SET SupplyFanPowerQ = %d        \
                      ,ReturnFanPowerQ = %d        \
                  FROM T_MMI_SENSING_INFO a        \
                       JOIN ( SELECT ROW_NUMBER() OVER (ORDER BY sensingtime) AS rownum, sensingtime \
                                  FROM T_MMI_SENSING_INFO              \
                                  WHERE MMIEquipmentID = 'AHU_12'      \
        ...
        """
        sql = "UPDATE T_MMI_SENSING_INFO    \
                   SET SupplyFanPowerQ = " + str(prediction) + ",ReturnFanPowerQ = " + str(label) + " \
                  FROM T_MMI_SENSING_INFO a        \
                       JOIN ( SELECT ROW_NUMBER() OVER (ORDER BY sensingtime) AS rownum, sensingtime \
                                FROM T_MMI_SENSING_INFO              \
                               WHERE MMIEquipmentID = 'AHU_12'      \
                                 AND format(CAST(sensingtime AS date), 'yyyyMMdd') = " + previous_day + ") b \
                            ON a.sensingtime = b.sensingtime           \
                 WHERE b.rownum = " + str(row_num)
        """

        val = (prediction, label, previous_day, row_num)
    ...
    if (err_no <= 1):
        ...
        val = (
        'AHU', 12, 'GGTMC042', 'M', 11, ddate, CoolHeatInd, s_cvrmse, s_wsd, s_cramerd, fault_yn, diagnosis_order,
        fault_type, sdate)
        ...
        for eno in np.arange(1, 22):
            ...
            fault_data = fault_total_data.query("DATE ==" + previous_day)
            # fault_data = fault_total_data.query('DATE == @previous_day')
            # fault_data = fault_total_data.query('SensingTime ==' + previous_day)

            sa_set_temp_c, sa_set_temp_h, sa_flow, oa_flow, ea_flow = ahu_info(EquipmentClassCode, EquipmentNo,
                                                                               SiteCode, BuildingNo, ZoneNo)
            ...
            if (AirCondition == '1'):
                path = "./KSB-cooling-fault-%02d.pkl" % eno

            if (AirCondition == '2'):
                path = "./KSB-heating-fault-%02d.pkl" % eno
            ...
            if (accuracy > 0):
                ...
                val = (
                EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, diagnosis_order, ddate, fault_type,
                class_accuracy, sdate)
        ...
        if (err_count == 0):
            ...
            fault_total = T_STD_SENSING_INFO(AirCondition)
            ...
            if (err_num == 1):
                ...
                sdate = dt_now.strftime("%Y%m%d %H:%M:%S")
                # value = 100.0
                print("sdate:", ddate, sdate, values)
            ...
            # KSB 고장 데이터 진단한 모델로 ASHRAE 고장 데이터 예측
            ######################################################
            ...
            if (class_accuracy > 0):
                ...
                sql = "INSERT INTO T_AHU_FAULT_DIAGNOSIS_INFO (EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, DiagnosisOrder, DiagnosisDate, FaultType, ClassificationAccuracy, SaveTime) VALUES (%s, %d, %s, %s, %d, %s, %s, %d, %d, %s)"
                val = ('AHU', 12, 'GGTMC042', 'M', 11, diagnosis_order, ddate, fault_type, class_accuracy, sdate)
...
# 실행 일시 및 진단(전날) 일시
dt_now = dt.datetime.now()
#yesterday = dt_now - pd.DateOffset(days=1)
#yesterday = yesterday.strftime("%Y%m%d")
tday = '20200831'

# 테스트일은 20200921
test_day = dt.datetime.strptime(tday, "%Y%m%d")
date_diff = (dt_now - test_day).days

print("오늘은: {0} \n테스트일은: {1} \n날짜 차이: {2}".format(dt_now, test_day, date_diff))
...
AirCondition = air_condition(SiteCode)
CoolHeatingIndication = AirCondition
...
for i in range(period):
    if (i >= 3):
        #days_cnt = days_cnt + i
        target_day = dt_now - pd.DateOffset(days=date_diff+i)
        previous_day = target_day.strftime("%Y%m%d")
        print('===========================================================================')
        print("previous_day:",previous_day)

        fault_detect(previous_day, EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, CoolHeatingIndication)
