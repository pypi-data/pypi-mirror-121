...
def criteria_STD_model(SiteCode):

    AirCondition = air_condition(SiteCode)
    df_fault = T_STD_SENSING_INFO(AirCondition)
    ...
    if(AirCondition == "1"):
        joblib.dump(afaultRFModel, "./KSB-cooling-regression.pkl")
    if(AirCondition == "2"):
        joblib.dump(afaultRFModel, "./KSB-heating-regression.pkl")
...
def criteria_STD_normal_model(SiteCode):

    AirCondition = air_condition(SiteCode)
    fault_predict = T_STD_SENSING_INFO(AirCondition)
    ...
    #aircondition = air_condition(criteria_file_name)
    #aircondition = air_condition('GGTMC042')
    if(AirCondition == "1"):
...
def criteria_STD_fault_model(SiteCode):
    kfault_type1 = 0
    AirCondition = air_condition(SiteCode)
    ...
    for eno in np.arange(1,22):
        ...
        if(AirCondition == "1"):
            joblib.dump(classRFModel, "./KSB-cooling-fault-%02d.pkl" %eno)

        if(AirCondition == "2"):
            joblib.dump(classRFModel, "./KSB-heating-fault-%02d.pkl" %eno)
...
"""
SiteCode = "GGTMC042"
criteria_STD_model(SiteCode)
criteria_STD_normal_model(SiteCode)
criteria_STD_fault_model(SiteCode)
"""