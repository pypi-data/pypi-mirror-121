...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        subparsers = parser.add_subparsers(
            help='Command to create a django app which has the frontend configuration', dest='frontend_library_or_framework', required=True)

        # add a parser for react because it can be javascript or typescript
        react_parser = subparsers.add_parser(
            'react', help='Command to have django app with React for Frontend Development')
        ...
        react_parser.add_argument(
            '--template', help='Language for the React Project', default='javascript', choices=language_choices)
