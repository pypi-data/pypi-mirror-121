...
v='1.97.97'
print("Welcome use civilianM,current version：1.97.97")

def upgrade():
    """
Get if have latest version.
havent parameter"""
    global v
    dd = []
    try:
...
def wait(t):
    if checkClass(t,'float') or checkClass(t,'int'):
        ...
        raise NumberError(
            """You enter's result must is a int/float-number""")

#Randomly generated number
def randint(mins,maxs):
    if checkClass(mins,'int') == False or checkClass(maxs,'int') == False:
        raise NumberError(
            """You enter's result must is a int-number""")
    if maxs == mins:
        raise NumberError(
            """Max-number cannot equal to min-number""")
    elif mins > maxs:
        raise NumberError(
            """Min-number cannot greater than max-number""")
...
def garbledCode(digit=20,types=None):
    ...
    if checkClass(digit,'int')==False:
        raise NumberError(
            """You enter's result must is a not string number!""")
...
def getAddress():
    try:
        ...
        raise NetworkError("Please check network connect status")
    ...
    #nr = [d['Address']['Country'],PR,C,d['IP'],ISP,comp,comip]
    nr = {'add':{'country':d['Address']['Country'],'province':PR,'city':C},
          'com':{'name':comp,'computerIP':comip},'networkIP':d['IP']}
    return nr
...
class author():
    ...
    def old(self):
        raise LibraryError("Cant from 'civilianMLibrary-OO' get about 'old' 's info")
    def gender(self):
        print('male')
    def phoneNumber(self):
        print('+86 13417083101')
    def From(self):
        print("CN")
    def live(self):
        print("GuangXi Zhuang Autonomous Region,China")
