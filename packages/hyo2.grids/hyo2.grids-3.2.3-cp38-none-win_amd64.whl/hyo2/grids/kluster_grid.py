...
try:
    ...
    has_kluster_bathygrid = True
except ImportError:
    has_kluster_bathygrid = False
    load_grid = None

logger = logging.getLogger(__name__)
logger.debug("Kluster's BathyGrid supported: %s" % has_kluster_bathygrid)
...
class KlusterGrid:
    ...
    class GridTile:
        ...
        def type(self, layer_name: str):
            ...
            raise RuntimeError("Unsupported layer type: %s" % layer_name)
    ...
    def __init__(self):
        if not has_kluster_bathygrid:
            raise RuntimeError("First install Kluster's Bathygrid module")
        self._cb = None
