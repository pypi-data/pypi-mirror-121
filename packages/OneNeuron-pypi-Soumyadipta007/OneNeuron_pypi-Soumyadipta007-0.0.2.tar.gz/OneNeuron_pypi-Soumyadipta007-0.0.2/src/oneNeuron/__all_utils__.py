...
def prepare_data(df):
  """Its used to separate the dependent and independent features.
...
def save_model(model, filename):
  ...
  logging.info(f"Saved the model as {filename}")
...
def save_plot(df, file_name, model):
  ...
  Args:
      ...
      file_name (path): File  to save
      model (python object): path to save the image
  """
  def _create_base_plot(df):
    logging.info("Creating the base_plot")
  ...
  logging.info(f"Savig the plot at {plotPath}")