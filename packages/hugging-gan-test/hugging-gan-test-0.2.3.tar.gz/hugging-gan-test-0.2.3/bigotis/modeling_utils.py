...
class ImageGenerator(
    ...
    def add_noise(
        ...
        noise = up_noise * torch.rand((img_batch.shape[0], 1, 1, 1)).to(
            self.device) * torch.randn_like(img_batch, requires_grad=False)

        # batch_mask = (torch.rand((img_batch.shape[0])) <= p).to(self.device)
    ...
    def augment(
        ...
        Args:
            img_batch (torch.Tensor): batch of images to augment
                with shape BxHxWx3.
            target_img_width (int): width of the augmented images.
            target_img_height (int): height of the augmented images.
            num_crops (int, optional): Number of augmentations to
                generate. Defaults to 32.
            crop_scaler (int, optional): Scale range used when
                cropping. Defaults to 1.

        Returns:
            torch.Tensor: augmented batch of images.
        """
        pad_scale = 3
        x_pad_size = target_img_width // pad_scale
        y_pad_size = target_img_height // pad_scale
        ...
        for crop_idx in range(num_crops):
            ...
            offsetx = torch.randint(
                0,
                int(target_img_width + target_img_width * 2 / pad_scale -
                    crop_size),
                (),
            )
            offsety = torch.randint(
                0,
                int(target_img_height + target_img_height * 2 / pad_scale -
                    crop_size),
                (),
            )
        ...
        img_batch = self.add_noise(img_batch, )
    ...
    def get_clip_img_encodings(
        ...
        Args:
            ...
            do_preprocess (bool, optional): if `True` the input 
                images are normalized and resized. Defaults to True.
