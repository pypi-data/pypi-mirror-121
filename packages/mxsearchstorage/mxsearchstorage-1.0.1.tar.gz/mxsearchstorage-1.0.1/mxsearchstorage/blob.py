...
class BlobStorage(object):
    ...
    def upload_object(self, container_name, file_name, file_path):
        self.upload_objects_single(Type.PRIMARY, container_name, file_name, file_path)
        self.upload_objects_single(Type.SECONDARY, container_name, file_name, file_path)
...
AZURE_PRIMARY_CONNECTION = "DefaultEndpointsProtocol=https;AccountName=525b7a16dbstg;AccountKey=TWPKO2y/GP9a2/X6tP/585JMvOKYVwEoj+25W3vH1SC2IPx/fgWRmuaJpgo5/mG2WWdEiX5+C563sBp/vbIWXA==;EndpointSuffix=core.windows.net"
AZURE_SECONDARY_CONNECTION = "DefaultEndpointsProtocol=https;AccountName=f34642900cstg;AccountKey=adUqvTiZW+8dZupVPUzOVSD2dpMlDRiZdEB8V3YAx1wrmHmXOmWk1JQwtlJaZk/kymOZQDY92UZAV9+kPNiyYw==;EndpointSuffix=core.windows.net"

conn = BlobStorage(AZURE_PRIMARY_CONNECTION, AZURE_SECONDARY_CONNECTION)
#conn.upload_objects("brand-extraction-prod", "requirements.txt", "/Users/h0l04gn/Workspace/mexico-search-object-store/mxsearchstorage/requirements.txt")
blob_list = conn.list_objects(Type.PRIMARY, "brand-extraction-prod")
for blob in blob_list:
    print(blob.name)
blob_list = conn.list_objects(Type.SECONDARY, "brand-extraction-prod")
for blob in blob_list:
    print(blob.name)
conn.delete_object(Type.PRIMARY, "brand-extraction-prod", "requirements.txt")
conn.delete_object(Type.SECONDARY, "brand-extraction-prod", "requirements.txt")
blob_list = conn.list_objects(Type.PRIMARY, "brand-extraction-prod")
for blob in blob_list:
    print(blob.name)
blob_list = conn.list_objects(Type.SECONDARY, "brand-extraction-prod")
...
conn.swpie_container(Type.PRIMARY, "teststorage")
blob_list = conn.list_objects(Type.PRIMARY, "teststorage")
for blob in blob_list:
    print(blob.name)