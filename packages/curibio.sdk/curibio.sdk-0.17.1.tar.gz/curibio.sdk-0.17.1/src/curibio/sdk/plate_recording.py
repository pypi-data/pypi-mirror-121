...
class PlateRecording(FileManagerPlateRecording):
    ...
    def _add_peak_detection_series(
        ...
        for idx in indices:
            uninterpolated_time_seconds = round(time_values[idx] / CENTIMILLISECONDS_PER_SECOND, 2)
            if self._is_optical_recording:
                row = int(
                ...
                row = uninterpolated_time_seconds * int(1 / INTERPOLATED_DATA_PERIOD_SECONDS) + 1
                ...
                try:
                    ...
                    if iter_metric_uuid == AMPLITUDE_UUID:
                        ...
                        if (
                            ...
                            value_to_write *= MICRO_TO_BASE_CONVERSION
                    if iter_metric_uuid in (TWITCH_PERIOD_UUID, WIDTH_UUID, IRREGULARITY_INTERVAL_UUID):
