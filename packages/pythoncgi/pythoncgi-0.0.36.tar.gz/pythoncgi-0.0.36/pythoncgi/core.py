...
def _should_return_304(fp: str = None):
    ...
    lastmodified = datetime.datetime.fromtimestamp(lastmodified, tz=tzoffset(None, 0))
    set_header("Last-Modified", lastmodified.strftime("%a, %d %b %Y %H:%M:%S GMT"))
    if "If-Modified-Since" in _HEADERS:
        ims = email.utils.parsedate(_HEADERS["If-Modified-Since"])[:6]
        if ims:
            ims = datetime.datetime(*ims, tzinfo=tzoffset(None, 0))
            if ims >= lastmodified:
                return True
    return False
