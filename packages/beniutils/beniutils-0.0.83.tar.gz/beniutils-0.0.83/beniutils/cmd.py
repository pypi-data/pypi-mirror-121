...
from . import (copy, execute, getPath, makeFolder, makeTempWorkspace, readFile,
               remove, writeFile)
...
def task_build(targetPath=None):
    try:
        ...
        _returncode, outBytes, errBytes = execute(f"pyinstaller {taskSpecFile}", ignoreError=True)
