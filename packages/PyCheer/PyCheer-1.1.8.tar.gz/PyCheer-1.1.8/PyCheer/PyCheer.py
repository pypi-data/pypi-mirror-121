...
set_language_dir(os.path.join(os.path.dirname(configpath), "language"))
config_content = {}
...
def run(params):
    ...
    def tree_handler():
        ...
        breadcrumb_code = ""
        if len(path_split) > 0:
            breadcrumb_code += '''<nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb"><ol class="breadcrumb" style="width:60%;"><li class="breadcrumb-item"><a href="/tree">（根目录）</a></li>'''
            for i in range(len(path_split) - 1):
                now_path = ""
                for j in range(i + 1):
                    now_path += "/" + path_split[j]
                breadcrumb_code += '<li class="breadcrumb-item"><a href="/tree?path=%s">%s</a></li>' % (
