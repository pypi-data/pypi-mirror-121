...
mumuzi:
	...
	def solve(self,filename):
		...
		if re_result:
			current_layer=re_result.group(0).decode().split('_')[3]
			ftype=filetype(layer_data_b[len(re_result.group(0)):])
		else:
			print('mumuzi:\n\t不要拿奇怪的东西来糊弄神好么')
			self.kua()
		if 'xor' in ftype:
			print('套神认为这层是xor加密，密钥是'+ftype[-1])
			return(decode_xor(filename,ftype[-1],current_layer))
		elif ftype in ['hex','base64','base85']:
			print('套神认为这层是文本加密，加密方法是'+ftype)
			return(decode_txt(filename,ftype,current_layer))
		else:
			print('套神认为这层是一个'+ftype+'文件')
			return(decode_file(filename,ftype,current_layer))
