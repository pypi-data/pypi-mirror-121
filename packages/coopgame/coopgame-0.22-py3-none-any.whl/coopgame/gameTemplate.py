...
class GameTemplate(MonitoredClass):
    ...
    def _handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                logger.info(f"Left click")
                self.handle_left_click(pressed_keys)
            elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
                logger.info(f"Right click")
    ...
    def register_monitored_class(self, new_class: MonitoredClass):
        self._monitored_class_logger.register_classes([new_class])