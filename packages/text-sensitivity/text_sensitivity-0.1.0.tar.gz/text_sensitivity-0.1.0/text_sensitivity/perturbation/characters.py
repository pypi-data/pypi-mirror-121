...
def __construct(cls_or_fn, constructor_one, constructor_many, **kwargs):
    ...
    if 'n' in kwargs and isinstance(kwargs['n'], int) and kwargs['n'] > 1:
        n = kwargs.pop('n')
        if not isinstance(cls_or_fn, (str, dict, Callable)):
...
def random_lower(n: int = 1) -> Perturbation:
    ...
    Returns:
        Perturbation: Object able to apply perturbations on strings or TextInstances.
    """
    return _function(lambda x: __random_character_fn(x, str.lower), label_to='random_upper', n=n)
...
def random_case_swap(n: int = 1) -> Perturbation:
    ...
    Returns:
        Perturbation: Object able to apply perturbations on strings or TextInstances.
    """
    return _function(lambda x: __random_character_fn(x, str.swapcase), label_to='random_upper', n=n)
