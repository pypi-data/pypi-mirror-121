...
def oneway_dictionary_mapping(instance: TextInstance,
    ...
    for idx in set(np.random.randint(len(all_options), size=n)):
        ...
        for i, option in enumerate(current_option):
            new_tokenized[options.keys()[i]] = option
...
def one_to_one_dictionary_mapping(instance: TextInstance,
                                  ...
                                  tokenizer,
                                  detokenizer):
    return list(oneway_dictionary_mapping(instance, dictionary, label_from=label_from, label_to=label_to,
                                          tokenizer=tokenizer, detokenizer=detokenizer, n=1))[0]
    ...
    def from_dictionary(cls,
        ...
        def perturbation_function(instance: TextInstance) -> Optional[Tuple[str, LT, LT]]:
            return one_to_one_dictionary_mapping(instance,
                                                 dictionary={k: as_list(v) for k, v in dictionary},
    ...
    def from_list(cls,
        ...
        Args:
            ...
            detokenizer (Callable, optional): Function to detokenize tokens into instance data.
                Defaults to default_detokenizer.        
        """
        mapping_list = [(a, b) for a, b in set(list(itertools.combinations(mapping_list, 2)))]
        return OneToManyPerturbation.from_tuples(mapping_list,
                                                 label_from=label_from,
                                                 label_to=label_to,
                                                 tokenizer=tokenizer,
                                                 detokenizer=detokenizer)
