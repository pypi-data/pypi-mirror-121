...
{{ import_str }}{% endfor %}
{% if checkserver %}
{% for import_str in checkserver.get("import_list") %}
{{ import_str }}{% endfor %}
flag = py_checkserver([{% for service in checkserver.get("required_service_list") %}{{service}}, {% endfor %}], [{% for service in checkserver.get("optional_service_list") %}{{service}}, {% endfor %}])
{% endif %}
...
{% endif %}
{% if checkserver %}
@pytest.mark.skipif(flag in ["Ignore", False], reason=str(flag))
{% endif %}
class {{ class_name }}(HttpRunner):
    ...
    def test_start_{{param_index}}(self, param):{% if parse_flag %}
        '''
            {{ doc_info }}
        '''{% else %}
        '''
    ...
    {% if doc_info and not parameters%}
    def test_start(self):
        '''
            {{ doc_info }}
        '''
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    parameters = parse_parameters(config.get("parameters")) if config.get("parameters") is not None else None
    ...
    data = {
        ...
        "doc_info": doc_info,
        "usefixtures": config.get("usefixtures"),
        "author": config.get("author"),
        "checkserver": config.get("checkserver"),
        "teststeps_chain_style": [
            make_teststep_chain_style(step) for step in teststeps
        ],
    }
    content = __TEMPLATE__.render(data, zip=zip)
