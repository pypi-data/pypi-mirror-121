...
class HttpRunner(object):
    ...
    def run_testcase(self, testcase: TestCase) -> "HttpRunner":
        ...
        if self.__config.skip_if_list:
            skip_result: dict = self.__call_skip_if(self.__config.skip_if_list, self.__config.variables, "*** skipIf testcase ***")
        ...
        for step in self.__teststeps:
            ...
            if step.skip_if_list:
                skip_result: dict = self.__call_skip_if(step.skip_if_list, step.variables, "<<<< skipIf teststep: {} >>>".format(step.name))
                skip_flag = True if True in list(skip_result.values()) else False
                if skip_flag:
