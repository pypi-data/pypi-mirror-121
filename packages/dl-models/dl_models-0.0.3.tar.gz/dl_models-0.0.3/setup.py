from setuptools import setup, find_packages
import os
from pathlib import Path


project_dir = Path(__file__).parent.parent


with open(os.path.join(project_dir, "requirements.txt")) as f:
    required_packages = f.read().splitlines()

setup(
    name="dl_models",
    version="0.0.3",
    description="Contains various neural networks",
    long_description="Contains various neural networks developed using PyTorch",
    author="Nitin Kumar Mittal",
    author_email="nitinmittal234@gmail.com",
    url="https://github.com/nitinkmittal/dl_models",
    packages=find_packages(include=["dl_models", "dl_models.*"]),
    install_requires=required_packages,
)
