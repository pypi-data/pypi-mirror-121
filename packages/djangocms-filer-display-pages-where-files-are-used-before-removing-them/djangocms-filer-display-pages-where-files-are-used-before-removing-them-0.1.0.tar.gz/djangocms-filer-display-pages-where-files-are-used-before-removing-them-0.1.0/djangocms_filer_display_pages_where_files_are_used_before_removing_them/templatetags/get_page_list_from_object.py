...
use_images_bootstrap4 = True
use_images_djangocms_picture = True
use_media = True

try:
    from djangocms_bootstrap4.contrib.bootstrap4_picture.models import Bootstrap4Picture
except ModuleNotFoundError:
    use_images_bootstrap4 = False
try:
    from djangocms_picture.models import Picture
except ModuleNotFoundError:
    use_images_djangocms_picture = False
try:
    ...
    if use_images_bootstrap4:
        for image in Bootstrap4Picture.objects.filter(picture_id=obj.id):
            try:
                ...
                pass
    elif use_images_djangocms_picture:
        for image in Picture.objects.filter(picture_id=obj.id):
            try:
                if not image.placeholder.page.publisher_is_draft:
                    images_list.append(
                        {
