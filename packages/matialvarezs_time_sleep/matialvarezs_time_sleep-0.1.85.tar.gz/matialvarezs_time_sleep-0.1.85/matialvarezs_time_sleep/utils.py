...
def should_stop(**kwargs):
    ...
    results = {}
    if method == "GET":
        results, error = matialvarezs_request_handler_utils.send_get_and_get_response(url, headers=headers).json()
    if method == "POST":
        results, error = matialvarezs_request_handler_utils.send_post_and_get_response(url, headers=headers, data=data,
