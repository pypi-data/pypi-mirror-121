...
class DeepLCRetentionTimePredictor(tf.keras.Model):
    ...
    def _build_aminoacid_branch(self):
        self.aminoacid_branch = tf.keras.Sequential([
            ...
            self._build_conv_pool_block(n_filters=64, kernel=8, padding='same', pool=False),
    ...
    def _build_onehot_encoding_branch(self):
        self.onehot_encoding_branch = tf.keras.Sequential([
            self._build_conv_pool_block(n_filters=2, kernel=2, padding='same', activation='tanh',
                                        pool_strides=10, pool_size=10),
            tf.keras.layers.Flatten()])
    ...
    def _build_conv_pool_block(self, n_conv_layers=2, n_filters=256, kernel=8, padding="same", activation='leaky_relu',
                               pool=True, pool_strides=2, pool_size=2):
        ...
        block = tf.keras.Sequential([tf.keras.layers.Conv1D(filters=n_filters, kernel_size=kernel, padding=padding,
                                                            activation=activation_fn) for _ in range(n_conv_layers)])
        if pool:
