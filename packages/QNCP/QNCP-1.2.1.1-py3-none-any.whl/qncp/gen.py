...
class Quantum_Composers:
    ...
    def t0(self,t):  # clock T0
        self.dev.write(':PULSE0:PER {}'.format(round(t,12)))
    @sleep_method
    ...
    @sleep_method            
    def wid(self,ch,w):
        self.dev.write(':PULSE{}:WIDth {}'.format(ch,round(w,12)))
    @sleep_method
    def dly(self,ch,d):
        self.dev.write(':PULSE{}:DELay {}'.format(ch,round(d,12)))
    @sleep_method
    def pol(self,ch,p):
        self.dev.write(':PULSE{}:POL {}'.format(ch,round(p,12)))
    @sleep_method
    def wcount(self,ch,w):  # wait number of T0 before enable output
        self.dev.write(':PULSE{}:WCOunter {}'.format(ch,round(w,12)))
    @sleep_method
