...
class DataTable(object):
    ...
    def mr_selections_indices(self):
        ...
        mr_dimensions_indices = [
            i
            for (i, dim) in enumerate(self.all_dimensions)
            if (i + 1 < len(self.all_dimensions) and dim.type == "multiple_response")
        ]
    ...
    def count(self, weighted=True):
        ...
        if weighted and self.is_weighted:
            return sum(self._cube["result"]["measures"].get("count", {}).get("data"))
    ...
    def flat_values(self, weighted, margin=False):
        ...
        values = [(val if not type(val) is dict else np.nan) for val in values]
        return values
