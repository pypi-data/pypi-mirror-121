...
from features_factory.input_features import (
    InputFeature,
    ProvidedInputFeatureWithControlOnNaN,
    BoolInputFeature,
    IntInputFeature,
    FloatInputFeature,
    DateTimeInputFeature,
    StringInputFeature,
    CountryCodeInputFeature,
    DateInputFeature,
    StringTimestampInputFeature,
)
...
class TestSimpleTypeFeature(unittest.TestCase):
    ...
    def test_datetime_type(self):
        ...
        df = pd.DataFrame(
            {
                feat.name(): [datetime(2018, 1, 1), "2018-01-02", datetime(2018, 1, 3)],
                "rock": [1, 2, 3],
            }
        )
        self.assert_there_is_no_error(feat.verify_input(df))
        ...
        df = pd.DataFrame(
            {
                feat.name(): [datetime(2018, 1, 1), "bubble", datetime(2018, 1, 3)],
                "rock": [1, 2, 3],
            }
        )
        self.assert_only_format_is_wrong(feat.verify_input(df))
    ...
    def test_date_type(self):
        ...
        df = pd.DataFrame(
            {feat.name(): ["2018-01-01", "2018-01-02", "2018-01-03"], "rock": [1, 2, 3]}
        )
        self.assert_only_format_is_wrong(feat.verify_input(df))
    ...
    def test_allow_nan_is_compatible_with_type_check_for_date(self):
        ...
        df = pd.DataFrame(
            {
                feat.name(): [datetime(2018, 1, 1).date(), None, float("nan"), np.nan],
                "other": [1, 2, 3, 4],
            }
        )
    ...
    def test_allow_nan_is_compatible_with_type_check_for_string(self):
        ...
        df = pd.DataFrame(
            {feat.name(): ["ciao", None, float("nan"), np.nan], "other": [1, 2, 3, 4]}
        )
    ...
    def test_string_timestamp_type_with_strict_format(self):
        feat = StringTimestampInputFeature(
            "date_as_string", timestamp_format="%Y-%m-%d"
        )
        ...
        df = pd.DataFrame({feat.name(): [2018, 45, 3], "rock": [1, 2, 3]})
        self.assert_only_format_is_wrong(feat.verify_input(df))

        df = pd.DataFrame(
            {feat.name(): ["2018-01-01", 2018, "2018-01-03"], "rock": [1, 2, 3]}
        )
        self.assert_only_format_is_wrong(feat.verify_input(df))
    ...
    def test_allow_nan_is_compatible_with_type_check_for_string_timestamp(self):
        ...
        df = pd.DataFrame(
            {
                feat.name(): ["2018-01-01", None, float("nan"), np.nan],
                "other": [1, 2, 3, 4],
            }
        )
    ...
    def test_allow_nan_is_compatible_with_type_check_for_country_code(self):
        ...
        df = pd.DataFrame(
            {feat.name(): ["DE", None, float("nan"), np.nan], "other": [1, 2, 3, 4]}
        )
