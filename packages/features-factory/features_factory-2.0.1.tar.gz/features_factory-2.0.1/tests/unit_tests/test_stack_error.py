...
class TestStackError(unittest.TestCase):
    ...
    def test_add(self):
        ...
        de = DependenciesError()
        de.add_missing_dependencies(
            victim="Gunther", missing_dependencies=["Reinhold", "Mutter"]
        )
        ...
        self.assertEqual(
            ("stone",), se_sum.get_input_data_error().get_columns_with_wrong_format()
        )
        self.assertEqual(
            ("bubble",), se_sum.get_input_data_error().get_missing_columns()
        )
