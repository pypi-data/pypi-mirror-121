...
class TestDurationFeature(unittest.TestCase):
    def test_generate(self):
        ...
        duration_h = DurationFeature(
            "duration_h", start_time_feat=start, end_time_feat=end, units="hours"
        )
        duration_m = DurationFeature(
            "duration_m", start_time_feat=start, end_time_feat=end, units="minutes"
        )
        duration_s = DurationFeature(
            "duration_s", start_time_feat=start, end_time_feat=end, units="seconds"
        )

        df = pd.DataFrame(
            {
                start.name(): [datetime(2018, 1, 18, 8, 30)] * 2,
