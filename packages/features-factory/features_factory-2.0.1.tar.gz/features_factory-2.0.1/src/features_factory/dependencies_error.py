...
class DependenciesError:
    ...
    def has_missing_dependencies(self) -> bool:
        return (
            True
            if sum([len(v) for v in self._missing_dependencies_map.values()]) > 0
            else False
        )
    ...
    def add_missing_dependencies(
        self, victim: str, missing_dependencies: Collection[str]
    ):
        if len(missing_dependencies) > 0:
            if victim not in self._missing_dependencies_map.keys():
                self._missing_dependencies_map[victim] = set()
            self._missing_dependencies_map[victim] = self._missing_dependencies_map[
                victim
            ].union(set(missing_dependencies))

    def to_string(self) -> str:
        lines = list()
        lines.append(
            "DependenciesError {}".format(
                "is empty." if self.is_empty() else "contains errors:"
            )
        )
        ...
        for victim in self.get_missing_dependencies_victims():
            lines.append(
                " * {} misses {}".format(
                    victim, ", ".join(self.get_missing_dependency_of_victim(victim))
                )
            )
