...
class MultiComponentFeature(GeneratedFeature, metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, name: str, dependencies: List[FeatureArchetype], mmap_function: Callable
    ):
        """
        E.g.
    ...
    def generate(self, df: pd.DataFrame, **kwargs):
        return df.loc[:, [f.name() for f in self._dependencies]].apply(
            self._mmap_function, axis=1
        )
