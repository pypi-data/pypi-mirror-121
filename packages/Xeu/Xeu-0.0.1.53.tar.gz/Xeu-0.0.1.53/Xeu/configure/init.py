...
LISTEN = collections.namedtuple('LISTEN', ['ipv4', 'port'])
Collector.set('RUNTIME', 'env', '未知')
RUNTIME = collections.namedtuple('RUNTIME', ['env', ])

Collector.set('DB_CONFIG', 'MYSQL', None)
Collector.set('DB_CONFIG', 'REDIS', None)
DB_CONFIG = collections.namedtuple('DB_CONFIG', ['MYSQL', 'REDIS'])

# print('1LISTEN: ', id(LISTEN))
...
class _Setup(object):
    ...
    def __load_base_data_config(self):
        self.BASE_CONFIG_RAW_DATA = self.__base_config_handler(
            os.path.join(BASE_DIR, BASE_CONFIG_DIR, self.BASE_CONFIG_FILE),
            Collector
        ).read()
    ...
    def __call__(self, base_config: Tuple[Union[str, ConfigFileOperation]]):
        ...
        # print('2LISTEN: ', id(LISTEN))
        # print('Collector call', id(Collector))
        LISTEN.port = Collector.LISTEN.port
        LISTEN.ipv4 = Collector.LISTEN.ipv4
        RUNTIME.env = Collector.runtime.env
        DB_CONFIG.MYSQL = Collector.mysql
        DB_CONFIG.REDIS = Collector.redis
