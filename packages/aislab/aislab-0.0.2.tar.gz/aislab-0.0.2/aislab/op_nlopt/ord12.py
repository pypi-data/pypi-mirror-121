...
# def cgrad():
###################################################################################
def newton(x, func, grad, hes, args, met_opt='newt', maxiter=50, fcnv=None, afcnv=None, xcnv=None, gcnv=1e-6, sc=None, dsp_op=2, smeth='fprev2', su=1, p_incr=1.2, p_decr=0.6, rcondH=1e-6, mdl_type_temporary='lgr'):
...
#        g = grad(args)
#        H = hes(args)
        if mdl_type_temporary == 'exp':
            F, args = f_exp(args); g = g_exp(args); H = H_exp(args)
        elif mdl_type_temporary == 'lgr':
            F, args = f_lgr(args); g = g_lgr(args); H = H_lgr(args)
        elif mdl_type_temporary == 'log':
            F, args = f_log(args); g = g_log(args); H = H_log(args)
        # Termination & Display
        if np.all(np.all(H == 0)):
            if np.any(dsp_op >= 1): print('Hessian is a zero matrix. Optimization is terminated.')
            iterate = 0
            msg = 'H'
            continue
        iterate, msg = stopcrt(x, F, xx, FF, g, H, itr, maxiter, fcnv, afcnv, xcnv, gcnv, sc, dsp_op)
