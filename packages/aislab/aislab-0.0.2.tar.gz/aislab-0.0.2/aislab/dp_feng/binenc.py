...
def enc_apl(bng, x, met, skipo=True):
    ...
    for i in range(m):
        ...
        for h in range(1, len(bns) + 1):
            ...
            if met == 'my':
                xe[dv] = bns[h]['my'][1];       cname_e = cname + '_encMY'
            elif met == 'woe':
                xe[dv] = bns[h]['int'][1];      cname_e = cname + '_encWOE'
            elif met == 'int':
                xe[dv] = h - 1;                 cname_e = cname + '_encINT'
            elif met == 'dv':
                xe[:, j] = dv.flatten(); cname_e = cname_e + [cname + '_encDV' + str(j + 1)]; j += 1
...
def sbng(bng, md=5, skip_mos=['m', 'o', 'sv'], met_dist='manh', met='co1y', nmin_uy=[50, 50], mar=np.inf, pt=0.05, ba=0,
         maxprm=20000, dGBInd=5, tolmindGBInd=5, mt=np.nan, minB=50, tolminB=20):
