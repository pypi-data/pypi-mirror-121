...
class TestStep(KamaTest):
  def test_field_patch_propagation(self):
    ...
    patched_fields = step.inflate_children(
      Field,
      **{
        mc.ATTR_KW: step_mod.FIELDS_KEY,
        mc.PATCH_KEY: gen_downstream_patches(op_state, {'i1': 'i1v'})
      }
    )
  ...
  def test_final_partitioning(self):
    ...
    expect = {
      consts.TARGET_STANDARD: {'a': {'f1': 'v1'}, 'extra': 'extra'},
      consts.TARGET_INLINE: {'foo': {'bar': {'baz': 'v1'}}},
      consts.TARGET_STATE: {},
      consts.TARGET_PREFS: {'f4': 'v4'}
    }
  ...
  def test_partition_vars_by_field(self):
    ...
    expected = {
      consts.TARGET_STANDARD: {'a': {'f1': 'v1'}},
      consts.TARGET_INLINE: {'a': {'f2': 'v2'}},
      consts.TARGET_STATE: {'b': {'f3': 'v3'}},
      consts.TARGET_PREFS: {'f4': 'v4'}
    }
...
step1_config = {
  ...
  step_mod.NEXT_STEP_KEY: {
    ...
    supplier.SRC_DATA_KEY: [
      {
        PREDICATE_KEY: {
          mc.KIND_KEY: Predicate.__name__,
  ...
  step_mod.ACTION_KEY: {
    ...
    supplier.SRC_DATA_KEY: {
      mc.KIND_KEY: Action.__name__,
      mc.ID_KEY: 'nested-action'
    }
  }}
...
step2_config = {
  'fields': [
    {'id': 'a.f1', 'target': consts.TARGET_STANDARD},
    {'id': 'a.f2', 'target': consts.TARGET_INLINE},
    {'id': 'b.f3', 'target': consts.TARGET_STATE},
    {'id': 'f4', 'target': consts.TARGET_PREFS}
  ],
  'commit': {
    consts.TARGET_STANDARD: {
      'kind': MergeSupplier.__name__,
      'source': [
        f'get::self>>default_commit->.{consts.TARGET_STANDARD}',
        {'extra': 'extra'}
      ]
    },
