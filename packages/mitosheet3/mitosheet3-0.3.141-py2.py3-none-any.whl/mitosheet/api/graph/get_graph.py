...
def get_graph(event, steps_manager):
    ...
    try:
        ...
        if graph_type == BAR:
            ...
            if x_axis:
                fig, is_zoomed = get_box_plot(X, df, x_axis_column_headers)
                generation_code = get_box_code(X, df, x_axis_column_headers, df_name)
            else:
                fig, is_zoomed = get_box_plot(Y, df, y_axis_column_headers)
            ...
            fig, is_zoomed = get_column_summary_graph(X, df, x_axis_column_headers)
            generation_code = ''
