...
class Askdata(SignUp):
    ...
    def join(self, df:pd.DataFrame, with_query:str, workspace:str, how="inner", on=None, left_on=None, right_on=None, lang="en"):
        ...
        if (on == None and (left_on == None and right_on == None)):
            join_conditions = self.__get_join_info(df, df_query)["join_list"][0]
            return df.merge(df_query, how, '', join_conditions["left_on"], join_conditions["right_on"])
    ...
    def __get_join_info(self, df1:pd.DataFrame, df2:pd.DataFrame):
        ...
        url = self.base_url + '/h2sql/smartjoin'

        r = s.post(url=url, headers=self._headers, json=tables)
        r.raise_for_status()

        try:
            return r.json()["join_list"][0]
        except:
    ...
    def get(self, query, workspace='', dataset='', lang="en", fields:list=[], filters=None, sorted_by=None):
        ...
        url = self.base_url + "/smartbot/agents/"+agent_id+"/v2/ask"
