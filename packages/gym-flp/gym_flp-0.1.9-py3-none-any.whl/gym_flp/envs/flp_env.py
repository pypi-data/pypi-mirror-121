...
class ofpEnv(gym.Env):
    ...
    def __init__(self, mode = None, instance = None, distance = None, aspect_ratio = None, step_size = None, greenfield = None):
        ...
        if self.mode == "rgb_array":
            ...
            observation_low[0::4] = max(self.fac_width_y)/2
            observation_low[1::4] = max(self.fac_length_x)/2
            observation_low[2::4] = min(self.fac_width_y)
            observation_low[3::4] = min(self.fac_length_x)
            
            observation_high[0::4] = self.plant_Y - max(self.fac_width_y)/2
            observation_high[1::4] = self.plant_X - max(self.fac_length_x)/2
    ...
    def reset(self):
        ...
        x = state_prelim[1::4] if self.mode == 'human' else rng.integers(low=max(self.fac_length_x)/2, high=(self.plant_X - max(self.fac_length_x)/2), size=(self.n,))
        y = state_prelim[0::4] if self.mode == 'human' else rng.integers(low=max(self.fac_width_y)/2, high=(self.plant_Y - max(self.fac_width_y)/2), size=(self.n,))
    ...
    def step(self, action):        
        ...
        if self.action_list[action] == "up": 
            ...
            if (temp_state[4*(m-1)] + temp_state[4*(m-1) + 2]*0.5 + step_size) > self.plant_Y:
                out_of_bounds = True
                penalty = -10
            ...
            if (temp_state[4*(m-1)+1] + temp_state[4*(m-1) + 3]*0.5 + step_size) > self.plant_X:
                out_of_bounds = True
                penalty = -10
                temp_state = np.array(old_state)
            else:
        ...
        collision_penalty = -2 * collisions
        ...
        reward = cost_penalty + collision_penalty + penalty
        
        # Check for terminality for observation
        done = True if self.counter >= self.pseudo_stability else False 
            
        
        return np.array(self.state), reward + penalty, done,  {'mhc': mhc}
    ...
    def ConvertCoordinatesToState(self, state_prelim):    
        ...
        data_transformed = np.moveaxis(data, 2, 0)  # Convert to channel-first for PyTorch CNN
        return np.array(data_transformed, dtype=np.uint8)
        
    def render(self, mode = None):       
        
        data = self.ConvertCoordinatesToState(self.internal_state)

        img = Image.fromarray(np.moveaxis(data, 0, 2) , 'RGB') #Convert channel-first back to channel-last for image display
