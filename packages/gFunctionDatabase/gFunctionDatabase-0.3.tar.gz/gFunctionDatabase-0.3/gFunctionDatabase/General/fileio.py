...
def list_directory_files(path_to_dir, file_ext=None):
    """
    This function returns a list of files in the path of the directory provided.
    If a file extension is supplied, then only files with the given file
    extension are returned.

    Parameters
    ----------
    path_to_dir : str
        A path to a directory (with ending slash provided
    file_ext : str (optional)
        A string containing a file extension.
    Returns
    -------
    files : list
        A list of files in the given directory that contain the supplied file
        extension.
    Examples
    ---------
    ...
    >>> path = '.'
    >>> file_ext = 'json'
    >>> files = gfdb.General.fileio.list_directory_files(path, file_ext)
    """

    directory_contents = os.listdir(path_to_dir)  # get directory contents
    slash = platform_specific.get_slash_style()
    files = [f for f in directory_contents if os.path.isfile(
        path_to_dir + slash + f)]
    ...
    if file_ext is not None:
        ...
        for i in range(len(files)):
            file = files[i].split('.')
            current_file_ext = file[-1]
            if current_file_ext == file_ext:
                files_tmp.append(files[i])
        files = deepcopy(files_tmp)
