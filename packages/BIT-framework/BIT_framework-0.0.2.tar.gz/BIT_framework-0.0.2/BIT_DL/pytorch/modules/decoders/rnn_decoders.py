...
from BIT_DL.pytorch.core import layers
from BIT_DL.pytorch.core.attention_mechanism import (
    AttentionMechanism, AttentionWrapperState)
from BIT_DL.pytorch.core.cell_wrappers import (
    AttentionWrapper, HiddenState, RNNCellBase)
from BIT_DL.pytorch.modules.decoders import decoder_helpers
from BIT_DL.pytorch.modules.decoders.decoder_base import (
    TokenEmbedder, TokenPosEmbedder)
from BIT_DL.pytorch.modules.decoders.decoder_helpers import Helper
from BIT_DL.pytorch.modules.decoders.rnn_decoder_base import RNNDecoderBase
from BIT_DL.pytorch.utils import utils
from BIT_DL.pytorch.utils.beam_search import beam_search
from BIT_DL.pytorch.utils.types import MaybeList, MaybeTuple
from BIT_DL.pytorch.utils.utils import check_or_get_instance, get_function
...
class BasicRNNDecoderOutput(NamedTuple):
    r"""The outputs of :class:`~BIT_DL.pytorch.modules.BasicRNNDecoder` that
    ...
    :class:`~BIT_DL.pytorch.modules.BasicRNNDecoder` with default hyperparameters,
    ...
    :class:`~BIT_DL.pytorch.modules.BasicRNNDecoder` with decoding strategy of
    ...
    :class:`~BIT_DL.pytorch.modules.Helper` for the detailed information."""
...
class AttentionRNNDecoderOutput(NamedTuple):
    r"""The outputs of :class:`~BIT_DL.pytorch.modules.AttentionRNNDecoder` that
    additionally includes attention results.
    """
    logits: torch.Tensor
    r"""The outputs of RNN (at each step/of all steps) by applying the
    output layer on cell outputs. For example, in
    :class:`~BIT_DL.pytorch.modules.AttentionRNNDecoder` with default
    hyperparameters, this is a :tensor:`Tensor` of shape
    ``[batch_size, max_time, vocab_size]`` after decoding the whole sequence."""
    sample_id: torch.LongTensor
    r"""The sampled results (at each step/of all steps). For example, in
    :class:`~BIT_DL.pytorch.modules.AttentionRNNDecoder` with decoding strategy of
    ``"train_greedy"``, this is a :tensor:`LongTensor` of shape
    ``[batch_size, max_time]`` containing the sampled token indices of all
    steps. Note that the shape of ``sample_id`` is different for different
    decoding strategy or helper. Please refer to
...
class BasicRNNDecoder(RNNDecoderBase[HiddenState, BasicRNNDecoderOutput]):
    ...
    Args:
        ...
        cell (RNNCellBase, optional): An instance of
            :class:`~BIT_DL.pytorch.core.cell_wrappers.RNNCellBase`. If `None`
        ...
        output_layer (optional): An instance of :torch_nn:`Module`. Apply to
            ...
            Set ``output_layer`` to :func:`~BIT_DL.pytorch.core.identity` if you do
    ...
    See :meth:`~BIT_DL.pytorch.modules.RNNDecoderBase.forward` for the inputs and
    outputs of the decoder. The decoder returns
    ``(outputs, final_state, sequence_lengths)``, where ``outputs`` is an
    instance of :class:`~BIT_DL.pytorch.modules.BasicRNNDecoderOutput`.
    ...
    def default_hparams():
        ...
        `"rnn_cell"`: dict
            ...
            :func:`~BIT_DL.pytorch.core.default_rnn_cell_hparams`.
        ...
        `"helper_train"`: dict
            ...
            from module :mod:`BIT_DL.pytorch.modules`, or
            :mod:`BIT_DL.pytorch.custom`. This is used only when both
            ``"decoding_strategy"`` and ``"helper"`` arguments are `None` when
            calling the decoder. See
            :meth:`~BIT_DL.pytorch.modules.RNNDecoderBase.forward` for more
            details.
...
class AttentionRNNDecoder(RNNDecoderBase[AttentionWrapperState,
    ...
    Args:
        ...
        output_layer (optional): An output layer that transforms cell output
            ...
            - If no output layer after the cell output is needed, set
              `(vocab_size=None, output_layer=BIT_DL.pytorch.core.identity)`.
    ...
    of :class:`~BIT_DL.pytorch.modules.AttentionRNNDecoderOutput`.
    ...
    def __init__(self,
        ...
        attn_modules = ['BIT_DL.pytorch.core']
    ...
    def default_hparams():
        ...
        :class:`~BIT_DL.pytorch.modules.BasicRNNDecoder`.
        :meth:`~BIT_DL.pytorch.modules.BasicRNNDecoder.default_hparams`.
        ...
        `"attention"`: dict
            ...
            `"type"`: str or class or instance
                ...
                :mod:`BIT_DL.pytorch.core` or :mod:`BIT_DL.pytorch.custom`.
                ...
                .. code-block:: python
                    ...
                    # module path
                    "type": "BIT_DL.pytorch.core.BahdanauMonotonicAttention"
                    "type": "my_module.MyAttentionMechanismClass"
                    # class
                    "type": BIT_DL.pytorch.core.LuongMonotonicAttention
                    # instance
                    "type": LuongAttention(...)
    ...
    def forward(  # type: ignore
        ...
        Args:
            ...
            inputs (optional): Input tensors for teacher forcing decoding.
                ...
                :class:`~BIT_DL.pytorch.modules.WordEmbedder`, then :attr:`inputs`
            ...
            helper (optional): An instance of
                :class:`~BIT_DL.pytorch.modules.Helper`
