...
from BIT_DL.pytorch.data.data.dataset_utils import Batch
from BIT_DL.pytorch.data.data.dataset_utils import _CacheStrategy, _LazyStrategy
...
class _CachedDataSource(DataSource[RawExample]):
    ...
    :class:`~BIT_DL.pytorch.data.data.DatasetBase`, while conforming to user
    ...
    def __init__(self, data_source: DataSource[RawExample],
        ...
        Args:
            ...
            erase_after_access: If `True`, cached examples are erased after
                ...
                :class:`~BIT_DL.pytorch.data.data.DatasetBase` hyperparameter
...
class DatasetBase(Dataset, Generic[RawExample, Example], ABC):
    ...
    Args:
        source: An instance of type :class:`~BIT_DL.pytorch.data.DataSource`,
        hparams: A `dict` or instance of :class:`~BIT_DL.pytorch.HParams`
    ...
    - :meth:`collate`: Combine a list of processed examples into a single batch,
      and return an object of type :class:`~BIT_DL.pytorch.data.Batch`.

    Example:

        Here, we define a custom data class named ``MyDataset``, which is
        equivalent to the most basic usage of
        :class:`~BIT_DL.pytorch.data.MonoTextData`.
    ...
    def to(self, device: Optional[torch.device]):
        ...
        device after :class:`~BIT_DL.pytorch.data.DataIterator` fetches the batch.
        """
    ...
    def _prefetch_processed(self, index: int):
        ...
        :meth:`BIT_DL.pytorch.data.data.DatasetBase._prefetch_source` if
    ...
    def _add_cached_examples(self, indices: List[int], examples: List[Example]):
        r"""Called by :class:`BIT_DL.pytorch.data.data._CacheDataLoaderIter` to
    ...
    def _start_iteration(self) -> None:
        r"""Called by :class:`BIT_DL.pytorch.data.data.SamplerBase` before a new
    ...
    @property
    def hparams(self):
        r"""A :class:`~BIT_DL.pytorch.HParams` instance of the
    ...
    def collate(self, examples: List[Example]) -> Batch:
        ...
        an instance of :class:`~BIT_DL.pytorch.data.Batch`.
    ...
    def _collate_and_maybe_return(self, examples: List[Example]) -> \
        ...
        r"""Called by :class:`~BIT_DL.pytorch.data.DataIterator` to obtain the
