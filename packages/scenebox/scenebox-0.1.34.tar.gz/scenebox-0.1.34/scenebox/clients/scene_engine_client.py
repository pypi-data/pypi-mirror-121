...
class SceneEngineClient(object):
    ...
    def _wait_for_job_completion(self,
        ...
        progress_callback:
            If supplied, ``progress_callback(job.progress)`` is called every ``increments_sec`` seconds until the
            job is complete or until an Exception is thrown.  Otherwise, has no effect.
    ...
    def add_assets_to_set(
        ...
        timeout:
            If True and ``wait_for_availability`` is True, represents the maximum amount of time
    ...
    def submit_video_indexing_request(
        ...
        -------
        dict
            The full response from the indexing Job.
    ...
    def get_searchable_field_statistics(self, asset_type: str) -> dict:
        ...
        return self.requests.get(
            route=join(
                asset_type,
                'meta',
                'searchable_field_statistics').replace("\\", "/"),
    ...
    def compress_lidars(self,
        ...
        -------
        str
            The full response of the job that carries out the LIDAR compression.
    ...
    def get_indexed_session_filenames(self, search):
        # Get all the indexed sessions
        indexed_sessions = self.get_asset_manager(
            AssetsConstants.SESSIONS_ASSET_ID).search_assets(search)
    ...
    def _get_asset_response_model(self, asset_type: str):
        ...
        return self.requests.get(
            route=join(asset_type, 'meta', 'response_model').replace("\\", "/"),
            trailing_slash=True
        ).json()
    ...
    def add_embeddings(
        ...
        wait_for_completion:
            ...
            Otherwise, continues execution and does not raise an error if the job does not complete.
        buffered_write:
            Ingests embeddings in a buffered fashion.
        threading:
    ...
    def add_image(
        ...
        """Upload an image onto SceneBox.
               ...
               Upload an image with a local file path, URL, URI, or image bytes.  Threaded wrapper for
               ``self.add_image``. This method is best used with singular images, or images that are not all located
               in the same folder. For several images all located in the same folder, check out
               ``self.add_images_from_folder``.
               ...
               buffered_write:
                    Writes images in a buffered fashion.
               add_to_session:
    ...
    def add_images_from_folder(
        ...
        """Upload images from a single local folder path onto SceneBox.
           ...
           ----------
           folder_path:
               The local path to the image to upload.
           session_uid:
                The session associated with the image.
           set_id:
        ...
        """Extracts objects from annotations.
           ...
           ----------
            ...
            wait_for_completion:
                ...
                complete.
            progress_callback:
                If wait_for_completion is True, calls progress_callback(job.progress) every 10 seconds until the celery
                job created by the endpoint call is complete.  Otherwise, has no effect.
...
class SceneEngineRequestHandler:
    ...
    def __get_scene_engine_url(self, route, trailing_slash, remove_version):
        ...
        url = join(scene_engine_url_, route).replace("\\", "/")
