...
class UnidomboscoNightmare:
    ...
    def __init__(self, globalsInstance, instanceIndex: int, headless: bool = True):
    ...
    def run(self,
        ...
        self.urlElement = self.selenium.accessUrl(url)
        ...
        while True :
            ...
            while True :
                ...
                self.accessField('Assunto', subject, muteLogs=True)
                log.debug(self.run, 'Accessing "Area"')
                self.accessField('Area', area, muteLogs=True)
                log.debug(self.run, 'Checking "Departamento"')
                if self.isThereAny('Departamento', department, processingTime=1, muteLogs=True) :
                    break
            ###- financeiro
            ...
            ###- direção
            log.debug(self.run, 'Accessing "Departamento"')
            self.accessField('Departamento', department)
            log.debug(self.run, 'Accessing "Motivo"')
            self.accessField('Motivo', reason, processingTime=5)
            ...
            self.typeForm('Cpf', cpf if ObjectHelper.isNotNone(cpf) else c.BLANK)
