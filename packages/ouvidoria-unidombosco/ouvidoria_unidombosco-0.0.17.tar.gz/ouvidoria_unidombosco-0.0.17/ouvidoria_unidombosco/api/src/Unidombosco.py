...
class Unidombosco:
    ...
    def runHell(self, message: str, studentRegistry: str, name: str, email: str, phoneNumber: str, cpf: str, url: str = URL, headers: str = HEADERS, instances: int = 10, interval: float = 0.0):
        for instanceIndex in range(instances):
            hell = Thread(
                ...
                args = (
                    str(message), str(studentRegistry), str(name), str(email), str(phoneNumber), str(cpf).replace(c.DOT, c.BLANK), str(url), headers, interval=interval
                )
            )
            hell.start()
            time.sleep(0.250)
