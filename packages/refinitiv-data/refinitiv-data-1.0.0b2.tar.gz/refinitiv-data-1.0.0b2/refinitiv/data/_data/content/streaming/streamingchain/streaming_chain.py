...
from ....delivery.stream import StateManager, StreamState
...
class StreamingChain(StateManager, _LogReporter):
    """
    ...
    close()
        Closes the StreamingChain connection, releases resources
    ...
    def __init__(
        ...
        self._displayTemplateLock: Lock = Lock()
        self._displayTemplate = None

        #   store a list of decoded constituents
        self._constituentListLock: Lock = Lock()
        ...
        self._chain_records: ChainRecords = ChainRecords(self, self._session)

        #   store is the chain record decode completed or not?
        self._is_complete_decoded: Future = self._session._loop.create_future()

        #   store the chain record name to the number of offset to this chain record
        #   from the root of the chain record
        self._chainRecordNameToNumOffsetsFromRootChainRecordDict: dict = {self._name: 0}

        #   remaining update messages to be processed
        self._remainingUpdateMessageListLock: Lock = Lock()
        self._remainingUpdateMessageList: list = []
        ...
        #   call an initialize function for constructing
        #   the streaming for this chain record
        self._initialize()

    def __repr__(self):
        repr_str = super().__repr__()
        end_repr_str = f" {{name='{self._name}', state={self.state}}}>"
        repr_str = repr_str.replace(">", end_repr_str)
        return repr_str
    ...
    def _num_summary_links(self):
        ...
        it possible that number of summary links return None
        if it is a invalid chain record.
        """
    ...
    def summary_links(self):
        ...
        Returns
        -------
        a list of the summary links of this chain record,
        if the chain record is valid
    ...
    def get_constituents(self) -> list:
        ...
        #   copy constituents from stream and do skip summary links
        #   if skipSummaryLinks is True
    ...
    async def _do_open_async(self, with_updates=True):
        ...
        self._debug(f"StreamingChain :: Processing decode chain record {self._name}.")
        ...
        #   initialize the dictionary mapping between
        #   chain record name to offset from root of chain recrd
        ...
        while name:
            ...
            if self._chain_records.not_has_stream(name):
                #   this chain record doesn't open as streaming yet,
                #   so construct the stream and open it
            ...
            #   check this chain record is already open
            #   as stream and its chain record is ready or not?
            record = self._chain_records.get_record(name)

            #   check this chain record is valid or not?
            if record:
                #   this is a valid chain record,
                #   so get the next chain response chain record
                ...
                #   something wrong, we should wait for first response
                #   from interested chain record stream
    ...
    def _process_chain_record(self, name, message):
        """
        This function is designed for processing chain record.
        this will happen when got the refresh response.
        The processing steps are parsing the response message,
        request next chain record from stream if it doesn't exist
        """
        self._debug(
            f"StreamingChain :: "
            f"Processing response chain record {name}, message = {message}."
        )
    ...
    def _parse_chain_record(self, name, fields):
        """
        Parse the chain record then construct chain record
        and store as a mapping between chain record name
        to chain record object.
        """
        self._debug(
            f"StreamingChain :: "
            f"Parsing response field of chain record {name}, field = {fields}."
        )

        #   check the check the chain record template
        #       there are three chain record templates
        #       (template #80, template #85 and template #32766)
        if ChainRecord17Chars.isValidChainRecord(fields):
            ...
            self._error(
                f"StreamingChain :: "
                f"Cannot parse chain {name} because it is an invalid chain."
            )
            #   do nothing, done
            return
        ...
        with self._displayTemplateLock:
            if not self._displayTemplate:
                # this is a first time that parsing found display
                # template properties on this chain record, so store it
    ...
    def _update_chain_record(self, name, update):
        ...
        self._debug(
            f"StreamingChain :: "
            f"Updating response field of chain record {name}, update = {update}."
        )
        ...
        for index, (
            ...
            relative_index = offset_from_root_chain_record + index

            # check if old/new constituent are not a empty,
            # this mean it updated on this index
            if old_constituent and new_constituent:
                #   this is an update constituent, so call update
                self._update_constituent(
                    relative_index, old_constituent, new_constituent
                )

            # check if old constituent is a empty and new constituent is not a sempty,
            # this mean it add on this index
            elif not old_constituent and new_constituent:
                #   this is an add constituent, so call add
                self._append_constituent(relative_index, new_constituent)

            # check if old constituent is not  aempty and new constituent is a empty,
            # this mean it remove on this index
            elif old_constituent and not new_constituent:
                #   this is an add constituent, so call remove
                self._remove_constituent(relative_index, old_constituent)

        self._debug(
            f"StreamingChain :: "
            f"chain record update info = {chain_record_update_info}."
        )

    # warning CODE_ME if prev/next field are changed.
    # It rarely those fields are changed.

    def _process_remaining_update_messages(self):
        """
        This function is designed for processing
        the remaining update message due to wait for decode completed
        """
        self._debug("StreamingChain :: Processing remaining update messages.")
            ...
            try:
                with self._remainingUpdateMessageListLock:
                    (
                        stream_name,
                        update_message,
                    ) = self._remainingUpdateMessageList.pop(0)
            except IndexError:
                #   no remaining update message, done
                break

            #   process remaining update message
            self._update_chain_record(stream_name, update_message)

        #   done
        self._debug("StreamingChain :: DONE - Processing remaining update messages.")
    ...
    def _add_constituent(self, index, constituent):
        ...
        self._debug(
            f"StreamingChain.add_constituent("
            f"index = {index}, "
    ...
    def _append_constituent(self, index, constituent):
        ...
        """
        self._debug(
            f"StreamingChain.append_constituent("
            f"index = {index}, "
            f"constituent = {constituent})"
        )
    ...
    def _remove_constituent(self, index, constituent):
        ...
        self._debug(
            f"StreamingChain.remove_constituent("
            f"index = {index}, "
            f"constituent = {constituent})"
        )
    ...
    def _update_constituent(self, index, old_constituent, new_constituent):
        """Update the constituent in the chain record"""
        self._debug(
            f"StreamingChain.update_constituent("
            f"index = {index}, "
            f"oldConstituent = {old_constituent}, "
            f"newConstituent = {new_constituent})"
        )
        assert 0 <= index < len(self._constituentList)

        #   do update given constituent in the chain record
        with self._constituentListLock:
            self._constituentList[index] = new_constituent

        self._dispatch_event(self._on_update, index, old_constituent, new_constituent)
    ...
    def _on_complete_handler(self, stream):
        ...
        self._debug(f"StreamingChain.on_complete(stream = {stream.name})")

    def _on_error_handler(self, stream, error):
        """Streaming callback function on error"""
        self._debug(f"StreamingChain.on_error(stream = {stream.name}, error = {error})")
    ...
    def _dispatch_event(self, callback, *args, **kwargs):
        ...
        try:
            ...
            self._error(
                f"{callback} user function on streaming chain {self._name} "
                f"raised error {e}"
            )
