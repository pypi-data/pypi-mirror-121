...
from ..tools._datetime import to_datetime, get_date_from_today
import pandas as pd
import numpy
...
def get_timeseries(
    ...
    start_date: string or datetime.datetime or datetime.timedelta
        ...
        You can use the helper legacy get_date_from_today, please see the usage in the examples section
    ...
    normalize: boolean, optional
        if set to True, the legacy will return a normalized data frame with the following columns 'Date','Security','Field'
    ...
    raw_output: boolean, optional
        ...
        if set to False, the legacy will return a data frame which shape is defined by the parameter normalize
        Default: False
    ...
    if start_date > end_date:
        with "end date ({})should be after than start date ({})".format(
            end_date, start_date
        ) as error_msg:
