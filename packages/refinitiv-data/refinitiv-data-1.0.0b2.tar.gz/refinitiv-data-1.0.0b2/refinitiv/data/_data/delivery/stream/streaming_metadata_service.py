...
class StreamingMetadataService(StateManager, LogReporter):
    ...
    def __init__(self, session, connection=None, service=None):
        ...
        self._dictionary_stream = DictionaryStream(
            ...
            name=MetadataType.RWF_FLD.value,
        ...
        self._metadata_state[MetadataType.RWF_FLD] = ServiceMetadataEvent.NotInitialized
        ...
        self._enumtype_stream = DictionaryStream(
            ...
            session=self._session,
            connection=self._connection,
            name=MetadataType.RWF_ENUM.value,
            service=self._service,
            on_refresh=self._on_refresh,
            on_status=self._on_status,
            on_error=self._on_error,
        ...
        self._metadata_state[
            MetadataType.RWF_ENUM
        ] = ServiceMetadataEvent.NotInitialized
    ...
    def _on_refresh(self, dictionary: MetadataType, message):
        with self._stream_lock:
            ...
            if dictionary == MetadataType.RWF_FLD.value:
                self._decode_field_dictionary(message)
            elif dictionary == MetadataType.RWF_ENUM.value:
    ...
    def _on_complete(self, dictionary: MetadataType):
        with self._stream_lock:
            if self.is_open:
                self._session.debug(f"Dictionary {self._name} - Receive complete")
...
class DictionaryStream(_OMMStream):
    def __init__(
        self,
        ...
        _OMMStream.__init__(self, session, connection=connection)
