...
from ....delivery.stream import OMMStream, StreamState


class ChainRecords(_LogReporter):
    def __init__(self, streaming_chain, session):
        ...
        # ie. complete chain record named ".DJI" contains
        # with three chain records including
        # "0#.DJI", "1#.DJI", "2#.DJI"
    ...
    async def open_stream(self, name, with_updates):
        self._debug(f"Opening stream of chain record = {name}.")
        stream = self.streams_by_name[name]
        await stream.open_async(with_updates=with_updates)

    def is_status_closed(self, name):
        # warning THIS IS A HACK.
        # THE STREAMING CHAIN NEED TO REFACTOR
        # FOLLOWING THE NEW DESIGN OF STREAMING WEBSOCKET
    ...
    def close_streams(self):
        ...
        with self.lock["streams"]:
            for name, stream in self.streams_by_name.items():
                ...
                self._debug(f"Closing stream of chain record = {name}.")
                stream.close()
    ...
    def refreshed(self, name):
        ...
        with self.lock["refreshing"]:
            ...
            # warning :: PREVENT AND ERROR WHEN IT HAS MULTIPLE REFRESH
            # MESSAGE FROM SERVER
            #            PLEASE RECHECK THE PROTOCOL ON SERVER SIDE
            if not refreshing.done():
                # it's possible that it's receiving
                # a refresh message multiple time from server
