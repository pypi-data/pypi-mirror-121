...
from ...tools._datetime import Converter
...
regexp_delta_files = re.compile(r".*(-(d|D)elta-).*\d{4}-\d{2}-\d{2}.*")
regexp_init_file = re.compile(r".*(-(i|I)nit-).*\d{4}-\d{2}-\d{2}.*")
...
def get_filesets_with_the_newest_init_file_and_delta_files(file_sets):
    init_filesets = filter(lambda a: "init" in a["name"], file_sets)
