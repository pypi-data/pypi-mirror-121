...
from ._stream_cxn_manager import stream_cxn_manager
from ... import log
...
class StreamConnection(threading.Thread, LogReporter, abc.ABC):
    """
    This class is designed to be a abstract class.
    this class manage the login and close to/from websocket protocol.
    ...
    The following must be called to set future object to be done
    when received login or close response.
    ...
    def __init__(
        ...
        connection_config: "StreamConnectionConfiguration",
    ...
    def run(self):
        ...
        #   initialize flags
        #       cleanup flag
        stream_cxn_manager.register(self)
        ...
        while not self.is_closing and self._do_reconnect:
            ...
            if self._session._logger.level == log.TRACE:
            ...
            self._websocket.run_forever(
                ...
                # TEMPORARY :: disable ping interval because it has a bug in websocket-client v.0.57
                # ping_interval=300,
            )
    ...
    async def _close_async(self):
        ...
        if self._state != self.State.CLOSED:
            ...
            stream_cxn_manager.unregister(self)
