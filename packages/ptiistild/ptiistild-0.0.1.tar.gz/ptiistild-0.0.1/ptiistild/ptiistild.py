...
class ptiistild:
    ...
    def _check_vulnerable(self):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Testing {self.url}", "TITLE", self.use_json))
        for method in self.METHOD_LIST:
            ...
            if r_1.status_code != r_2.status_code:
                ...
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Different status codes for method {method} [{r_1.status_code}] & [{r_2.status_code}]", "INFO", self.use_json))
                grab_method = method
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"HTTP Head Server response: {r_1.headers.get('Server', 'None')}", "INFO", self.use_json))
        ...
        if self.is_vulnerable:
            ...
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"{self.url} is vulnerable to IIS Tilde Enumeration", "VULN", self.use_json))
            return grab_method
        else:
            self.ptjsonlib.set_vulnerable(self.json_no, "False")
            ptmisclib.ptprint(ptmisclib.out_ifnot(f"{self.url} not vulnerable to IIS Tilde Enumeration", "NOTVULN", self.use_json))

    def _grab_filenames(self):
        """Grabs/Bruteforces all the information"""
        ptmisclib.ptprint(ptmisclib.out_ifnot("Grabbing information", "INFO", self.use_json))
    ...
    def _grab_thread(self, target):
        ...
        ptmisclib.ptprint(ptmisclib.out_ifnot(target, "", self.use_json), end=f"{' '*10}\r")
        if "." in target:
            extension = True
            wildcard = "*"
        else:
        ...
        if extension:
            ...
            if len(target[:dot_char]) < 3:
                ptmisclib.ptprint(ptmisclib.out_ifnot(f"Complete filename without extension found {target}~1", "", self.use_json))
                self._add_targets(target)
            else:
                tilda_char = target.find("~")
                if len(target[dot_char+1:]) < 3:
                    ptmisclib.ptprint(ptmisclib.out_ifnot(f"File found {target} [{target.replace('~1', '*')}]", "", self.use_json))
            ...
            if len(target) < 6:
                ...
                if self._check_filename(target+"*~1"+"/.aspx"):
                    ptmisclib.ptprint(ptmisclib.out_ifnot(f"Directory found {target}~1", "", self.use_json))
    ...
    def _check_filename(self, filename):
        ...
        for _ in range(20): 
            try:
                ...
                ptmisclib.ptprint(ptmisclib.out_ifnot(f".", "", self.use_json), end=" ")
                time.sleep(0.5)
                continue
    ...
    def _print_result(self):
        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Found directories: {len(self.result['directories'])}", "", self.use_json))
        ptmisclib.ptprint(ptmisclib.out_ifnot('\n'.join(i for i in self.result["directories"]), "", self.use_json))

        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Found files: {len(self.result['files'])}", "", self.use_json))
        ptmisclib.ptprint(ptmisclib.out_ifnot('\n'.join(i for i in self.result["files"]), "", self.use_json))

        ptmisclib.ptprint(ptmisclib.out_ifnot(f"Found complete files: {len(self.result['complete_files'])}", "", self.use_json))
