...
class BlackBox(ABC):
    ...
    def evaluate_objective(self, parameters: Dict[str, float], check_input=True) -> float:
        """
    ...
    def evaluate_multiple_inputs(self, parameters_list: List[Dict[str, float]], check_input=True) \
            -> List[Dict[str, float]]:
        return [self.evaluate(params, check_input) for params in parameters_list]

    def evaluate_objective_multiple_inputs(self, parameters_list: List[Dict[str, float]], check_input=True) \
            -> List[float]:
        return [self.evaluate_objective(params, check_input) for params in parameters_list]

    def evaluate_multiple_inputs_np_array(self, parameters_list: np.ndarray, check_input=True) \
            -> List[Dict[str, float]]:
        return [self.evaluate_np_array(params, check_input) for params in parameters_list]

    def evaluate_objective_multiple_inputs_np_array(self, parameters_list: np.ndarray, check_input=True) \
            -> List[float]:
        return [self.evaluate_objective_np_array(params, check_input) for params in parameters_list]

    def evaluate_np_array(self, parameters: np.array, check_input=True) -> Dict[str, float]:
        """
        ...
        if check_input and not self.is_input_valid(params):
            raise ValueError("The provided input is not valid.")
        return self.evaluate(params, check_input)

    def evaluate_objective_np_array(self, parameters: np.array, check_input=True) -> float:
        """
