        ...
        while not self._done:
            ...
            for h in self._config.get("harvesters", default=[]):
                ...
                for w in ws:
                    ...
                    if p is None:
                        logging.info("[HarvesteringExecutor::__arun__] error unknown harvester ({})".format(h.strip().lower()))
                        raise SystemExit
            ...
            for w in workers:
                if w[1] not in self._plugins:
                    logging.error("[HarvesteringExecutor::__arun__] is plugin '{}' installerd in system?".format(w[1]))
                    sys.exit(1)
