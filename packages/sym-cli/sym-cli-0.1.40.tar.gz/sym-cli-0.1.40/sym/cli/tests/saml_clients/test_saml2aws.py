...
def test_saml2aws(
    ...
    capture_command.assert_command(
        f"saml2aws --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- env",
        f"saml2aws --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- env",
        f"saml2aws --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml login",
        f"saml2aws --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- 'aws ssm start-session --target i-0123456789abcdef'",
    )
...
def test_saml2aws_debug(
    ...
    with test_context(debug=True) as saml2aws:
        with capture_command():
            saml2aws.exec("env")
    capture_command.assert_command(
        f"saml2aws --verbose --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- env",
        f"saml2aws --verbose --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- env",
        f"saml2aws --verbose --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml login",
        f"saml2aws --verbose --config {saml2aws.config_file} --idp-account sym-catamorphic --skip-prompt --cache-saml exec -- env",
    )
...
def test_saml2aws_multi_threads(subprocess_friendly_asserter) -> None:
    ...
    commands = (
        f"saml2aws --config {config_file} --idp-account sym-test --skip-prompt --cache-saml exec -- env",
        f"saml2aws --config {config_file} --idp-account sym-test --skip-prompt --cache-saml exec -- env",
        f"saml2aws --config {config_file} --idp-account sym-test --skip-prompt --cache-saml login",
        f"saml2aws --config {config_file} --idp-account sym-test --skip-prompt --cache-saml exec -- env",
    )
