...
def taskchecker():
    try:
        ...
        if FOUNDFUNCTIONTIMEOUT == FUNCTIONTIMEOUT:
            print(bcolors.OKGREEN + "Function timout is correct" + bcolors.ENDC)
        else:
            print(bcolors.FAIL + "Function timout is not correct, it should be %s, please update by Function >> Configuration >> >> General Configuration" % TOTALMEMORY + bcolors.ENDC)
            sys.exit(1)


    except:
        print(bcolors.FAIL + "Something went wrong, please redo the task" +  bcolors.ENDC)
