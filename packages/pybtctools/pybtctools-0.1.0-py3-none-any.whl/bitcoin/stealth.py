...
def pubkeys_to_basic_stealth_address(scan_pubkey, spend_pubkey, magic_byte=42):
    ...
    hex_data = "00{0:066x}01{1:066x}0100".format(
        int(hex_scankey, 16), int(hex_spendkey, 16)
    )
...
def basic_stealth_address_to_pubkeys(stealth_address):
    ...
    if len(hex_data) != 140:
        raise Exception(
            "Stealth address is not of basic type"
            + "(one scan key, one spend key, no prefix)"
        )
...
def mk_stealth_metadata_script(ephem_pubkey, nonce):
    ...
    return (
        ...
        + version
        + "{0:08x}".format(nonce)
        + main.encode_pubkey(ephem_pubkey, "hex_compressed")
    )
