...
def is_testnet(inp):
    ...
    # TXID
        ...
        base_url = (
            "http://api.blockcypher.com/v1/btc/{network}/txs/{txid}?includesHex=false"
        )
        try:
            # try testnet fetchtx
    ...
    for a in addrs:
        ...
        try:
            ...
            for o in jsonobj["unspent_outputs"]:
                ...
                u.append(
                    {"output": h + ":" + str(o["tx_output_n"]), "value": o["value"]}
                )
        except Exception:
...
def helloblock_unspent(*args):
    ...
    if network == "testnet":
        url = (
            "https://testnet.helloblock.io/v1/addresses/%s/unspents?limit=500&offset=%s"
        )
    elif network == "btc":
        url = (
            "https://mainnet.helloblock.io/v1/addresses/%s/unspents?limit=500&offset=%s"
        )
    o = []
    for addr in addrs:
...
def bci_pushtx(tx):
    if not re.match("^[0-9a-fA-F]*$", tx):
        tx = tx.encode("hex")
    return make_request(
        "https://blockchain.info/pushtx", from_string_to_bytes("tx=" + tx)
    )
...
def helloblock_pushtx(tx):
    if not re.match("^[0-9a-fA-F]*$", tx):
        tx = tx.encode("hex")
    return make_request(
        "https://mainnet.helloblock.io/v1/transactions", "rawTxHex=" + tx
    )
...
def blockr_fetchtx(txhash, network="btc"):
    ...
    if isinstance(txhash, list):
        txhash = ",".join(
            [
                x.encode("hex") if not re.match("^[0-9a-fA-F]*$", x) else x
                for x in txhash
            ]
        )
...
def get_block_at_height(height):
    j = json.loads(
        make_request(
            "https://blockchain.info/block-height/" + str(height) + "?format=json"
        ).decode("utf-8")
    )
    for b in j["blocks"]:
...
def _get_block(inp):
    if len(str(inp)) < 64:
        ...
        return json.loads(
            make_request("https://blockchain.info/rawblock/" + inp).decode("utf-8")
        )
...
def get_block_timestamp(height, network="btc"):
    ...
    if isinstance(height, list):
        k = json.loads(
            make_request(blockr_url + ",".join([str(x) for x in height])).decode(
                "utf-8"
            )
        )
        o = {
            x["nb"]: calendar.timegm(time.strptime(x["time_utc"], "%Y-%m-%dT%H:%M:%SZ"))
            for x in k["data"]
        }
...
def get_block_header_data(inp, **kwargs):
    f = block_header_data_getters.get(
        kwargs.get("source", ""), bci_get_block_header_data
    )
    return f(inp, **kwargs)
...
def get_block_height(txhash):
    j = json.loads(
        make_request("https://blockchain.info/rawtx/" + txhash).decode("utf-8")
    )
