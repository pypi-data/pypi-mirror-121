...
class Interactive_UNet3d(nn.Module):
    def __init__(self, n_modaliities = 1, prev_seg_channels = 1, patch_size = (128, 128, 128), final_output_channels = 1):
        super().__init__()
        self.patch_size = np.array(patch_size)
        self.networks_depth = self.get_networks_depth()
        self.encoder_blocks = []
        ...
        conv_kwargs1 = {'kernel_size' : 3, 'padding' : 1, 'stride' : 1}
        self.encoder1 = nn.Sequential(
            ConvDropoutNormNonlin(n_modalities + prev_seg_channels, 32, conv_kwargs= conv_kwargs1, conv_op = nn.Conv3d),
            ConvDropoutNormNonlin(32, 32, conv_kwargs = conv_kwargs1, conv_op = nn.Conv3d)
        )
        conv_kwargs1 = {'kernel_size' : 3, 'padding' : 1, 'stride' : 2}
        conv_kwargs2 = {'kernel_size' : 3, 'padding' : 1, 'stride' : 1}
    ...
    def points_to_volume(self, shape, points, mode = 'positive', device = None):
        result = torch.zeros(shape).to(device)
        if len(points) == 0:
            return result
        bs = torch.tensor(points).T[0]
        x = torch.tensor(points).T[1]
        y = torch.tensor(points).T[2]
        z = torch.tensor(points).T[3]
        if mode == 'positive':
            result[bs, x, y, z] = 1
    ...
    def forward(self, x, positive_points, negative_points, prev_seg = None):
        ...
        positive_input = self.points_to_volume((bs, 1, nx, ny, nz), positive_points, mode = 'positive', device = x.device.index)
        negative_input = self.points_to_volume((bs, 1, nx, ny, nz), negative_points, mode = 'negative', device = x.device.index)

        pointers = torch.cat((positive_input, negative_input), dim =1)
        ...
        x1_ = F.sigmoid(self.final_layer1(x1_).view(bs, self.final_output_channels, -1), dim = 1).view(bs, self.final_output_channels, nx, ny, nz)
        x2_ = F.sigmoid(self.upsample2(self.final_layer2(x2_)).view(bs, self.final_output_channels, -1), dim = 1).view(bs, self.final_output_channels, nx, ny, nz)
        x3_ = F.sigmoid(self.upsample3(self.final_layer3(x3_)).view(bs, self.final_output_channels, -1), dim = 1).view(bs, self.final_output_channels, nx, ny, nz)
        x4_ = F.sigmoid(self.upsample4(self.final_layer4(x4_)).view(bs, self.final_output_channels, -1), dim = 1).view(bs, self.final_output_channels, nx, ny, nz)
