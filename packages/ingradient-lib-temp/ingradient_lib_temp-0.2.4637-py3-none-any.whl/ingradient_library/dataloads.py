...
class CustomDataset(Dataset):
    def __init__(self, root_dir, device = None, normalizer = None, mode = 'train', dim = '3d', resample = None):
    ...
    def __getitem__(self, idx):
        ...
        if self.resample != None:
            x = self.resample(x.unsqueeze(0), info_data, mode = 'x').squeeze(0)
            y = self.resample(y.unsqueeze(0), info_data, mode = 'y').squeeze(0)
...
class DataLoader3D(object):
    ...
    def __init__(self, dataset, augmentation = None, num_iteration = 2, patch_size = (128,128,128), device = 0, batch_size = 2, seg_one_hot = False, is_half = False, info_class = None):
