...
class Interactive_DataLoader(DataLoader3D):
    ...
    def generate_train_batch(self):
        ...
        print("Error Check Seg0", torch.unique(oversample_seg))
        if self.transform != None: 
            oversample_images, oversample_seg = self.transform(oversample_images, oversample_seg, self.info)
            
        print("Error Check Seg1", torch.unique(oversample_seg))
    ...
    def sampling(self, images, seg):
        print("Error Check Seg2", torch.unique(seg))
        for i in range(self.batch_size):
            random_index = torch.randint(low = 1, high = seg.max().long() + 1, size =(1,)).to(seg.device.index)
            if i == 0:
                new_seg = (seg == random_index).long()

            else:
                new_seg = torch.vstack((new_seg, (seg == random_index).long()))
