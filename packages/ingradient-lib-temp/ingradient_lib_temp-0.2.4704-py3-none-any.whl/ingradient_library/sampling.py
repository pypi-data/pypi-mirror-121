...
def get_pointer_tensor(bs, nx, ny, nz, positive_points, negative_points, device = None):
    positive_input = points_to_volume((bs, 1, nx, ny, nz), positive_points, mode = 'positive', device = x.device.index)
    negative_input = points_to_volume((bs, 1, nx, ny, nz), negative_points, mode = 'negative', device = x.device.index)

    return torch.cat((positive_input, negative_input), dim =1)
