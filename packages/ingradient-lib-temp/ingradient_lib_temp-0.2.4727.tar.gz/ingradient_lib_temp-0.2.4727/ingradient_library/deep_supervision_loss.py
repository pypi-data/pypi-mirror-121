...
class Deep_Supervision_Loss(nn.Module):
    ...
    def __forward__(self, output, target):
        ...
        if self.loss2 != None:
            result += self.loss2(output, target) * self.regularizer  # Loss Shape => (batch_size * deep supervision shape, -1)
...
class Multi_Dice_Loss(nn.Module):
    ...
    def __forward__(self, output, target):
        ...
        target = F.one_hot(target, num_classes = self.num_classes)[:,:, self.mask]
