...
def deepsupervision_CE_loss(output, target, device = 0):
    n_bs, n_ds, n_cls = output.shape[0], output.shape[1], output.shape[2]
    ...
    return F.cross_entropy(output, target, weight = torch.tensor([1, 141.08, 55.133, 200, 163.91]).float().to(device), reduction = 'none')
