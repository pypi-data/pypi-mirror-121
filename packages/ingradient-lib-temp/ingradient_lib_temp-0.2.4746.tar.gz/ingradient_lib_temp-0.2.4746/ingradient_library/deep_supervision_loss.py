...
class Deep_Supervision_Loss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        loss1 = self.loss1(output, target)
        if isinstance(loss1, nn.CrossEntropyLoss):
                loss1 = loss1.mean(-1)
        if self.loss2 != None:
            loss2 = self.loss2(output, target) # Loss Shape => (batch_size * deep supervision shape, -1)
            if isinstance(loss2, nn.CrossEntropyLoss):
                loss2 = loss2.mean(-1)


        result = loss1 + loss2 * self.regularizer 
        self.writer.add_scalar('Iteration_Loss/loss1', loss1[0])
        self.writer.add_scalar('Iteration_Loss/loss2', loss2[0])
...
class Multi_Dice_Loss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        intersection = output * target #각 class 별 intersection score를 합함.
        union = output + target #각 class 별 union score를 합함.
        dice = 2.0 * (intersection + self.smooth) / (union + self.smooth)
        
        return 1 - dice.sum(-1).mean(-1)
