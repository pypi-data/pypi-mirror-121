...
class Batch_Affine_3D(object):
    ...
    def gpu_calculate_coornidates(self, coords, matrices, img_size):
        ...
        print("ERORR Check Affine 0",torch.unique(coords))
        ...
        print("ERORR Check Affine 1",torch.unique(coords))

        for i in range(len(matrices)-1):
            matrices[i+1] = torch.matmul(matrices[i], matrices[i+1])
        print("ERORR Check Affine 2",torch.unique(coords))
    
        coords = torch.matmul(matrices[-1], coords.reshape(3, -1)).reshape(*before_shape)
        print("ERORR Check Affine 3",torch.unique(coords))
        coords = coords + half_img_size
        print("ERORR Check Affine 4",torch.unique(coords))
        coords = coords.reshape(3, -1).round() #Mapping 해주는 함수
        print("ERORR Check Affine 5",torch.unique(coords))
        coords = torch.max(torch.min(coords,  torch.Tensor([[0], [0], [0]]).long().to(self.device)) ,torch.Tensor(coords_bound).long().to(self.device)).long()
        print("ERORR Check Affine 6",torch.unique(coords))
