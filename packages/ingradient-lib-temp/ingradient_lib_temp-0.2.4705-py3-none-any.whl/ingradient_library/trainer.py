    ...
    def run(self):
        ...
        for e in range(self.n_epoch):
            ...
            while not self.tr_dl.is_end():
                ...
                pointers = get_pointer_tensor(bs, nx, ny, nz, positive_points, negative_points, device = images.device.index)
                output = self.model(images, pointers)
                self.optimizer.zero_grad()
                loss = self.loss_func(output, seg)
                train_loss = loss.item()
                ...
                for i in range(self.n_point):
                    ...
                    pointers = get_pointer_tensor(bs, nx, ny, nz, positive_points, negative_points, device = images.device.index)
                    output = self.model(images, pointers, prev_seg)
                    loss = self.loss_func(output, seg)
                    train_loss = loss.item()
