...
class Deep_Supervision_UNet3d(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.is_binary:
            ...
            x3_ = F.sigmoid(self.upsample3(self.final_layer3(x2_)).view(bs, self.final_output_channels, -1)).view(bs, self.final_output_channels, nx, ny, nz)
            x4_ = F.sigmoid(self.upsample4(self.final_layer4(x2_)).view(bs, self.final_output_channels, -1)).view(bs, self.final_output_channels, nx, ny, nz)
...
class Interactive_UNet3d(nn.Module):
    def __init__(self, n_modalities = 1, prev_seg_channels = 1, patch_size = (128, 128, 128), final_output_channels = 1):
        ...
        self.model = resnet200(**kwargs)
        input = n_modalities + final_output_channels + 1
        self.initial_layer = nn.Sequential(
            nn.Conv3d(input, 32, 3, 1, 1),
            nn.InstanceNorm3d(32),
            nn.LeakyReLU(),
    ...
    def forward(self, x, positive_points, negative_points, prev_seg = None):
        bs, nc, nx, ny, nz  = x.shape
        if prev_seg == None:
            prev_seg = torch.zeros((bs, nx, ny, nz)).to(x.device.index)
        
        x = torch.cat((x, (positive_points - negative_points).unsqueeze(1)), dim = 1)
        x = torch.cat((x, prev_seg.unsqueeze(1)), dim = 1)
