...
class DataLoader3D(object):
    ...
    def get_image_seg(self, current_index): #patch size보다 전체 이미지 크기가 작을 경우 padding을 진행
        ...
        if np.any(image_shape < self.patch_size):
            ...
            even_pad = even_pad[::-1].astype(int)
            images = F.pad(torch.tensor(images), tuple(even_pad.copy()), "constant", 0 )
            seg = F.pad(torch.tensor(seg), tuple(even_pad.copy()), "constant", 0 )
...
class DataLoader3DTest(DataLoader3D):
    ...
    def generate_train_batch(self):
        ...
        oversample_images, oversample_seg, pointers = self.sampling(oversample_images, oversample_seg)
        self.next_index()

        return oversample_images, oversample_seg, pointers
    
    def sampling(self, images, seg):
        #bs, x,y, z
        for i in range(self.batch_size):
            if i == 0:
                new_seg = (seg == torch.randint(low = 1, high = seg.max().long(), size =(1,))).long()
                pointers = torch.vstack((self.get_negative_pointer(new_seg), self.get_positive_pointer(new_seg))).unsqueeze(0)

            else:
                new_seg = torch.vstack((new_seg, (seg == torch.randint(low = 1, high = seg.max().long(), size =(1,))).long()))
                pointers = torch.vstack((pointers, torch.vstack((self.get_negative_pointer(new_seg), self.get_positive_pointer(new_seg))).unsqueeze(0)))
        new_images = torch.tile(images, [self.batch_size, *(torch.tensor(images.shape)//torch.tensor(images.shape))])
        
        return new_images, new_seg, pointers
    
    def get_negative_pointer(self, new_seg):
        negative_result = torch.zeros(new_seg.shape)
        negative_pos = torch.where (new_seg != 1)
        n_points = 10
        num_points_in_current_seg = np.random.randint(low =0 ,high = n_points, size = (1,))
        index = np.random.choice(np.arange(len(negative_pos[0])), size = num_points_in_current_seg, replace = False)
        negative_result[negative_pos[0][index], negative_pos[1][index]] = -1
        return negative_result

    def get_positive_pointer(self, new_seg):
        positive_result = torch.zeros(new_seg.shape)
        positive_pos = torch.where (new_seg == 1)
        n_points = 10
        num_points_in_current_seg = np.random.randint(low =1 ,high = n_points, size = (1,))
        index = np.random.choice(np.arange(len(positive_pos[0])), size = num_points_in_current_seg, replace = False)
        positive_result[positive_pos[0][index], positive_pos[1][index]] = 1
