    ...
    def run(self):
        ...
        for e in range(self.n_epoch):
            ...
            while not self.tr_dl.is_end():
                ...
                pointers = get_pointer_tensor(bs, nx, ny, nz, positive_points, negative_points)
                print("error0",pointers.shape)
                output = self.model(images, pointers.to(images.device.index))
                self.optimizer.zero_grad()
                loss = self.loss_func(output, seg)
                train_loss = loss.item()
                writer.add_scalar('Loss/Iteration_'+'0', train_loss)
                loss.backward()
                self.optimizer.step()
                ...
                for i in range(self.n_point):
                    ...
                    pointers = get_pointer_tensor(bs, nx, ny, nz, positive_points, negative_points)
                    print("error0",i,pointers.shape)
                    output = self.model(images, pointers.to(images.device.index), prev_seg)
                    loss = self.loss_func(output, seg)
                    train_loss = loss.item()
                    writer.add_scalar('Loss/Iteration_'+str(i+1), train_loss)
                    loss.backward()
                    self.optimizer.step()
