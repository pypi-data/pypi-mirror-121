...
class Cropping(object):
    def get_non_zero(self, seg):
        self.non_zero_index = np.where(seg != 0)
    def __call__(self, images, mode ='x'):
        if mode != 'x' and mode != 'y':
            raise ValueError("mode is wrong.")
        non_zero_index = np.where(images != 0)
        min_val = np.min(self.non_zero_index, axis = 1)
        max_val = np.max(self.non_zero_index, axis = 1)
