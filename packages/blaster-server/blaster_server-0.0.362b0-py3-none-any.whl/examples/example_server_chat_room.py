...
def decode_user(func):
	def new_func(*args , **kwargs):
		...
		if(auth_key):
			user_data = decode_signed_value("user_data", auth_key, SERVER_SECRET, max_age_days=10)
			#_type = user_data.get("type",None)
			if(user_data):
				kwargs["user"] = json.loads(user_data)
			
			else: # simple user_id
				user_id = decode_signed_value("user_key", auth_key, SERVER_SECRET, max_age_days=10)
				if(user_id):
...
def get_internal_auth_key():
	return create_signed_value("user_data", json.dumps({"user_id": "internal", "name": "don"}), SERVER_SECRET)
...
def create_or_update_user(request_params=None, user=None):
	...
	return {
		"type": TYPE_USER, "user_name": user_name,
		"auth_key": create_signed_value("user_data", json.dumps({"name": user_name, "user_id" : user_id or get_random_id(10)}), SERVER_SECRET)
	}
