...
class CBCharacteristic(ABleCharacteristic):
    ...
    :param permission: the characteristic permissions (read, write, read_encryption_required, etc.)
    ...
    async def notify_central_of_change(  # pylint: disable=arguments-differ
        ...
        try:
            result = peripheral_manager.updateValue_forCharacteristic_onSubscribedCentrals_(
        ...
        if not result:
            logger.warning(
                "could not send data... peripheral delegate manager cache full... trying again when ready"
            )
            ...
            logger.debug("peripheral manager delegate ready to send")
            await self.notify_central_of_change(central, data, notify_all)

        logger.debug("succesfully sent data to central(s)")
        self.value = data  # type: ignore
        self.obj.setValue_(data)
