...
@curry
def finds(ps: Iterable[Callable[[X], bool]], xs: Iterable[X]) -> Iterable[Option[X]]:
  buffer = []
...
def match_(*fs: Callable[..., Option[Y]]) -> Callable[..., Option[Y]]:
...
def catch(*fs: Callable[..., Option[Y]], default: Callable[..., Y]) -> Callable[..., Y]:
  f = match_(*fs)
...
def stripby(f: Callable[[X, X], bool]) -> Callable[[Iterable[X]], Iterable[X]]:
  return compose(lift(fst), groupby(f))
