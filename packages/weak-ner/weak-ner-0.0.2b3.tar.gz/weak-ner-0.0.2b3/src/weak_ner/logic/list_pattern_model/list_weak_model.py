...
from weak_ner.logic.utils import create_entity_set, get_list_from_dict
...
class WeakNERModel:
    ...
    def __init__(self, directory_path: str, use_preprocess: bool = True, pre_processing_options: tp.List[str] = []):
        ...
        self.__set_entities()
        self.__list_based_entities = {'DOC': self.documents,
                                      'COMP': self.companies,
                                      'WD': self.week_days,
                                      'REL': self.relatives,
                                      'VOC': self.vocatives}
    ...
    def __create_allowed_dict(self) -> None:
        ...
        """
        names_set = self.prepositions.union(self.names, self.surnames, {'e'})
        location_set = self.locations
        financial_set = self.prepositions.union(self.financial)
        dates_set = self.months.union(self.prepositions, self.week_days, {'date', 'number'})
        not_permitted_generic = self.financial.union(self.names,
                                                     self.surnames,
                                                     self.documents,
                                                     self.months,
                                                     self.companies,
    ...
    def __set_entities(self) -> None:
        ...
        """
        self.months = create_entity_set(self.__vocab_dict,
                                        'substantivos_meses')
        
        self.names = create_entity_set(self.__vocab_dict,
                                       'substantivos_nomes')
        self.surnames = create_entity_set(self.__vocab_dict,
                                          'substantivos_sobrenome')
        self.companies = create_entity_set(self.__vocab_dict,
                                           'substantivos_empresas',
                                           'substantivos_empresas_internacionais')
        self.documents = create_entity_set(self.__vocab_dict,
                                           'substantivos_documentos')
        self.vocatives = create_entity_set(self.__vocab_dict,
                                           'substantivos_vocativos')
        self.locations = create_entity_set(self.__vocab_dict,
                                           'substantivos_paises',
                                           'substantivos_cidades',
                                           'substantivos_continentes',
                                           'substantivos_estados')
        self.financial = create_entity_set(self.__vocab_dict,
                                           'substantivos_financeiros')
        self.week_days = create_entity_set(self.__vocab_dict,
                                           'substantivos_dias_da_semana')
        self.animals = create_entity_set(self.__vocab_dict,
                                         'substantivos_animais')
        self.relatives = create_entity_set(self.__vocab_dict,
                                           'substantivos_parentescos')
        self.cars = create_entity_set(self.__vocab_dict,
                                      'substantivos_carros')
    ...
    def __set_pos(self) -> None:
        ...
        """
        self.pronouns = get_list_from_dict(self.__vocab_dict, 'pronomes')
        self.articles = get_list_from_dict(self.__vocab_dict, 'artigos')
        self.prepositions = get_list_from_dict(self.__vocab_dict, 'preposicoes')
    ...
    def __correct_function(self, identified_entity: str, target_label: str) -> str:
        ...
        if len(split_correction) > 0 and split_correction[0] in self.prepositions:
            split_correction = split_correction[1:]
        if len(split_correction) > 0 and split_correction[-1] in self.prepositions:
            split_correction = split_correction[:-1]
    ...
    def __find_pattern(self, entity_pattern: str, tags: str, sentence: str, target_entity: str) -> tp.List[str]:
        ...
        if target_entity == 'DATE':
            found_pattern_lst += [word for word in split_message if word in self.months]
    ...
    def __check_entities_list(self, sentence: str) -> None:
        ...
        :type sentence: ``str``
        """
        if any(word in self.names or word in self.surnames for word in sentence.split()):
            self.__entities.append('PERS')
        if any(word in self.locations for word in sentence.split()):
            self.__entities.append('LOC')
        if any(word in self.financial for word in sentence.split()):
            self.__entities.append('FIN')
        if any(word in self.months or word == 'date' for word in sentence.split()):
