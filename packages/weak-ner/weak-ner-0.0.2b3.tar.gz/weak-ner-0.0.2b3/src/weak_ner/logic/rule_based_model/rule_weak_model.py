...
class WeakNERRules:
    ...
    def __set_entities(self) -> None:
        """
        Create a set with words of lists entities.

        Example: create a set with the 'names' words.
        """
        self.names = create_entity_set(self.__vocab_dict, 'substantivos_nomes')
        self.surnames = create_entity_set(self.__vocab_dict, 'substantivos_sobrenome')
    ...
    def __set_pos(self) -> None:
        """
        Create a set with words of lists Part of speech, pronouns,
        article and prepositions.

        Example: create a set with the 'pronouns' words.
        """
        self.preposition = get_list_from_dict(self.__vocab_dict, 'preposicoes')
        self.interjection = get_list_from_dict(self.__vocab_dict, 'interjeicoes')
    
    def __set_local_name(self):
        """
        Create a set with the interception between the list of local and the list of names and surnames.
        """
        self.local_or_name = [local for local in self.locations if self.__check_local_is_name(local)][1:]
    ...
    def __check_local_is_name(self, local: str) -> bool:
        ...
        :rtype: ``bool``
        """
        return all([word in self.names or word in self.surnames
                    or word in self.preposition for word in local.split()])
    ...
    def __apply_rules_names(self, sentence: str, ner: str) -> str:
        ...
        for match in self.person_pattern.finditer(ner, overlapped=True):
            ind = len(ner[:match.end() - 2].split()) - 1
            if msg_list[ind] in self.surnames and msg_list[ind] not in self.names:
                ner_list[ind] = 'O'
        return ' '.join(ner_list)
    ...
    def __apply_rules_financial(self, sentence: str, ner: str) -> str:
        ...
        for match in self.financial_pattern.finditer(ner, overlapped=True):
            ...
            if msg_list[ind] in self.preposition:
    ...
    def __apply_rules_wd(self, sentence: str, ner: str) -> str:
        ...
        for match in self.wd_pattern.finditer(sentence):
            ...
            if start < len(msg_list) - 3 and msg_list[start + 1] in ['via'] and \
                    msg_list[start + 2] in self.preposition and ner_list[start + 3] != 'O':
