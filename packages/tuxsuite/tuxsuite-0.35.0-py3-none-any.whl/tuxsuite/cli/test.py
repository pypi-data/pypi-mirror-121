...
def handle_list(options, config):
    ret = get(config, f"/v1/groups/{config.group}/projects/{config.project}/tests")
    if ret.status_code != 200:
        raise NotImplementedError()
    ...
    if options.json:
        ...
        for test in tests:
            ...
            state_msg = f"{colors.state(test.state, test.result)}{state}{colors.reset}"
            tests = ",".join(test.tests)
            pt = test.provisioning_time
            if pt is None:
                pt = "....-..-..T..:..:........."
            pt = pt[:-7]
            print(
                f"{datediff(previous_pt, pt)} {test.uid} [{state_msg}] {tests}@{test.device} {test.kernel}"
            )
