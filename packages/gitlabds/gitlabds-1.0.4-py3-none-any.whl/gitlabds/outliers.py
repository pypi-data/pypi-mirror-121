def mad_outliers (df, dv=None, min_levels=10, columns = 'all', threshold=4, inplace=False, verbose=True, windsor_threshold=0.01):
    """Median Absoutely Deviation for outlier detection and correction. 
    By default will windsor all numeric values in your dataframe that are more than 4 standard deviations above or below the median ('threshold').
    Will look at all numeric variables by default. Entering your outcome variable in dv will prevent it from being windsored. Min Levels will only 
    look at variables in which have more than the indicated levels. To limit to just  a subset of variables, you can pass the list of field names
    into to the 'variables' parameter. This will ignore any constraints put on by the 'dv' and 'min_levels' paramaters. 
    Inplace will replace existing df. Verbose will print outputs of windsoring being done."""
    ...
    if inplace==True:
        for v in var_list:
            ...
            if mad == 0:
                z_scores = [0.6745 * (x - mad) 
                                    for x in df[v]] #0.6745 to standardize it to 1 SD
                new_min = old_min
                new_max = old_max
            else:
        ...
        for v in var_list:
            ...
            if mad == 0:
                z_scores = [0.6745 * (x - mad) 
                                    for x in new_df[v]] #0.6745 to standardize it to 1 SD
                new_min = old_min
                new_max = old_max
            else:
                z_scores = [0.6745 * (x - median) / mad
