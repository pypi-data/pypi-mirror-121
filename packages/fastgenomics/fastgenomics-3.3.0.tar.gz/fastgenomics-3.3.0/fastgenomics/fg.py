...
def get_homedir() -> str:
    """ get the homedir """
    return path.normpath(path.expanduser("~/.fgcli"))


def run_zip(
    files_or_directory: List[str], zip_filename, zip_password: str
) -> str:
    ...
    params = [sevenZip, "a", zip_filename]
...
class LargeFileStorageAccessInformation:
    ...
    def __init__(
        ...
        title: str,
        download_uri: str,
        download_account: dict,
        user_accesstoken: str,
        upload_uri: str,
        upload_account: dict,
...
class FASTGenomicsClient:
    ...
    def __init__(self):
        ...
        retry = Retry(total=5, read=5, connect=5, backoff_factor=0.3)
    ...
    def _aws_run_cli_command(
        self,
        arguments: List[str],
        aws_account: Union[dict, None] = None,
    ...
    def _aws_create_account(self, user_name: str, aws_path: str) -> str:
        ...
        assert (
            region != ""
        ), "No AWS region found. Please check AWS configuration."

        configuration = self._get_configuration()

        org_user_name = user_name.lower()
        if PSEUDOMIZE_EMAIL_ADDRESSES:
            user_name = "fg-" + FASTGenomicsClient._pseudomize_email_address(
                user_name
            )
        else:
        ...
        json_filename = path.join(
            configuration.lfs_vault_directory,
            "aws-account-" + org_user_name + ".json",
        )
    ...
    def aws_configure_cloud(self) -> dict:
        ...
        * create a Managed Policy to 'FASTGenomics-S3Bucket-Read-Write-Policy' as policy for this user
        ...
        logger.info(f"creating AWS user '{user_name}'")
        process = self._aws_run_cli_command(
            [
                "iam",
                "create-user",
        ...
        logger.info(f"creating policy for limit access to s3 buckets")
        process = self._aws_run_cli_command(
            [
                "iam",
                "create-policy",
                ...
                policy_doc,
                "--description",
                "Policy for FASTGenomics Access to manage S3 buckets",
                "--path",
                aws_path,
            ],
            None,
        ...
        logger.info(f"attaching policy '{policy_name}' to user {user_name}")
        process = self._aws_run_cli_command(
            [
                "iam",
                "attach-user-policy",
    ...
    def version():
        """ get the version """
        return {"fg.py": f"{FG_CLI_VERSION}"}

    def _get_bearer_token(self) -> str:
        """ get a bearer token """
        account = self._get_fastgenomics_account()

        if account.fastgenomics_login_method == "bearer":
            return account.fastgenomics_passphrase
    ...
    def _get_fastgenomics_account(self) -> FASTGenomicsAccount:
        ...
        if not path.exists(filename):
            raise Exception(
                "You are not logged in! Please login using 'fg-cli login'"
            )
    ...
    def login(self, user: str, passphrase: str, login_method: str, url: str):
        ...
        passphrase   : if login_method is "pat" the personal access token (pat), if login_method is "bearer" the bearer token else the password of the user
        login_method : pat | password | bearer
        ...
        if login_method.lower() == "pat":
            login_method = "pat"
        elif login_method.lower() == "bearer":
            login_method = "bearer"
        elif login_method.lower() == "password":
            login_method = "password"
        else:
            raise KeyError(
                '"login_method must be "pat", "bearer", or "password".'
            )
...
class FASTGenomicsLargeFileStorageClient(FASTGenomicsClient):
    ...
    def __store_account(self, account: dict, email: str, provider_type: str):
        ...
        json_filename = path.join(
            configuration.lfs_vault_directory,
            f"{provider_type}-account-{email}.json",
        )
    ...
    def __get_stored_account(self, email: str, provider_type: str):
        ...
        if path.exists(json_filename):
            logger.info(f"reading upload account from file '{json_filename}'")
            with open(json_filename, "r") as f:
                data = f.read()
            return json.loads(data)
        return None

    def __create_large_file_storage(
        self,
        provider_type: str,
        full_filename: str,
        title: str,
        recipientInformation: dict,
        uploaderinformation: dict,
    ) -> LargeFileStorageAccessInformation:
        """
        ...
        logger.info(f"creating large file storage for file '{filename}'")
        ...
        try:
            ...
            self.__store_account(
                sai["uploadAccount"],
                self._get_fastgenomics_account().fastgenomics_user,
                provider_type,
            )
            self.__store_account(
                sai["downloadAccount"], recipient_email, provider_type
            )
    ...
    def __update_state(
        self,
        sai: LargeFileStorageAccessInformation,
        state: LargeFileStorageState,
    ):
    ...
    def __upload_to_azure(
        self, sai: LargeFileStorageAccessInformation, full_filename: str
    ):
        ...
        params = [azcopy, "copy", full_filename, sai.upload_uri]
    ...
    def __upload_to_aws(
        ...
        params = [aws, "s3", "cp", full_filename, sai.upload_uri]
        ...
        subprocess.run(params, shell=False, check=True, env=my_env)

        return

    def __create_dataset_and_attach_lfs(
        self,
        sai: LargeFileStorageAccessInformation,
        title: str,
        dataset_type: str = "cs-singlecell",
    ) -> str:
        ...
        request_data = {"title": title, "typeId": dataset_type, "files": []}
    ...
    def __store_report(
        self,
        storage_info: LargeFileStorageInformation,
        sai: LargeFileStorageAccessInformation,
        recipient_email: str,
        zip_password: str,
        full_filename: str,
        signed_downloadurl: str,
    ):
        ...
        report_filename = path.join(
            configuration.lfs_vault_directory,
            f"{storage_info.storage_id}-report.txt",
        )
        ...
        with open(report_filename, "w") as f:
            ...
            if signed_downloadurl != "" and sai.provider_type.lower() == "aws":
                print(
                    "The signed download url is valid for 7 days only.", file=f
                )
                print("", file=f)
            ...
            print(f"  {v}  ", file=f)
            print(
                f"  Click download button of large file storage and provide your user access token",
                file=f,
            )
            ...
            print("", file=f)
            print(
                f"  Extract files using 7-zip. Use this password to enable extraction",
                file=f,
            )
            ...
            if signed_downloadurl != "":
                ...
                print("", file=f)
                filename = path.basename(full_filename)
                print(f'  curl -o "{filename}" "{signed_downloadurl}" ', file=f)
                print("", file=f)
                print(
                    f"  Extract files using 7-zip. Use this password to enable extraction",
                    file=f,
            ...
            if sai.provider_type.lower() == "azure":
                ...
                print(f'  azcopy copy "{uri}" "{filename}"', file=f)
                print("", file=f)
                print(
                    f"  Extract files using 7-zip. Use this password to enable extraction",
                    file=f,
                ...
                print("", file=f)
                print(
                    "  Configure AWS using this credentials. (aws configure)",
                    file=f,
                )
                ...
                filename = path.basename(full_filename)
                uri = report["Download URI"]
                print(f'  aws cp "{uri}" "{filename}"', file=f)
    ...
    def upload_file_to_lfs(
        ...
        try:
            ...
            logger.info(f"UserAccessToken: {sai.user_accesstoken}")
            logger.info(f"Download URI   : {sai.download_uri}")

        except Exception as err:
            logger.error("Failed to create large file storage")
            self.__update_state(sai, LargeFileStorageState.ERROR)
            raise

        self.__update_state(sai, LargeFileStorageState.READY)
    ...
    def _ensure_recipient_account(
        self, provider_type: str, recipient_email: str
    ) -> dict:
...
class FASTGenomicsDatasetClient(FASTGenomicsClient):
    ...
    def add_file_to_dataset(
        ...
        if not path.isfile(full_filename):
            raise IOError(f"Path '{full_filename}' is no file.")

        if (
            filetype.lower() != "expressiondata"
            and filetype.lower() != "metadata"
        ):
        ...
        request_data = {"name": filename, "size": filesize}
        request_json = json.dumps(request_data)
        ...
        request_data = {"uploadId": uploadId, "type": filetype}
        request_json = json.dumps(request_data)
        ...
        show_bar = show_progress_bar is not False and (
            show_progress_bar or log_level <= logging.INFO
        )
    ...
    def set_metadata(self, dataset_id: str, metadata: dict):
        """ set bulk metadata for a dataset """
        assert dataset_id != "", "You have have to provide a dataset id"

        bearer_token = self._get_bearer_token()

        logging.info(f"setting metadata of dataset {dataset_id}")
        url = self._get_url(f"webclient/api/datasets/{dataset_id}/metadata")

        request_json = json.dumps(metadata)
        headers = {
            "Content-type": "application/json",
            "Accept": "application/json",
            "Authorization": f"Bearer {bearer_token}",
        ...
        response = self.session.put(url, data=request_json, headers=headers)
        self._handle_response(response)
    ...
    def create_dataset(
        self, title: str, dataset_type: str
    ) -> requests.Response:
    ...
    def get_dataset_types(self) -> requests.Response:
        """ get the available dataset types """

        bearer_token = self._get_bearer_token()

        logging.info(f"getting dataset types")
        url = self._get_url("webclient/api/datasettypes")
    ...
    def get_dataset_type_by_id(self, id) -> requests.Response:
        """ get details for a dataset type """

        bearer_token = self._get_bearer_token()

        logging.info(f"getting dataset types")
        url = self._get_url(f"webclient/api/datasettypes/{id}")
    ...
    def get_dataset_editable_fields(self, id) -> requests.Response:
        """ get all editable fields for a dataset type """

        bearer_token = self._get_bearer_token()

        logging.info(f'getting editable fields for dataset type "{id}".')
        url = self._get_url(f"webclient/api/datasettypes/{id}/editablefields")
