...
from CalculatingFeatures.common_mapper import calc_features_common
from CalculatingFeatures.acceleration_mapper import calc_features_acc
from CalculatingFeatures.gyro_mapper import calc_features_gyro
...
def calculate_rest(x2d, y2d, z2d, time2d, fs, feature_names, show_progress):
    ...
    for x, y, z, time in progress:
        ...
        if len(set(feature_names) & set(frequency_features)) > 0:
            row.update({str(key) + "_X": val for key, val in f.computeFreqFeatures(x, feature_names, fs).items()})
            if y is not None:
                row.update({str(key) + "_Y": val for key, val in f.computeFreqFeatures(y, feature_names, fs).items()})
                row.update({str(key) + "_Z": val for key, val in f.computeFreqFeatures(z, feature_names, fs).items()})
