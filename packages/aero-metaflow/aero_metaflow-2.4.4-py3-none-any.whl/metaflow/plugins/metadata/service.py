...
class ServiceMetadataProvider(MetadataProvider):
    ...
    def compute_info(cls, val):
        v = val.rstrip('/')
        try:
            resp = requests.get(os.path.join(v, 'ping'),
    ...
    def _get_or_create(
        ...
        create_path = self._create_path(
            obj_type, self._flow_name, run_id, step_name)
    ...
    def _get_request_data(cls, http_function, url, headers, auth, data=None):
        ...
        while 'LastEvaluatedKey' in resp.json():
            # task_id=N=2520
            base_last_evaluated_key = resp.json()['LastEvaluatedKey'].replace(
                "}", "").replace("{", "")
            ...
            exclusive_start_key[split_last_evaluated_key[0]
                                ][split_last_evaluated_key[1]] = split_last_evaluated_key[2]
                ...
                if data is None:
                    ...
                    if monitor:
                        with monitor.measure('metaflow.service_metadata.get'):
                            body, status_code = cls._get_request_data(
                                requests.get, url, METADATA_SERVICE_HEADERS, auth)
                    ...
                    if monitor:
                        with monitor.measure('metaflow.service_metadata.post'):
                            resp = requests.post(
                                url, headers=METADATA_SERVICE_HEADERS, json=data, auth=auth)
