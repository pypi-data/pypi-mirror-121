...
class Batch(object):
    ...
    def wait(self, stdout_location, stderr_location, echo=None):
        ...
        def wait_for_slot(pre_job):
            ...
            while job_blocked:
                print(TRACKING_URL.format(
                    vcpus=requested_cpu, memory=requested_memory))
                resp = requests.get(TRACKING_URL.format(
                    vcpus=requested_cpu, memory=requested_memory), headers=BATCH_METADATA_SERVICE_HEADERS)

                if resp.status_code == 200:
                    job_blocked = False
                    echo(
                        f"New job of {requested_cpu} vcpus has been submitted")

                    parsed_body = resp.json()

                    bar, percent = progress_bar(
                        parsed_body['vcpus'], parsed_body['vcpus_limit'])
                    echo(f"Currently used vCPUs: {bar} {str(percent)}% Usage")

                    bar, percent = progress_bar(
                        parsed_body['vcpus'] + parsed_body['requested_vcpus'], parsed_body['vcpus_limit'])
                    ...
                    if parsed_body['vcpus_limit'] < parsed_body['requested_vcpus']:
                        echo(
                            f"You have requested {parsed_body['requested_vcpus']} VCPUs, but you only have access to {parsed_body['vcpus_limit']}")
                    ...
                    echo(
                        f"New job of {requested_cpu} is currently over the limit of utilised vCPUs")

                    time.sleep(60)

                    if requested_cpu + current_vcpus > cpu_limit:
                        echo(
                            f"New job of {requested_cpu} would push CPU usage to {requested_cpu + current_vcpus}, which is higher than the limit of {cpu_limit}")
                    else:
                        echo(
                            f"Requesting {requested_cpu} vcpus, current usage now {requested_cpu + current_vcpus}")
                        job_blocked = False

        def wait_for_launch(job):
            status = job.status
            echo('Aero: Task submitted')
            t = time.time()

            while True:
                if status != job.status:
                    status = job.status
                    echo('Aero: Allocating resources')

                    t = time.time()
                if job.is_running or job.is_done or job.is_crashed:
                    echo("Aero: Task starting")
                    break

        prefix = b'[%s] ' % util.to_bytes("pre")
