...
def metadata(ms):
    ...
    if infos[0] in types:
        current_metadata = [
            m for m in METADATA_PROVIDERS if m.TYPE == infos[0]][0]
        if len(infos) > 1:
...
class MetaflowObject(object):
    ...
    def __init__(self,
        ...
        self._created_at = time.strftime(
            '%Y-%m-%dT%H:%M:%SZ', time.gmtime(self._object['ts_epoch']))
    ...
    def _get_object(self, *path_components):
        result = self._metaflow.metadata.get_object(
            self._NAME, 'self', None, *path_components)
    ...
    def __iter__(self):
        ...
        if current_namespace == "production":
            query_filter = {
                'any_tags': f"production:{self._aero_user}_{flow_id}"}
        else:
    ...
    def parent(self):
        ...
        if self._parent is None:
            ...
            self._parent = _CLASSES[self._PARENT_CLASS](
                parent_pathspec, _namespace_check=False)
        return self._parent
