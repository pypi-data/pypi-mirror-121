...
class Report:
    ...
    def get_diff(self):
        ...
        for start_time, end_time in zip(self.start_log_list, self.end_log_list):
            start = (start_time.split("_")[-1])
            end = (end_time.split("_")[-1])
            _s = datetime.strptime(end, "%I:%M:%S.%f")
            _e = datetime.strptime(start, "%I:%M:%S.%f")
            diff = \
                timedelta(hours=_s.hour, minutes=_s.minute, seconds=_s.second, microseconds=_s.microsecond)\
                -\
                timedelta(hours=_e.hour, minutes=_e.minute, seconds=_e.second, microseconds=_e.microsecond)
