...
class PulsarMessageConsumer(PulsarConnection):
    ...
    def connect(self):
        ...
        while not self.connectionAttempts > PulsarMessageConsumer.MAX_CONNECTION_ATTEMPTS:
            try:
                ...
                print(f'unable to establish connection (reason: {e}), next attempt in {self.reconnectTimeout} sec.')
    ...
    def _pulsar_ping(self, timeout, event: threading.Event):
        while not event.wait(timeout):
            if self.subscription and self.subscription.connected:
                ...
                print('quitting pulsar ping loop due to unconnected or missing subscription')
                break
                ...
                if response_payload:
                    ...
                    if len(response_payload) > self.MAX_PAYLOAD_SIZE:
                        ...
                        payload_fragments: List[str] = textwrap.wrap(
                            text=base64.b64encode(response_payload).decode('utf-8'), width=self.MAX_PAYLOAD_SIZE)

                        num_fragments = len(payload_fragments)
                        for i in range(num_fragments):
                            fragment_props = response_properties.copy()
                            fragment_props[PropertyKeys.FRAGMENT] = i
                            fragment_props[PropertyKeys.NUM_FRAGMENTS] = num_fragments
