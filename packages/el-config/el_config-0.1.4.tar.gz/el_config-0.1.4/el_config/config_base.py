...
class ConfigBase:
    ...
    Attributes:
        _CONFIGS_DIR  (str     ) [static]: Default configs directory. Defaults to '${PWD}/configs'.
        _PRE_LOAD     (function) [static]: Default lambda function for 'pre_load'. Defaults to <lambda config: config>.

        configs_dir   (str     ) [object]: Main configs directory to load all config files. Defaults to _CONFIGS_DIR.
        required_envs (str     ) [object]: Required environment variables to check. Defaults to []
        valid_schema  (dict    ) [object]: Validation schema to validate 'config'. This schema is based on 'Cerberus' package. Defaults to {}.
        config        (box.Box ) [object]: Main 'config' object based on 'python-box' package. Defaults to Box().
    ...
    def __init__(self, configs_dir: str=_CONFIGS_DIR, required_envs: list=[], valid_schema: dict={}, pre_load: callable=_PRE_LOAD):
