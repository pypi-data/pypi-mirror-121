...
from el_validator import checkers, Validator
...
class ConfigBase:
    ...
    Attributes:
        configs_dir   (str    ): Main configs directory to load all config files. Defaults to '${PWD}/configs'.
        required_envs (str    ): Required environment variables to check. Defaults to []
        valid_schema  (dict   ): Validation schema to validate 'config'. This schema is based on 'Cerberus' package. Defaults to {}.
        config        (box.Box): Main 'config' object based on 'python-box' package. Defaults to Box().

    Methods:
        _set_pre_load()        : Setting method for custom 'pre_load' method.
        ...
        _pre_load()            : Custom pre-load method, this method will executed before validating 'config'.
        _validate()            : Validate the 'config' by 'valid_schema'.
        _freeze_config()       : Freeze 'config' into immutable variable.
    """

    _CURRENT_DIR = os.path.join(os.getcwd(), 'configs')
    _PRE_LOAD = lambda config: config

    def __init__(self, configs_dir: str=_CURRENT_DIR, required_envs: list=[], valid_schema: dict={}, pre_load: callable=_PRE_LOAD):
        """ConfigBase constructor method.

        Args:
            configs_dir   (str,      optional): Main configs directory to load all config files. Defaults to '${PWD}/configs'.
            required_envs (list,     optional): Required environment variables to check. Defaults to [].
            valid_schema  (dict,     optional): Validation schema to validate 'config'. This schema is based on 'Cerberus' package. Defaults to {}.
            pre_load      (function, optional): Custom pre-load method, this method will executed before validating 'config'. Defaults to <lambda config: config>.
        """

        self.configs_dir = configs_dir
        self.required_envs = required_envs
        self.valid_schema = valid_schema
        self._set_pre_load(pre_load)

        self.config = Box()


    def _set_pre_load(self, pre_load: callable=_PRE_LOAD):
    ...
    def configs_dir(self):
        try:
            ...
            return ConfigBase._CURRENT_DIR
