...
import timeit
from typing import Iterable, Iterator, List, Optional, Tuple, cast, Union
...
class NcsReader(GordoBaseDataProvider):
    ...
    def can_handle_tag(self, tag: SensorTag):
        ...
        """
        return self.dl_base_path is not None or self.base_paths_from_asset(tag.asset)
        ...
        **kwargs,
        ...
        if self.threads:
            with ThreadPoolExecutor(max_workers=self.threads) as executor:
                ...
                for series in fetched_tags:
                    yield self.filter_series(series, train_start_date, train_end_date)
        else:
            for tag_dirs in tag_dirs_iter:
                series = self._load_series_mapper(tag_dirs, partitions, dry_run)
                yield self.filter_series(series, train_start_date, train_end_date)
    ...
    def read_tag_files(
        ...
        """
        tag_base_paths: List[str] = []
        if self.dl_base_path:
            tag_base_paths = [self.dl_base_path]
        if not tag_base_paths:
            base_paths = self.base_paths_from_asset(tag.asset)
            if base_paths:
                tag_base_paths = base_paths
    ...
    def base_paths_from_asset(self, asset: str) -> Optional[List[str]]:
        """
        ...
        full_paths = path_spec.full_paths(self.storage)

        logger.debug(f"Found asset code {asset}, returning {full_paths}")
        return full_paths
