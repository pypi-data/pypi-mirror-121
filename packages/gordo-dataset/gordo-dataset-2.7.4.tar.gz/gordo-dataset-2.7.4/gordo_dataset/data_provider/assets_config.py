...
class PathSpec:
    ...
    def full_paths(self, fs: FileSystem) -> List[str]:
        return [fs.join(self.base_dir, path) for path in self.paths]
...
class AssetsConfig:
    ...
    def load(cls, raw_config: dict, file_path: Optional[str] = None) -> "AssetsConfig":
        ...
        PathSpec(reader='ncs_reader', base_dir='/ncs_data', paths=['path/to/asset2'])
        ...
        for storage, reader_items in valid_config["storages"].items():
            ...
            for reader_item in reader_items:
                ...
                for asset in reader_item["assets"]:
                    ...
                    paths: List[str]
                    if "paths" in asset and asset["paths"]:
                        paths = asset["paths"]
                    elif "path" in asset and asset["path"]:
                        paths = [asset["path"]]
                    else:
                        message = (
                            f"'path' and 'paths' is empty in storage "
                            f"'{storage}' for asset '{name}', reader '{reader}'"
                        )
                        raise ConfigException(exception_message(message))
                    path_spec = PathSpec(reader, base_dir, paths)
