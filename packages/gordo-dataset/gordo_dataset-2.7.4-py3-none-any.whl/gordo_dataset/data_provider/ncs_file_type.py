...
from typing import Iterable, Optional, List, Tuple, Type, Dict, Pattern, cast
...
class NcsMonthlyParquetFileType(NcsFileType):
    ...
    def _file_name_re(self, tag_name_upper: str, partition: MonthPartition) -> Pattern:
        file_extension = self._file_type.file_extension
        file_name_pattern = (
            r"^"
            + re.escape(f"{tag_name_upper}")
            + r"\.?"
            + re.escape(
                f"_{partition.year}{partition.month:02d}{file_extension.upper()}"
            )
            + r"$"
        )
    ...
    def paths(
        ...
        for partition in month_partitions:
            ...
            file_name_re = self._file_name_re(tag_name_upper, partition)
            for path, file_info in files_list:
                _, file_name = fs.split(path)
                if cast(FileInfo, file_info).isfile():
                    if file_name_re.match(file_name.upper()):
...
class NcsYearlyParquetFileType(NcsFileType):
    ...
    def _file_name_re(self, tag_name_upper: str, partition: YearPartition) -> Pattern:
        file_extension = self._file_type.file_extension
        file_name_pattern = (
            r"^"
            + re.escape(f"{tag_name_upper}")
            + r"\.?"
            + re.escape(f"_{partition.year}{file_extension.upper()}")
            + r"$"
        )
        return re.compile(file_name_pattern)
