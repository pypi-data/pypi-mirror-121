...
class IrocReader(GordoBaseDataProvider):
    def can_handle_tag(self, tag: SensorTag):
        return bool(self.base_paths_from_asset(tag.asset))
    ...
    def load_series(
        ...
        **kwargs,
        ...
        base_paths_from_assets = []
        for tag in tag_list:
            base_paths = self.base_paths_from_asset(tag.asset)
            if base_paths:
                base_paths_from_assets.extend(base_paths)
    ...
    def base_paths_from_asset(self, asset: str):
        """
        ...
        if path_spec.reader != self.reader_name:
            return None
        full_paths = path_spec.full_paths(cast(FileSystem, self.storage))
