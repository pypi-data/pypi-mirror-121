...
from typing import List, Iterable, Tuple, Optional, Dict, Iterator, Union, Set, cast
...
class NcsLookup:
    ...
    def tag_dirs_lookup(
        self, base_dirs: List[str], tag_list: List[SensorTag]
        ...
        ----------
        base_dirs: List[str]
            List of base directories where all of the tags are placed
        ...
        """
        tags: Set[SensorTag] = set()
        tag_names: Dict[str, SensorTag] = {}
        for tag in tag_list:
            tags.add(tag)
            tag_name = self.quote_tag_name(tag.name).upper()
            tag_names[tag_name] = tag
            tag_name_without_suffixes = tag_name.rstrip(TAG_DIRS_OPTIONAL_SUFFIXES)
            if tag_name != tag_name_without_suffixes:
                tag_names[tag_name_without_suffixes] = tag
        for base_dir in base_dirs:
            for path, file_info in self.storage.ls(base_dir):
                if file_info is not None and file_info.isdir():
                    ...
                    if file_tag_name in tag_names:
                        tag = tag_names[file_tag_name]
                        if tag in tags:
                            yield tag, path
                            tags.remove(tag)
        for tag in tags:
    ...
    def assets_config_tags_lookup(
        ...
        for path_spec, asset_tags in asset_path_specs:
            for tag, tag_dir in self.tag_dirs_lookup(
                path_spec.full_paths(storage), asset_tags
            ):
                yield tag, tag_dir
