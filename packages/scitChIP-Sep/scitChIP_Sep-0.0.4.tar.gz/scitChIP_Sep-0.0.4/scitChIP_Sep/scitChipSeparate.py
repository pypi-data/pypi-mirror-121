...
def Separate_itChip(out_prefix,outfolder,f_filename,r_filename,f_ref,r_ref,start_forward,start_reverse,thresh_forward,thresh_reverse,range_check,compress):
    ...
    for i in range(int(len(f_list)/4)):
        match = f_list[i*4+1][(start_forward-range_check):(start_forward+range_check+len(f_ref[0]))]
        matchcount = 0
        matchref = ''
        for ref in f_ref:
    ...
    for i in range(int(len(r_list)/4)):
        match = r_list[i*4+1][(start_reverse-range_check):(start_reverse+range_check+len(r_ref[0]))]
        matchcount = 0
        matchref = ''
        for ref in r_ref:
    ...
    for key in barcodes.keys():
        ...
        if compress == True:
            pd.DataFrame(f_list).iloc[allindexes].to_csv(outfolder+'/'+out_prefix+'_'+key+'_R1.fq.gz',header=False,index=False,sep='\t',compression='gzip')
            pd.DataFrame(r_list).iloc[allindexes].to_csv(outfolder+'/'+out_prefix+'_'+key+'_R2.fq.gz',header=False,index=False,sep='\t',compression='gzip')
        else:
...
def main():
    ...
    optional.add_argument("-c", "--compress", dest="compress",
                        help="Indicate if result files Should be compressed. True or False accepted.", metavar="COMPRESS")
    ...
    if args['compress'] == None:
        ...
        compress = True
    elif args['compress'] == 'False':
        compress = False
    else:
        print('Only True or False accepted for --compress')
        sys.exit(1)

    Separate_itChip(args['out_prefix'],args['outfolder'],args['f_filename'],args['r_filename'],f_ref,r_ref,start_forward,start_reverse,thresh_forward,thresh_reverse,range_check,compress)