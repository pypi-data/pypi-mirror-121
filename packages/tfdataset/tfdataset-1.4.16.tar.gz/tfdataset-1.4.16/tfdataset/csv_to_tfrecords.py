...
def main(args):
    ...
    path_to_object_label_list = '{}.json'.format(object_name)
    f = open(Path(__file__).parent / Path(path_to_object_label_list), "rb")
    label_list = json.load(f)

    # write tfrecords
    writer = tf.python_io.TFRecordWriter(
        args.csv_path + '/{}.record'.format(args.data_type))
