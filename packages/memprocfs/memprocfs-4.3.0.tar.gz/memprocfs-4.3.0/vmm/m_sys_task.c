...
VOID VmmSysTask_Initialize_AddInfo(_In_ PVMM_TASK_SETUP_CONTEXT ctx, _In_ POB_REGISTRY_KEY pKey, _In_ PVMM_MAP_TASKENTRY pTask)
{
    ...
    ZeroMemory(pb, 0x24);
    if(VmmWinReg_ValueQuery5(ctx->pHive, pKey, "DynamicInfo", NULL, pb, sizeof(pb), NULL)) {
    ...
    if(VmmWinReg_ValueQuery5(ctx->pHive, pKey, "Actions", NULL, pb, sizeof(pb), &cb)) {
        dwVer = *(PWORD)pb;
...
VOID VmmSysTask_Initialize_Tree(_In_ PVMM_TASK_SETUP_CONTEXT ctx, _In_ POB_REGISTRY_KEY pKey, _In_ BOOL fTopLevel, _In_opt_ PVMM_MAP_TASKENTRY pParentTask)
{
    ...
    if(!fTopLevel) {
        ...
        if(!VmmWinReg_ValueQuery5(ctx->pHive, pKey, "Id", &dwKeyValueTp, pbRegGUID, sizeof(pbRegGUID) - 2, NULL)) { return; }
        if(!CharUtil_WtoU((LPWSTR)pbRegGUID, -1, pbBuffer, sizeof(pbBuffer), &uszGUID, NULL, 0)) { return; }
        if(!(pTask = VmmSysTask_Initialize_AddTask(ctx, KeyInfo.ftLastWrite, KeyInfo.uszName, uszGUID, pParentTask))) { return; }
    }
...
*/
PVMM_MAP_TASKENTRY VmmSysTask_Initialize_GetTaskByPath(_In_ PVMM_TASK_SETUP_CONTEXT ctx, _In_ LPSTR uszPath, _In_opt_ PVMM_REGISTRY_KEY_INFO pKeyInfo)
{
    QWORD qwHash;
    LPSTR uszNewName;
    ...
    if((uszNewPath[0] != 0) && (uszNewPath[1] != 0)) {
        pTaskParent = VmmSysTask_Initialize_GetTaskByPath(ctx, uszNewPath, NULL);
        if(!pTaskParent) { return NULL; }
    }
    return VmmSysTask_Initialize_AddTask(ctx, (pKeyInfo ? pKeyInfo->ftLastWrite : 0), uszNewName, (pKeyInfo ? pKeyInfo->uszName : NULL), pTaskParent);
}

VOID VmmSysTask_Initialize_AddInfoOrType(_In_ PVMM_TASK_SETUP_CONTEXT ctx, _In_ LPSTR uszKeyPath, _In_ DWORD dwTypeOpt)
{
    ...
    LPSTR uszPath;
    if((pObKey = VmmWinReg_KeyGetByPath(ctx->pHive, uszKeyPath)) && (pmObKeyList = VmmWinReg_KeyList(ctx->pHive, pObKey))) {
        while((pObSubKey = ObMap_Pop(pmObKeyList))) {
            ...
            if(!pTask && !dwTypeOpt) {
                ...
                if(VmmWinReg_ValueQuery5(ctx->pHive, pObSubKey, "Path", NULL, pbBufferPath, sizeof(pbBufferPath) - 2, NULL)) {
...
VOID VmmSysTask_Initialize_DoWork(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP)
{
    ...
    if(!VmmWinReg_KeyHiveGetByFullPath(TASKREGROOT "Tree", &ctxInit.pHive, &pObKey)) { goto fail; }
    VmmSysTask_Initialize_Tree(&ctxInit, pObKey, TRUE, NULL);
    // 3: ADD TASK ADDITIONAL INFO:
    VmmSysTask_Initialize_AddInfoOrType(&ctxInit, TASKREGROOT_LOCALHIVE "Tasks", 0);
    // 4: ADD TASK TYPE:
    VmmSysTask_Initialize_AddInfoOrType(&ctxInit, TASKREGROOT_LOCALHIVE "Boot", VMM_MAP_TASKENTRY_TP_BOOT);
    VmmSysTask_Initialize_AddInfoOrType(&ctxInit, TASKREGROOT_LOCALHIVE "Logon", VMM_MAP_TASKENTRY_TP_LOGON);
    VmmSysTask_Initialize_AddInfoOrType(&ctxInit, TASKREGROOT_LOCALHIVE "Maintenance", VMM_MAP_TASKENTRY_TP_MAINTENANCE);
    VmmSysTask_Initialize_AddInfoOrType(&ctxInit, TASKREGROOT_LOCALHIVE "Plain", VMM_MAP_TASKENTRY_TP_PLAIN);
...
*/
DWORD MSysTask_InfoFromEntry(_In_ PVMM_MAP_TASKENTRY pe, _Out_writes_(cbu) LPSTR usz, _In_ DWORD cbu)
{
...
*/
NTSTATUS MSysTask_ReadSingleEntry(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset, _In_ PVMM_MAP_TASKENTRY pTask, _In_ LPSTR uszPath)
{
    DWORD cbInfoFile;
    CHAR usz[MAX_PATH];
...
NTSTATUS MSysTask_Read(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
    ...
    if(!_stricmp(ctxP->uszPath, "tasks.txt")) {
        nt = Util_VfsLineFixed_Read(
            (UTIL_VFSLINEFIXED_PFN_CB)MSysTask_ReadLine_CB, pObTaskMap, MSYSTASK_LINELENGTH, MSYSTASK_LINEHEADER,
    ...
    if(!_strnicmp("by-guid\\", ctxP->uszPath, 8) || !_strnicmp("by-name\\", ctxP->uszPath, 8)) {
        ...
        nt = MSysTask_ReadSingleEntry(ctxP,pb, cb, pcbRead, cbOffset, pe, uszSubPath);
        goto finish;
    }
finish:
...
*/
VOID MSysTask_ListSingleEntry(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Inout_ PHANDLE pFileList, _In_ PVMM_MAP_TASKENTRY pTask, _In_ LPSTR uszPath)
{
    DWORD cbInfoFile;
    CHAR usz[MAX_PATH];
    CHAR szu8InfoFile[0x1000];
...
BOOL MSysTask_List(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Inout_ PHANDLE pFileList)
{
    ...
    if(!ctxP->uszPath[0]) {
        ...
        VMMDLL_VfsList_AddFile(pFileList, "tasks.txt", UTIL_VFSLINEFIXED_LINECOUNT(pObTaskMap->cTask) * MSYSTASK_LINELENGTH, NULL);
        goto finish;
    }
    ...
    if(!_strnicmp("by-guid\\", ctxP->uszPath, 8) || !_strnicmp("by-name\\", ctxP->uszPath, 8)) {
        ...
        MSysTask_ListSingleEntry(ctxP, pFileList, pe, uszSubPath);
        goto finish;
    }
finish:
    Ob_DECREF(pObTaskMap);
...
VOID MSysTask_FcTimeline(
    ...
    _In_ VOID(*pfnAddEntry)(_In_ HANDLE hTimeline, _In_ QWORD ft, _In_ DWORD dwAction, _In_ DWORD dwPID, _In_ DWORD dwData32, _In_ QWORD dwData64, _In_ LPSTR uszText),
    ...
    if(pObTaskMap) {
        for(i = 0; i < pObTaskMap->cTask; i++) {
            ...
            if(pe->ftRegLastWrite) { pfnAddEntry(hTimeline, pe->ftRegLastWrite, FC_TIMELINE_ACTION_MODIFY, 0, 0, 0, usz); }
            if(pe->ftCreate) { pfnAddEntry(hTimeline, pe->ftCreate, FC_TIMELINE_ACTION_CREATE, 0, 0, 0, usz); }
            if(pe->ftLastRun) { pfnAddEntry(hTimeline, pe->ftLastRun, FC_TIMELINE_ACTION_READ, 0, 0, 0, usz); }
            if(pe->ftLastCompleted) { pfnAddEntry(hTimeline, pe->ftLastCompleted, FC_TIMELINE_ACTION_DELETE, 0, 0, 0, usz); }
        }
    }
}
...
VOID M_SysTask_Initialize(_Inout_ PVMMDLL_PLUGIN_REGINFO pRI)
{
    ...
    strncpy_s(pRI->reg_info.uszTimelineFile, 32, "timeline_task.txt", _TRUNCATE);
