...
# TODO: Maybe add data cleaning within the download job
...
class BaseJob:
    ...
    CsPy Uploading Python Package Version: 1.0.2""")
            print('=' * 99)
    ...
    def get_vm_status(self):
        ...
        if left(computer_node, 2) == 'de' or left(computer_node, 2) == 'gb':
            ...
            self.job_details['Script Name'] = right(file, (len(file) - file.rfind('/')) - 1)
    ...
    def send_failure_email(self):
        ...
        if self.run_error:
            if not self.setting.testing and self.vm_status and self.use_account:
                ...
                receiver = '{0}; {1}; {2}'.format(
                    self.account.info['data_team_vm_email'],
                ...
                message = """
                    Time Ran: {0}
                    The Script located at: {1}{2}
                    The Script failed at: {3} - {4}
                    
                    Error Message: {5}
                    Error Solution: {6}
                    
                    For Further Details Please Read The Script Logs On The VM
    ...
    def send_email(self):
        ...
        if not self.setting.testing and self.vm_status and self.use_account:
            ...
            message = """
                Time Ran: {0}
                The Script located at: {1}{2}
                The Script failed at: {3}
                Failed to run successfully due to python error message: {4}
