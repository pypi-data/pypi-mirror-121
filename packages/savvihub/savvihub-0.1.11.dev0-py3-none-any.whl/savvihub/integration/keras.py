...
from savvihub.experiment import list_experiments
from savvihub.integration.common import get_current_experiment, log
from savvihub.model import create_model
from savvihub.util import logger
from savvihub.util.constant import (
    VESSL_PLOTS_FILETYPE_IMAGE,
    VESSL_PLOTS_FILETYPE_IMAGES,
)
from savvihub.util.exception import InvalidExperimentError, InvalidParamsError
...
class ModelCallback(keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        if self._should_upload_model(logs):
            create_model(
                experiment=get_current_experiment().id,
