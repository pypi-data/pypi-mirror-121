    ...
    def upload_hooks(self, *, callback=None):
        def fn(resp, **kwargs):
            if resp.status_code != 200:
                logger.warning(
                    f"Upload for {resp.request.url} failed. Detail: {resp.data}"
                )
...
class DownloadableFileObject:
        ...
        def fn(resp, **kwargs):
            ...
            with open(self.full_path, "wb") as f:
                for chunk in resp.iter_content(chunk_size=8192):
                    ...
                    if callback:
                        callback(chunk)
            logger.debug(f"Downloaded {self.full_path}.")
    ...
    def download(self, session=requests.Session(), progressable=None):
        ...
        future = session.get(
            self.url, stream=True, hooks=self.download_hooks(callback=progress_callback)
        )
        return future
