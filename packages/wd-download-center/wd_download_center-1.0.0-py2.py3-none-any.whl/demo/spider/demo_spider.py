...
class DemoSpider(BaseSpider):
    ...
    def start_requests(self):
        ...
        for i in range(100):
            ...
            for id,keyword,device in rows:
                headers = self.get_headers(device)
                if device==1:
                    kwd = urllib.parse.quote(keyword)
                    url = "https://www.baidu.com/s?wd={}".format(kwd)
                else:
                    kwh = {'wd': keyword}
            ...
            request = SpiderRequest(urls=urls)
    ...
    def deal_response_results_status(self, response_status, objUrl, result, request):
        ...
        if response_status:
            ...
            print('失败的链接{},html = {}'.format(objUrl,result['result']))
            self.log.info('抓取失败: {}'.format(objUrl))
