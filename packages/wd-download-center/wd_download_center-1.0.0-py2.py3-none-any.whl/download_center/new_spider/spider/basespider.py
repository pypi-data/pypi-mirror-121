...
class BaseSpider(object):
    ...
    def is_finish(self):
        ...
        """
        return self.sending_queue.empty() and len(self.sended_queue) == 0  and self.response_queue.empty() and self.store_queue.empty()
    ...
    def send_requests(self, max_idle_time):
        ...
        while True:
            try:
                if len(self.sended_queue) < self.sended_queue_max and self.response_queue.qsize() < self.response_queue_max:
    ...
    def get_wait(self):
        ...
        if self.thread_wait != 0:
            ...
            if self.response_queue.qsize() > 4000:
                time.sleep(5)
            elif len(self.sended_queue) < 2000:
                time.sleep(1)
            else:
                time.sleep(0.1)
        ...
        while True:
            try:
                if self.store_queue.qsize() < self.store_queue_max and self.response_queue.qsize() > 0:
                    ...
                    try:
                        self.deal_response_results(results, self.stores)
    ...
    def deal_request_results(self, request, results):
        if results['success']:
            self.sended_queue.append(request)
        else:
                ...
                for request in self.sended_queue:
                    ...
                    for u in request.urls: # request.urls 列表
                        ...
                        if uid in json.dumps(result_list): #url的uid是否在结果里，在则说明是当前任务的结果
                            ...
                            for res in result_list: #因为result_list一次获取10条，所以要循环处理。todo 需要升级
    ...
    def record_log(self):
        ...
        while True:
            ...
            table.add_row(["sended_queue", len(self.sended_queue)])
