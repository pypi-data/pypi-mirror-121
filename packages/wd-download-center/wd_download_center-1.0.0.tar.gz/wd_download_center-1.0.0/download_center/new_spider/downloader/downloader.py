...
class Downloader(object):
    ...
    def validate_accout(self, user="", password=""):
        try:
            ...
            for i in range(2):
                try:
                    r = reqs.post(config.VERUFY_USER_URL, data=data, timeout=config.REQUEST_TIMEOUT)
                    rdata = json.loads(r.text)
                    break
                except:
                    time.sleep(config.REQUEST_RETYR_SLEEP)
    ...
    def get(self,task_id):# page=1
        ...
        try:
            ...
            for i in range(5):
                try:
                    rdata = reqs.post(config.DOWNLOADER_GETRESULT.format(config.IP_HOST), data=data, timeout=config.REQUEST_TIMEOUT)
