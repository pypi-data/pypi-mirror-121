...
class PDList(object):
    ...
    def from_rips_filtration(distance_matrix, maxdim, maxvalue=np.inf,
        ...
        if simplicial:
            ...
            filtration = dm.build_rips_filtration(save_graph)
...
class Pair(object):
    ...
    def optimal_1_cycle(self):
        ...
        * Bitmap filtration by :meth:`PDList.from_bitmap_levelset`: `save_suppl_info=True`
        ...
        if self.diagram.pd.has_chunk("graph_weights"):
            return self.graph_optimal_1_cycle()
    ...
    def graph_optimal_1_cycle(self):
        return GraphOptimal1Cycle(self, graph_opt1cyc.search(
            self.diagram.pd.graph_adjacent_matrix, self.birth_time()
        ))
