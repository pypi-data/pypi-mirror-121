...
class Client(Callbacks, SocketHandler):
    ...
    def login(self, email: str, password: str): # I working on it
        """
        ...
        **Returns**
        ...
        response = requests.post("https://aminoapps.com/api/auth", headers=self.web_headers, data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else:
            self.run_socket()
            self.authenticated = True
            self.json = json.loads(response.text)
            self.userId = response.json()["result"]["uid"]
            ...
            self.sid = self.web_headers["cookie"][3: sid_index]
