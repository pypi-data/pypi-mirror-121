...
def fund_stock_holding_compare_china(fund,quarter1,quarter2,top=10):
    ...
    """
    print("\nSearching fund stock holding info, it may take time, please wait ...\n")
    ...
    try:
        ...
        print("  #Error(fund_stock_holding_compare_china): fund",fund,"not exist or wrong year",years[0])
        return
    if len(data)==0:
        print("  #Error(fund_stock_holding_compare_china): fund",fund,"not exist or wrong year",years[0])
        return      
...
# 合并后，在dataframe 中 NaN 的数据应为 0
    ...
    if top <0: 
        order='后'
        top=-top
    print("\n===== 中国基金持仓股票分析："+name+'，'+s1+"对比"+s2,"(按后者持仓比例高低排列，"+order+str(top)+"名股票) =====\n")
    print(df.to_string(index=False))
    import datetime; today = datetime.date.today()
    print("*** 注：持股数为万股，持仓市值为万元，持仓比例为占基金资产净值比例%，包括A股与非A股")
...
def fund_stock_holding_rank_china(fund,year_num=2):
    ...
    data = ak.fund_em_portfolio_hold(code=fund,year=years[0])
    if len(data)==0:
        print("  #Error(fund_stock_holding_rank_china): fund",fund,"not exist")
        return          
    ...
    print("\n=== 中国基金持仓股票排行变化："+name,"(按持仓比例高低排列，近"+str(year_num)+"年前10名) ===\n")
    print(df.to_string(index=False))
    import datetime; today = datetime.date.today()
    print("*** 注：包括A股与非A股。数据来源：天天基金/东方财富,",today)          
