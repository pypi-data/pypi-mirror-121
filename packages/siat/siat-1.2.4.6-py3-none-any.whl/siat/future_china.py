...
def future_type_china_0(trade_date):
    ...
    df=ak.futures_rule(trade_date=trade_date1)
...
def future_type_china(code=''):
    """
    综合程序：
    合约code为空或品种symbol不存在时列示中国期货内盘的全部代码，否则列示该合约品种明细
    """
    import datetime
    today = datetime.date.today()
...
def future_price_china(code,start,end,power=0):
    ...
    p1=p[p['symbol']==code]
    if len(p1)==0: 
        print("  #Error(future_price_china): future contract",code,'not found in market',market)
        contracts=set(list(p[p['variety']==variety]['symbol']))
        contracts1=sorted(contracts) 
        print("\n提示："+varietyname+variety+'的期货合约：'+mktnamefull+', '+str(end))
        print(contracts1)
        return None        
    ...
    titletxt="中国期货内盘交易走势分析："+varietyname+code
    #避免code被翻译
    acode=' '+code
    #"""
    #收盘价vs结算价
...
def future_price_foreign(code,start,end,power=0):
    """
    综合程序：
    ...
    titletxt="中国期货外盘交易走势分析："+varietyname+code
