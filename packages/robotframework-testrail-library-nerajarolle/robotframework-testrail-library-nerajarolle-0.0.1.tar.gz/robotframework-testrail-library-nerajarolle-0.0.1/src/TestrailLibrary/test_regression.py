import unittest
import TestrailLibrary 
from TestrailLibrary import DuplicateException
import random
...
class RunsResponse(BaseResponse):
    ...
    def __init__(self, size=250, offset=0, limit=250) -> None:
        ...
        self.runs = [
                ...
                'status_id': random.randint(1, 5), 'created_on': fake.unix_time(), 'assignedto_id': None, 
                'comment': fake.text(32), 'version': None, 'elapsed': f'{random.randint(1, 120)}s', 'defects': None, 
...
class ResultsResponse(BaseResponse):
    ...
    def __init__(self, size=250, offset=0, limit=250) -> None:
        ...
        self.results = [{"id": i+5000, "test_id": i+1000, 
                        ...
                        "comment": fake.text(24)} for i in range(min(size, limit))
                        ]
...
class CasesResponse(BaseResponse):
    ...
    def __init__(self, size=250, offset=0, limit=250) -> None:
        ...
        self.cases = [{"id": i+offset+1, "title": f'Test Case {i+offset+1}', 
...
class BasicTests(unittest.TestCase):     
    ...
    def setUp(self) -> None:
        self.tr = TestrailLibrary.TestrailLibrary("host")
        return super().setUp()

    @patch('TestrailLibrary.Apiclient.send_get')
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=[]))
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=ProjectsResponse(10).get))
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value={"message": "Error"}))
    def test_get_project_by_name_when_error_response(self):
        res = self.tr.get_project_by_name("some project")
        self.assertFalse(res)

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=GET_SUITE))
    def test_get_suite(self):
        res = self.tr.get_suite(GET_SUITE.get("id"))
        self.assertTrue(res)
        self.assertEqual(res.get('id'), GET_SUITE.get('id'))

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=GET_SUITES))
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(10).get))
    def test_get_run_by_name_when_not_exists(self):
        res = self.tr.get_runs_by_name(17, "test")
        self.assertFalse(res)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(0).get))
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(side_effect=[RunsResponse().get, 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(50).get))
    def test_get_runs_with_size_and_no_limit(self):
        res = self.tr.get_runs(17)
        self.assertTrue(res)
        self.assertTrue(len(res)==50)

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(10, limit=50).get)) 
    def test_get_runs_with_size_smaller_than_limit(self):
        res = self.tr.get_runs(17, limit=50)
        self.assertTrue(res)
        self.assertTrue(len(res)==10)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(50, limit=50).get)) 
    def test_get_runs_with_size_equal_to_limit(self):
        res = self.tr.get_runs(17, limit=50)
        self.assertTrue(res)
        self.assertTrue(len(res)==50)

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(60, limit=50).get)) 
    def test_get_runs_with_size_greater_than_limit(self):
        res = self.tr.get_runs(17, limit=50)
        self.assertTrue(res)
        self.assertTrue(len(res)==50)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(1).get)) 
    def test_get_runs_with_size_one(self):
        res = self.tr.get_runs(17)
        self.assertTrue(res)
        self.assertTrue(len(res)==1)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(1, limit=1).get)) 
    def test_get_runs_with_size_one_and_limit_one(self):
        res = self.tr.get_runs(17, limit=1)
        self.assertTrue(res)
        self.assertTrue(len(res)==1)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(50, offset=5).get)) 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(50, offset=5, limit=2).get)) 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(20).get))
    def test_get_case_by_name_when_no_exists(self):
        res = self.tr.get_cases_by_name(17, 1000, 1000, "Case 500")
        self.assertFalse(res)  
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(0).get))
    ...
    def test_get_case_by_name_when_duplicates(self, mock_get):
        ...
        res = self.tr.get_cases_by_name(17, 1000, 1000, "Case 1")
        self.assertFalse(res)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(side_effect=[CasesResponse().get, 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(20, limit=5).get))
    def test_get_cases_with_size_and_limit(self): 
        res = self.tr.get_cases(17, 1000, limit=5)
        self.assertTrue(res)
        self.assertTrue(len(res)==5)

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(5, limit=10).get))
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(5, limit=5).get))
    def test_get_cases_with_size_equal_to_limit(self): 
        res = self.tr.get_cases(17, 1000, limit=5)
        self.assertTrue(res)
        self.assertTrue(len(res)==5)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(20, limit=10).get))
    def test_get_cases_with_size_greater_than_limit(self): 
        res = self.tr.get_cases(17, 1000, limit=10)
        self.assertTrue(res)
        self.assertTrue(len(res)==10)

    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(1).get))
    def test_get_cases_with_size_one(self): 
        res = self.tr.get_cases(17, 1000)
        self.assertTrue(res)
        self.assertTrue(len(res)==1)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(1, limit=1).get))
    def test_get_cases_with_size_one_and_limit_one(self): 
        res = self.tr.get_cases(17, 1000, limit=1)
        self.assertTrue(res)
        self.assertTrue(len(res)==1)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(50, offset=5).get)) 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=CasesResponse(50, offset=5, limit=2).get)) 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=RunsResponse(50).get))
    @patch('TestrailLibrary.Apiclient.send_post', Mock(return_value=RunsResponse(1, offset=1000).runs[0]))
    ...
    @patch('TestrailLibrary.Apiclient.send_post', Mock(return_value={"id": GET_SUITE.get('id')}))
    @patch('TestrailLibrary.Apiclient.send_get') 
    ...
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=ResultsResponse(50).get)) 
    def test_get_results_for_case(self):
        res = self.tr.get_results_for_case(1, 1)
        self.assertTrue(res)
        self.assertTrue(len(res)==50)
    
    @patch('TestrailLibrary.Apiclient.send_get', Mock(return_value=ResultsResponse(0).get)) 
    ...
    @patch('TestrailLibrary.Apiclient.send_post')
    @patch('TestrailLibrary.Apiclient.send_get') 
    ...
    def test_set_status_on_test_run(self, mock_get, mock_post):
        ...
        result = results.results[0]
        mock_post.return_value = result
        mock_get.return_value = CasesResponse(10).get
    ...
    def test_set_status_on_test_run_using_suite_name(self, mock_get, mock_post):
        ...
        result = results.results[0]
        mock_post.return_value = result
        mock_get.side_effect = [ GET_SUITES, CasesResponse(10).get ]
    ...
    def test_set_status_on_test_run_using_suite_and_section_name(self, mock_get, mock_post):
        ...
        mock_post.return_value = result
        mock_get.side_effect = [ GET_SUITES, sections.get, CasesResponse(10).get ]
        res = self.tr.set_status_on_test_run(1, project_id=17, suite_name=GET_SUITES[0].get("name"), 
    ...
    def test_set_status_on_test_run_using_suite_and_section_and_test_name(self, mock_get, mock_post):
        ...
        case = cases.cases[0]
        mock_post.return_value = result
        mock_get.side_effect = [ GET_SUITES, sections.get, cases.get, 
    ...
    def test_set_status_on_test_run_using_section_name(self, mock_get, mock_post):
        ...
        section = sections.sections[0]
        mock_post.return_value = result
        mock_get.side_effect = [ sections.get, CasesResponse(10).get ]
    ...
    def test_set_status_on_test_run_using_test_name(self, mock_get, mock_post):
        ...
        case = cases.cases[0]
        mock_post.return_value = result
        mock_get.side_effect = [ cases.get, SectionsResponse(10).get, cases.get, cases.get ]
        res = self.tr.set_status_on_test_run(1, project_id=17, suite_id=GET_SUITES[0].get("id"), 
    ...
    def test_set_status_on_test_run_using_run_name(self, mock_get, mock_post):
        ...
        run = runs.runs[0]
        mock_post.return_value = result
        mock_get.side_effect = [ runs.get, cases.get ]
        res = self.tr.set_status_on_test_run(1, project_id=17, suite_id=GET_SUITES[0].get("id"), 
    ...
    def test_set_status_on_test_run_using_test_name_when_duplicate(self, mock_get, mock_post):
        ...
        cases.cases.append(new_case)
        mock_post.return_value = result
        mock_get.side_effect = [ SectionsResponse(10).get, cases.get, cases.get ]
        res = self.tr.set_status_on_test_run(1, project_id=17, suite_id=GET_SUITES[0].get("id"), 
            section_id=1000, test_name=case.get("title"), run_id=1 )
        self.assertTrue(res)
        self.assertEqual(res, result)
