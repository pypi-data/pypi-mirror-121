...
class TestrailLibrary:
    ...
    def __init__(self, server, testrail_username=None, testrail_key=None):
        self.server = server
        self.test_case_result = self.TestCaseResult()
        self.client = Apiclient(server)
        self.client.user = testrail_username
        self.client.password = testrail_key

    def set_testrail_credentials(self, testrail_username, testrail_key):
        self.client.user = testrail_username
        self.client.password = testrail_key
    ...
    def get_projects(self):
        url = f'get_projects/'
        return self.__get_with_offset(url, "projects")
    ...
    def get_suite_by_name(self, project_id, suite_name):
        ...
        if suite and len(suite) == 1:
            return suite[0]
        elif suite and len(suite) > 1:
            logger.info(suite)
            raise DuplicateException(f'More than 1 suite found with name {suite_name} for project {project_id}')
        return {}

    def add_suite(self, project_id, suite_name, description):
        suite = self.get_suite_by_name(project_id, suite_name)
        if suite:
            return suite
        else:
    ...
    def get_sections(self, project_id, suite_id):
        ...
        return self.__get_with_offset(url, "sections")
    ...
    def get_sections_by_name(self, project_id, suite_id, section_name, expect_duplicates=False):
        ...
        if section:
            if not expect_duplicates and len(section) > 1:
                logger.warn(f'Found more than 1 section with name {section_name} for project' 
                '{project_id}, suite {suite_id}...returning first section')
        return section
        # if section and len(section) == 1:
        #     return section[0]
        # elif section and len(section) > 1:
        #     raise Exception(f'Found more than 1 section with name {section_name} for project {project_id}, suite {suite_id}')
        # else:
        #     pass 
        # return {}
    ...
    def add_section(self, project_id, name, description, suite_id, parent_id=None):
        ...
        #found_section = list(filter(lambda x: x.get('name') == name, existing_sections))
        if section:
            return section[0]
        else:
    ...
    def get_cases_by_name(self, project_id, suite_id, section_id, title, expect_duplicates=False):
        ...
        case = list(filter(lambda x: x.get('title') == title and x.get('section_id') == section_id,
                                           cases))
        #print(case)
        if not expect_duplicates and len(case) > 1:
            raise DuplicateException(f'More than 1 case found with name {title} for project {project_id} ' 
                    f'suite {suite_id} and section {section_id}: {case}')
        return case
        # if case and len(case) == 1:
        #     #logger.info(f'Test case {title} is already existing')
        #     return case[0]
        # elif case and len(case) > 1:
        #     logger.info(case)
        #     raise Exception(f'More than 1 case found with name {title} for project {project_id} ' 
        #             f'suite {suite_id} and section {section_id}: {case}')
        # else:
        #     return {}

    def add_case(self, project_id, suite_id, title, section_id=None, **kwargs):
        #section_id = kwargs.get("section_id") or None
        section_name = kwargs.get('section_name') or 'New Section'
        #suite = self.get_suite(suite_id)
        #if not suite:
        #    raise AssertionError(f'Could not find a suite with id {suite_id}')
        #suite_id = suite.get('id')
        if not section_id:
        ...
        if case: 
            if len(case) > 1:
                logger.warn(f'More than 1 cases found with name {title} for project {project_id} '
                f'and suite {suite_id}')
            return case[0]
        else:
    ...
    def get_tests_by_case_name(self, run_id, title):
        ...
        #     return test[0]
        # elif test and len(test) > 1:
        #     logger.info(test)
        #     raise Exception(f'More than 1 test found with name {title} for run {run_id}')
        # else:
        #     return {}
    ...
    def get_runs_by_name(self, project_id, run_name, expect_duplicates=False):
        runs = self.get_runs(project_id)
        if not isinstance(runs, list): runs = []
        run = list(filter(lambda x: x.get('name')== run_name, runs))
        if not expect_duplicates and len(run) > 1: 
            raise DuplicateException(f'Found more than 1 test runs with name {run_name} for project {project_id}')
        return run
        # if run and len(run) == 1:
        #     return run[0] 
        # elif run and len(run) > 1:
        #     logger.info(run)
        #     raise Exception(f'Found more than 1 test runs with name {run_name} for project {project_id}')
        # return {}
    ...
    def add_run(self, project_id, suite_id, name, description, include_all=True, refs=None, case_ids=None):
        ...
        if run:
            if len(run) > 1:
                logger.warn(f'Found more than 1 run with name {name} for project {project_id}...'
                        f'returning only first one')
            return run[0]
        description = description
    ...
    def update_run(self, run_id, **kwargs):
        for key, value in kwargs.items():
            if key not in ['suite_id', 'name', 'description', 'include_all', 'case_ids', 'refs']:
                logger.error(f'{key} attribute is not supported')
                kwargs.pop(key)
    ...
    def set_status_on_test_run(self, status_id, **kwargs):
        ...
        if str(status_id).lower() in ('pass', 'passed'):
        ...
        elif str(status_id).lower() in ('skip', 'skipped'):
            status_id = TEST_STATUS_NA
            ...
            if not (str(status_id).isdigit() and 0 < int(status_id) < 8):
                raise Exception(f'{status_id} is not a valid status id.' 
                'Should be either a number from 1 to 7 or one of the following strings (case insensitive):'  
    ...
    def delete_attachments_cleanup(self, run_id, max_to_keep=7):
        attachments = self.get_attachments_for_run_in_testrail(run_id)
        ...
        if len(attachments_sorted) > max_to_keep:
            ...
            for attachment in to_delete:
                ...
                try:
                    deleted_response = self.delete_attachment(attachment)
