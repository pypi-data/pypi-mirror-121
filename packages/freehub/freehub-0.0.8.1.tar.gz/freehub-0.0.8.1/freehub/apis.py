...
def fetch(address):
    repo_address, branch_name, relative_path = parse_address(address)
    repo_url = get_repo_url(repo_address)
    cache_dir = download_branch_to_cache(repo_url, branch_name, overwrite=True)
...
def freehub_cat(address:str):
    repo_address, branch_name, relative_path = parse_address(address)
    repo_url = get_repo_url(repo_address)
    cache_dir = download_branch_to_cache(repo_url, branch_name, overwrite=True)
    file_path = join_path(cache_dir, relative_path)
    with open(file_path,'r') as f:
        print(address.center(100,'*'))
        print(f.read())
        print(('End of %s'%(address)).center(100, '*'))
...
def freehub_search(pattern: str):
    ...
    cache_dir = download_branch_to_cache(repo_url, branch_name_pattern, overwrite=True)
