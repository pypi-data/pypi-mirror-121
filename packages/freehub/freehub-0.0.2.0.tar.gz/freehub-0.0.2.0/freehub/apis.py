...
def get_freehub_location():
    cfg=read_config()
    if cfg and 'username' in cfg.keys() and 'password' in cfg.keys():
        return _gen_location(user_name=cfg['username'],password=cfg['password'],repo_path=cfg['repo'])
...
def freehub_download(src,dst,overwrite=False):
    ...
    download_to_dir(get_freehub_location(),src,dst,overwrite=overwrite)
def freehub_upload(src,dst,overwrite=False):
    upload_to_remote(get_freehub_location(),dst,src,overwrite=overwrite)
