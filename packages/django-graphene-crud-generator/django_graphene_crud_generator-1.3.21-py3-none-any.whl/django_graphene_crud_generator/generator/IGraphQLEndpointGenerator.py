...
class IGraphQLEndpointGenerator(IGraphQLEndpointComponent, abc.ABC):
    ...
    def generate(self, **kwargs) -> TGrapheneQuery:
        ...
        def perform_query(root, info, *args, **kwargs) -> any:
            ...
            def generator_body_to_graphene_basic_mapper():
                def decorator(generator_body):
                    ...
                    def wrapper(*aargs, **akwargs):
                        ...
                        result = f(runtime_context.root, runtime_context.info, *runtime_context.args, **runtime_context.kwargs)
                        return result
            ...
            def graphene_basic_to_generator_body_mapper():
                def decorator(f):
                    @functools.wraps(f)
                    def wrapper(*aargs, **akwargs):
                        # f has the signature cls, info, *args, **kwargs
                        result = f(runtime_context.root, runtime_context.info, *runtime_context.args,
            ...
            for x in self.components:
                if hasattr(x, "_graphql_body_function_decorator_native"):
                    ...
                    if dec is not None:
                        ...
                        body = generator_body_to_graphene_basic_mapper()
                        body = dec(body)
                    ...
                    dec = x._graphql_body_function_decorator(body)
                    if dec is not None:
                        body = dec(body)
