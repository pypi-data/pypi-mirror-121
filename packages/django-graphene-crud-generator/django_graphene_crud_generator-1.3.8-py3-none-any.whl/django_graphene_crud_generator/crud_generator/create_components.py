...
class CreateMutationReturnAddedValueComponent(AbstractGraphQLCrudComponent):
    ...
    def _graphql_body_function_decorator(self, body_function: GrapheneGeneratorBodyFunction,
                                         runtime_context: GraphQLRuntimeContext):
        @functools.wraps(body_function)
        def wrapper(*args, **kwargs):
            result = body_function(*args, **kwargs)
            # result is something like dict(obj=item_in_db[0], added=False)
            obj = result["obj"]
            added = result["added"]
            return runtime_context.build_context.action_class_name(added, obj)
        return wrapper
...
class CreateMutationReturnTrueComponent(AbstractGraphQLCrudComponent):
    ...
    def _graphql_body_function_decorator(self, body_function: GrapheneGeneratorBodyFunction,
        ...
        def wrapper(*args, **kwargs):
            ...
            # result is something like dict(obj=item_in_db[0], added=False)
            added = result["added"]
            return runtime_context.build_context.action_class_name(added)
        return wrapper
...
class StandardCrudCreateMutation(AddCRUDContextComponentMixIn, AbstractCreateGraphQLMutationGenerator):
    ...
    def _get_mutation_input_parameter_name(self, build_context: GraphQLBuildtimeContext) -> str:
        ...
        for name, field in build_context.action_arguments.items():
            if expected in name and isinstance(field, graphene.InputObjectType):
                return name
        else:
