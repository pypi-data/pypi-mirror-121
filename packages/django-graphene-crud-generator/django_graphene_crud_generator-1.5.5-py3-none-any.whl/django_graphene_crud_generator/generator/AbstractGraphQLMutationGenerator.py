...
class AbstractGraphQLMutationGenerator(IGraphQLEndpointGenerator, abc.ABC):
    ...
    def _compute_graphene_class(self, build_context: GraphQLBuildtimeContext) -> type:
        ...
        assert logic_helpers.implies(len(build_context.action_arguments) > 0, isinstance(list(build_context.action_arguments.keys())[0], str)), f"argument keys are not strings, but {type(list(build_context.action_arguments.keys())[0])}!"
