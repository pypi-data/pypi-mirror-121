        ...
        for run in runs:
            ...
            if len(new_circs) > 0:
                ...
                if rewriteable_and_in_basis_p and len(run) < len(new_circ):
                    ...
                    warnings.warn(
                        "Resynthesized \n\n"
                        + "\n".join([str(node.op) for node in run])
                        + "\n\nand got\n\n"
                        + "\n".join([str(node[0]) for node in new_circ])
                        + f"\n\nbut the original was native (for {self._target_basis}) and the new "
                        "value is longer.  This indicates an efficiency bug in synthesis.  Please "
                        "report it by opening an issue here: "
