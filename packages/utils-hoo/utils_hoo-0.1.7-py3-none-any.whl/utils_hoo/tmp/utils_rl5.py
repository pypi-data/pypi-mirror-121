...
def play_policy(env, policy=None):
    ...
    print(f'初始observation = {observation}')
    while True:
        print(f'玩家 = {env.player}, 庄家 = {env.dealer}')
        action = policy()
        print(f'动作 = {action}')
        observation, reward, done, _ = env.step(action)
        print(f'观测 = {observation}, 奖励 = {reward}, 结束指示 = {done}')
        total_reward += reward
        if done:
            break
    return total_reward
...
if __name__ == '__main__':
    ...
    print(f'Mean reward of {N} rounds: {np.mean(rewards)}')
