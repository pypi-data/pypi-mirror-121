...
if __name__ == '__main__':
    ...
    print(f'随机策略{N}次平均奖励：{np.mean(rewards)}')
    ...
    print(f'状态价值函数 = \n{v_vi.reshape(4, 4)}')
    print(f'最优策略 = \n{np.argmax(policy_vi, axis=1).reshape(4, 4)}')
    
    print(f'策略迭代最优策略{N}次平均奖励：' + \
          f'{np.mean([play_policy(env, policy_pi) for _ in range(N)])}')
    print(f'价值迭代最优策略{N}次平均奖励：' + \
      f'{np.mean([play_policy(env, policy_vi) for _ in range(N)])}')
