...
def plot_Candle(data,
    ...
    Parameters
    ----------
    data: 绘图数据，必须有'time'|'date'、'open'、'high'、'low'、'close'五列，
          以及plot_below指定的列名
    ...
    cols_other_x: 其他需要画线的列设置，格式为{col: (lnstyl, label), ...}
    cols_to_label_info: 设置需要特殊标注的列绘图信息，格式形如:
        {col1: [[col_lbl1, (v1, v2, ..), (ln1, ln2, ..), (lbl1, lbl2, ..)],
                [col_lbl2, (v1, v2, ..), ...]],
         col2: ..}，其中col是需要被特殊标注的列，col_lbl为标签列；
    ...
    if args_boll:
        ...
        ln = ax1.plot(data['timeI'], data['boll_mid'], '-k',
                      label=f'Boll({args_boll[0]}, {args_boll[1]})')
        lns.append(ln)
