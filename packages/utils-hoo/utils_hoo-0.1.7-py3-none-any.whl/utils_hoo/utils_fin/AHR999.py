...
def get_AHR999(data, birthday, col_price='close', poly=1, fit_log=True,
               model='lr', NFixInv=200, methodFI='gmean', col_weight='volume',
               plot=True, kwargs_plot={'title': None, 'figsize': (12, 8)}):
    '''
    AHR999指标计算
    data中需包含['time'|'date', col_price]列记忆VWAP用到的[col_vol, col_val]列
    
    poly设置拟合模型输入X进行多项式扩展的最高次方数
    fit_log设置拟合模型是否进行对数变换
    model指定模型类别：
        `lr`为线性回归，`gbr`为gbdt回归
    NFixInv设置几定投历史期数
    methodFI设置定投成本计算方法，可选`gmean`, `vwap`
    ...
    for p in range(1, poly+1):
        df[f'Alog{p}'] = df['Alog'] ** p
    
    Xcols = [f'Alog{p}' for p in range(1, poly+1)]
    ...
    # 定投成本
    if methodFI == 'gmean': # 几何平均值
        df[f'定投{NFixInv}成本'] = df[col_price].rolling(NFixInv).apply(gmean)
    elif methodFI == 'vwap':
        df['tmp_val'] = df[col_price] * df[col_weight]
        df[f'定投{NFixInv}成本'] = df['tmp_val'].rolling(NFixInv).sum() / \
                                  df[col_weight].rolling(NFixInv).sum()
        df.drop('tmp_val', axis=1, inplace=True)
    
    df['Price/Cost'] = df[col_price] / df[f'定投{NFixInv}成本']
...
if __name__ == '__main__':
    import time
    from utils_hoo.utils_fin.load_his_data import load_btc_daily
    from utils_hoo.utils_fin.load_his_data import load_index_daily
    ...
    data['AHR999'] = get_AHR999(data, birthday, NFixInv=150, fit_log=False,
    ...
    data['AHR999'] = get_AHR999(data, birthday, NFixInv=150, fit_log=False,
                                ...
                                kwargs_plot={'title': code})
    data['AHR999vwap'] = get_AHR999(data, birthday, NFixInv=150, fit_log=False,
                                poly=2, model='gbr', methodFI='vwap',
    ...
    data = load_index_futures_daily(code+'9999')[['date', 'close']]
    data.set_index('date', drop=False, inplace=True)
    data['AHR999'] = get_AHR999(data, birthday, NFixInv=100, fit_log=False,
