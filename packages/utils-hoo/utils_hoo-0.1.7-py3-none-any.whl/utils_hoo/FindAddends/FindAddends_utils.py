...
#%%
def get_alts_sml(tgt_sum, alts, typeSort='descend', tol=0.0, add_num=None):
    '''
    ...
    tgt_sum: 目标和
    alts: list，备选数列表
    typeSort: 对alts进行排序的方式，默认'descend'降序，可选'ascend'升序、None不排
    tol: 两个数进行比较时的绝对误差控制范围
    add_num: 限制在加起来和大于等于tgt_sum的基础上增加的备选数个数，默认无限制
...
def backfind_sml1st_index(tgt_sum, alts, tol=0.0, loop_count=None):
    ...
    alts: list，待比较数列表
    tol: 两个数进行比较时的绝对误差控制范围
    loop_count: 初始迭代次数值，默认为None；若loop_count为None，则不记录迭代次数，
                否则在loop_count基础上继续记录迭代次数
                    
    Returns
    -------
    idx: 从后往前搜索，alts中小于等于tgt_sum的第一个数的索引
    loop_count: 搜索结束时的迭代次数
    '''
    if len(alts) == 0:
        return -1, loop_count
...
if __name__ == '__main__':
    ...
    alts_new = get_alts_sml(tgt_sum, alts, typeSort=typeSort, tol=tol,
                            add_num=add_num)
    print(alts_new)
