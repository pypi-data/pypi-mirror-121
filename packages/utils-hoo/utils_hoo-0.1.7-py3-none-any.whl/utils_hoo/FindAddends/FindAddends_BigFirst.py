...
import time 
from .FindAddends_utils import get_alts_sml
from .FindAddends_utils import tol2side_x_eq_y
from .FindAddends_utils import backfind_sml1st_index
from utils_hoo.utils_general import simple_logger
    

def FindAddends_BigFirst(tgt_sum, alts, n_adds=None, check_alts=False,
    ...
    tgt_sum: 目标和
    alts: list，备选数列表
    n_adds: 限制备选加数个数为n_adds，默认无限制
    check_alts: 是否检查alts（提前删除大于目标和的备选数），默认否
                注：该参数只有当alts没有负数时才起作用
    add_num: 在加起来和大于等于tgt_sum的基础上增加的备选数个数，意义同get_alts_sml
             函数中的add_num参数，在check_alts起作用时才起作用
    tol_below: 下界误差，若目标和减去当前和小于等于tol_below，则结束搜索，默认为0.0
    tol_above: 上界误差，若当前和减去目标和小于等于bol_above，则结束搜索，默认为0.0
    max_loop: 最大搜索次数限制，默认一百万
    global_loop: 是否将寻找下一个最优备选数时的循环次数计入总搜索次数
    log_info: 是否打印搜索次数和时间等信息
    save_process: 是否保存搜索过程
    logger: logging日志记录器
        
    Returns
    -------
    choseds_best: list，最优备选数列表
    choseds_addends: 最终备选数列表
    adds_process: 搜索中间过程
    '''
    
    if log_info and logger is None:
    ...
    if log_info:    
        ...
        logger.info('used time {}s.'.format(round(time.time()-start_time, 6)))
...
if __name__ == '__main__':
    ...
    best, choseds, mid = FindAddends_BigFirst(tgt_sum, alts, n_adds=n_adds,
