...
from utils_hoo.utils_optimizer.utils_Heuristic import rand_init, sortPopulation


def rouletteWheelSelectionId(fvals): 
    '''
    ...
    current = 0
    for individualId in range(len(fvals)):
        current += reverseScores[individualId]
        if current > pick:
            return individualId
        

def RandomWalAroundAntlion(antlion, now_iter, Max_iter, x_lb, x_ub, dim):
    '''随机游走'''    
...
def ALO(objf, func_opter_parms):
    ...
    XantlionSorted, FvalsSorted_antlion = sortPopulation(Xantlion,
                                                         fvals_antlion)
    ...
    for l in range(0, Niter):
        for i in range(PopSize):
            RoletteId = rouletteWheelSelectionId(FvalsSorted_antlion)
            # ALO.pdf公式(2.13)
            RA = RandomWalAroundAntlion(XantlionSorted[RoletteId],
                                        l, Niter, x_lb, x_ub, dim)
            RE = RandomWalAroundAntlion(gBest, l, Niter, x_lb, x_ub, dim)
        ...
        # 蚁狮位置保持由于蚂蚁位置
        XallSorted, FvalsAllSorted = sortPopulation(Xall, fvals_all)
...
if __name__ == '__main__':
    ...
    logger = get_logger('./test/ALO_test.txt', screen_show=True)
