...
from utils_hoo.Sorts.insert_sort import rank_of_insert
from utils_hoo.Sorts.insert_sort_bin import rank_of_insert_bin
...
def fFit(series):
    '''对series（pd.Series或np.array）进行F分布参数估计'''
...
def Fit_f_Pdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个F分布，并计算在该分布下取值为x的概率
    '''
...
def Fit_f_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个F分布，并计算在该分布下取值为x的分布函数值
    '''
    df1, df2, loc, scale = fFit(series)
...
def fTest(series, plot=False, **kwargs_plot):
    '''
    对series（pd.Series或np.array）进行F分布检验（目前采用KS检验）
    '''
    df1, df2, loc, scale = f.fit(series)
    Stat, P = kstest(series, 'f', args=(df1, df2, loc, scale))
    if plot:
        from utils_hoo.utils_plot.plot_HistDist import plot_HistDist
...
def tFit(series):
    '''对series（pd.Series或np.array）进行T分布参数估计'''
...
def Fit_t_Pdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个t分布，并计算在该分布下取值为x的概率
    '''
...
def Fit_t_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个t分布，并计算在该分布下取值为x的分布函数值
    '''
    df, loc, scale = tFit(series)
...
def tTest(series, plot=False, **kwargs_plot):
    '''
    对series（pd.Series或np.array）进行t分布检验（目前采用KS检验）
    '''
    df, loc, scale = t.fit(series)
...
def chi2Fit(series):
    '''对series（pd.Series或np.array）进行卡方分布参数估计'''
...
def Fit_chi2_Pdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个卡方分布，并计算在该分布下取值为x的概率
    '''
...
def Fit_chi2_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个卡方分布，并计算在该分布下取值为x的分布函数值
    '''
    df, loc, scale = chi2Fit(series)
...
def chi2Test(series, plot=False, **kwargs_plot):
    '''
    对series（pd.Series或np.array）进行卡方分布检验（目前采用KS检验）
    '''
    df, loc, scale = chi2.fit(series)
...
def Fit_norm_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个正态分布，并计算在该分布下取值为x的分布函数值
    '''
    mu, sigma = normFit(series)
...
def Fit_lognorm_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个对数正态分布，并计算在该分布下取值为x分布函数值
    '''
    mu, sigma = lognormFit(series)
...
def Fit_weibull_Cdf(series, x):
    '''
    用series（pd.Series或np.array）拟合一个威布尔分布，并计算在该分布下取值为x的分布函数值
    '''
    k, lmd = weibullFit(series)
