...
def CkSupFreqLk(dataSet, Ck, minSup):
    '''
    筛选频繁项集
    扫描数据集dataSet（dataSet为list格式，每条事务记录以set或frozenset格式保存），
    计算Ck中项集的支持度CkSup，并筛选出Ck中不小于最小支持度minSup的频繁项集列表FreqLk。
    '''
...
def get_CkSupFreqLk_all(dataSet, minSup=0.5):
    '''
    Apriori算法寻找最大频繁K项集
    ...
    Return:
        FreqLk_all: 所有频繁项集FreqLk。list格式，每个元素均为频繁项集列表，
    ...
    CkSup_all, FreL1 = CkSupFreqLk(dataSet, C1, minSup) # FreL1为频繁1项集
    
    FreqLk_all = [FreL1]
    k = 2
    while len(FreqLk_all[k-2]) > 0:
        Ck = FreqLk_1ToCk(FreqLk_all[k-2], k) # 生成k项集Ck
        # 从Ck中筛选频繁k项集FreqLk
        CkSup, FreqLk = CkSupFreqLk(dataSet, Ck, minSup)
