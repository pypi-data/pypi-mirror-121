...
from utils_hoo import simple_logger
from utils_hoo.utils_general import con_count
from utils_hoo.utils_general import gap_count
from utils_hoo.utils_general import isnull, cal_pct
from utils_hoo.utils_general import replace_repeat_pd
from utils_hoo.utils_general import get_pre_val_Fcond
from utils_hoo.utils_datsci.utils_stats import norm_linear
from utils_hoo.utils_plot.plot_Common import plot_MaxMins, plot_Series

#%%
def FindMaxMin(series, Tmin=2, Tmax=np.inf, minDifPct=None, minDifVal=None,
               maxDifPct=None, maxDifVal=None, Pct_v00=1, skip_nan=True,
               logger=None):
    '''
    todo: 
    ...
    Tmax: 设置极大极小值直接最大间隔期数
    minDifVal/minDifPct（二选一）: 若两个相邻极值点之间的间隔大于等于Tmin，
        但是两者差值/百分比小于minDifVal/minDifPct，则删除该极值点对
    maxDifVal/maxDifPct（二选一）: 若两个相邻极值点之间的间隔小于Tmin，
        但是两者差值/百分比大于maxDifVal/maxDifPct，则保留该极值点对  
    Pct_vv0: 当使用百分比控制差值时，若前一个值为0时，指定的百分比变化值
    
    Returns
    -------
    df['label']: label列，其值中1表示极大值点，-1表示极小值点，0表示普通点
    '''
    ...
    if not isnull(minDifPct) and not isnull(minDifVal):
        if logger:
            logger.warning(
                        '同时设置`minDifPct`和`minDifVal`，以`minDifPct`为准！')
        else:
            print('同时设置`minDifPct`和`minDifVal`，以`minDifPct`为准！')
        minDifVal = None
    if not isnull(maxDifPct) and not isnull(maxDifVal):
        if logger:
            logger.warning(
                        '同时设置`maxDifPct`和`maxDifVal`，以`maxDifPct`为准！')
        else:
            print('同时设置`maxDifPct`和`maxDifVal`，以`maxDifPct`为准！')
        maxDifVal = None
    ...
    if Tmin:
        def maxDifOK(v2, v3):
            '''判断v2和v3差值是否大于maxDifPct/maxDifVal'''
            if not isnull(maxDifPct) and \
                                abs(cal_pct(v2, v3, Pct_v00)) >= maxDifPct:
                return True
            elif not isnull(maxDifVal) and abs(v3-v2) >= maxDifVal:
                return True
            else:
                return False
            
        def minDifOK(v2, v3):
            '''判断v2和v3差值是否小于minDifPct/minDifVal'''
            if not isnull(minDifPct) and \
                                abs(cal_pct(v2, v3, Pct_v00)) <= minDifPct:
                return True
            elif not isnull(minDifVal) and abs(v3-v2) <= minDifVal:
                return True
            else:
                return False
        
        def toDel(df, k1, k2, k3, k4):
            '''
            判断是否需要删除(k2, k3)位置的极值点对，判断条件见del_Tmin函数
            '''
            if k4 < df.shape[0] and k1 < 0: # 开头位置特殊处理
                v2, v3, v4 = df.loc[k2, col], df.loc[k3, col], df.loc[k4, col]
                if k3-k2 < Tmin+1: # 间隔小于Tmin
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif maxDifOK(v2, v3): # 差值满足条件，保留
                            return False, None
                        else:
                            return True, k2
                        ...
                        if v2 <= v4: # 若影响前后趋势，保留
                            ...
                            return True, k2
                elif k3-k2 >= Tmax: # 间隔大于Tmax
                    return False, None
                else: # 间隔大于等于Tmin
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif minDifOK(v2, v3): # 不满足差值条件，删除
                            return True, k2
                        else:
                            return False, None
                    elif df.loc[k2, 'label'] == -1:
        ...
        def del_Tmin(df):
            ...
            删除不满足条件的相邻极大极小值点对，删除条件：
                若间隔小于Tmin：
                    若删除后影响前后趋势，保留
                    若差值大于maxDifPct/maxDifVal，保留
                    否则删除
                若间隔大于等于Tmin：
                    若差值小于minDifPct/minDifVal，删除
                    否则保留
            ...
            while k2 < df.shape[0]:
                if df.loc[k2, 'label'] == 0:
                    ...
                    to_del, idxs = toDel(df, k1, k2, k3, k4)
                    if to_del:
                        df.loc[idxs, 'label'] = 0
...
def get_his_maxmin_info(df, col, col_label, max_lbl=1, min_lbl=-1):
    ...
    df['pre_label_val'] = get_pre_val_Fcond(df, col, col_label,
...
def get_last_pos_info(df, col, col_label, min_gap=2, max_lbl=1, min_lbl=-1):
    ...
    min_gap: 若大于0，则强制将最后min_gap条记录的标签去除
    '''
    df = df.reindex(columns=[col, col_label])
    if min_gap > 0:
        df.loc[df.index[-min_gap:], col_label] = 0
    df['pre_max_val'] = get_pre_val_Fcond(df, col, col_label,
                                                lambda x: x == max_lbl)
    df['pct_pre_max_val'] = df['pre_max_val'] / df[col] - 1
    df['dif_pre_max_val'] = df['pre_max_val'] - df[col]
    df['pre_min_val'] = get_pre_val_Fcond(df, col, col_label,
                                                lambda x: x == min_lbl)
...
#%%
def LabelMaxMinNeighbor(df, label_col, base_col, labels=[-1, 1], label0=0,
                        maxT=3, maxPct=1.0/100):
    '''
    todo：处理不同标签重复标注问题
    极大极小值邻近点标签标注
    '''
    ...
    for lbl in labels:
        ...
        for idx in idxs:
            ...
            strt = max(0, idx-maxT)
            end = min(idx+maxT, imax)
            for k in range(strt, end+1):
                if abs(df[base_col].iloc[k] / v0 - 1) < maxPct:
                    df.loc[df.index[k], label_col+'_new'] = lbl
    return df[label_col+'_new']

#%%
def delClosedMaxMin(df, label_col, base_col, minPct=5.0/100):
    '''
    删除FindMaxMin结果中极大值和极小值差别太小的极值点
    '''
    df = df.reindex(columns=[base_col, label_col])
    k, k1 = 0, 0
...
def get_last_sure_maxmin_info(df, maxmin_col='maxmin', price_col='ma',
    ...
    if sure_gap > sure_gap_min and abs(sure_pct) > sure_pct_min:
        ...
        sure = False
    return sure, last_is_maxmin, \
            (sure_maxmin, sure_loc, sure_gap, sure_price, sure_pct)
...
#%%
def dyFindMaxMin(df, col, Tmin, his_lag=None, use_all=True,
    ...
    Parameters
    ----------
    his_lag: 从第his_lag条记录处开始动态标注，若为None则默认为4*Tmin
    use_all: 若为True，每次动态标注时使用之前的所有历史数据，否则只使用前his_lag期数据
    动态确定标签参数和规则（以确定极小值信号为例，极大值同理）：
        （待补充）
    logger: 日志记录器
    n_log: 没个n_log期日志提示进度
    fig_save_dir: 若为False，则不做图；若为文件夹路径，则画图并保存到文件夹；
                  若为None，则绘图但不保存
    plot_all: 若为False，则只有在确定为极大/小值是才绘图，否则每期都绘图
    plot_lag: 指定每次画图时往前取plot_lag期历史数据进行绘图。若为None，则取his_lag；
              若为`all`，则取所有历史期数；
    '''
    ...
    N = len(it_idxs)
    # 开始历史期数
    his_lag = 4 * Tmin if his_lag is None else his_lag
    ...
    for it in range(his_lag, N):
        ...
        if n_log and it % n_log == 0:
            logger.info(f'{it} / {N}, {it_idx} dyFindMaxMin ...')
        ...
        # 极值点
        df['mm'] = FindMaxMin(df[col], Tmin=Tmin, **kwargs_findMaxMin)
...
if __name__ == '__main__':
    ...
    df1['label'] = FindMaxMin(df1['col'])
    plot_MaxMins(df1, 'col', 'label',
                 title='寻找极大极小值test1')   
    OK, e = check_peaks(df1, df1.columns[0], df1.columns[1])
    if OK:
        print('极值点排列正确！')
    else:
        print('极值点排列错误:', e)
    ...
    df2['label'] = FindMaxMin(df2['col'])
    plot_MaxMins(df2, 'col', 'label',
                 title='寻找极大极小值test2')   
    OK, e = check_peaks(df2, df2.columns[0], df2.columns[1])
    if OK:
        print('极值点排列正确！')
    else:
        print('极值点排列错误:', e)
    ...
    # Tmin = None
    Tmin = 5
    df['label'] = FindMaxMin(df['test'], Tmin=Tmin, minDifVal=4, Tmax=10)
    ...
    series_ = df[col]
    Tmin = 5
    df['label'] = FindMaxMin(series_, Tmin=Tmin, minDifPct=0.15/100, Tmax=10)
    ...
    #%%
    # 动态标签
    # df['signal'], df['sig_type'] = dyFindMaxMin(df, col, Tmin,
    #                                 his_lag=50,
    #                                 use_all=False,
    #                                 logger=None,
    #                                 n_log=10,
    #                                 # fig_save_dir='./test/dyFindMaxMin/50ETF',
    #                                 fig_save_dir=None,
    #                                 plot_all=True,
    #                                 plot_lag='all',
    #                                 kwargs_findMaxMin={},
