...
def today_month(joiner='-', forbid_joiners=['%']):
    '''获取今日所在年月'''
    return get_YearMonth(today_date(joiner, forbid_joiners))
...
def get_dates_between(date1, date2, keep1=False, keep2=True,
    ...
    '''
    _, joiner1 = get_format(date1)
    _, joiner2 = get_format(date2)
...
def get_recent_workday_ChnCal(date=None, dirt='post'):
    ...
    注：若chinese_calendar库没更新，可能导致结果不准确
    '''
    if isnull(date):
        date = today_date()
    if dirt == 'post':
...
def cut_date(start_date, end_date, n=500):
    ...
    比如start_date, end_date, n = '20200201', '20200225', 10，
    则应返回:
        [['20200201', '20200210'],
         ['20200211', '20200220'],
         ['20200221', '20200225']]
