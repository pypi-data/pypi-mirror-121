...
def HCPSOBOA(objf, func_opter_parms):
    ...
    for t in range(1, Niter+1):
        ...
        for i in range(1, PopSize+1):
            ...
            # 自适应权重
            w = np.sin((np.pi*t) / (2*Niter)+ np.pi) + 1

            # 全局|局部搜索
            r1, r2 = np.random.rand(), np.random.rand()
            p_ = p - 0.1 * (Niter-t) / Niter # 动态切换概率
        ...
        if nshow:            
            if t % nshow ==0:
                ...
                logger.info(f'{opter_name} for {func_name}, iter: {t}, ' + \
                            f'best fval: {best_y}')
...
if __name__ == '__main__':
    ...
    logger = get_logger('./test/HCPSOBOA_test.txt', screen_show=True)
