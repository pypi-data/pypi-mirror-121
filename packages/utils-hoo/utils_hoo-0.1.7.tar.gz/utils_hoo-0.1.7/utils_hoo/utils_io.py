...
from .utils_general import simple_logger, isnull, cut_df_by_con_val
# from utils_hoo.utils_logging.logger_utils import logger_show
# from utils_hoo.utils_general import simple_logger, isnull, cut_df_by_con_val
...
def install_pkg(pkg_name, version=None, upgrade=False, ignore_exist=False,
    ...
    if pkg_name[-4:] == '.whl' and os.path.exists(pkg_name):
        cmd_str = f'pip install {os.path.abspath(pkg_name)} {ignr}'
    else:
        if version is not None:
            cmd_str = f'pip install {pkg_name}{version} {ignr}'
        else:
            upgrade_str = '--upgrade' if upgrade else ''
            cmd_str = f'pip install {upgrade_str} {pkg_name} {ignr}'
    ...
    logger_show(logger, f'安装{pkg_name} ...')
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise IOError(
                  f'读取{fpath}出错，请检查文件（如编码或文件末尾多余字符等问题）！')
