...
def cal_beta(values_target, values_base, gain_type='pct', vP0=1):
    ...
    return np.cov(pcts_target, pcts_base)[0][1] / np.var(pcts_base)
...
def cal_sig_gains(data, sig_col, sig_type=1, shift_lag=0,
    ...
    data: 行情数据，其列须包含[sig_col, col_price, col_price_buy, col_price_sel]。
          ...
          sig_col列为信号列，其值为：
              ...
              -1为做多（买入）信号，0为不操作
              当sig=2时，sig_col列的值为正|负整数或0，其中正整数表示买入（做多）交易量，
    ...
    assert (data[sig_col] == 0).sum() < data.shape[0], f'{sig_col}信号列全为0！'
    ...
    for col in cols:
        if df[col].isna().sum() > 0:
            raise ValueError(f'{col}列存在无效值，请检查！')
    ...
    for k in range(0, df.shape[0]):
        if nshow and k % nshow == 0:
            logger_show(logger,
                        f'simTrading: {k} / {df.shape[0]}, {ori_index[k]} ...')
...
def get_open_gain_info(df_gain, col_price='close', col_price_buy='close',
    ...
    while i < N:
        if nshow and i % nshow == 0:
            logger_show(logger,
                        f'GetOpenGainInfo: {i} / {df.shape[0]} ...')
...
def get_yield_curve(data, sig_col, NPeriod=252, ext_type=1,
    ...
    for col_name in cols_to_plot:
        if col_name in df_gain.columns:
            logger_show(logger, f'{col_name}列画图数据被更新！', 'warn')
            continue
...
def print_key_infos(trade_gain_info, net_type='fundnet'):
    print(f'年化收益率：{round(trade_gain_info["年化收益(净值)"], 4)}；' + \
          f'年化收益率（市场）：{round(trade_gain_info["年化收益(市场)"], 4)}')
    print(f'年化收益率（超额）：{round(trade_gain_info["年化超额"], 4)}')
    print(f'最大回撤：{round(trade_gain_info["最大回撤(净值)"], 4)}；' + \
          f'最大回撤（市场）：{round(trade_gain_info["最大回撤(市场)"], 4)}')
    print(f'胜率(平)：{round(trade_gain_info["胜率(平仓或减仓)"], 4)}；' + \
          f'胜率(开)：{round(trade_gain_info["胜率(开仓)"], 4)}')
    if net_type in ['prod', 'sum']:
        print(f'盈亏比(平): {round(trade_gain_info["百分比盈亏比(平仓或减仓)"], 4)}；' + \
              f'平均赢率：{round(trade_gain_info["百分比平均赢率(总交易次数)"], 4)}')
    else:
        print(f'盈亏比(平): {round(trade_gain_info["盈亏比(平仓)"], 4)}；' + \
              f'平均赢率：{round(trade_gain_info["平均赢率(总交易次数)"], 4)}')
    print(f'夏普比率：{round(trade_gain_info["夏普(净值)"], 4)}；' + \
          f'夏普比率（市场）：{round(trade_gain_info["夏普(市场)"], 4)}')
    print(f'单笔最大回撤(平): {round(trade_gain_info["单笔最大回撤(平仓或减仓)"], 4)}；' + \
          f'单笔最大亏损(平)：{round(trade_gain_info["单笔最大亏损(平仓或减仓)"], 4)}')
    print(f'最大回撤区间长度: {round(trade_gain_info["最大回撤区间长度"], 4)}')
    print(f'平均持仓周期: {round(trade_gain_info["平均持仓周期(所有平均)"], 4)}；' + \
          f'开仓频率: {round(trade_gain_info["开仓频率"], 4)}')
...
def get_yield_curve2(data, col_gain, col_cost, col_price=None, NPeriod=252,
    ...
    if isnull(col_price) and \
        ...
        logger_show(logger, '未识别价格列，收益曲线无法与市场基准比较！', 'warn')
    if isnull(col_price):
        show_mkt = False
    ...
    if show_key_infos:
        if not isnull(col_price) and show_mkt:
            print(f'年化收益率：{round(gain_info["年化收益"], 4)}；' + \
                  f'年化收益率（市场）：{round(gain_info["年化收益(市场)"], 4)}')
            print(f'年化收益率（超额）：{round(gain_info["年化超额"], 4)}')
            print(f'最大回撤：{round(gain_info["最大回撤"], 4)}；' + \
                  f'最大回撤（市场）：{round(gain_info["最大回撤(市场)"], 4)}')
            print(f'胜率：{round(gain_info["胜率"], 4)}；' + \
                  f'盈亏比: {round(gain_info["盈亏比"], 4)}')
            print(f'平均赢率：{round(gain_info["平均赢率"], 4)}')
            print(f'夏普比率：{round(gain_info["夏普"], 4)}；' + \
                  f'夏普比率（市场）：{round(gain_info["夏普(市场)"], 4)}')
            print(f'最大回撤区间长度: {round(gain_info["最大回撤区间长度"], 4)}')
        else:
            print(f'年化收益率：{round(gain_info["年化收益"], 4)}')
            print(f'最大回撤：{round(gain_info["最大回撤"], 4)}')
            print(f'胜率：{round(gain_info["胜率"], 4)}；' + \
