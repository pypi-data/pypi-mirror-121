...
#%%
class TimesCaster_simple(object):
    '''
    简单倍投法博弈过程计算
    适用场景：
    ...
    def get_next_new_cost(self, pre_total_cost, pre_total_n):
        '''
    ...
    def play_Nround(self, N):
        ...
        for k in range(0, N-1):
            new_n, new_cost = self.get_next_new_cost(total_cost, total_n)
            total_n += new_n
            total_cost += new_cost
            back_if_over = new_n * self.base_back
            gain_if_over = back_if_over - total_cost
...
#%%
class TimesCaster_ConFuture(object):
    '''
    永续合约交易倍投法交易博弈过程计算
    ...
    def __init__(self, player, start_price, pct_add=2.0/100, lever=100,
                 expect_gain_pct=2.0/100, VolF_add='ctrl_loss_pct_1.0/100',
                 init_vol=1, max_hold_rate=10/100, fee_pct=0.1/100,
                 n_future2target=0.001, seller_cost_base='open'):
        '''
        todo：增加更多加仓方式选择（如每次加仓量跟当前加仓总次数相关联等）
              fee卖出和买入分开考虑
        
        player: 交易角色，买方设置为：
                  ['buyer', 'Buyer', 'b', 'B', 'buy', 'Buy']，
                卖方设置为：
                  ['seller', 'Seller', 's', 'S', 'sell', 'Sell', 'sel', 'Sel']
        start_price: 开始时（一个单位标的证券对应的）合约价格
                    （eg. 一个BTC对应的永续合约价格为50000，注意：不是BTC的价格）
        pct_add: 合约价格每次跌|涨幅达到pct_add就加仓
        lever: 杠杆倍数
        expect_gain_pct: 期望盈利比例（注：成本为实际参与交易的资金总额，不是账户总额）
        VolF_add: 加仓时的交易量确定方式
            当VolF_add指定为'base_x'时，每次加仓量为底仓init_vol的x倍
            当VolF_add指定为'hold_x'时，每次加仓量为持有量的x倍
            当VolF_add为'ctrl_loss_pct_x': 每次加仓量根据加仓完成之后的目标亏损比例
                来计算（计算目标成本时暂不考虑平仓时的交易成本），
                例如'ctrl_loss_pct_2.0/100'表示加仓完成之后亏损控制在2%
            当VolF_add指定为'base_kth_x'时，第k次加仓量为底仓init_vol的k倍乘以x
            当VolF_add指定为'hold_kth_x'时，第k次加仓量为底仓init_vol的k倍乘以x
        init_vol: 初次下单合约张数（基准开仓量）
        max_hold_rate: 最大仓位限制（用于计算账户所需保证金最低额）
        fee_pct: 交易成本（手续费）比例（单次）
        n_future2target: 一张合约对应多少个标的证券
                        （用于计算保证金，eg. 一张BTC永续合约对应0.001个BTC）
        seller_cost_base: 计算卖方-做空盈亏比时成本用开仓价`open`还是平仓价`close`：
        ...
        if 'ctrl_loss_pct_' in VolF_add:
            ctrl_loss_pct = eval(VolF_add.split('pct_')[-1])
            self.ctrl_loss_pct = ctrl_loss_pct
            self.F_add_vol = self.get_add_info_by_ctrl_loss
        elif 'base_kth_' in VolF_add:
            self.multer_add_base = int(VolF_add.split('base_kth_')[-1])
            self.F_add_vol = self.get_add_info_by_base_vol_kth
        elif 'hold_kth_' in VolF_add:
            self.multer_add_hold = int(VolF_add.split('hold_kth_')[-1])
            self.F_add_vol = self.get_add_info_by_hold_vol_kth
        elif 'base_' in VolF_add:
            self.multer_add_base = int(VolF_add.split('base_')[-1])
            self.F_add_vol = self.get_add_info_by_base_vol
        elif 'hold_' in VolF_add:
            self.multer_add_hold = int(VolF_add.split('hold_')[-1])
            self.F_add_vol = self.get_add_info_by_hold_vol
        
    def get_Punit(self, price):
        '''根据当前单位标的证券对应的合约价格price计算一张合约的价格（所需保证金）'''
        return price * self.n_future2target / self.lever
    
    def get_price(self, Punit):
        '''根据一张合约的价格（所需保证金）计算单位标的证券对应的合约价格price'''
        return Punit / (self.n_future2target / self.lever)
    
    def get_endPunit_gain(self, Pcost):
    ...
    def get_add_hold_info(self, vol, Punit, pre_total_vol, pre_total_cost):
        '''
    ...
    def get_add_info_by_base_vol_kth(self, Punit, pre_total_vol,
                                     pre_total_cost, k):
        '''
        计算加仓量并更新加仓后的持仓信息
        ...
        total_vol, total_cost, mean_cost, total_val = self.get_add_hold_info(
                                    vol, Punit, pre_total_vol, pre_total_cost)
        return vol, total_vol, total_cost, mean_cost, total_val
    
    def get_add_info_by_hold_vol_kth(self, Punit, pre_total_vol,
                                     pre_total_cost, k):
        '''
        计算加仓量并更新加仓后的持仓信息
    ...
    def get_add_info_by_base_vol(self, Punit, pre_total_vol, pre_total_cost,
                                 **kwargs):
        '''
        计算加仓量并更新加仓后的持仓信息
        加仓量确定方式：基准开仓量的倍数
    ...
    def get_add_info_by_hold_vol(self, Punit, pre_total_vol, pre_total_cost,
                                 **kwargs):
        '''
        计算加仓量并更新加仓后的持仓信息
        加仓量确定方式：持仓量的倍数
    ...
    def get_add_info_by_ctrl_loss(self, Punit, pre_total_vol, pre_total_cost,
                                  **kwargs):
        '''
        计算加仓量并更新加仓后的持仓信息
    ...
    def play_Nround(self, N):
        ...
        # 首次开仓
        Punit0 = self.get_Punit(self.start_price) # 一张合约价格
        Punit = Punit0
        vol = self.init_vol
        total_vol = vol
        if self.player == 'buyer':
        ...
        endPunit, gain_unit_end, gain_pct_end = \
                                             self.get_endPunit_gain(mean_cost)
        if self.player == 'buyer':
        ...
        results['成交价'] = results['成交价'].apply(lambda x: self.get_price(x))
        results['平仓价'] = results['平仓价'].apply(lambda x: self.get_price(x))
...
if __name__ == '__main__':
    ...
    #%%
    # 彩票
    tc_lottery = TimesCaster_simple(base_cost=2, base_back=5,
                                    expect_gain_pct=50/100, init_n=1)
    results_lottery = tc_lottery.play_Nround(N=30)
    ...
    VolF_add = 'ctrl_loss_pct_2.0/100'
    # VolF_add = 'base_kth_3'
    # VolF_add = 'hold_1'
    init_vol = 5
    N = 10
    ...
    # BTC永续-做多
    tcBTC_buy = TimesCaster_ConFuture('buyer', start_price,
                                      pct_add=pct_add,
                                      expect_gain_pct=expect_gain_pct,
                                      VolF_add=VolF_add,
                                      init_vol=init_vol)
    resultsBTC_buy, resultsBTC_buy_u = tcBTC_buy.play_Nround(N)
    
    # BTC永续-做空
    tcBTC_sel_open = TimesCaster_ConFuture('seller', start_price,
                                           pct_add=pct_add,
                                           expect_gain_pct=expect_gain_pct,
                                           VolF_add=VolF_add,
                                           init_vol=init_vol,
    ...
    resultsBTC_sel_open, resultsBTC_sel_open_u = \
                                            tcBTC_sel_open.play_Nround(N)
    
    tcBTC_sel_close = TimesCaster_ConFuture('seller', start_price,
                                            pct_add=pct_add,
                                            expect_gain_pct=expect_gain_pct,
                                            VolF_add=VolF_add,
                                            init_vol=init_vol,
