...
def get_recent_trade_date_ChnCal(date=None, dirt='post'):
    '''
    若date为交易日，则返回，否则返回下一个(post)或上一个(pre)交易日
    注：若chinese_calendar库统计的周内工作日与交易日有差异或没更新，可能导致结果不准确
    '''
    if isnull(date):
        date = utl_dttm.today_date()
    if dirt == 'post':
...
def get_trade_dates(start_date, end_date=None, trade_dates_df_path=None,
    ...
    _, joiner1 = utl_dttm.get_format(start_date)
    if isnull(end_date):
        end_date = utl_dttm.today_date(joiner=joiner1)
    
    if isnull(trade_dates_df_path):
        return utl_dttm.get_dates_between(start_date, end_date, keep1=True,
        ...
        _, joiner2 = utl_dttm.get_format(end_date)
        ...
        if last_date == end_date:
            ...
            start_date_ = utl_dttm.date_add_Nday(last_date, N=1)
            dates_ = utl_dttm.get_dates_between(start_date_, end_date, 
                                 keep1=True, keep2=True, only_workday=True,
                                 del_weekend=True, joiner=joiner)
            return dates + dates_
    
    
def get_trade_dates_bk(trade_dates_df_path, start_date, end_date):
    '''
    获取起止日期之间(从start_date到end_date)的交易日期，返回列表
    trade_dates_df_path可以为历史交易日期数据存档路径，也可以为pd.DataFrame
    注：默认trade_dates_df_path数据格式为tushare格式（GetData1_tushare.py）：
