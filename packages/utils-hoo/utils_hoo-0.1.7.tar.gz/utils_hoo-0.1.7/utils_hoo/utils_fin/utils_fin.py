...
from utils_hoo.utils_datsci.utils_stats import cumScale, cumPctLoc
from statsmodels.sandbox.regression.predstd import wls_prediction_std
from utils_hoo.utils_datsci.utils_stats import rollingScale, rollingPctLoc


def calIC(data, colFactor, colP='close', colT=None, colCode=None,
          lag=1, icType='rank'):
    '''
    计算因子IC值
    colFactor、colP、colT、colCode分别为因子列、价格列、时间列、代码列
    lag设置期数，ICType设置IC类型，可选`normal`和`rank`
    '''
    df = data.copy()
    if isnull(colT):
        df.index.name = 'T'
    ...
    if isnull(colCode):
        df['code'] = 'code0'
    df = df.reindex(columns=['code', 'T', colP, colFactor])
    df_ = []
    def get_pct(df):
        df.sort_values('T', inplace=True)
        df['pct'] = df[colP].shift(-lag) / df[colP] - 1
        return df
    for code in df['code'].unique():
        df_.append(get_pct(df[df['code'] == code].copy()))
    df = pd.concat(df_, axis=0)
    if icType == 'normal':
        return df[[colFactor, 'pct']].corr().loc[colFactor, 'pct']
    elif icType == 'rank':
        return df[[colFactor, 'pct']].corr(method='spearman').loc[colFactor, 'pct']
...
def simPrice_MC(his_prices, nT, nCut=240, Nsim=1000, keepP0=True,
                random_seed=None, kwargs_plot=None):
    '''
    蒙特卡洛法模拟股价路径
    ...
    nCut: 将一期时间划分为多少个小时段
    Nsim: 模拟次数
    keepP0: 模拟结果中是否保留第一个初始价格
    kwargs_plot: dict, 绘图参数
    
    Returns
    -------
    df_sim: 模拟结果，shape为(nT, Nsim)，即每列为一条模拟路径
    '''
    
    np.random.seed(random_seed)
    
    df = pd.DataFrame({'p': his_prices})
    df['pcts'] = df['p'].pct_change() # 历史收益率序列
    mu = df['pcts'].mean() # 收益率均值
    ...
    P0 = pd.Series(his_prices).iloc[-1]
    df_sim_ = pd.DataFrame({'P0': np.ones(Nsim) * P0})
    for k in range(1, nT_+1):
        df_sim_[f'P{k}'] = np.random.randn(Nsim)
        df_sim_[f'P{k}'] = df_sim_[f'P{k-1}'] + df_sim_[f'P{k-1}'] * \
                           (mu_deltaT + df_sim_[f'P{k}'] * sigma_deltaT_sqrt)
    if keepP0:
        cols = [f'P{k*nCut}' for k in range(0, nT+1)]
    else:
        cols = [f'P{k*nCut}' for k in range(1, nT+1)]
    ...
    df_sim.columns = [f'sim_{k}' for k in range(1, df_sim.shape[1]+1)]
    if not kwargs_plot is None:
...
def MAsOrder(series, lags=[3, 5, 15, 20, 30, 60]):
    '''
    ...
    df = pd.DataFrame({'p': series})
    for lag in lags:
        df[f'ma_{lag}'] = df['p'].rolling(lag).mean()
    def get_order_type(x):
        order_type = 0
        for k in range(1, len(lags)):
            if x[f'ma_{lags[k]}'] >= x[f'ma_{lags[k-1]}']:
                order_type -= 1
            elif x[f'ma_{lags[k]}'] <= x[f'ma_{lags[k-1]}']:
                order_type += 1
        if order_type == len(lags) - 1:
            ...
            return 0
    df['order_type'] = df[[f'ma_{lag}' for lag in lags]].apply(lambda x:
                                                get_order_type(x), axis=1)
...
def KDJ(data, Nhis=9, nK=3, nD=3, nK_=1, nD_=1):
    '''
    ...
    df['high_'] = df['high'].rolling(Nhis).max()
    df['low_'] = df['low'].rolling(Nhis).min()
    df['RSV'] = 100 * ((df['close']-df['low_']) / (df['high_']-df['low_']))
    df['K'], df['D'] = 0, 0
    for k in range(Nhis-1, df.shape[0]):
        RSV_now = df.loc[df.index[k], 'RSV']
        K_pre = df.loc[df.index[k-1], 'K']
        K_now = (nK_*RSV_now + (nK-nK_)*K_pre) / nK
        df.loc[df.index[k], 'K'] = K_now
        D_pre = df.loc[df.index[k-1], 'D']
        D_now = (nD_*K_now + (nD-nD_)*D_pre) / nD
...
def ATR(df, lag=14):
    '''平均真实波幅ATR，df中必须包含['high', 'low', 'close']三列'''
...
def ADX(df, lag=14):
    '''
    平均趋向指标ADX，df中必须包含['high', 'low', 'close']三列
    https://zhuanlan.zhihu.com/p/64827704
    '''
    raise NotImplementedError


def ADX_talib(df, lag=14):
    '''
    平均趋向指标ADX，df中必须包含['high', 'low', 'close']三列
    '''
...
def MeanCandle(data):
    '''
    平均K线计算，data(pd.DataFrame)中必须包含['open', 'close', 'high', 'low']
...
def Cross2_plot(data, col_buy, base_buy, col_sel, base_sel,
                buy_sig=1, sel_sig=-1, col_price='close', plot=True,
                **kwargs_plot):
    '''在Cross2的信号基础上绘图'''
    df = data.copy()
...
def Cross_cumMaxMinDif(data, col, rlag, thr_rmax2now, thr_rmin2now,
...
def ACPMP(values, periods=range(2, 60), mode='sum', std_type='pct',
    ...
    '''
    计算多周期平均累计百分比(Average Cumulative Percentage of Multi-Period)
    periods指定多个周期列表
    mode设置动量累计模式：
    ...
    std_type设置滚动标准化方式：
        ...
        若为`pct`，则转化为百分位
    lag_roll设置为整数时，标准化时采用滚动方式而不是累计方式
    std_order设置标准化顺序：
    ...
    if mode == 'sum':
        ...
        for n in periods:
            df[f'acc_pct_{n}'] = df['pct'].rolling(n).sum()
    elif mode == 'dirt':
        for n in periods:
            df[f'acc_pct_{n}'] = df['v'].pct_change(n)
            
    if isnull(std_order) or not std_order:
        df['acpmp'] = df[[f'acc_pct_{n}' for n in periods]].mean(axis=1)
    else:
        if std_type == '01':
            if isnull(lag_roll) or not lag_roll:
                stdFunc = cumScale
            else:
                stdFunc = lambda x: rollingScale(x, lag_roll)
        elif std_type == 'pct':
            if isnull(lag_roll) or not lag_roll:
                stdFunc = cumPctLoc
            else:
                stdFunc = lambda x: rollingPctLoc(x, lag_roll)
        
        if std_order == 'first':
            for n in periods:
                df[f'acc_pct_{n}_std'] = stdFunc(df[f'acc_pct_{n}'])
            df['acpmp'] = df[[f'acc_pct_{n}_std' for n in periods]].mean(axis=1)
        elif std_order == 'last':
            df['acpmp'] = df[[f'acc_pct_{n}' for n in periods]].mean(axis=1)
...
if __name__ == '__main__':
    ...
    from utils_hoo.utils_general import replace_repeat_iter
    from utils_hoo.utils_plot.plot_Candle import plot_Candle
    from utils_hoo.utils_fin.utils_gains import get_yield_curve 
    ...
    # # 纠错
    # df['sig_pre'] = get_pre_val_Fcond(df, 'signal', 'signal', lambda x: x != 0)
    ...
    # trade_gain_info, gain_stats, df_gain = get_yield_curve(
    #                                         df.iloc[:, :], 'signal',
    #                                         baseMny=None, baseVol=100,
    #                                         fee=1.5/1000, max_loss=None,
    #                                         max_gain=None, max_down=None,
    #                                         show_dyMaxDown=False,
    ...
    df_ = df.iloc[:, :].copy()
    N, Lag = 9, 4    
    df_['dmktd'] = DeMarkTD(df_['close'], N, Lag)
