...
from utils_hoo.utils_datetime import today_date
from utils_hoo.utils_datetime import get_format
from utils_hoo.utils_datetime import date_reformat
from utils_hoo.utils_fin.utils_CHN import get_recent_trade_date_ChnCal
from utils_hoo.utils_datetime import get_recent_workday_ChnCal
from utils_hoo.utils_datetime import get_recent_inweekday_ChnCal
...
def find_target_dir(dir_name):
    ...
    for dr in prefix_dirs:
        if os.path.exists(dr+dir_name):
            return dr + dir_name
    raise ValueError(f'未找到文件夹`{dir_name}`路径，请检查！')
...
def check_daily_data_is_new(df_path, date_col='date', only_trade_day=True,
    ...
    _, joiner = get_format(exist_last_date)
