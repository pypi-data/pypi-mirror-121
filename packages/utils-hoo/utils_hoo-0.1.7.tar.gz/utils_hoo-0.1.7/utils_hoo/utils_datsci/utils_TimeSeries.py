...
from utils_hoo import isnull, simple_logger
...
def genXy_X2d_y1d1step(df, ycol, Xcols_series_lag=None, Xcols_other=None,
    ...
    *注：序列自变量视为不能拿到跟因变量预测同期的数据，非序列自变量视为可以拿到同期数据
    gap: 预测跳跃期数（eg.用截止今天的数据预测gap天之后的数据）
    
    Returns
    -------
    X: 二维np.array，行数为样本量，列数为特征数量
    y: 一维np.array，长度等于样本量
    '''
...
def Predict_X2d_y1d1step(model, funcPred, df_train, df_valid, df_test,
    ...
    if isEffect(df_valid) and df_train.shape[0] < y_lag+gap:
        raise ValueError(
              f'根据输入结构判断，训练数据df_train记录数必须大于等于{y_lag+gap}！')
...
def SeriesPredict_X2d_y1dsteps(model, funcPred, df_train, df_valid, df_test,
    ...
    if isEffect(df_valid):
        ...
        if n_valid % y_step == 0:
            ...
            if df_train.shape[0] >= y_lag+gap+y_step:
                logger.warning('用训练集数据扩充验证集使得其样本数能被y_step整除！')
    ...
    if isEffect(df_test) and isEffect(df_valid):
        ...
        if n_test % y_step == 0:
            ...
            n_ext = y_step - n_test % y_step
            if df_train.shape[0] >= y_lag+gap+y_step:
                logger.warning('用训练集数据扩充测试集使得其样本数能被y_step整除！')
...
if __name__ == '__main__':
    ...
    from utils_hoo.utils_general import cut_range_to_subs, replace_repeat_pd
    from utils_hoo.tmp.utils_SignalDec import dec_emds, merge_high_modes
    from utils_hoo.tmp.utils_SignalDec import plot_modes
