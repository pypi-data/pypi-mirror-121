...
from utils_hoo.utils_io import pickleFile, unpickleFile
    ...
    if objective is None:
        ...
        if isinstance(parms_mdl, dict) and 'objective' in parms_mdl.keys() \
            ...
            if logger:
                ...
                print('objective与parms_mdl设置不一样，以前者为准！')
        ...
        if not isinstance(num_class, int):
            ...
            if isinstance(parms_mdl, dict) and 'num_class' in \
                ...
                if logger:
                    ...
                    print('num_class与parms_mdl设置不一样，以前者为准！')
...
def lgb_cv_GridSearch(X_train, y_train, objective=None, parms_mdl=None,
    ...
    if len(parms_mdl['metric']) > 1 and \
        ...
        if logger:
            ...
            print('发现多个metric，将以{}为优化目标！'.format(metric))
    ...
    for grid_parm in grid_parms:
        if logger:
            logger.info(f'交叉验证网格搜索中：{k} / {len(grid_parms)} ...')
            logger.info(f'当前参数：{grid_parm}')
        k += 1
