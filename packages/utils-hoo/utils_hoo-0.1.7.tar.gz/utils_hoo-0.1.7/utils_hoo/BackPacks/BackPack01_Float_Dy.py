...
# 参考https://my.oschina.net/u/3242615/blog/1940533/
...
def BackPack01_Float_Dy(c, v, w):
    ...
    c_v: 最大容量-最大价值跳跃点完整记录
        注: c_v[_] = [W, V]表示某个物品加入之后，最大容量为W时最大价值为V
    '''
    ...
    max_v = c_v[-1][1]
    trace = traceBack(v, w, c_v, head)
    
    return max_v, trace, c_v


def traceBack(v, w, c_v, head):
    '''
...
def BackPack01_Float_Dy_smp(c, v, w):
    ...
    -------
    max_v: 最大价值
    values: 最大容量-最大价值跳跃点记录
        注: values[_] = [W, V]表示某个物品加入之后，最大容量为W时最大价值为V
    '''
