...
def label_rep_index_str(df):
    ...
    df['_idx_idx_idx_'] = df[['_idx_idx_idx_', '_tmp_tmp_']].apply(lambda x:
                            ...
                            f'{x["_idx_idx_idx_"]}_{x["_tmp_tmp_"]}', axis=1)
...
def get_update_kwargs(key, arg, kwargs, arg_default=None, update=True):
    '''
    取出并更新kwargs中key参数的值
    若update为True，则取出的参数将被更新为arg，否则保留原来的值或默认值
    '''
    ...
    if update:
        if isinstance(arg, dict):
            arg_new = {} if isnull(arg_new) else arg_new
            arg_new.update(arg)
        else:
            arg_new = arg
...
def con_count_(series, Fcond, via_pd=True,
               Fignore=lambda x: x == 0 or isnull(x)):
    '''
    在con_count的基础上增加连续性判断条件：
        当series中的值满足Fignore函数值为True，不影响连续性判断
    '''
    df = pd.DataFrame({'v': series})
    df['ignore'] = df['v'].apply(lambda x: Fignore(x)).astype(int)
    df['count'] = con_count(df[df['ignore'] == 0]['v'], Fcond, via_pd=via_pd)
...
def val_gap_cond_new():
    '''
    val_gap_cond函数升级修改（与count_between_gap函数结合？）
