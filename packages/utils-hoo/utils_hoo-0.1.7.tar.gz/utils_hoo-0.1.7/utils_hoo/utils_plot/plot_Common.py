...
from utils_hoo.utils_general import simple_logger, isnull, get_update_kwargs
...
def plot_Series(data, cols_styl_up_left, cols_styl_up_right={},
                ...
                title=None, nXticks=8, xticks_rotation=None,
    ...
    cols_to_label_info: 设置需要特殊标注的列绘图信息，格式形如:
        {col1: [[col_lbl1, (v1, v2, ..), (styl1, styl2, ..), (lbl1, lbl2, ..),
                 {v1: {kwargs1}, v2: {kwargs2}}],
                [col_lbl2, (v1, v2, ..), ...]],
         col2: ..}，其中col是需要被特殊标注的列，col_lbl为标签列；v指定哪些标签值对应的
        数据用于绘图；styl设置线型；lbl设置图例标签，若为None，则设置为v，若为False，
        则不设置图例标签；{v1: {kwargs1}, v2: {kwargs2}}设置其他绘图标注参数
    cols_to_fill_info: 需要进行颜色填充的列信息，格式形如:
        {col1 : {'color': 'c', 'alpha': 0.3}, ...}
    yscales: y轴标轴尺度设置，若为None，则默认普通线性坐标，可设置为list指定每个坐标尺度
    ...
    fills_yparl_up, fills_yparl_low: 分别设置上下图平行于y轴的填充区域信息，格式形如:
        [([x1, x2], clor1, alpha1, kwargs), (...)]
    fills_xparl: 设置平行于x轴的填充区域信息，格式形如：
        {'col1': [([y1, y2], clor1, alpha1, kwargs), ...], 'col2': ...}
    ylabels: None或列表，设置四个y轴标签文本内容，若为None则不设置标签文本，
        若为False则既不设置y轴标签文本内容，也不显示y轴刻度
    xlabels: None或列表，设置两个x轴标签文本内容，若为None则不设置标签文本，
        若为False则既不设置x轴标签文本内容，也不显示x轴刻度
    grids: 设置四个坐标轴网格，若grids=True，则将顶部左轴和底部左轴绘制网格；
        若grids=False，则全部没有网格，grids可设置为列表分别对四个坐标轴设置网格
        （注：当某个坐标轴设置为不显示刻度时，其对应的网格线也会不显示？）
    legend_locs设置上下两个图的legend位置，默认设置为[0, 0]
    字体默认设置为None，可替换其他字体（常用字体包括`Courier New`、
    `Times New Roman`等，matplotlib默认字体为`sans-serif`）
    '''
    ...
    def get_parls_info(parlInfo):
        if len(parlInfo) == 5 and isinstance(parlInfo[-1], dict):
            ...
            lnwidth, kwstyl = get_update_kwargs('lw', None, kwstyl, update=False)
            ...
            lnstyl, kwstyl = get_update_kwargs('ls', None, kwstyl, update=False)
            lnwidth, kwstyl = get_update_kwargs('lw', None, kwstyl, update=False)
            ...
            val, kwstyl = parlInfo
            clor, kwstyl = get_update_kwargs('c', None, kwstyl, update=False)
            lnstyl, kwstyl = get_update_kwargs('ls', None, kwstyl, update=False)
            lnwidth, kwstyl = get_update_kwargs('lw', None, kwstyl, update=False)
        else:
            val = parlInfo[0]
    ...
    def get_fill_info(fillInfo):
        if len(fillInfo) == 4 and isinstance(fillInfo[-1], dict):
            ...
            alpha, kwstyl = get_update_kwargs('alpha', None, kwstyl, update=False)
            ...
            clor, kwstyl = get_update_kwargs('color', None, kwstyl, update=False)
            alpha, kwstyl = get_update_kwargs('alpha', None, kwstyl, update=False)
        else:
            locs = fillInfo[0]
...
def plot_Series_conlabel(data, conlabel_info, del_repeat_lbl=True, **kwargs):
    ...
    注：(v1, ...)中的最后一个值会被当成默认值，其余的当成特殊值（绘图时未来保证连续会将默认值与特殊值连接起来）
    '''
