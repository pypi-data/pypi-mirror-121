...
def FindAddends_SmlFirst(tgt_sum, alts, n_adds=None,
    ...
    while loop_count < max_loop:
        ...
        if idx_last == 0 and len(chosed_idxs) == 1:
            if log_info:
                logger.info('无最优解（搜索到最大值且只剩它一个备选数）结束搜索。')
            break
...
if __name__ == '__main__':   
    ...
    best, choseds, mid=FindAddends_SmlFirst(tgt_sum, alts, n_adds=n_adds,
                                            tol_below=tol_below,
                                            tol_above=tol_above,
                                            max_loop=max_loop,
