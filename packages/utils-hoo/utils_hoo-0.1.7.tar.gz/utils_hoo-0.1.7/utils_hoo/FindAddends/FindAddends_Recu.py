...
# 参考：https://blog.csdn.net/mandagod/article/details/79588753


from .FindAddends_utils import tol2side_x_eq_y


def FindAddends_Recu(tgt_sum, alts, max_idx, choseds=[], results=[], 
    ...
    tgt_sum: 目标和
    alts: list，备选数列表
    max_idx: 从索引max_idx处开始从后往前搜索，初始值一般设为alts的最大索引
    choseds: 预选中的备选数，初始值默认为空
    results: 已存在的所有结果，初始值默认为空
    tol_below: 下界误差，若目标和减去当前和小于等于tol_below，则为可行解，默认为0.0
    tol_above: 上界误差，若当前和减去目标和小于等于bol_above，则为可行解，默认为0.0
    n_result: 返回的可行解个数限制，当results的长度大于n_result时返回results
    keep_order: 是否保结果中备选数的进入顺序与alts中的顺序一致，默认是
    ...
    results = FindAddends_Recu(tgt_sum-alts[max_idx], alts, max_idx-1,
    ...
    # 不选max_idx继续递归
    del choseds[-1]
    results = FindAddends_Recu(tgt_sum, alts, max_idx-1, choseds=choseds,
...
if __name__ == '__main__':
    ...
    results = FindAddends_Recu(tgt_sum, alts, max_idx, choseds=choseds,
                               results=results, tol_below=tol_below,
                               tol_above=tol_above, n_result=n_result,
                               keep_order=keep_order)
