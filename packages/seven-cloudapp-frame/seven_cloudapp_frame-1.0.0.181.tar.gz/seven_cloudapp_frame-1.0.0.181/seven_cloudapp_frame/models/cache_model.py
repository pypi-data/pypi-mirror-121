...
@LastEditTime: 2021-08-26 10:28:51
...
class CacheModel(BaseModel):
    ...
    def _get_dependency_key(self, dependency_key=''):
        """
        :Description: 获取依赖建
    ...
    def _get_cache_key(self, where='', group_by='', order_by='', limit='', params=None):
        """
        :Description: 获取缓存key
        ...
        cache_key = str(where + "_" + group_by + "_" + order_by + "_" + limit + params_str).replace(" ", "").lower()
        return CryptoHelper.md5_encrypt(cache_key)
        
    def _get_dependency_cache_key(self, dependency_key, cache_key):
        """
    ...
    def _get_dependency_cache(self, dependency_key, cache_key):
        """
        :Description: 获取依赖缓存值
        :param dependency_key: 依赖键
        :param cache_key: 缓存key
        ...
        :last_editors: HuangJianYi
        """
        dependency_cache_key = self._get_dependency_cache_key(dependency_key,cache_key)
        redis_init = self._redis_init()
    ...
    def _set_dependency_cache(self, dependency_key, cache_key,value, cache_expire):
        """
        :Description: 获取依赖缓存值
        :param dependency_key: 依赖键
        :param cache_key: 缓存key
    ...
    def _redis_init(self, db=None, decode_responses=True):
        """
    ...
    def delete_dependency_key(self, dependency_key=''):
        ...
        """
        redis_init = self._redis_init()
        return redis_init.delete(self._get_dependency_key(dependency_key))
    ...
    def get_cache_list(self, where='', group_by='', order_by='', limit='', params=None,dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "entitylist_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,limit,params)
        list_row = self._get_dependency_cache(dependency_key,cache_key)
        if list_row and config.get_value("is_redis_cache",True) == True:
            ...
            if not cache_value:
                cache_value = None
            self._set_dependency_cache(dependency_key,cache_key,cache_value,cache_expire)
            return self._BaseModel__row_entity_list(list_row)
    ...
    def get_cache_page_list(self, field="*", page_index=0, page_size=20, where='', group_by='', order_by='', params=None, dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "entitypagelist_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,limit,params)
        page_list = self._get_dependency_cache(dependency_key,cache_key)
        if page_list and config.get_value("is_redis_cache",True) == True:
            ...
            self._set_dependency_cache(dependency_key,cache_key,page_list,cache_expire)
    ...
    def get_cache_entity(self, where='', group_by='', order_by='',  params=None, dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "entity_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,'1',params)
        one_row = self._get_dependency_cache(dependency_key,cache_key)
        if one_row and config.get_value("is_redis_cache",True) == True:
            ...
            self._set_dependency_cache(dependency_key,cache_key,one_row,cache_expire)
            return self._BaseModel__row_entity(one_row)
    ...
    def get_cache_entity_by_id(self, primary_key_id, dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "entityone_" + self.table_name + "_" + self._get_cache_key(where,'','','',params)
        one_row = self._get_dependency_cache(dependency_key,cache_key)
        if one_row and config.get_value("is_redis_cache",True) == True:
            if one_row == 'null':
                return None 
    ...
    def get_cache_total(self, where='', group_by='', field=None, params=None,dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "total_" + self.table_name + "_" + self._get_cache_key(where,group_by,'','',params)
        total = self._get_dependency_cache(dependency_key,cache_key)
        if total and config.get_value("is_redis_cache",True) == True:
            ...
            if list_row and 'count' in list_row:
                self._set_dependency_cache(dependency_key,cache_key,list_row['count'],cache_expire)
    ...
    def get_cache_dict(self, where='', group_by='', order_by='', limit='', field="*", params=None, dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "dict_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,limit,params)
        one_row = self._get_dependency_cache(dependency_key,cache_key)
    ...
    def get_cache_dict_by_id(self, primary_key_id, dependency_key='', cache_expire=600, field="*"):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "dictone_" + self.table_name + "_" + self._get_cache_key(where,'','','',params)
        one_row = self._get_dependency_cache(dependency_key,cache_key)
        if one_row and config.get_value("is_redis_cache",True) == True:
            if one_row == 'null':
                return None    
    ...
    def get_cache_dict_list(self, where='', group_by='', order_by='', limit='', field="*", params=None, dependency_key='',cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "dictlist_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,limit,params)
        list = self._get_dependency_cache(dependency_key,cache_key)
        if list and config.get_value("is_redis_cache",True) == True:
            return list
        else:
            list = self.db.fetch_all_rows(sql, params)
            self._set_dependency_cache(dependency_key,cache_key,list,cache_expire)
            return list
    ...
    def get_cache_dict_page_list(self, field="*", page_index=0 ,page_size=20, where='', group_by='' ,order_by='' ,params=None, dependency_key='', cache_expire=600):
        ...
        dependency_key = self._get_dependency_key(str(dependency_key))
        cache_key = "dictpagelist_" + self.table_name + "_" + self._get_cache_key(where,group_by,order_by,limit,params)
        page_list = self._get_dependency_cache(dependency_key,cache_key)
        if page_list and config.get_value("is_redis_cache",True) == True:
