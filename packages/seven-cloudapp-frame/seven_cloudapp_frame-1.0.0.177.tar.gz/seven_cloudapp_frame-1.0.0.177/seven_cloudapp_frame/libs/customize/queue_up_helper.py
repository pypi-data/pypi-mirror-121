...
@LastEditTime: 2021-09-24 10:24:45
...
class QueueUpHelper:
    ...
    def _delete_user(self, app_id, queue_name):
        ...
        if SevenHelper.is_continue_request(f"delete_and_notice_user:{queue_name}", 500) == False:
            try:
                redis_init = SevenHelper.redis_init()
                zset_name = self._get_zset_name(app_id, queue_name)
    ...
    @classmethod
    def queue(self, app_id, queue_name, user_id, user_nick="", queue_length=10, all_queue_limit_num=500, info_dict={}):
        """
        :description: 加入排队
        :param app_id：应用标识
        :param queue_name：队列名称
        ...
        :param info_dict：信息字典，由于业务端存储需要的数据
        :return: 
        :last_editors: HuangJianYi
        """
        invoke_result_data = InvokeResultData()
    ...
    def query(self, app_id, queue_name, user_id):
        ...
        try:
            ...
            if score:
                ...
                query_user["progress_status"] = 1  #进度0未排队1已排队2办理中
