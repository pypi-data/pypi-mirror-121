...
class ConfigFilesBuilding(ConfigDashTagUnitTimestamp):
    ...
    def computeAutoConso(self,timeRange,compteurs,formula='g+f-e+pv'):
        ...
        dfAutoConso['Batiment']      = df['puissance totale']
    ...
    def compute_EnergieMonth(self,timeRange,compteurs,formula='g+f-e+pv'):
        ...
        if not df.empty:
            ...
            df = df.resample('10s').mean().ffill().bfill()
            dfMonth = df.resample('1d').last().diff()[1:]
        ...
        if formula=='g+f-e+pv':
            ...
            df['energie totale'] = df[pTotal].sum(axis=1) - df[pvEnergie]
        elif formula=='sum':
            df['energie totale'] = df[energieTotale].sum(axis=1)

        df = df[['energie totale',pvEnergie]]
