...
def max_sharpe_ratio(df, limits=None, date_len=None):

    if date_len != None:
        rts = excise(log_returns(df), date_len)
    else:
        rts = log_returns(df)
...
def min_variance(df, limits=None, date_len=None):
...
# 有效前沿组合
def efficient_frontier(df, n_prtf, limits=None, date_len=None):
...
# 投资风险比
# 找出单个基金的收益风险比（年化）
def return_risks_ef(df, date_len=None):
...
# 既定目标收益
def ef_target_returns(tgt_min, tgt_max, n_prtf, df, limits=None, date_len=None):
