...
from cyberrl.envs import cyberrlEnv
from cyberrl.scenarios.benchmark import AVAIL_BENCHMARKS
from cyberrl.scenarios import \
...
def make_benchmark(scenario_name,
                   ...
                   flat_actions=True,
                   flat_obs=True):
    """Make a new benchmark cyberrl environment.
    ...
    return cyberrlEnv(scenario, **env_kwargs)
...
def load(path,
         ...
         flat_obs=True,
         name=None):
    """Load cyberrl Environment from a .yaml scenario file.
...
# Register cyberrlEnv with OpenAI gym
def _register(id, entry_point, kwargs, nondeterministic, force=True):
    """Registers cyberrl Open AI Gym Environment.
...
for benchmark in AVAIL_BENCHMARKS:
    ...
    # testag should yield testag and testag-small should yield testagSmall
    for fully_obs in [True, False]:
        ...
        _register(
            id=f"{name}-v0",
            entry_point='cyberrl.envs:cyberrlGymEnv',
            kwargs={
                "scenario": benchmark,
                "fully_obs": fully_obs,
                "flat_actions": True,
