...
To run 'testag' benchmark scenario with default settings, run the following from
the cyberrl/agents dir:

$ python bruteforce_agent.py testag
...
def run_bruteforce_agent(env, step_limit=1e6, verbose=True):
    """Run bruteforce agent on cyberrl environment.

    Parameters
    ----------
    env : cyberrl.cyberrlEnv
        the cyberrl environment to run agent on
...
if __name__ == "__main__":
    ...
    cyberrlenv = cyberrl.make_benchmark(
        args.env_name,
        args.seed,
    ...
    if not args.param_actions:
        print(cyberrlenv.action_space.n)
    else:
        print(cyberrlenv.action_space.nvec)
    run_bruteforce_agent(cyberrlenv)
