...
class iNumberParser:
    ...
    def __init__(self, s, prec=4, sosujeom='.'):
        ...
        # PartGubun('파싱가능한 입력값인지 검사')
        if isinstance(s, numbers.Number):
            self._parsable_s = str(s)
        elif isinstance(s, str):
            if '%' in s:
                logger.info(f'{self} | 퍼센트-문자열은 처리하지 않는다 | s --> {s} {type(s)}')
            else:
                self._parsable_s = s.strip()
        else:
            logger.info(f'{self} | 문자형숫자를 입력해라. s --> {s} {type(s)}')

        # PartGubun('파싱')
        if hasattr(self, '_parsable_s'):
            s = self._parsable_s
...
def iNumber(s, prec=4, sosujeom='.'):
    return iNumberParser(s, prec, sosujeom).num
...
class Percent:
    ...
    # 30 -> 3000%
    # 0.23 -> 23%
    _err_msg = "사용예시: ('30%' -> 30%), ('30' -> 30%), (30 -> 3000%), (0.23 -> 23%)"
    ...
    def __init__(self, s, prec=2, sosujeom='.'):
        ...
        if isinstance(self._s, str):
            m = re.search('([0-9\.,\+-]+)(%$)*', s)
            print(m, m.groups())
            num_part, pct_part = m[1], m[2]
            print('num_part:', num_part, type(num_part), 'pct_part:', pct_part)
            parser = iNumberParser(num_part, self._p_prec, self._sosujeom)
            # 숫자파트는 퍼센트 단위이므로, 순수숫자 단위로 변경한다
            num = parser.num / 100
            self.parser = iNumberParser(num, self._p_prec, self._sosujeom)
        elif isinstance(self._s, numbers.Number):
            self.parser = iNumberParser(self._s, self._n_prec, self._sosujeom)
        else:
            logger.info(f"{self} | {self._err_msg} | s: {self._s} {type(self._s)}")

    @property
    def pct(self):
        # 퍼센트 단위의 숫자
        return round(self.parser.num * 100, self._p_prec)

    @property
    def num(self):
        return round(self.parser.num, self._n_prec)

    @property
    def str(self):
        # 소수점이하 자릿수를 예쁘지 채워주기
        # 소수점이하 수를 문자열로 변경하고, 소수점으로 분리한 후, 소수점이하 자릿수를 조정한다
        _int, prec = str(self.pct).split('.')
        _sosu_str = prec.ljust(self._p_prec, '0')
        return _int + self._sosujeom + _sosu_str + '%'
