...
def colours_per_batch_id(
    ...
    colours = (
        list(colour_map(n_colours)) if not (use_default_colour) else [(0.5, 0.5, 0.5)] * n_colours
    )
    random.shuffle(colours)
    colours = [get_rgba_from_triplet(c, as_string=True) for c in colours]
    colour_assignment = {
        key: dict(width=default_line_width, color=val) for key, val in zip(list(batch_ids), colours)
    }
    for key, val in batches_to_highlight.items():
...
def plot_multitags(
    ...
    for index in np.linspace(0, longest_time_length, settings["animate_n_frames"]):
        ...
        frame_name = (
            f"{index}"  # this is the link with the slider and the animation in the play button
        )
