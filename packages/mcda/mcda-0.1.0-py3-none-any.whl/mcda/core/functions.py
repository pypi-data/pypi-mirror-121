...
class Interval:
    ...
    def normalize(self, x: NumericValue) -> NumericValue:
        ...
        """
        return (
            (x - self.dmin) / (self.dmax - self.dmin)
...
class VectorizedDiscreteFunction(VectorizedFunction, DiscreteFunction):
    """This class implements vectorized discrete MCDA function.
    ...
    def __init__(self, values: Dict[Value, Value]):
        """Constructor method"""
        DiscreteFunction.__init__(self, values)
        VectorizedFunction.__init__(self, {}, self.apply)


class VectorizedConstantFunction(VectorizedFunction):
    """This class implements vectorized constant MCDA function.

    :param constant:
    :raises ValueError: if `constant` is ``None``
    """

    _TYPE = "VectorizedConstantFunction"

    def __init__(self, constant: NumericValue = None):
        """Constructor method"""
        VectorizedFunction.__init__(
            self, {np.ndarray: self._apply_np_array}, self._apply
        )
        if constant is None:
            raise ValueError(f"{self._TYPE} needs a 'constant' value")
        self.constant = constant

    def _apply(self, x: NumericValue) -> NumericValue:
        """Apply function to single value.
    ...
    def _apply_np_array(self, x):
        ...
        :rtype: np.array
        """
        return self.constant * np.ones_like(x)


class VectorizedAffineFunction(VectorizedConstantFunction):
    """This class implements vectorized constant MCDA function.

    :param slope:
    :param constant:
    :param points: two points used to define the function
    :raises ValueError:
        * if `constant` is ``None``
        * if `slope` is ``None``
        * if `points` are used and are wrongly formatted

    .. note::
        * if `points` is used, `slope` and `constant` are taken from them
        * `points` must be a sequence of two points, each containing at least
          an abscissa and an ordinate in a sequence
    """

    _TYPE = "VectorizedAffineFunction"

    def __init__(
        self,
        slope: NumericValue = None,
        constant: NumericValue = None,
        points: List[List[NumericValue]] = None,
    ):
        """Constructor method"""
        if points is not None:
            if len(points) < 2:
                raise ValueError(
                    f"two points are needed to define {self._TYPE}"
                )
            ...
            if points[0][0] == points[1][0]:
                raise ValueError(
                    f"{self._TYPE} needs two points with different abscissa"
                )
            slope = (points[0][1] - points[1][1]) / (
                points[0][0] - points[1][0]
            )
            constant = (
                points[1][1] * points[0][0] - points[0][1] * points[1][0]
            ) / (points[0][0] - points[1][0])
        VectorizedConstantFunction.__init__(self, constant=constant)
        if slope is None:
            raise ValueError(f"{self._TYPE} needs a 'slope' value")
        self.slope = slope
    ...
    def _apply_np_array(self, x):
        """Override :meth:`ConstantFunction._apply_np_array`.
...
class VectorizedPieceWiseFunction(VectorizedFunction, PieceWiseFunction):
    """This class implements vectorized piecewise MCDA function.
    ...
    _TYPE = "VectorizedPieceWiseFunction"

    def __init__(
        self,
        intervals: List[Interval] = None,
        functions: List[Union[NumericFunction, VectorizedFunction]] = None,
        segments: List[List[List]] = None,
    ):
        """Constructor method"""
        PieceWiseFunction.__init__(self, intervals, functions, segments)
    ...
    def _parse_segments(self, segments: List[List[List]]):
        ...
        for seg in segments:
            ...
            b = seg[1] if len(seg[1]) > 2 else seg[1] + [True]
            self.intervals.append(Interval(a[0], b[0], a[2], b[2]))
            self.functions.append(VectorizedAffineFunction(points=seg))


class VectorizedFuzzyNumber(VectorizedPieceWiseFunction):
    """This class implements a vectorized fuzzy number.
    ...
    :raises ValueError:
        ...
        * if functions are discontinuous
    :raises TypeError:
        if any supplied function is not :class:`VectorizedConstantFunction`
    """
    ...
    def __init__(
        ...
        """Constructor method"""
        VectorizedPieceWiseFunction.__init__(
            self, intervals, functions, segments
        )
        if len(self.functions) == 0:
            raise ValueError(
                "VectorizedFuzzyNumber must have at "
                "least one VectorizedConstantFunction instance"
            )
        for f in self.functions:
            if not isinstance(f, VectorizedConstantFunction):
                raise TypeError(
                    "VectorizedFuzzyNumber can only have"
                    "VectorizedConstantFunction instance"
                )
        if not self.continuous():
            raise ValueError(
                "VectorizedFuzzyNumber functions must be continuous"
            )
