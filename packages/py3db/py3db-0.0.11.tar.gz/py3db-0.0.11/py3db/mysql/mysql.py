...
class MySql:
    def __init__(self, ip, user_name, password, database, table=None):
        self.ip = ip
    ...
    def update(self, table=None, update_dict: dict = {}, condition_dict: dict = {}, equal_str="="):
        update_list = []
        condition_list = []
        self.table_name = table if table else self.table_name
    ...
    def delete(self, table=None, condition_dict={}, equal_str="="):
        condition_list = []
        self.table_name = table if table else self.table_name
        for key, item in condition_dict.items():
            item = item if item else "NULL"
            condition_list.append("`%s` %s '%s'" % (key, equal_str, item))
        condition_sql = ",".join(condition_list)
...
#     mysql = MySql("localhost", "root", "123456", "sex")
#     mysql.delete("scontent", {"a": 10})
