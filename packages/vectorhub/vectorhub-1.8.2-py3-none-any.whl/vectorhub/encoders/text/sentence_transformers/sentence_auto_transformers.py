...
class SentenceTransformer2Vec(BaseText2Vec):
    ...
    def get_sentences_list(self, fields, documents, chunksize=500):
        sentences = []
        for c in self.chunk(documents, chunksize=chunksize):
            sentences += self.get_fields_across_documents(fields, c)
        return sentences
...
Set use_amp to True if your GPU supports FP16 cores
        """
        train_sentences = self.get_sentences_list(fields, documents)
...
Set use_amp to True if your GPU supports FP16 cores
    ...
    def run_mlm(self, 
        ...
        use_fp16: bool = True,                #Set to True, if your GPU supports FP16 operations
        max_length = 100,                #Max length for a text input
        do_whole_word_mask = True,       #If set to true, whole words are masked
        mlm_prob = 0.15,                 #Probability that a word is replaced by a [MASK] token
        output_dir = ".",
        random_split_pct=0.2,
        overwrite_ouput_dir: bool=True,
        save_total_limit=1,
        prediction_loss_only=True
    ):
        """Running MLM
        Params:
            save_steps: save model every 1k steps 
            num_train_epochs: Number of epochs 
            use_fp16: Set True if GPU supports FP16 ops 
            max_length: Max length for text input 
            do_whole_word_mask: If set to True, whole words are masked 
            mlm_prob: Probability that a wrod is replaced by a [MASK] token
            output_dir: The output directory of everything 
            random_split_pct: The random split percentage
        """
        if not hasattr(self, "tokenizer"):
            self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        if sentences is None:
            assert train_sentences is not None and dev_sentences is not None, "If sentences is None, you need to provide your own train and dev"
        else:
            random.shuffle(sentences)
            total_dev_split = int(random_split_pct * len(sentences))
            train_sentences = sentences[:total_dev_split]
            dev_sentences = sentences[total_dev_split:]
        train_dataset = TokenizedSentencesDataset(train_sentences, self.tokenizer, max_length)
        dev_dataset = TokenizedSentencesDataset(dev_sentences, self.tokenizer, max_length, 
        ...
        if do_whole_word_mask:
            ...
            data_collator = DataCollatorForLanguageModeling(tokenizer=self.tokenizer, 
                mlm=True, mlm_probability=mlm_prob)
        ##### Training arguments
        ...
        training_args = TrainingArguments(
            ...
            overwrite_output_dir=overwrite_ouput_dir,
