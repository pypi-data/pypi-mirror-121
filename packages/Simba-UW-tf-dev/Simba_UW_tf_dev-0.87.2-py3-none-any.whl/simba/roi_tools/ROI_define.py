...
class ROI_definitions:
    def __init__(self, config_path, video_path):
        ...
        self.file_ext = Path(self.video_path).suffix
        self.file_name = os.path.basename(self.video_path.rsplit(self.file_ext, 1)[0])
        self.project_path = config.get('General settings', 'project_path')
        self.roi_data_folder = os.path.join(self.project_path, 'logs', 'measures', 'ROI_definitions')
        ...
        self.video_info_df = pd.read_csv(self.video_info_path, sep='\t')
        print(self.video_info_df)
    ...
    def show_video_info(self):
        self.video_info_frame = LabelFrame(self.master, text='Video information', font=("Arial", 16, "bold"), padx=5, pady=5)
        self.video_info_frame.grid_configure(ipadx=55)
        self.video_name_lbl_1 = Label(self.video_info_frame, text="Video name: ", font=("Arial", 14)).grid(row=0, column=0)
        self.video_name_lbl_2 = Label(self.video_info_frame, text=str(self.file_name), font=("Arial", 14, "bold"))

        self.video_ext_lbl_1 = Label(self.video_info_frame, text="Video format: ", font=("Arial", 14)).grid(row=0, column=2)
        self.video_ext_lbl_2 = Label(self.video_info_frame, text=str(self.file_ext), font=("Arial", 14, "bold"))

        self.video_fps_lbl_1 = Label(self.video_info_frame, text="FPS: ", font=("Arial", 14)).grid(row=0, column = 4)
        self.video_fps_lbl_2 = Label(self.video_info_frame, text=str(self.curr_fps), font=("Arial", 14, "bold"))

        self.video_frame_lbl_1 = Label(self.video_info_frame, text="Display frame #: ", font=("Arial", 14)).grid(row=0, column=6)
        self.video_frame_lbl_2 = Label(self.video_info_frame, text=str(self.img_no), font=("Arial", 14, "bold"))

        self.video_frame_time_1 = Label(self.video_info_frame,text="Display frame (s): ", font=("Arial", 14)).grid(row=0, column=8)
        self.video_frame_time_2 = Label(self.video_info_frame, text=str(round((self.img_no / self.curr_fps), 2)), font=("Arial", 14, "bold"))
    ...
    def select_img(self):
        ...
        self.pos_1s.grid(row=1, column=0, sticky=W, pady=10)
        self.neg_1s.grid(row=1, column=1, sticky=W, pady=10)
        self.reset_btn.grid(row=1, column=2, sticky=W, pady=10)
        self.seconds_fw_label.grid(row=1, column=3, sticky=W, pady=10)
        self.seconds_fw_entry.grid(row=1, column=4, sticky=W, pady=10)
        self.custom_run_seconds.grid(row=1, column=5, sticky=W, pady=10)
    ...
    def apply_from_other_videos_menu(self):
        ...
        self.apply_from_other_video.grid(row=2, sticky=W)
    ...
    def select_shape(self):
        ...
        self.rectangle_button.grid(row=1, sticky=W, pady=10)
        self.circle_button.grid(row=1, column=1, sticky=W, pady=10)
        self.polygon_button.grid(row=1, column=2, sticky=W, pady=10)
    ...
    def select_shape_attr(self):
        ...
        self.shape_thickness_dropdown = OptionMenu(self.shape_attr_frame, self.shape_thickness, *list(self.shape_thickness_list))
        ...
        self.shape_thickness_dropdown.grid(row=1, column=1, sticky=W, pady=10)
        self.ear_tag_sizes_lbl.grid(row=1, column=2)
        self.ear_tag_size_dropdown.grid(row=1, column=3, sticky=W, pady=10)
    ...
    def interact_menus(self):
        ...
        self.zoom_pct_label = Label(self.interact_frame, text="Zoom %: ").grid(row=1, column=5)
        ...
        self.move_shape_button.grid(row=1, column=0, sticky=W, pady=10)
        self.pan.grid(row=1, column=1, sticky=W, pady=10)
        self.zoom_in_button.grid(row=1, column=2, sticky=W, pady=10)
        self.zoom_out_button.grid(row=1, column=3, sticky=W, pady=10)
        self.zoom_home.grid(row=1, column=4, sticky=W, pady=10)
    ...
    def draw_menu(self):
        ...
        self.draw_frame.grid(row=7, sticky=W)
        self.draw_button.grid(row=1, column=1, sticky=W, pady=2)
        self.delete_all_rois_btn.grid(row=1, column=2, sticky=W, pady=2)
        self.select_roi_label.grid(row=1, column=3, sticky=W, pady=2)
        self.roi_dropdown.grid(row=1, column=4, sticky=W, pady=2)
        self.apply_delete_button.grid(row=1, column=5, sticky=W, pady=2)
        self.duplicate_ROI_btn.grid(row=1, column=6, sticky=W, pady=2)
        self.chg_attr_btn.grid(row=1, column=7, sticky=W, pady=2)

    def save_menu(self):
        self.save_frame = LabelFrame(self.master, text='Save', font=("Arial", 16, "bold"), padx=5, pady=5)
        self.save_button = Button(self.save_frame, text='Save ROI', fg=self.non_select_color, command=lambda: self.save_data())
    ...
    def get_duplicate_shape_name(self):
        c_no = 1
        while True:
            self.new_name = self.current_shape_data['Shape_name'] + '_copy_' + str(c_no)
            if str(self.shape_type)  + ': ' + self.new_name in self.video_ROIs:
                c_no += 1
            else:
                self.new_shape_data['Shape_name'] = str(self.shape_type)  + ': ' + self.new_name
                break

    def get_duplicate_coords(self):
        print(self.duplicate_jump_size)
        if self.shape_type == 'Rectangle':
            self.new_shape_x = int(self.current_shape_data['Top_left_X'] + self.duplicate_jump_size)
            self.new_shape_y = int(self.current_shape_data['Top_left_Y'] + self.duplicate_jump_size)
            for shape in self.image_data.out_rectangles:
                if (shape['Top_left_X'] == self.new_shape_x) and (shape['Top_left_Y'] == self.new_shape_y):
                    self.new_shape_x += self.duplicate_jump_size
                    self.new_shape_y += self.duplicate_jump_size
        if self.shape_type == 'Circle':
            self.new_shape_x = int(self.current_shape_data['Center_X'] + self.duplicate_jump_size)
            self.new_shape_y = int(self.current_shape_data['Center_Y'] + self.duplicate_jump_size)
            for shape in self.image_data.out_circles:
                if (shape['Center_X'] == self.new_shape_x) and (shape['Center_Y'] == self.new_shape_y):
                    self.new_shape_x += self.duplicate_jump_size
                    self.new_shape_y += self.duplicate_jump_size
    ...
    def call_duplicate_ROI(self):
        ...
        if shape_name[0] != 'None':
            ...
            self.current_shape_data = [d for d in all_roi_list if d.get('Shape_name') == shape_name][0]
            self.new_shape_data = copy.deepcopy(self.current_shape_data)
            self.get_duplicate_shape_name()
            self.get_duplicate_coords()
            if self.shape_type == 'Rectangle':
                self.new_shape_data['Top_left_X'] = self.new_shape_x
                self.new_shape_data['Top_left_Y'] = self.new_shape_y
                self.new_shape_data['Shape_name'] = self.new_shape_data['Shape_name'].split("Rectangle: ", 1)[-1]
                update_all_tags(self.new_shape_data)
                self.image_data.out_rectangles.append(self.new_shape_data)

            if self.shape_type == 'Circle':
                self.new_shape_data['Center_X'] = self.new_shape_x
                self.new_shape_data['Center_Y'] = self.new_shape_y
                self.new_shape_data['Shape_name'] = self.new_shape_data['Shape_name'].split("Circle: ", 1)[-1]
                update_all_tags(self.new_shape_data)
            ...
            if self.shape_type == 'Polygon':
                ...
                self.new_shape_data['Shape_name'] = self.new_shape_data['Shape_name'].split("Polygon: ", 1)[-1]
                self.image_data.out_polygon.append(self.new_shape_data)


            self.video_ROIs.append(self.shape_type + ': ' + self.new_shape_data['Shape_name'])
            self.image_data.insert_all_ROIs_into_image()
    ...
    class ChangeAttrMenu:
        def __init__(self, shape_data, image_data):
            ...
            if shape_name[0] != 'None':
                ...
                current_shape_data = [d for d in self.all_roi_list if d.get('Shape_name') == self.shape_name][0]
                attr_win = Toplevel()
                attr_win.minsize(400, 300)
                attr_win.wm_title("Selected Shape Attributes")
                attr_lbl_frame = LabelFrame(attr_win, text='Attributes', font=("Arial", 16, 'bold'), pady=5, padx=5,fg='black')
                selected_shape_name_lbl = Label(attr_win, text="Shape name: ")
                self.selected_shape_name_entry_txt = StringVar()
                self.selected_shape_name_entry_txt.set(current_shape_data['Shape_name'])

                selected_shape_name_entry = Entry(attr_win, width=25, textvariable=self.selected_shape_name_entry_txt)
                selected_shape_thickness_lbl = Label(attr_win, text="Shape thickness: ")
                self.selected_shape_thickness = IntVar()
                self.selected_shape_thickness.set(current_shape_data['Thickness'])
                selected_shape_thickness_dropdown = OptionMenu(attr_win, self.selected_shape_thickness, *list(shape_data.shape_thickness_list))

                selected_shape_eartag_size_lbl = Label(attr_win, text="Ear tag size: ")
                self.selected_shape_eartag_size = IntVar()
                self.selected_shape_eartag_size.set(current_shape_data['Ear_tag_size'])
                selected_shape_eartag_size_dropdown = OptionMenu(attr_win, self.selected_shape_eartag_size,*list(shape_data.ear_tag_size_list))

                selected_shape_color_lbl = Label(attr_win, text="Shape color: ")
                self.selected_shape_color = StringVar()
                self.selected_shape_color.set(current_shape_data['Color name'])
                selected_shape_color_dropdown = OptionMenu(attr_win, self.selected_shape_color, *list(shape_data.named_shape_colors.keys()))

                save_button = Button(attr_win, text='Save', fg=shape_data.non_select_color, command=lambda: self.save_attr_changes(shape_data, image_data))
        ...
        def save_attr_changes(self, shape_data, image_data):
            ...
            for shape in [image_data.out_rectangles, image_data.out_circles, image_data.out_polygon]:
                for e in shape:
                    shape_type = e['Shape_type']
                    if e['Shape_name'] == self.shape_name:
                        e['Shape_name'] = new_shape_name
