...
1. main() : A wizard that guides you to choosing a version that does not conflict with any git tags or PyPi versions.
...
def _version_is_valid(last_versions, version):
    ...
    The version is tested against both the tags, and the PyPi versions.
    """
    versions = _sorted_numeric_versions(
        [last_versions["testpypi"],
        last_versions["prodpypi"], 
        last_versions["tag"], version]
    )
    print("Current version: ", version)

    return versions[-1] == version and len(versions)>1 and versions[-2] != versions[-1]
...
def _get_all_versions(repo):
    ...
    test_pypi_versions = _get_test_pypi_versions(pip_name)
    prod_pypi_versions = _get_prod_pypi_versions(pip_name)
    return _sorted_numeric_versions(repo.tags + test_pypi_versions + prod_pypi_versions) 
...
def _resolve_version(repo, version, version_file):
    ...
    versions = _sorted_numeric_versions([last_versions["testpypi"], last_versions["prodpypi"], last_versions["tag"], version])
