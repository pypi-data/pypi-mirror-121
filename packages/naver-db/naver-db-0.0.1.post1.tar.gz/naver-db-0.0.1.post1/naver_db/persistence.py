...
class Persistence(NaverConfig):
    myApp = Flask(__name__)

    def __init__(self, myApp, session=None):
        super().__init__(myApp, session)
    """
       Métodos SQL Core 
    """
# TESTED

    def _sql(self, rawSql, sqlVars={}):
...
# TESTED
    ...
    def _sqltran(self, rawSql, sqlVars={}):
        self.myDb = SQLAlchemy(self.myApp)
        assert type(rawSql) == str
        assert type(sqlVars) == dict
        cursor = self.myDb.session.execute(rawSql, sqlVars)
...
# TESTED
    ...
    def setWrite(self, stm, table, logtable="log", loglevel=1, logtype=1):
        self.core.getEnvCnx(table)
