...
class TestCliAuthCommand(unittest.TestCase):
    def setUp(self):
        ...
        set_cli_config(
            self.runner, "oauth_token", ["scope", TEST_AUTH_SCOPES["publisher"]]
        )
    ...
    def login(test_class):
        ...
        login_thread = threading.Thread(
            target=TestCliAuthCommand.do_login_steps,
            args=(
                proc,
                test_class,
            ),
    ...
    def test_login(self):
        # set one of the
        set_cli_config(self.runner, "collections-url", TEST_COLLECTIONS_URI)

        proc = self.login(self)
