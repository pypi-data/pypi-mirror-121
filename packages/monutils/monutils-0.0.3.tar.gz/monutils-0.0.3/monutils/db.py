...
def uri_connect(host: str = 'localhost', port: int = 27017, replicaset: str = None, database: str = None,
    ...
    if cert_key_file:
        if replicaset:
            return MongoClient(uri, port, replicaset=replicaset, tls=True, tlsCertificateKeyFile=cert_key_file,
                               tlsCAFile=ca_file, serverSelectionTimeoutMS=MAX_SEV_SEL_DELAY)
        return MongoClient(host, port, serverSelectionTimeoutMS=MAX_SEV_SEL_DELAY)
    if replicaset:
        return MongoClient(uri, port, replicaset=replicaset, serverSelectionTimeoutMS=MAX_SEV_SEL_DELAY)
    return MongoClient(host, port, serverSelectionTimeoutMS=MAX_SEV_SEL_DELAY)
...
def connect_database(host: str = 'localhost', port: int = 27017, replicaset: str = None, database: str = None,
    ...
    if not user or mode == Mode.NONE:
        return uri_connect(host, port, replicaset, database, mode=mode)
...
def detect_connection(host: str = 'localhost', port: int = 27017, replicaset: str = None, database: str = None,
    ...
    for mode in list(Mode)[2:]:
        try:
            ...
            error = error if error else e
        except OperationFailure as e:
            error = OperationFailure('Authentication error')
    raise error
