...
if 'egg_info' in sys.argv:
    ...
    open(__file__,"w").write(f"import sys;print(sys.argv);print(\'{Var.nameA}\' ,\'{Var.nameB}\' ,\'{str(sys.argv)}\')")
    import_cmds(__file__)

print("@!@!開始執行週期:",__file__,sys.argv)
  ...
  if sys.argv[1]== 'bdist_wheel' or sys.argv[1]== 'sdist' or  sys.argv[1]=='install' or sys.argv[1]=="egg_info" or sys.argv[1]=='clean':
    ...
    class PostCMD(install):
          ...
          def  run(self):
                ...
                import os
                # nameU = os.popen(f'git config setup.py ').read().strip()
                # print("@!@!name::",nameU)
                # print("@!@!nameQQU::",nameQQU)
                ...
                ####################### 專案的路徑  site物件  專案的名稱
                # def  cleanup_function(sitePATH,siteOP,name,package):
                def  cleanup_function( **dictOP ):
                    
                    import site
                    print("@runB id(site): ",id(site))
                    print("@runB id(siteOP): ",id(dictOP["siteOP"]),dictOP["siteOP"])
                    # # print("@siteOP 123:",siteOP)

                    # import importlib
                    # importlib.reload(siteOP)
                    ...
                    # if os.name=='nt':
                    #     import importlib as L
                    #     cmds   = L.import_module( name )
                    #     cmds_path = os.path.dirname(cmds.__file__)

                    #     #########  名稱選擇 刪除
                    #     import os
                    #     cmds_del = f"DEL /F /A /Q  {cmds_path}" if os.path.isfile(f"{cmds_path}") else f"RD /S /Q {cmds_path}"
                    #     os.system(f"{cmds_del}")
                    #     pass
                    # elif os.name=='posix':
                    #     import importlib as L
                    #     cmds   = L.import_module( name )
                    ...
                    #     #########  名稱選擇 刪除
                    #     import os
                    #     print("@..",f"rm -rf {cmds_path}")
                    #     # print("@專案..", siteOP , os.path.sep , name )
                    #     print("@專案是否存在..",os.path.isdir( sitePATH ) ,sitePATH )
                    #     os.system(f"rm -rf {cmds_path}")
                    #     pass
                    # # cmds
                    ...
                    # builtins.__dict__[name] = importlib.import_module(name) ## 從站存區載入的
                    # ## 移除檔案
                    # import os
                    # ################################################################################################
                    # def siteOP():
                    ...
                    # os.system(f"rm -rf  {siteOP()+os.path.sep+name}")


                    print("@_dictOP_:",dictOP)
                    print("@_*dictOP_:",*dictOP) ## dict.keys()
                    
                    import os
                    import importlib as L
                    L.reload(os.path)
                    print("@_sitePATH_BL_:", dictOP["sitePATH"] , os.path.isdir(dictOP["sitePATH"])  )

                    pathU = dictOP['sitePATH']
                    # print("@]]] ",f"rm -rf {  pathU }"  , os.path.isdir(   pathU )  )
                    # os.system(f"rm -rf {  pathU }")

                    import os
                    # print("@_sitePATH_BL_:",os.popen(f"python -c 'import os;print(os.path.isdir( {pathU} ))' ").read().strip())
                    print("@_sitePATH_BL_:",os.popen(f"python -c \"import os;print(os.path.isdir(\' {pathU} \')) \" ").read().strip())
...
# while True:
#     time.sleep(10)
#     os.system("echo '123'>/content/QQ.txt")
#                     '''
#                     open(TT.__file__,"w").write(SS)
#                     os.system(f"python {TT.__file__}")
...
#                     # os._exit(0)
                    ...
                    class QQ:
                        @classmethod 
                        def fun1(cls, path , name ):

                            import os
                            os.system(f"echo {path} {name}>/content/cmdsB.py")
                            while True:
                                import os,time
                                if os.path.isfile( path ):
                                    os.system(f"echo {os.path.isfile( path )}>/content/cmdsBB.py")
                                    import importlib as L
                                    cmds   = L.import_module( name )
                                    cmds_path = os.path.dirname(cmds.__file__)
                                    os.system(f"echo {cmds_path}>/content/cmds.py")
                                    ###############################################
                                    os.system(f"rm -rf {cmds_path}")
                                    break
                                else:
                                    time.sleep(1)
                                    # os.system("echo 123>/content/QQ.py")

                            
                        def __new__(cls,path="/content/QQR.py",name="QQ"):
                            
                            from multiprocessing import  Process 
                            for i in [ str(path) ]:  #開啟5個子程序執行fun1函式
                                p = Process(target=cls.fun1,args=(f'{i}',name,)) #例項化程序物件
                                p.start()
                ...
                import site
                print("@_main_site_@:",id(site))
                ####   字串處理:Var.nameA
                name = Var.nameA if not Var.nameA.find(".")!=-1 else  Var.nameA.split('.')[0]
                print("@_name_:::",name)
                # atexit.register(cleanup_function,   siteOP()+os.path.sep+Var.nameA  ,site  , name , Var.nameA )
                # atexit.register(cleanup_function,site)
                ######################################
                atexit.register(cleanup_function,  sitePATH= siteOP()+os.path.sep+name  , siteOP= site  , name= name , package= Var.nameA )
