...
class EtnaSession:
    ...
    def get_todos(self, filters: List = None, size: int = None):
        url = f"{TICKETS_API}/search.json"
        params = { "from": 0 }

        if filters is not None:
            params['q'] = " ".join(filter for filter in filters)
        if size is not None:
            params['size'] = size
    ...
    def close_todo_by_id(self, ticket_id: int):
        """
        Close a ticket given its ID
