...
def get_report(delta, ret, long_short_split=False):
    """
...
def get_composite_report(delta, ret, long_short_split=False):
    """
...
def get_pnl_plot(delta, ret, fig_return=False, alpha=False, **kwargs):
    """
    delta: pd.Series with DatatimeIndex
    ret: pd.Series with DatatimeIndex
    """
    ...
    if 'drop_holiday' in kwargs:
        dh = indicator.index.to_series().apply(lambda d: 0.0 if all([cc.is_holiday(d + datetime.timedelta(days=i))
    ...
    ax1.set_title('PnL={} PoT={} Sharpe={} WR={:.2f} Worst={:.2f} HP={:.2f}'.format(
        report_all.loc['PnL'], report_all.loc['PoT'], report_all.loc['Sharpe'], report_all.loc['WR'],
        report_all.loc['Worst'], report_all.loc['HP']
    ))
    ax1.set_ylabel('PnL')
    ax1.legend(loc=2)
    ax1.set_xlabel('')
...
def get_composite_pnl_plot(delta, ret, fig_return=False, alpha=False, **kwargs):
    """
    Multi-Assets PnL
    delta: pd.DataFrame with DatatimeIndex
    ret: pd.DataFrame with DatatimeIndex
