...
class Implementation:
    ...
    def find_proj_file_path(project_dir: str) -> str:
        ...
        results = list(Path(Implementation.normalize_path(
            project_dir)).rglob("*.csproj"))
        if not len(results):
            raise Exception(
                f"Could not find *.csproj file in project directory '{project_dir}'")

        proj_file_name = str(results[0])
        proj_file_path: str = Implementation.resolve_path(
            os.path.join(project_dir, proj_file_name))
