...
class NWService:
    ...
    def __init__(self, context: "NWServiceContext", config_path: str):
        ...
        try:
            ...
            for task in self.tasks.values():
                task.requires += [f"{t.service.name}:{t.id}" for t in self.get_depending_tasks_for(task)]
    ...
    def get_depending_tasks_for(self, t: "NWTask") -> List["NWTask"]:
...
class NWTask:
    ...
    def __init__(self, name: str, service: "NWService", members: Dict[str, Any]):
        ...
        self.commands: List[Dict[str, Dict[str, Union[str, float, bool]]]] = list(members["commands"])
    ...
    async def is_changed_async(self) -> bool:
        ...
        if self._is_changed is None:
            ...
            if len(self.service.get_depending_tasks_for(self)) == 0:
