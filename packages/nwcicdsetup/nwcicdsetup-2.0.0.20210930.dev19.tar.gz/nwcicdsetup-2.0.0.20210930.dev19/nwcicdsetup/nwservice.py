...
class NWService:
    ...
    def __init__(self, context: "NWServiceContext", config_path: str):
        ...
        try:
            ...
            # build up self.layers containing task ids which can be used to resolve
            # tasks via self.tasks dictionary
    ...
    @property
    def all_jobs(self) -> Dict[str, "NWTask"]:
        """returns a dictionary containing all jobs or this service"""
        return self.tasks

    async def get_depending_tasks_async(self, t: "NWTask") -> List["NWTask"]:
        """returns all tasks of preceeding layers this task depends on"""
        depending: List["NWTask"] = []
        for layer_tasks in self.layers:
            if t.id in layer_tasks:
                break
            for task_id in layer_tasks:
                depending.append(self.tasks[task_id])
    ...
    async def check_changes_for_required(self, depency_ids: List[str]) -> Tuple[bool, Dict[str, Any]]:
        ...
        def tasks_for(id: str) -> List[Coroutine[Any, Any, bool]]:
            ...
            if not job_id in all_jobs and not requires_all:
                raise Exception(
                    f"No task-reference found for {job_id} in '{id}'")
    ...
    async def service_to_yaml_async(self) -> Dict[str, Any]:
        """returns all taks in form of a yml compliant dictionary"""
...
class NWTask:
    ...
    def __init__(self, name: str, service: "NWService", members: Dict[str, Any]):
        ...
        first_command_name = next(iter(self.commands)) if isinstance(
            self.commands[0], str) else next(iter(self.commands[0]))
    ...
    def command_steps(self) -> List[Union[str, Dict[str, Any]]]:
        ...
        for command_entry in [c for c in self.commands if not isinstance(c, str)]:
        ...
        # commands as raw strings
        for command_name in [c for c in self.commands if isinstance(c, str)]:
            steps.append(command_name)
    ...
    def is_deployment(self) -> bool:
        # raw string commands
        if "deploy" in [name.lower() for name in self.commands if isinstance(name, str)]:
            return True
        # dictionary commands
        return "deploy" in [name.lower() for command in self.commands if not isinstance(command, str) for name in command.keys()]
    ...
    async def is_changed_async(self) -> bool:
        ...
        if self._is_changed is None:
            ...
            # handle depending tasks in preceeding layers:
            depending_tasks = await self.service.get_depending_tasks_async(self)
            if len(depending_tasks):
                result = await self.check_task_changed_async(depending_tasks)
                self.change_info |= result[1]
                self._is_changed = self._is_changed or result[0]
            else:
            ...
            # check depending external tasks by the requires attribute:
