...
from nwcicdsetup.nwservice import (NWDeployment, NWService, NWServiceContext,
                                   NWTask)
from nwcicdsetup.validationschema import general_config_validate


async def validate(general_config_path: str):
    cwd = Path(".")
    ...
    try:
        ...
        def valdiate_deployment_attr_infra_yml_path(cwd: Path, service: NWService, d: NWDeployment):
            if len(d.infra_yml_path):
                path = Path(os.path.join(service.path, d.infra_yml_path)
                            ).relative_to(cwd)
        ...
        context = NWServiceContext(CircleCIContext.create_dummy_env(), services, general_config, str(cwd.resolve()))
        services |= {s.name: s for s in [NWService(context,str(configPath.relative_to(cwd))) for configPath in configs]}
