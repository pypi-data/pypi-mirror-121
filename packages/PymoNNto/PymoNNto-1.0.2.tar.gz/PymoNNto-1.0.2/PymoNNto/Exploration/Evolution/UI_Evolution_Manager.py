...
def evolution_thread_worker(name, python_cmd, conn):
    ...
    try:
        ...
        if 'darwin' in system_str:#mac os
            os.system(python_cmd+' execute_evolution.py')#todo: test dfdsgdf
        elif 'win' in system_str:#windows
            os.system('start /wait cmd /c '+python_cmd+' execute_evolution.py')
        else:
            os.system('gnome-terminal -x '+python_cmd+' execute_evolution.py')
...
class UI_Evolution_Manager(UI_Base):
    ...
    def add_tab(self, name):
        ...
        def start_continue():
            ...
            if 'local' in tab.server:
                if tab.process is None:
                    ...
                    tab.process = Process(target=evolution_thread_worker, args=(name, tab.python_cmd, child_conn))#python
                    tab.process.start()
                else:
                    print('thread already running')
        ...
        def help():
            layout = QVBoxLayout()
            pte = QPlainTextEdit()
            pte.setPlainText('''Every tab corresponds to a folder in:
...
The project folders also contain the file execute_evolution.py which is used to start the evolution.
