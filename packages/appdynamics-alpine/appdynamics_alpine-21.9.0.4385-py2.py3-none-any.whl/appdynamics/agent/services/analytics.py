...
class AnalyticsService(threading.Thread):
    ...
    def _get_request_experience(self, bt):
        ...
        elif bt_info_response:
            if bt_info_response.currentStallThreshold and \
                    bt.timer.duration_ms >= bt_info_response.currentStallThreshold:
                request_experience = self.STALL
            elif bt_info_response.currentVerySlowThreshold and \
                    bt.timer.duration_ms >= bt_info_response.currentVerySlowThreshold:
                request_experience = self.VERY_SLOW
            elif bt_info_response.currentSlowThreshold and \
                    bt.timer.duration_ms >= bt_info_response.currentSlowThreshold:
    ...
    def record_transaction(self, current_bt):
        ...
        segment = {
            ...
            'requestExperience': self._get_request_experience(current_bt),
        ...
        event = {
            ...
            'userExperience': self._get_request_experience(current_bt),
    ...
    def report_analytics(self, current_bt):
        """This method checks if BT data needs to be sent for analytics, records the transaction and reports data in case
        buffer limit is reached.
        It will be called when a BT ends and acts as an interface between transaction service and analytics service.
        """
        try:
            if not self.enabled:
