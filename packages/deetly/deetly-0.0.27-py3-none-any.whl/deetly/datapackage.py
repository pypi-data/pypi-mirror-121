...
class Datapackage:
    ...
    def add(self, type: str, content: Any, name: str, description: str = "") -> None:
        ...
        if type == "grid":
            return self.grid(content, name, description)
    ...
    def grid(
        ...
        self.plot("grid", spec, name, description)
    ...
    def plotly(
        self, fig: Dict, name: str, description: str = "", filter: Dict = {}
    ) -> None:
    ...
    def table(
        self,
        value: Union[pd.DataFrame, object],
        name: str = "",
        description: str = "",
        *args: Any,
        **kwargs: Any,
        ...
        if isinstance(value, pd.DataFrame):
            resource = self.df(value, name, description, sep=";")
            spec = kwargs.get("spec", {})
            spec["path"] = resource.get("path", "")
            resource_name = resource.get("name", "name")
            self.plot("table", spec, name, description, [resource_name], args, kwargs)
        else:
            self.plot("table", value, name, description, "", args, kwargs)
    ...
    def datatable(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            description: Description of the view. Can be plain text or Markdown.
        Raises:
            TypeError: The first parameter (df) must be of type pandas.Dataframe() or object.
        """

        if not isinstance(value, pd.DataFrame) and not isinstance(value, object):
            raise TypeError(
                f"The first parameter df must be of type pandas.Dataframe() or a spec object"
            )
        ...
        if isinstance(value, pd.DataFrame):
            ...
            resource_name = resource.get("name", "name")
            self.plot(
                "datatable", spec, name, description, [resource_name], args, kwargs
            )
        else:
            self.plot("datatable", value, name, description, "", args, kwargs)
    ...
    def df(
        ...
        for dname, dtype in zip(df.columns, df.dtypes):
            ...
            fields.append({"name": dname, "type": valuetype})
    ...
    def _upload_resources(self, space: str, token: str, api: str) -> List:
        ...
        for resource in self.resources:
            try:
                ...
                _resource["path"] = json.loads(public_url).get(
                    "url", resource.get("path", "")
                )
                _resource["type"] = resource.get("type", "dataset")
                _resource["schema"] = resource.get("schema", {})
    ...
    def publish(self, space: str = None, token: str = None, api: str = None) -> None:
        ...
        try:
            ...
            if datapackage_url is not None:
                ...
                print(f"Declaration: {datapackage_url}", "\n")
            else:
