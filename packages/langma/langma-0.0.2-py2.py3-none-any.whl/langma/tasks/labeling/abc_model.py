...
from langma.generators import CorpusGenerator, BatchDataSet
from langma.layers import KConditionalRandomField
from langma.logger import logger
from langma.metrics.sequence_labeling import get_entities
from langma.metrics.sequence_labeling import sequence_labeling_report
from langma.processors import SequenceProcessor
from langma.tasks.abs_task_model import ABCTaskModel
from langma.types import TextSamplesVar
...
class ABCLabelingModel(ABCTaskModel, ABC):
    ...
    def predict_entities(self,
        ...
        for index, seq in enumerate(new_res):
            ...
            for entity in seq:
                ...
                for i, e in enumerate(text_seq[index][entity[1]:entity[2] +
                                                      1]):
            ...
            final_res.append({'tokenized': x_data[index], 'labels': seq_data})
        return final_res
    ...
    def evaluate(self,
        ...
        Example:
            ...
            >>> from langma.tasks.labeling import BiGRU_Model
            >>> model = BiGRU_Model()
