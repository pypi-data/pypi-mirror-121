...
from langma.tasks.classification.abc_model import ABCClassificationModel
...
class CNN_Attention_Model(ABCClassificationModel, ABC):
    ...
    def build_model_arc(self) -> None:
        if tuple(tf.__version__.split('.')) < tuple('2.1.0'.split('.')):
            logger.warning(
                "Attention layer not serializable because it takes init args "
        ...
        input_layer = L.Concatenate(axis=-1)(
