...
from langma.embeddings import ABCEmbedding, BareEmbedding
from langma.generators import BatchDataSet, CorpusGenerator
from langma.layers import L
from langma.logger import logger
from langma.metrics.multi_label_classification import multi_label_classification_report
from langma.processors import ABCProcessor
from langma.processors import ClassificationProcessor
from langma.processors import SequenceProcessor
from langma.tasks.abs_task_model import ABCTaskModel
from langma.types import TextSamplesVar, ClassificationLabelVar, MultiLabelClassificationLabelVar
...
class ABCClassificationModel(ABCTaskModel, ABC):
    ...
    def build_model(
        self, x_train: TextSamplesVar,
        y_train: Union[ClassificationLabelVar,
                       MultiLabelClassificationLabelVar]
    ) -> None:
        """
    ...
    def build_model_generator(self, generators: List[CorpusGenerator]) -> None:
        ...
        if self.sequence_length is None:
            self.sequence_length = self.embedding.get_seq_length_from_corpus(
                generators)
    ...
    def fit(self,
            ...
            y_validate: Union[ClassificationLabelVar,
                              MultiLabelClassificationLabelVar] = None,
            *,
            batch_size: int = 64,
            epochs: int = 5,
    ...
    def predict(
        ...
        *,
        ...
        if predict_kwargs is None:
            predict_kwargs = {}
        with langma.utils.custom_object_scope():
            if truncating:
                seq_length = self.sequence_length
            else:
                seq_length = None
            ...
            logger.debug(
                f'predict input shape {np.array(tensor).shape} x: \n{tensor}')
            ...
            if self.multi_label:
                ...
                res = multi_label_binarizer.inverse_transform(
                    pred, threshold=multi_label_threshold)
            else:
    ...
    def evaluate(
        ...
        x_data: TextSamplesVar,
        y_data: Union[ClassificationLabelVar,
                      MultiLabelClassificationLabelVar],
        *,
        batch_size: int = 32,
        digits: int = 4,
        ...
        if self.multi_label:
            report = multi_label_classification_report(
                y_data,  # type: ignore
                y_pred,  # type: ignore
            ...
            original_report = sklearn_metrics.classification_report(
                y_data, y_pred, output_dict=True, digits=digits)
            print(
                sklearn_metrics.classification_report(y_data,
                                                      y_pred,
                                                      output_dict=False,
                                                      digits=digits))
            report = {
