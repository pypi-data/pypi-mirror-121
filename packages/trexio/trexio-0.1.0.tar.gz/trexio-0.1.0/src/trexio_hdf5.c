...
#define BASIS_NUM_NAME            "basis_num"
...
#define BASIS_NUCLEUS_SHELL_NUM_NAME           "basis_nucleus_shell_num"
#define BASIS_SHELL_ANG_MOM_NAME           "basis_shell_ang_mom"
#define BASIS_SHELL_PRIM_NUM_NAME           "basis_shell_prim_num"
#define BASIS_SHELL_FACTOR_NAME           "basis_shell_factor"
#define BASIS_SHELL_PRIM_INDEX_NAME           "basis_shell_prim_index"
...
trexio_exit_code
trexio_hdf5_has_ecp_local_num_n_max (trexio_t* const file)
{
  ...
  htri_t status = H5Aexists(f->ecp_group, ECP_LOCAL_NUM_N_MAX_NAME);
  /* H5Aexists returns positive value if attribute exists, 0 if does not, negative if error */
  if (status > 0){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_non_local_num_n_max (trexio_t* const file)
{
  ...
  htri_t status = H5Aexists(f->ecp_group, ECP_NON_LOCAL_NUM_N_MAX_NAME);
  /* H5Aexists returns positive value if attribute exists, 0 if does not, negative if error */
  if (status > 0){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_basis_num (trexio_t* const file)
{
  ...
  htri_t status = H5Aexists(f->basis_group, BASIS_NUM_NAME);
  /* H5Aexists returns positive value if attribute exists, 0 if does not, negative if error */
  if (status > 0){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_lmax_plus_1 (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_LMAX_PLUS_1_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_local_n (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_LOCAL_N_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_local_exponent (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_LOCAL_EXPONENT_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_local_coef (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_LOCAL_COEF_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_local_power (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_LOCAL_POWER_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_non_local_n (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_NON_LOCAL_N_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_non_local_exponent (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_NON_LOCAL_EXPONENT_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_non_local_coef (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_NON_LOCAL_COEF_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_ecp_non_local_power (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->ecp_group, ECP_NON_LOCAL_POWER_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_basis_nucleus_shell_num (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->basis_group, BASIS_NUCLEUS_SHELL_NUM_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_basis_shell_prim_num (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->basis_group, BASIS_SHELL_PRIM_NUM_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_has_basis_shell_prim_index (trexio_t* const file)
{
  ...
  herr_t status = H5LTfind_dataset(f->basis_group, BASIS_SHELL_PRIM_INDEX_NAME);
  /* H5LTfind_dataset returns 1 if dataset exists, 0 otherwise */
  if (status == 1){
    return TREXIO_SUCCESS;
  } else if (status == 0) {
...
trexio_exit_code
trexio_hdf5_read_ecp_local_num_n_max (trexio_t* const file, int64_t* const num)
{
  ...
  /* Read the ecp_local_num_n_max attribute of ecp group */
  const hid_t num_id = H5Aopen(f->ecp_group, ECP_LOCAL_NUM_N_MAX_NAME, H5P_DEFAULT);
  if (num_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_read_ecp_non_local_num_n_max (trexio_t* const file, int64_t* const num)
{
  ...
  /* Read the ecp_non_local_num_n_max attribute of ecp group */
  const hid_t num_id = H5Aopen(f->ecp_group, ECP_NON_LOCAL_NUM_N_MAX_NAME, H5P_DEFAULT);
  if (num_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_read_basis_num (trexio_t* const file, int64_t* const num)
{
  ...
  const trexio_hdf5_t* f = (const trexio_hdf5_t*) file;
  /* Quit if the dimensioning attribute is missing in the file */
  if (H5Aexists(f->basis_group, BASIS_NUM_NAME) == 0) return TREXIO_FAILURE;

  /* Read the basis_num attribute of basis group */
  const hid_t num_id = H5Aopen(f->basis_group, BASIS_NUM_NAME, H5P_DEFAULT);
  if (num_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_read_ecp_lmax_plus_1 (trexio_t* const file, int64_t* const ecp_lmax_plus_1, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_z_core (trexio_t* const file, double* const ecp_z_core, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_local_n (trexio_t* const file, int64_t* const ecp_local_n, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_local_exponent (trexio_t* const file, double* const ecp_local_exponent, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_local_coef (trexio_t* const file, double* const ecp_local_coef, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_local_power (trexio_t* const file, int64_t* const ecp_local_power, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_non_local_n (trexio_t* const file, int64_t* const ecp_non_local_n, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_non_local_exponent (trexio_t* const file, double* const ecp_non_local_exponent, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_non_local_coef (trexio_t* const file, double* const ecp_non_local_coef, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_ecp_non_local_power (trexio_t* const file, int64_t* const ecp_non_local_power, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_read_basis_nucleus_shell_num (trexio_t* const file, int64_t* const basis_nucleus_shell_num, const uint32_t rank, const uint64_t* dims)
{
  ...
  // open the dataset to get its dimensions
  hid_t dset_id = H5Dopen(f->basis_group, BASIS_NUCLEUS_SHELL_NUM_NAME, H5P_DEFAULT);
  if (dset_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_read_basis_shell_prim_num (trexio_t* const file, int64_t* const basis_shell_prim_num, const uint32_t rank, const uint64_t* dims)
{
  ...
  // open the dataset to get its dimensions
  hid_t dset_id = H5Dopen(f->basis_group, BASIS_SHELL_PRIM_NUM_NAME, H5P_DEFAULT);
  if (dset_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_read_basis_shell_prim_index (trexio_t* const file, int64_t* const basis_shell_prim_index, const uint32_t rank, const uint64_t* dims)
{
  ...
  // open the dataset to get its dimensions
  hid_t dset_id = H5Dopen(f->basis_group, BASIS_SHELL_PRIM_INDEX_NAME, H5P_DEFAULT);
  if (dset_id <= 0) return TREXIO_INVALID_ID;
...
trexio_exit_code
trexio_hdf5_write_ecp_local_num_n_max (trexio_t* const file, const int64_t num)
{
  ...
  const hid_t num_id = H5Acreate(f->ecp_group, ECP_LOCAL_NUM_N_MAX_NAME, 
                                 dtype, dspace, H5P_DEFAULT, H5P_DEFAULT);
  if (num_id <= 0) {
    H5Sclose(dspace);
    H5Tclose(dtype);
...
trexio_exit_code
trexio_hdf5_write_ecp_non_local_num_n_max (trexio_t* const file, const int64_t num)
{
  ...
  const hid_t num_id = H5Acreate(f->ecp_group, ECP_NON_LOCAL_NUM_N_MAX_NAME, 
                                 dtype, dspace, H5P_DEFAULT, H5P_DEFAULT);
  if (num_id <= 0) {
    H5Sclose(dspace);
    H5Tclose(dtype);
...
trexio_exit_code
trexio_hdf5_write_basis_num (trexio_t* const file, const int64_t num)
{
  ...
  const hid_t num_id = H5Acreate(f->basis_group, BASIS_NUM_NAME, 
                                 dtype, dspace, H5P_DEFAULT, H5P_DEFAULT);
  if (num_id <= 0) {
    H5Sclose(dspace);
    H5Tclose(dtype);
...
trexio_exit_code
trexio_hdf5_write_ecp_lmax_plus_1 (trexio_t* const file, const int64_t* ecp_lmax_plus_1, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_z_core (trexio_t* const file, const double* ecp_z_core, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_local_n (trexio_t* const file, const int64_t* ecp_local_n, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_local_exponent (trexio_t* const file, const double* ecp_local_exponent, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_local_coef (trexio_t* const file, const double* ecp_local_coef, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_local_power (trexio_t* const file, const int64_t* ecp_local_power, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_non_local_n (trexio_t* const file, const int64_t* ecp_non_local_n, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_non_local_exponent (trexio_t* const file, const double* ecp_non_local_exponent, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_non_local_coef (trexio_t* const file, const double* ecp_non_local_coef, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_ecp_non_local_power (trexio_t* const file, const int64_t* ecp_non_local_power, const uint32_t rank, const uint64_t* dims)
{
...
trexio_exit_code
trexio_hdf5_write_basis_nucleus_shell_num (trexio_t* const file, const int64_t* basis_nucleus_shell_num, const uint32_t rank, const uint64_t* dims)
{
  ...
  if ( H5LTfind_dataset(f->basis_group, BASIS_NUCLEUS_SHELL_NUM_NAME) != 1 ) {
...
trexio_exit_code
trexio_hdf5_write_basis_shell_prim_num (trexio_t* const file, const int64_t* basis_shell_prim_num, const uint32_t rank, const uint64_t* dims)
{
  ...
  if ( H5LTfind_dataset(f->basis_group, BASIS_SHELL_PRIM_NUM_NAME) != 1 ) {
...
trexio_exit_code
trexio_hdf5_write_basis_shell_prim_index (trexio_t* const file, const int64_t* basis_shell_prim_index, const uint32_t rank, const uint64_t* dims)
{
  ...
  if ( H5LTfind_dataset(f->basis_group, BASIS_SHELL_PRIM_INDEX_NAME) != 1 ) {
