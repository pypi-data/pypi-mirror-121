...
class ServerSocket(SocketConnection):
    def __init__(self, connection: socket.socket, message_size: int, model: str):
        super().__init__(connection=connection, message_size=message_size, info_string=f'{model} model server')
...
class ModelServer:
    __slots__ = ('model', 'model_name', 'device', 'server_socket', 'verbose')
    ...
    def run(self):
        ...
        while True:
            ...
            if self.server_socket in new_connections:
                ...
                connection = ServerSocket(connection, Config.server_message_size, self.model_name)
                logger.debug(f'Added connection to model {self.model_name} server, fileno: {connection.fileno()}')
            ...
            for connection in new_connections:
                try:
                    ...
                    logger.warning(f'ConnectionError at {self.model_name} model server:\n{e}')
                    continue
                if not inputs:
                    continue
            ...
            try:
                ...
                logger.exception(
                    f'Exception at {self.model_name} model server, socket {self.server_socket.getsockname()}:\n{e}'
                )
