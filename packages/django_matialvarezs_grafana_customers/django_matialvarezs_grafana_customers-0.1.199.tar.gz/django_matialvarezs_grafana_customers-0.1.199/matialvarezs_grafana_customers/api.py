...
def create_organisation(name):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(settings.GRAFANA_API_BASE_URL + 'orgs',
                                                                               data=data,
...
def change_current_organization(organisation_id):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(url,
                                                                               data={},
                                                                               headers=settings.GRAFANA_API_HEADERS,
                                                                               convert_data_to_json=True)
    if res.status_code == 200:
...
def create_folder(name):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'folders',
        data=data, headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
    if res.status_code == 200:
        return res.json()
...
def update_folder(name, folder_uid):
    ...
    res, error = matialvarezs_request_handler_utils.send_put(settings.GRAFANA_API_BASE_URL + 'folders/' + folder_uid,
                                                             data=data, headers=settings.GRAFANA_API_HEADERS,
                                                             convert_data_to_json=True)
    return res
...
def restore_folder_permissions(folder_uid):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(url=url, data=json.dumps(data),
                                                                               headers=settings.GRAFANA_API_HEADERS)
...
def create_global_user(username, email, name, password):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'admin/users',
        data=data, headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
    print(res)
    if res.status_code == 200:
...
def add_global_user_to_organisation(email):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'org/users',
        data=data, headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
...
def get_user_by_username_or_email(username):
    res, error = matialvarezs_request_handler_utils.send_get_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'users/lookup?loginOrEmail=' + username,
        headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
    if res:
        return res.json()
...
def delete_user_in_organisation(organisation_id, user_id):
    res, error = matialvarezs_request_handler_utils.send_delete(
...
def create_dashboard(dashboard, **options):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'dashboards/db',
        data=data, headers=settings.GRAFANA_API_HEADERS,
...
def create_team(name, email):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(settings.GRAFANA_API_BASE_URL + 'teams',
                                                                               data=data,
                                                                               headers=settings.GRAFANA_API_HEADERS,
                                                                               convert_data_to_json=True)
    if res:
...
def add_member_to_team(team_id, user_id):
    ...
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'teams/' + str(team_id) + '/members',
        data=data, headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
    if res:
        return res.json()
...
def remove_member_from_team(team_id, user_id):
    res, error = matialvarezs_request_handler_utils.send_post_and_get_response(
        settings.GRAFANA_API_BASE_URL + 'teams/' + str(team_id) + '/members' + str(user_id),
        headers=settings.GRAFANA_API_HEADERS,
        convert_data_to_json=True)
    if res:
        return res.json()
