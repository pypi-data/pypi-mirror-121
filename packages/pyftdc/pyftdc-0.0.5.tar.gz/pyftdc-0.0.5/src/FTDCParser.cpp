...
FTDCParser::parseFiles(std::string file_paths,  bool metadata, bool metric_names) {
    ...
    return parseFiles(vector_files,metadata,metric_names);
}

int
FTDCParser::parseFiles(std::vector<std::string> &file_paths,   const bool only_metadata, const bool only_metric_names) {
    bson_reader_t *reader;
