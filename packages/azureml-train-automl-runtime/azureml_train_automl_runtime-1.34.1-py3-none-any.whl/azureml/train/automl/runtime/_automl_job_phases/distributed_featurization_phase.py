...
def _build_transformer(training_dataset: TabularDataset,
    ...
    tif = None
    for idx, step in enumerate(forecasting_pipeline.steps):
        if step[0] == TimeSeriesInternal.MAKE_TIME_INDEX_FEATURES:
            break
    ...
    features = tif.preview_time_feature_names(tsdf)
    forecasting_pipeline.steps[idx][1]._force_feature_list = features
