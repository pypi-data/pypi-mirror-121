...
class CommonGamesView(GeodeGtkCommon, Gtk.ScrolledWindow):
    ...
    def reset_item(self, identifier, data):
        """ Reset a specific item in games view
        ...
        if identifier in self.storage:
            treeiter = self.get_treeiter(identifier)

            for column, value in enumerate(data):
                self.view.set_value(treeiter, column, value)
...
class GeodeGEMViews(GeodeGtkCommon, Gtk.Box):
    ...
    def __init__(self, interface, *args, **kwargs):
        ...
        self.drag_dest_set(Gtk.DestDefaults.ALL, list(), Gdk.DragAction.COPY)
    ...
    def reset_game(self, console, game):
        """ Reset a game from games views
        ...
        self.view_list.reset_item(console, game)
        self.view_grid.reset_item(console, game)
...
class GeodeGEMTreeView(CommonGamesView):
    ...
    def append_item(self, console, game):
        ...
        row_data = self.get_item_information(console, game)

        return super().append_item(game.id, row_data)

    def get_item_information(self, console, game):
        """ Retrieve game information from API object
        ...
        -------
        list
            Item information as list structure
        """

        thumbnail = self.interface.get_pixbuf_from_cache(
            "games", 22, game.id, game.cover, use_blank=False)
        ...
        return [
            ...
            self.interface.get_ui_icon(
                ...
                game.has_custom_parameters(console)),
            self.interface.get_ui_icon(
                Columns.List.SCREENSHOT, len(game.screenshots) > 0),
            self.interface.get_ui_icon(
                Columns.List.SAVESTATE, len(game.savestates) > 0),
            game,
            thumbnail,
        ]
    ...
    def reset_item(self, console, game):
        """ See geode_gem.ui.view.CommonGamesView.reset_item
        ...
        return super().reset_item(game.id, row_data)
...
class GeodeGEMIconView(CommonGamesView):
    ...
    def get_item_information(self, console, game):
        ...
        thumbnail = self.interface.get_pixbuf_from_cache(
            "games", 96, game.id, game.cover, use_blank=False)
        if thumbnail is None:
            thumbnail = self.interface.icons_games_views.get("iconview")

        return [thumbnail, game.name, game]
