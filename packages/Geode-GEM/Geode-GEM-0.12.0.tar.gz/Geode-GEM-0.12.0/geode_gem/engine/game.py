...
class Game(object):
    ...
    def __init_attributes(self):
        ...
        if self.__parent is not None:
            ...
            if environment is not None and self.id in environment.keys():
    ...
    def has_custom_parameters(self, console):
        """ Check if current game has some custom parameters

        Parameters
        ----------
        console : geode_gem.engine.console.Console
            Console used to check if emulator is the same

        Returns
        -------
        bool
            True is the game has custom parameters, False otherwise
        """
        ...
        return (len(self.default) > 0
                ...
                or (console is not None
                    ...
                    and not console.emulator.id == self.emulator.id))

    @cached_property
    def installed(self):
        """ Retrieve Game filepath installed datetime
        ...
        return self.get_installation_date()
    ...
    def get_installation_date(self):
        ...
        return get_creation_datetime(self.__path)
