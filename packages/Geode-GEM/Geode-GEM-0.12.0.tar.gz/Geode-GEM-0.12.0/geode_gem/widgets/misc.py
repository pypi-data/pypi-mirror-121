        ...
        for element in args:
            ...
            if not issubclass(type(self), Gtk.HeaderBar) \
               and method.startswith("pack_"):
                args.extend([
                    getattr(element, "is_expandable", True),
                    getattr(element, "is_fillable", True),
                    0
                ])
...
class GeodeGtkAdjustment(GeodeGtkCommon, Gtk.Adjustment):

    def __init__(self, *args, **kwargs):
        """ Constructor
        """

        GeodeGtkCommon.__init__(self, Gtk.Adjustment, **kwargs)
...
class GeodeGtkScale(GeodeGtkCommon, Gtk.Scale):

    def __init__(self, *args, **kwargs):
        """ Constructor
        """

        GeodeGtkCommon.__init__(self, Gtk.Scale, **kwargs)

        for element in args:

            if isinstance(element, Gtk.Adjustment):
                self.append_widget(element)
                self.set_adjustment(element)

        if "marks" in kwargs:
            for mark in kwargs.get("marks"):
                self.add_mark(*mark)


class GeodeGtkScrolledWindow(CommonBox, Gtk.ScrolledWindow):
...
class GeodeGtkSearchBar(GeodeGtkCommon, Gtk.SearchBar):

    def __init__(self, *args, **kwargs):
        """ Constructor
        """

        GeodeGtkCommon.__init__(self, Gtk.SearchBar, **kwargs)
...
class GeodeGtkViewport(CommonBox, Gtk.Viewport):

    def __init__(self, *args, **kwargs):
        """ Constructor
        """

        CommonBox.__init__(self, Gtk.Viewport, *args, **kwargs)
