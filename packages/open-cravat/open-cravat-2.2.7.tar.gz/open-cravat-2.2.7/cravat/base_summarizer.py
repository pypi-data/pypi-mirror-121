...
class BaseAnnotator(object):
    ...
    def run(self):
        try:
            ...
            self.logger.info(
                "Completed %s in %s seconds" % (self.annotator_name, round(run_time, 3))
            )
        except Exception as e:
            self._log_exception(e)
    ...
    def get_gene_summary_data(self, cf):
        cols = [
            self.annotator_name + "__" + coldef["name"]
    ...
    def _log_runtime_exception(self, lnum, input_data, e):
        try:
            ...
            err_line = "\t".join(
                [input_data[self._id_col_name], error_classname, str(e)]
            )
    ...
    def _setup_secondary_inputs(self):
        try:
            ...
            if num_expected > num_provided:
                raise Exception(
                    "Too few secondary inputs. %d expected, "
            ...
            for sec_name, sec_input_path in self.secondary_paths.items():
                key_col = self.conf["secondary_inputs"][sec_name]["match_columns"][
                    "secondary"
                ]
    ...
    def _setup_outputs(self):
        try:
            ...
            self.output_path = os.path.join(
                self.output_dir,
                ".".join([self.output_basename, self.annotator_name, output_suffix]),
            )
            self.invalid_path = os.path.join(
                self.output_dir,
                ".".join([self.output_basename, self.annotator_name, "err"]),
            )
            if self.plain_output:
                ...
                self.output_writer.write_meta_line("name", self.annotator_name)
            ...
            for col_index, col_def in enumerate(self.conf["output_columns"]):
                self.output_writer.add_column(
                    col_index, col_def["title"], col_def["name"], col_def["type"]
                )
    ...
    def _open_db_connection(self):
        db_dirs = [self.data_dir, os.path.join("/ext", "resource", self.annotator_name)]
        db_path = None
    ...
    def _setup_logger(self):
        try:
            ...
            log_path = os.path.join(
                self.output_dir,
                ".".join([self.output_basename, self.annotator_name, "log"]),
            )
        ...
        for lnum, line, reader_data in self.primary_input_reader.loop_data():
            try:
                ...
                for annotator_name, fetcher in self.secondary_readers.items():
                    input_key_col = self.conf["secondary_inputs"][annotator_name][
                        "match_columns"
                    ]["primary"]
