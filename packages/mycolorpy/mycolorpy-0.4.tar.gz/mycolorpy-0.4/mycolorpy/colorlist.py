...
def gen_color(see_map,n,reverse=False):
...
def gen_color_normalized(see_map,data_arr,reverse=False):
    ...
    if reverse==True:
        ...
        for c in data_arr:
            norm=(c-data_min)/(data_max-data_min)*0.99
            rgba=cmap(1-norm) #select the rgba value of the cmap at point c which is a number between 0 to 1
            clr=colors.rgb2hex(rgba) #convert to hex
            colorlist_normalized.append(str(clr)) # create a list of these colors
