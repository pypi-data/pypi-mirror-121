...
from grid.openapi import Body3, Body4, DatastoreServiceApi, V1CompletePresignedUrlUpload
from grid.sdk._gql.queries import complete_datastore_upload, create_datastore
...
def create_datastore_api_client() -> DatastoreServiceApi:
    ...
    return DatastoreServiceApi(api_client)
...
class DatastoreUploadSession(UploadProgressCallback):
    ...
    def _get_presigned_urls(self):
        ...
        response = api_client.datastore_service_create_datastore_presigned_urls(
            cluster_id=self.cluster_id, datastore_id=self.id, body=Body3(count=self.part_count, object_key=object_key)
        )
    ...
    def _mark_upload_complete(self):
        ...
        api_client.datastore_service_complete_datastore_presigned_urls_upload(
            ...
            body=Body4(object_key=key, urls=urls)
        )

        self._update_progress("Completing datastore upload...")
        size_mib = math.ceil(self.original_size / (1024**2))
        # TODO - move to external API
        complete_datastore_upload(datastore_id=self.id, size_mib=size_mib)
