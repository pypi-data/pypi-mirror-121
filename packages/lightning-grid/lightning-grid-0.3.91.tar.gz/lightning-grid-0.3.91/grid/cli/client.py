...
from grid.openapi import V1ClusterType
...
"""
    ...
    def login(self, username: str, key: str):
        ...
        if settings_path.exists():  # else case should not happen
            ...
            for c in cluster_list.clusters:
                if c.id == default_cluster:
                    break
            else:  # N.B. for-else loop (ie. no-break)
                raise RuntimeError(
                    "Internal Error: Please report. default_cluster spec not listed"
                    f"in list clusters response. default_cluster={default_cluster} "
                    f"clusters={[c.id for c in cluster_list.clusters]}"
                )

            settings['context'] = default_cluster
            if 'context' not in settings:
                raise click.ClickException(f"Could not find default cluster! Please report to support@grid.ai")
