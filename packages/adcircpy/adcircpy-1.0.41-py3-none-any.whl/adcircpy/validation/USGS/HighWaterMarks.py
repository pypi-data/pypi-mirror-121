...
class HighWaterMarks(OrderedDict):
    ...
    def __init__(self, event_name, event_year, filter_dict=default_filter, **station_data):
    ...
    def from_event_name(cls, event_name, filter_dict=None):
        event_name, event_year, cls.params['Event'] = cls._get_event_id_from_name(event_name)
            ...
            for i, line in enumerate(csvfile):
                ...
                for j, column in enumerate(headers):
                    if column in [
                        ...
                        hwm_stations[station_id][column] = float(line[headers.index(column)])
                    elif column in ['hwm_quality_id']:
                        hwm_stations[station_id][column] = int(line[headers.index(column)])
                    else:
                        hwm_stations[station_id][column] = line[headers.index(column)]
        return cls(event_name, event_year, filter_dict=filter_dict, **hwm_stations)
