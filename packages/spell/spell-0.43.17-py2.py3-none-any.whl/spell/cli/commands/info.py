...
def info(ctx, run_id):
    ...
    if run.pip_env_packages:
        lines.append(("Python Environment", run.pip_env_packages[0]))
        for pip in run.pip_env_packages[1:]:
            lines.append(("", pip))

    if run.requirements_file:
        requirements_file_lines = run.requirements_file.split("\n")
        lines.append(("Requirements File", requirements_file_lines[0]))
        for requirements_file_line in requirements_file_lines[1:]:
            if requirements_file_line.strip() == "":
                continue
            lines.append(("", requirements_file_line))
    if run.pip_packages:
