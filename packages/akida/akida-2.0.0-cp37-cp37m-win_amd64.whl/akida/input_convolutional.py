...
class InputConvolutional(Layer):
    ...
    def __init__(self,
        ...
        if len(input_shape) != 3:
            raise ValueError(
                "The input shape of the image should have 3 parameters")
        if len(kernel_size) != 2:
            raise ValueError(
                "The kernel size of the convolution should have 2 parameters")
        try:
            params = InputConvolutionalParams(
                InputLayerParams(
                    InputParams(input_shape[0], input_shape[1],
                                input_shape[2])),
