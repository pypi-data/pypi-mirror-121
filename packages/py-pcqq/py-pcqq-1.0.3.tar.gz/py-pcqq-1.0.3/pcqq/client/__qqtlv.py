from pcqq.utils import GetRandomBin
from pcqq.binary import Writer, TeaEncrypt
...
def TLV_0006_TGTGT(BinQQ: bytes, TGTGTKey: bytes, Md5Once: bytes,
                   Md5Twice: bytes, ConnTime: bytes, LocalIP: bytes,
                   ComputerID: bytes, TGTGT: bytes):
    writer = Writer()
    if TGTGT == b'':
        writer.WriteBytes(GetRandomBin(4))
    ...
    return _TlvPack("00 06", TeaEncrypt(writer.ReadAll(), Md5Twice))


def TLV_0015_ComputerGuid() -> bytes:
    writer = Writer()
    writer.WriteHex(
        "00 01 01 74 83 F2 C3 00 10 14 FE 77 FC 00 00 00 00 00 00 00 00 00 00 00 00 02 17 65 6E 9D 00 10 78 8A 33 DD 00 76 A1 78 EB 8E 5B BB FF 17 D0 10"
    )
...
def TLV_001A_GTKeyTGTGTCryptedData(Tgtkey: bytes) -> bytes:
    ...
    return _TlvPack("00 1A", TeaEncrypt(writer.ReadAll(), Tgtkey))
...
def TLV_0114_DHParams() -> bytes:
    ...
    writer.WriteHex(
        "02 F0 3C 70 7B 85 60 78 04 0F 8F 26 3D 43 1F 66 5E F3 6C 5D C0 45 AD 61 A6"
    )  # PublicKey
...
def TLV_0103_SID() -> bytes:
    ...
    writer.WriteBytes(GetRandomBin(16))
...
def TLV_0313_GUID_Ex() -> bytes:
    writer = Writer()
    writer.WriteHex(
        "01 01 02 00 10 EE 47 7F A4 BC D6 EE 65 02 65 4D E9 43 38 4C 3D 00 00 00 EB"
    )
...
def TLV_0102_Official(Token0038From0825: bytes) -> bytes:
    ...
    writer.WriteBytes(GetRandomBin(20))
    return _TlvPack("01 02", writer.ReadAll())


def TLV_0309_Ping_Strategy(ServerIp: bytes, RedirectionHistory: str,
                           RedirectionTimes: int) -> bytes:
    writer = Writer()
    writer.WriteHex("00 01")
...
def TLV_001F_DeviceID() -> bytes:
    writer = Writer()
    writer.WriteHex("00 01")
    writer.WriteBytes(GetRandomBin(32))
...
def TLV_010B_QDLoginFlag() -> bytes:
    writer = Writer()
    writer.WriteHex("00 02")
    writer.WriteBytes(GetRandomBin(17))
    writer.WriteHex(
        "10 00 00 00 00 00 00 00 02 00 63 3E 00 63 02 04 00 03 07 00 04 00 49 F5 00 00 00 00 78 8A 33 DD 00 76 A1 78 EB 8E 5B BB FF 17 D0 10 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 FE 26 81 75 EC 2A 34 EF 02 3E 50 39 6D B1 AF CC 9F EA 54 E1 70 CC 6C 9E 4E 63 8B 51 EC 7C 84 5C 68 00 00 00 00"
    )
